sap.ui.define([
	...
	"sap/ui/core/Core",
	"sap/gantt/library"
], function (Controller, JSONModel, Relationship, Utility, Core, GanttLibrary) {
	"use strict";

	var TimeUnit = GanttLibrary.config.TimeUnit, oUIModel;

	var getRange = function (range1, range2) {
		return function (){
			var bEnableHideNonWorkingTime = oUIModel.getProperty("/enableHideNonWorkingTime");
			return bEnableHideNonWorkingTime ? range1 : range2;
		};
	};
	...
	return Controller.extend("sap.gantt.sample.GanttChart2Relationship.Gantt", {
		onInit: function() {
			oUIModel = new JSONModel({enableHideNonWorkingTime: true});
			this.getView().setModel(oUIModel, "UI");
			var oZoomStrategy = this.getView().byId("zoomStrategy");
			oZoomStrategy.setCoarsestTimeLineOption(oTimeLineOptions["1month"]);
			oZoomStrategy.setFinestTimeLineOption(oTimeLineOptions["5min"]);
			oZoomStrategy.setTimeLineOption(oTimeLineOptions["1day"]);
