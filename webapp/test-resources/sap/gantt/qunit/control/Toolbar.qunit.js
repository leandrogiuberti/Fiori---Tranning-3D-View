...
sap.ui.define([
...
], function (Toolbar, AssociateContainer, SettingGroup, SettingItem, GanttChartContainer, ContainerLayout, GanttChartWithTable, ToolbarScheme, StepwiseTimeLineOptions, GanttChartConfigurationUtils) {
	"use strict";
	...
	QUnit.test("Test local toolbar", function (assert) {
		...
		GanttChartConfigurationUtils.setLegacyDateFormat("2");
	...
	QUnit.test("Test slideronly", function (assert) {
		...
		var oTimeLineOptions = StepwiseTimeLineOptions;
		var aItems = [],
			iIndex = 0;
	...
	QUnit.module("Setting item tests", {
		beforeEach:  function () {
			this.oContainer = new GanttChartContainer({
				...
				toolbarSchemes: [
					new ToolbarScheme({
						...
						settings: new SettingGroup({
							position: "R1",
							overflowPriority: sap.m.OverflowToolbarPriority.Low,
							items: sap.gantt.config.DEFAULT_TOOLBAR_SETTING_ITEMS.concat(new SettingItem({
								key: "CUST_SETTING1",
								checked: true,
								displayText: "Customized Setting 1",
								tooltip: "Narrow Down"
				...
				ganttCharts: [new GanttChartWithTable()]
			});

			this.oGlobalToolbar = this.oContainer.getAggregation("_toolbar");

			this.oContainer.placeAt("content");
			sap.ui.getCore().applyChanges();
		},
		afterEach: function () {
			this.oGlobalToolbar._oSettingsDialog.destroy();
			this.oContainer.destroy();
			this.oContainer = undefined;
			this.oGlobalToolbar = undefined;
		}
	});

	QUnit.test("Test confirm", function (assert) {
		var oSettingButton = this.oGlobalToolbar.getAllToolbarItems()[1];
		oSettingButton.firePress();
		var oIndicateCurrentTimeSettingItem = this.oGlobalToolbar._oSettingsBox.getItems()[0],
			oCustomSettingItem = this.oGlobalToolbar._oSettingsBox.getItems()[8];
		oIndicateCurrentTimeSettingItem.setSelected(false);
		oCustomSettingItem.setSelected(false);
		this.oGlobalToolbar._oSettingsDialog.fireConfirm();
		assert.ok(!oIndicateCurrentTimeSettingItem.getSelected(), "Confirm saves the selection change");
		assert.ok(!this.oContainer.getEnableNowLine(), "Now line is disabled");
		assert.ok(!oCustomSettingItem.getSelected(), "Confirm saves the selection change for custom settings");
	});

	QUnit.test("Test cancel", function (assert) {
		var oSettingButton = this.oGlobalToolbar.getAllToolbarItems()[1];
		oSettingButton.firePress();
		var oIndicateCurrentTimeSettingItem = this.oGlobalToolbar._oSettingsBox.getItems()[0],
			oCustomSettingItem = this.oGlobalToolbar._oSettingsBox.getItems()[8];
		oIndicateCurrentTimeSettingItem.setSelected(false);
		oCustomSettingItem.setSelected(false);
		this.oGlobalToolbar._oSettingsDialog.fireCancel();
		assert.ok(oIndicateCurrentTimeSettingItem.getSelected(), "Cancel removes the selection change");
		assert.ok(this.oContainer.getEnableNowLine(), "Now line is enabled");
		assert.ok(oCustomSettingItem.getSelected(), "Cancel removes the selection change for custom setting");
	});

	QUnit.test("Test confirm and reset", function (assert) {
		var oSettingButton = this.oGlobalToolbar.getAllToolbarItems()[1];
		oSettingButton.firePress();
		var oIndicateCurrentTimeSettingItem = this.oGlobalToolbar._oSettingsBox.getItems()[0],
			oCustomSettingItem = this.oGlobalToolbar._oSettingsBox.getItems()[8];
		...
		assert.ok(!this.oContainer.getEnableNowLine(), "Now line is disabled");
		assert.ok(!oCustomSettingItem.getSelected(), "Confirm saves the selection change for custom settings");
		this.oGlobalToolbar._oSettingsDialog.fireReset();
		assert.ok(oIndicateCurrentTimeSettingItem.getSelected(), "Reset to initial state");
		assert.ok(oCustomSettingItem.getSelected(), "Reset to initial change for custom setting");
	});
