...
sap.ui.define([
...
], function (Utility, Context, TimezoneUtils, AxisTimeStrategyBase, TimeHorizon, DateFormat, GanttChartContainer, GanttChartConfigurationUtils) {
	"use strict";
	...
	var fnCreateAxisTimeStrategyBase = function(_oOptionsObject) {
	...
	QUnit.module("Test timezone conversion", {
		beforeEach: function () {
			this.oTestDate = new Date();
			this.oTestDate.setHours(0,0,0,0);
			var sLocalTimezone = TimezoneUtils.getLocalTimezone();
			var sFLPTimezone = Intl.supportedValuesOf('timeZone').filter(function(sTimezone){
				return sTimezone !== sLocalTimezone;
			})[0];
			this.oGetTimezoneStub = sinon.stub(GanttChartConfigurationUtils, "getTimezone").returns(sFLPTimezone);
			this.oAxisTimeStrategyBase = fnCreateAxisTimeStrategyBase();
		},
		afterEach: function () {
			this.oAxisTimeStrategyBase.destroy();
			this.oTestDate = undefined;
			this.oAxisTimeStrategyBase = undefined;
			this.oGetTimezoneStub.restore();
		}
	});
	QUnit.test("Test TimeAxis Labels and cursor labels", function (assert) {
		assert.equal(Utility.getFormattedDate(this.oAxisTimeStrategyBase.getUpperRowFormatter(), this.oTestDate), this.oAxisTimeStrategyBase.getUpperRowFormatter().format(this.oTestDate), "Axistime is converted to local timezone");
		assert.equal(Utility.getFormattedDate(this.oAxisTimeStrategyBase.getLowerRowFormatter(), this.oTestDate), this.oAxisTimeStrategyBase.getLowerRowFormatter().format(this.oTestDate), "Axistime is converted to local timezone");

	});
	QUnit.test("Test Status bar Labels", function (assert) {
		var oContainer = new GanttChartContainer();
		var oDateFormat = DateFormat.getDateTimeWithTimezoneInstance({ pattern: oContainer.getStatusBarDatePattern(), showTimezone: false }); //Returns a DateFormat instance for date
	    var oTimeFormat = DateFormat.getDateTimeWithTimezoneInstance({ pattern: oContainer.getStatusBarTimePattern(), showTimezone: false }); //Returns a DateFormat instance for time
		assert.equal(Utility.getFormattedDate(oDateFormat, this.oTestDate), oDateFormat.format(this.oTestDate), "Statusbar date is converted to local timezone");
		assert.equal(Utility.getFormattedDate(oTimeFormat, this.oTestDate), oTimeFormat.format(this.oTestDate), "Statusbar time is converted to local timezone");
