...
sap.ui.define([
	...
	QUnit.test("Test rowIndex of expand chart scheme [ulc_main, ac_expand_overlap]", function (assert) {
		...
		function createAsserts(oTreeTable, isExpanded) {
			var aRowCheckboxContent = oTreeTable.getDomRef().querySelectorAll(".sapUiTableContentRow > div.sapUiTableRowSelectionCell");
			var aRows = oTreeTable.getRows();
			if (isExpanded) {
				assert.equal(aRows[0].getDomRef().classList.contains("sapGanttExpandedRow"), true, "Expand row is present");
				assert.equal(aRowCheckboxContent[0].classList.contains("sapGanttExpandedRow"), true, "Expand row is present");
			} else {
				assert.equal(aRows[0].getDomRef().classList.contains("sapGanttExpandedRow"), false, "Expand row is not present");
				assert.equal(aRowCheckboxContent[0].classList.contains("sapGanttExpandedRow"), false, "Expand row is not present");
			}
		}
		setTimeout(function(){
			...
			createAsserts(this.oGantt._oTT, false);
			...
			setTimeout(function(){
				...
				createAsserts(this.oGantt._oTT, true);
