sap.ui.define([
	...
	"sap/ui/core/library"
], function(uid, Controller, JSONModel, MockServer, ODataModel, AdhocLine, Format, BaseRectangle, ListLegend, ListLegendItem, DimensionLegend, LegendColumnConfig, LegendRowConfig, GanttPrinting, Utility, Fragment, coreLibrary) {
	"use strict";
	...
	return Controller.extend("sap.gantt.simple.test.Controller", {
		...
		handleExpandShape: function(oEvent) {
			...
			if (oSourceButton.getId().endsWith("ulcExpandBtn")) {
				...
				oGantt1.expand("task_to_step", oGantt1.getTable().getSelectedIndices()[0]);
			}
		},
		...
		handleCollapseShape: function(oEvent) {
			...
			if (oSourceButton.getId().endsWith("ulcCollapseBtn")) {
				...
				var oGantt1 = this.getView().byId("gantt1");
				oGantt1.collapse("task_to_step", oGantt1.getTable().getSelectedIndices()[0]);
			}
		},
		...
		handleMouseEnterLeave: function(oEvent, bEnter) {
			...
			if (!this.oPopover) {
				Fragment.load({
					name: "sap.gantt.simple.test.view.QuickViewPopover",
					controller: this
		...
		handleShapeDrop: function(oEvent) {
			...
			if (oShape) {
				...
				var shapeId = Utility.getIdByUid(sLastShapeUid, false);
		...
		switchRowSettings: function(oEvent) {
			...
			Fragment.load({
				name: "sap.gantt.simple.test.view." + oEvent.getParameter("item").getKey(),
				controller: this
		...
		onPressAddTask: function(oEvent) {
			...
			if (!this.oAddTaskPopover) {
				Fragment.load({
					name: "sap.gantt.simple.test.view.AddTask",
					controller: this
				}).then(function (oDialog) {
