...
], function(vizFrameData, $, createAndAppendDiv) {

  document.body.insertBefore(createAndAppendDiv("markers"), document.body.firstChild);
  document.body.insertBefore(createAndAppendDiv("content"), document.body.firstChild);
  document.body.firstElementChild.style = "width: 100%;height:800px;";
...
$(function() {
  ...
  $('#content').css('width', (window.screen.width-8)+'px');
});
...
//unstable test case. skip it now.
QUnit.skip("Check small data point." , function( assert ) {
  assert.expect(10);
  ...
  var checkSmallDataPoint = function(e){
    ...
    assert.equal($('.viz-controls-chartPopover-measure-name').css('text-align'), 'left', "Measure label should align in the left side"
      + " (" + $("head > link[id^='sap-ui-theme-']").get().map(function(link) { return link.id.slice(13);}).join(", ") + ")");
