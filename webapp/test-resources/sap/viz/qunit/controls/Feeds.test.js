...
sap.ui.define([
...
], function(AnalysisObject, FeedItem) {
	"use strict";
	...
	/*
	 ...
	 steps: 1. Create a AnalysisObject
	...
	QUnit.test("Feeds.AnalysisObject",function(assert){
		assert.expect(6);
		var oAnalysisObject = new AnalysisObject({
			'uid' : 'id',
			'name' : 'name',
			'type' : 'type'
		});
	...
	*/
	QUnit.test("Feeds.FeedItem", function(assert) {
	    ...
	    assert.equal(genAnalysisObject(0).getUid(), FeedItem.toLightWeightFmt(feeds)[0].values[0].id, 'value0 is correct');
	    assert.equal(genAnalysisObject(1).getUid(), FeedItem.toLightWeightFmt(feeds)[0].values[1].id, 'value1 is correct');
	    ...
	    assert.equal( genAnalysisObject(1).getUid(), FeedItem.toLightWeightFmt(feeds)[1].values[0].id, "setValues() is correct. ");
	});
