sap.ui.define(
	...
	for (var j = 0; j < title_txt.length; j++) {
		...
		jQuery(title_txt[j].parentNode).css('margin', '0 8px');
		jQuery(title_txt[j].parentNode).css('width', '172px');
		jQuery(title_txt[j].parentNode).css('height', '32px');
		// jQuery(title_txt[j]).css('white-space','nowrap');
		// jQuery(title_txt[j]).css('text-overflow','ellipsis');
		// jQuery(title_txt[j]).css('overflow','hidden');
		//jQuery(title_txt[j]).css('display','block');
