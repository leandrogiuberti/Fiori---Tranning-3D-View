...
sap.ui.define([
	...
	function mockify(sOrignalUrl, sMockdsPath, bIsAnalytical) {
		...
		mockADataService({
			url: sDataUrl,
			metadata: require.toUrl('../mockds/' + sMockdsPath + '/metadata.xml'),
			mockdata: require.toUrl('../mockds/' + sMockdsPath + '/data')
		}, bIsAnalytical);
		return sDataUrl;
	}
	...
	var oODataModel, oModel, oChart;

	QUnit.module('PatternColoring Test', {
		beforeEach: function() {
			// always creating the ODataModel allows to handle the async loading solely in the beforeEach hook
			oODataModel = new ODataModel(sDataURL, true);
			return oODataModel.getMetaModel().loaded();
		},
		afterEach: function() {
			if (oModel && oModel !== oODataModel) {
				oModel.destroy();
				oModel = null;
			}
			if (oODataModel) {
