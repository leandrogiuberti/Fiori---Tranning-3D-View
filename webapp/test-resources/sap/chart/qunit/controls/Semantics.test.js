...
sap.ui.define([
	...
	"../mockds/semantic/normal",
	"../mockds/semantic/time",
	"../mockds/semantic/timeYearmonthday"
], function(
	Chart,
	ChartType,
	TimeUnitType,
	...
	function retrieveThemeParameter(sParameterName, oChart) {
		return new Promise(function(resolve) {
			var sSyncValue = Parameters.get({
				name: sParameterName,
				element: oChart,
				callback: function(sAsyncValue) {
					resolve(sAsyncValue);
				}
			});
			if ( sSyncValue != null ) {
	...
	QUnit.test('Draw chart with an invalid projectedValueStartTime', function(assert){
		...
		oChart.attachEventOnce("renderComplete", function() {
			retrieveThemeParameter("sapUiChartPaletteSequentialHue1", oChart)
			.then(function(sExpectedColor) {
				...
				assert.equal(dashLine.attr('stroke'), sExpectedColor, 'Should use actual measure color.');
	...
	QUnit.test('Reset semantic relation with undefined tag', function(assert){
		...
		oChart.attachEventOnce("renderComplete", function(){
			retrieveThemeParameter("sapUiChartPaletteSequentialHue2", oChart)
			.then(function(sExpectedColor) {
				var dashLine = $('#content .v-legend-marker line[stroke-dasharray]');
				assert.equal(dashLine.length, 1, 'Semantic relationship should be reset.');
				assert.equal(dashLine.attr('stroke'), sExpectedColor, 'No relation projected measure should use default color.');
				done();
			}, function(err) {
				assert.notOk(err);
				done();
