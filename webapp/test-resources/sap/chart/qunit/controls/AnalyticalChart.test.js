...
sap.ui.define([
	...
	'../analytics/anaChartFakeService',
	'../analytics/anaChartMetadata',
	'../analytics/AChart_ServiceDocument',
	'../analytics/AChart_Batch_Contexts'
], function(
	Localization,
	isPlainObject,
	AutoScaleMode,
	...
	QUnit.test("Supported Chart Types and Localized Chart Names", function(assert) {
		function getBarName(sLocale) {
			...
			return chartAPI.getChartTypes().bar;
		}
	...
	QUnit.test("API - getChartTypeLayout", function(assert) {
		...
		function initCb() {
			assert.deepEqual(chartAPI.getChartTypeLayout("line"),
			...
			assert.deepEqual(chartAPI.getChartTypeLayout("line", [{name:"CostElement"}]), {
				dimensions: [],
				measures: [],
				errors: [{cause:"missing",detail:{dim:0,msr:1,time:0}}]
			}, "using visible dimensions only, no default measures are inferred");
			assert.deepEqual(chartAPI.getChartTypeLayout("line", null, [{name:"ActualCosts"}]), {
				dimensions: [],
				measures: ["ActualCosts"],
				errors: []
			}, "using measures only, no default dimensions are inferred");
			assert.deepEqual(chartAPI.getChartTypeLayout("line", [{name:"CostElement"}], [{name:"ActualCosts"}]), {
			...
			assert.deepEqual(chartAPI.getChartTypeLayout("line", [dimension], [measure]), {
				"dimensions":["CostElement"],
				"measures":["ActualCosts"],
				"errors":[]
			},"using provided dimensions and measures is correct");
			...
			assert.deepEqual(chartAPI.getChartTypeLayout("bubble", [tDim, dimension], [measure, m2]), {
				"dimensions":["CostTime"],
				"measures":["ActualCosts", "Revenue"],
				"errors":[]
			},"using provided timedimensions and measures is correct");

			assert.deepEqual(chartAPI.getChartTypeLayout("bubble",
			...
			assert.deepEqual(chartAPI.getChartTypeLayout("line", [], []).errors, [{cause:"missing",detail:{dim:0,msr:1,time:0}}], "errors is detected (filter out MND)");
			assert.deepEqual(chartAPI.getChartTypeLayout("dual_column",
					[{"name":"CostElement"},{"name":"CostCenter"}],
			...
			QUnit.assert.throws(chartAPI.getChartTypeLayout.bind(oChart, null,
					[{"name":"CostElement"},{"name":"CostCenter"}],
					[{"name":"ActualCosts"},{"name":"PlannedCosts"}]),
					new Error("Invalid chart type: null"),
					"Invalid chartType parameter throws exception");
			QUnit.assert.throws(chartAPI.getChartTypeLayout.bind(oChart, "column",
					[{"name":"CostElement"},{}],
					[{"name":"ActualCosts"},{"name":"PlannedCosts"}]),
	...
	QUnit.test("Properties (valueAxisScale) test for non-stacked chart", function(assert) {
		...
		function initRenderCb() {
			...
			oChart.setValueAxisScale({
				scaleBehavior: ScaleBehavior.FixedScale,
				fixedScaleSettings: {
		...
		function setValueAxisScaleCb1() {
			oChart.getVizScales().forEach(function(v) {
				if (v.feed === 'valueAxis') {
		...
		function setValueAxisScaleCb2() {
			...
			oChart.setValueAxisScale({
				autoScaleSettings:{
					...
					syncWith:AutoScaleMode.VisibleData
			}});
		}
	...
	QUnit.test("init chart with vizScales and valueAxisScale", function(assert) {
		...
		function initRenderCb() {
			oChart.getVizScales().forEach(function(v) {
				if (v.feed === 'valueAxis') {
					assert.equal(v.min, 100, "the priority of valueAxisScale is higher than vizScales");
					assert.equal(v.max, 1000, "the priority of valueAxisScale is higher than vizScales");
				}
				if (v.feed === 'color') {
