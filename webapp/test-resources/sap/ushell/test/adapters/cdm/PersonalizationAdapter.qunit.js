...
sap.ui.define([
     ...
     *
     * @param {object} oResponses The Responses.
     */
    function createHttpClientStub (oResponses) {
        ...
        Object.keys(oResponses).forEach((sMethod) => {
            oHttpClientStub[sMethod] = sinon.stub().returns(
                new Promise((resolve, reject) => {
                    if (oResponses[sMethod].success) {
                        resolve(oResponses[sMethod]);
                    } else {
                        const oResponse = oResponses[sMethod];
                        const oError = new LaunchpadError(`Failed to execute HTTP request: ${oResponse.status} (${oResponse.status})`, oResponse);
                        reject(oError);
                    }
                })
            );
    ...
    QUnit.test("PersonalizationAdapter", function (assert) {
        const oSystem = createSystemStub();
        const oConfig = {
            config: {
        ...
        const oPersAdapter = new PersonalizationAdapter(oSystem, null, oConfig);
    ...
    QUnit.test("PersonalizationAdapter: construct an http client with the expected configuration", function (assert) {
        const oFnHttpClientStub = sinon.stub();
        const oConfig = {
            config: {
                storageResourceRoot: "path/to/storage/resource/location/"
            }
    ...
    QUnit.test(".getAdapterContainer", function (assert) {
        ...
        // act
        const oActualContainer = oAdapterInternals.getAdapterContainer(
            oFixture.input.config,
    ...
    QUnit.test(".getStorageResourceRoot", function (assert) {
        // -- Test proper usage
        const sExpectedStorageResourceRoot = "/path/to/resource/root";
        const sActualStorageResourceRoot = oAdapterInternals
    ...
    [{
        ...
        QUnit.test(`.getContainerPath - ${oFixture.testDescription}`, function (assert) {
            const sActualContainerPath = oAdapterInternals.getContainerPath(
                oFixture.input.config,
                oFixture.input.scope,
                oFixture.input.containerKey,
                oFixture.input.storageResourceRoot
    ...
    [{
        ...
        QUnit.test(`trimContainerKey is correct when ${oFixture.testDescription}`, function (assert) {
            const oLogMock = oTestUtils.createLogMock();

            if (oFixture.expectedErrorMessage) {
                oLogMock.error.apply(oLogMock, oFixture.expectedErrorMessage);
            }

            const sActualContainerKey = oAdapterInternals.trimContainerKey(oFixture.input.containerKey);
    ...
    [{
        ...
        QUnit.test(`.getContainerCategory is correct when ${oFixture.testDescription}`, function (assert) {
            // act
            const sContainerCategory = oAdapterInternals.getContainerCategory(oFixture.input.scope);
            // assert
            assert.equal(
                sContainerCategory,
                oFixture.expected,
    ...
    [{
        ...
        QUnit.test(`#save ${oFixture.testDescription}`, function (assert) {
            ...
            const sOriginalInputContainerData = extend({}, oFixture.input.containerData, true);
            ...
            oAdapterInternals
                ...
                .catch((oError) => {
                    afterSaving.call(this, false);
                    throw oError;
                })
                .then(afterSaving.bind(this, true));
        });
    });
    ...
    [{
        ...
        QUnit.test(`#load ${oFixture.testDescription}`, function (assert) {
            ...
            oAdapterInternals
                ...
                .fail(afterLoading.bind(this, false))
                .then(afterLoading.bind(this, true));
        });
    });
    ...
    [{
        ...
        QUnit.test(`#del ${oFixture.testDescription}`, function (assert) {
            ...
            oAdapterInternals
                ...
                .fail(afterDeleting.bind(this, false))
                .done(afterDeleting.bind(this, true));
        });
    });
    ...
    QUnit.test("#getItemKeys", function (assert) {
        const oFixture = {
            ...
            expected: {
                itemKeys: ["item-1", "item-2", "item-n", "ITEM#item-with-prefix"].map((sItemKey) => {
    ...
    [{
        ...
        QUnit.test(`setItemValue:  ${oFixture.testDescription}`, function (assert) {
            const oContainerData = extend({}, oFixture.input.oContainerData, true);
    ...
    [{
        ...
        QUnit.test(`getItemValue: ${oFixture.testDescription}`, function (assert) {
            const vActualValue = oAdapterInternals.getItemValue(
                oFixture.input.oContainerData,
                oFixture.input.key
            );
    ...
    [{
        ...
        QUnit.test(`#getHttpHeaderValue: ${oFixture.description}`, function (assert) {
            // Act
            const sHeaderValueAct = oAdapterInternals.getHttpHeaderValue(oFixture.input.aHeaders, oFixture.input.sHeaderName);
