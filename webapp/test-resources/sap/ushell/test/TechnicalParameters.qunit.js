...
sap.ui.define([
    ...
    [{
        ...
        QUnit.test(`getParameterValue: returns the correct array of values when ${oFixture.testDescription}`, async function (assert) {
            // Arrange
            const fnErrorLogStub = sinon.stub(Log, "error");

            const sNot = oFixture.expectedReject ? "" : " not";

            // Act
            await oTechnicalParameters.getParameterValue(
                ...
                .catch((oError) => {
                    assert.ok(oFixture.expectedReject, `Promise did${sNot} reject`);
                    assert.deepEqual(oError.message, oFixture.expectedResult, "The returned error was as expected");
                });
    ...
    QUnit.test("Calls getParameters with oFilterOptions=undefined", function (assert) {
        // Act
        const aResult = oTechnicalParameters.getParameters();
    ...
    QUnit.test("Calls getParameters with defined oFilterOptions", function (assert) {
        // Arrange
        const oFilterOptions = { injectFrom: "startupParameter" };
        const aExpectedResultNames = [
            "sap-fiori-id",
            "sap-ui-fl-max-layer",
            "sap-ui-fl-control-variant-id",
            "sap-ui-fl-version",
        ...
        // Act
        let aResults = oTechnicalParameters.getParameters(oFilterOptions);
        aResults = aResults.map((oResult) => {
            return oResult.name;
        });
        // Assert
