...
sap.ui.define([
    ...
    function fnParseShellHashNoUndefined (sHashFragment) {
        const oParsedHash = (new URLParsing()).parseShellHash(sHashFragment);
        return Object.keys(oParsedHash).reduce((oParsedNoUndefined, sKey) => {
    ...
    [{
        ...
        const sOccursOrNot = oFixture.expectedMatch ? "occurs" : "does not occur";

        QUnit.test(`match: match ${sOccursOrNot} when ${oFixture.testDescription}`, function (assert) {
            const oParseSrvc = new URLParsing();
            const oParsedShellHash = oParseSrvc.parseShellHash(oFixture.sShellHash);
            ...
            return oSearch.match(
                ...
                .then(
                    (oMatchResults) => {
                        ...
                        const bMatches = oMatchResults.matchResults.length > 0;
    ...
    [
        ...
        QUnit.test(`matchOne: (additional parameters)${JSON.stringify(oFixture)} `, function (assert) {
            const oParseSrvc = new URLParsing();
            const oArgs = oParseSrvc.parseShellHash(`#SO-aBC~CONTXT?${oFixture.url}`);

            oArgs.action = undefined;
            const oMatchResult = oSearch.matchOne(oArgs,
                {
                    deviceTypes: {
                        desktop: true,
                        phone: true,
    ...
    QUnit.test("matchOne: adds user default parameters to the in/out parameter", function (assert) {
        ...
        const oMissingUserDefaultRefsOut = {};
        const oMatchResult = oSearch.matchOne(oParsedShellHash, oTm, {}, oMissingUserDefaultRefsOut);
    ...
    (function () {
         ...
         */
        const oSampleInboundWithParameterMapping = {
        ...
        const oSampleInboundWithNoParameterMapping = deepExtend({}, oSampleInboundWithParameterMapping);
        ...
        const oSampleInboundWithParameterMappingAndHideIntentLink = deepExtend({}, oSampleInboundWithParameterMapping);
        oSampleInboundWithParameterMappingAndHideIntentLink.hideIntentLink = true;

        const oSampleInboundNoPars = {
            action: "toappnopar",
            deviceTypes: {
                desktop: true,
                phone: true,
        ...
        const oSampleInboundWithHideIntentLink = deepExtend({}, oSampleInboundNoPars);
        ...
        const oSampleInboundIgnoredPars = {
            semanticObject: "Action",
        ...
        const oSampleInboundWithExtendedUserDefault = {
            semanticObject: "Action",
            action: "toappstatesample",
            deviceTypes: {
                desktop: true,
        ...
        [{
            ...
            QUnit.test(`matchOne: returns expected result when sShellHash is ${oFixture.sShellHash}`, function (assert) {
                const oParseSrvc = new URLParsing();
                const oParsedShellHash = oParseSrvc.parseShellHash(oFixture.sShellHash);
                const oMatchResult = removeCountsAndSortString(oSearch.matchOne(
                    oParsedShellHash,
                    oFixture.oInbound,
                    {} /* oKnownUserDefaultRefsIn */,
                    {} /* oMissingUserDefaultRefsOut */
    ...
    [
        ...
        QUnit.test(`matchesFilter: matching ${fnValToString(oFixture.value)}`, function (assert) {
            const oMissingUserDefaultRefsOut = {};
            const bRes = oSearch.matchesFilter(oFixture.value, oFixture.filter, {}, oMissingUserDefaultRefsOut);
    ...
    [{
        ...
        QUnit.test(`checkAdditionalParameters: ${oFixture.name}`, function (assert) {
            const oEffectiveParams = (new URLParsing()).parseParameters(oFixture.params);
            const oResult = oSearch.checkAdditionalParameters(oFixture.Inbound, oEffectiveParams);
    ...
    [{
        ...
        QUnit.test(`addDefaultParameterValues: ${oFixture.name} case`, function (assert) {
            const aGotDefaultParameters = [];
            const oResult = oSearch.addDefaultParameterValues(oFixture.actual, oFixture.signatureParameters, {}, [], aGotDefaultParameters);
    ...
    QUnit.test("addDefaultParameterValues", function (assert) {
        ...
        const aGotDefaultParameterValues = [];

        const oIntentParamsPlusDefaults = oSearch.addDefaultParameterValues(oIntentParams, oTmParams, {}, [], aGotDefaultParameterValues);
    ...
    QUnit.test("_matchToInbound: matchesVirtualInbound is set when _isVirtualInbound returns true", function (assert) {
        ...
        const oTestInbound = { semanticObject: "Action", action: "search", signature: { parameters: {} } };

        sinon.stub(oVirtualInbounds, "isVirtualInbound")
            .returns(true);

        const oMatchResult = oSearch.matchOne(
            oParsedShellHash,
            oTestInbound,
            {}, /* refs in */
            {} /* refs out */
    ...
    [{
        ...
        QUnit.test(`_matchToInbound: returns expected sort string when ${oFixture.testDescription}`, function (assert) {
            ...
            const oMatchResult = oSearch.matchOne(oFixture.oParsedShellHash, oSampleInbound, {}, {});
    ...
    QUnit.test("_matchToInbound: returns expected sort string with CURCP=0 when contentProviderId is not defined in inbound", function (assert) {
        const expectedSortString = "AIDM=0 CURCP=0 x TECM=0 MTCH=000 MREQ=000 NFIL=000 NDEF=000 POT=000 RFRE=999 TECP=2";
        this.oParsedShellHash.params = {
            "sap-app-origin-hint": ["origin1"]
        };

        const oMatchResult = oSearch.matchOne(this.oParsedShellHash, this.oSampleInbound, {}, {});
    ...
    QUnit.test("_matchToInbound: returns sort string with CURCP=1 when contentProviderId === sap-app-origin-hint", function (assert) {
        const expectedSortString = "AIDM=0 CURCP=1 x TECM=0 MTCH=000 MREQ=000 NFIL=000 NDEF=000 POT=000 RFRE=999 TECP=2";
    ...
    QUnit.test("sortMatchingResultsDeterministic: prioritize result with the same origin", function (assert) {
        ...
        const oMatchResult1 = oSearch.matchOne(deepExtend({}, this.oParsedShellHash), deepExtend({}, this.oSampleInbound), {}, {});

        this.oSampleInbound.contentProviderId = "origin1";
        const oMatchResult2 = oSearch.matchOne(deepExtend({}, this.oParsedShellHash), deepExtend({}, this.oSampleInbound), {}, {});

        this.oSampleInbound.contentProviderId = "origin2";
        const oMatchResult3 = oSearch.matchOne(deepExtend({}, this.oParsedShellHash), deepExtend({}, this.oSampleInbound), {}, {});

        const aSortedResult = oSearch.sortMatchingResultsDeterministic([oMatchResult1, oMatchResult2, oMatchResult3]);
    ...
    QUnit.test("Not matched when there is no data origin in the content provider lookup", function (assert) {
        const sHash = "#SO-aBC?sap-system=ALIAS1";
        const oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
        const oLookup = {
            CP1: {
                ALIAS2: true,
                SYSTEM1: true
        ...
        const oGetLookupStub = sinon.stub().returns(new jQuery.Deferred().resolve(oLookup).promise());
    ...
    QUnit.test("Matched when data origin is in the content provider lookup", function (assert) {
        const sHash = "#SO-aBC?sap-system=ALIAS2";
        const oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
        const oLookup = {
            CP1: {
                ALIAS2: true,
    ...
    QUnit.test("Not matched when content provider is not matched", function (assert) {
        const sHash = "#SO-aBC?sap-app-origin=CP2";
        const oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
    ...
    QUnit.test("Not matched when sap-app-origin is not matched and sap-app-origin-hint is matched", function (assert) {
        const sHash = "#SO-aBC?sap-app-origin=CP2&sap-app-origin-hint=CP1";
        const oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
    ...
    QUnit.test("Matched when content provider is matched", function (assert) {
        const sHash = "#SO-aBC?sap-app-origin=CP1";
        const oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
    ...
    QUnit.test("Matched when content provider is empty", function (assert) {
        const sHash = "#SO-aBC?sap-app-origin=";
        const oParsedShellHash = this.oParseSrvc.parseShellHash(sHash);
