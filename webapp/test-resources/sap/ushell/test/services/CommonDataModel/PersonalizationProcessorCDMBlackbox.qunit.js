...
sap.ui.define([
    ...
    const S_TEST_SITE_NAME_DEFAULT = sap.ui.require.toUrl("sap/ushell/cdmSiteData/3.0/CDMData4BlackboxTests.json");
    const S_TEST_SITE_NAME_HCP = sap.ui.require.toUrl("sap/ushell/cdmSiteData/3.0/sampleHCPSitejson.json");
    ...
    const oExecuteActionFunctions = {
        ...
        addTile: function (oArgs) {
            ...
            this.oLPA.getCatalogs().done((aCatalogs) => {
                                        ...
                                        oPersonalizationProcessor
                                            ...
                                            .done((oPersonalizedSite2) => {
                                                ...
                                                if (oPersonalizationDelta && oFixture.expectedDelta) {
                                                    const oMaskedDelta = JSON.parse(JSON.stringify(oPersonalizationDelta).replace(rId, fMaskId));
                                                    nReplacementCount = 0;
                                                    const oMaskedFixDelta = JSON.parse(JSON.stringify(oFixture.expectedDelta).replace(rId, fMaskId));
