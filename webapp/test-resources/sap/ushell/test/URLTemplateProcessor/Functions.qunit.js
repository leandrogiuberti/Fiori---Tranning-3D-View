...
sap.ui.define([
    ...
    QUnit.module("url with arguments", {
        beforeEach: function () {
            const that = this;
            const sMockedURL = "https://user:pass@my.example.com:8080/sites/index.html?mode=admin&helpset=fcc&system=U1YCLNT120#ContentPackages-Manage&/Detail/New_catalog_15";
    ...
    QUnit.test("Can return the expected URL part", function (assert) {
        ...
        Object.keys(oExpectedUrlPart).forEach((sPart) => {
            const sExpectedResult = oExpectedUrlPart[sPart];
            const oFunctionArgs = [sPart];
            const sResult = oFunctions.applyFunctionInValueContext("url", oFunctionArgs);
