...
sap.ui.define(["sap/cards/ap/generator/pages/Application", "sap/cards/ap/generator/pages/FreeStyle", "sap/cards/ap/generator/pages/ObjectPage", "sap/ui/core/UIComponent", "sap/ui/model/odata/v2/ODataModel", "sap/ui/model/odata/v4/ODataModel"], function (sap_cards_ap_generator_pages_Application, sap_cards_ap_generator_pages_FreeStyle, sap_cards_ap_generator_pages_ObjectPage, UIComponent, ODataModel, ODataV4Model) {
  "use strict";

  const Application = sap_cards_ap_generator_pages_Application["Application"];
  const ODataModelVersion = sap_cards_ap_generator_pages_Application["ODataModelVersion"];
  const FreeStyle = sap_cards_ap_generator_pages_FreeStyle["FreeStyle"];
  ...
  jest.mock(sap.jest.resolvePath("sap/cards/ap/generator/helpers/IntegrationCardHelper"), () => {
    return {
      ...
      getObjectPageContext: jest.fn().mockReturnValue("SalesPlanUUID=guid'{{parameters.SalesPlanUUID}}',IsActiveEntity={{parameters.IsActiveEntity}}")
    };
  });
  describe("Application", () => {
    let windowSpy;
    const requestObjectMock = jest.fn();
    ...
    it("should return the same instance if one already exists and create a new once if does not exists", () => {
      const firstInstance = FreeStyle.createInstance(rootComponent);
      expect(firstInstance).toBeInstanceOf(Application);
      const secondInstance = FreeStyle.createInstance(rootComponent);
      expect(firstInstance).toBe(secondInstance);
    });
    it("should validate card generation for freestyle application", function () {
      try {
    ...
    it("should initialize and update the freeStyle Model for dialog", function () {
      try {
        const model = rootComponent.getModel();
        model.read = jest.fn((url, options) => {
          options.success({});
        });
        const freeStyleInstance = FreeStyle.createInstance(rootComponent);
        ...
        return Promise.resolve(freeStyleInstance.updateObjectContextFreeStyleModel()).then(function () {
          expect(freeStyleInstance.getFreeStyleModelForDialog().getData()).toMatchSnapshot();
        });
      } catch (e) {
        return Promise.reject(e);
    ...
    it("test for OData V4 based freestyle application", function () {
      try {
        const originalFetch = global.fetch;
        global.fetch = jest.fn(() => Promise.resolve({
          json: () => Promise.resolve({})
        }));
        rootComponent.getModel = jest.fn().mockReturnValue(new ODataV4Model({
        ...
        const firstInstance = FreeStyle.createInstance(rootComponent);
        return Promise.resolve(firstInstance.updateObjectContextFreeStyleModel()).then(function () {
          expect(firstInstance).toBeInstanceOf(Application);
  ...
  describe("Invalidate Card Generation", () => {
    ...
    it("should fetch the correct details in case of FCL Layout app or url has instance of `?` in string", () => {
      ...
      windowSpy.mockImplementation(() => ({
        hasher: {
          getHash: () => "test-intent&/testEntity('1234')?layout=TwoColumnsMidExpanded&sap-iapp-state=TASUI4HMOZD7OLBHKR0P0RGGG1PBWF1HGTRD33J1P"
        }
      }));
      const instance = ObjectPage.createInstance(rootComponent);
      const details = instance.fetchDetails();
