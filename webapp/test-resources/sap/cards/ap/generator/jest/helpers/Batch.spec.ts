...
describe("createURLParams", () => {
	test("should create URL parameters with multiple properties", () => {
		const properties = [{ name: "salesOrder" }, { name: "Net_amount" }, { name: "Created_at" }];
		const result = createURLParams(properties);
		expect(result).toEqual({ $select: "salesOrder,Net_amount,Created_at" });
	});

	test("should create URL parameters with a single property", () => {
		const properties = [{ name: "Created_at" }];
		const result = createURLParams(properties);
		expect(result).toEqual({ $select: "Created_at" });
	});
