...
describe("Card Actions", () => {
	...
	test("addActionToCardManifest : Add the actions to card manifest for OData V2 Card", async () => {
		...
		await addActionToCardManifest(cardManifest, controlProperties);
	...
	test("getCardActionInfo: Test the Generation of the Card Action information for CardGenerator Model when there exists actions for entityType", async () => {
		...
		ObjectPage.createInstance(component);
		const cardActionsInfo = await getCardActionInfo({}, undefined, cardManifest);
...
describe("getActionParameterConfigurationV2", () => {
	...
	it("returns configured action parameters", async () => {
		...
		ObjectPage.createInstance(component);
		const result = await getActionParameterConfigurationV2(mockAdditionalParameter);
