...
sap.ui.define(["sap/cards/ap/generator/app/handlers/FreeStyle", "sap/cards/ap/generator/pages/Application", "sap/cards/ap/generator/pages/FreeStyle", "sap/ui/core/Element"], function (freeStyleHandlers, sap_cards_ap_generator_pages_Application, sap_cards_ap_generator_pages_FreeStyle, CoreElement) {
  "use strict";
  ...
  describe("onServiceChange", () => {
    ...
    test("should change the service", () => {
      ...
      freeStyleHandlers.onServiceChange(event);
  ...
  describe("onEntitySetPathChange, onContextPathChange and applyServiceDetails functions of the controller", () => {
        ...
        getMetaModel: jest.fn().mockReturnValue({
          getODataEntityContainer: jest.fn().mockReturnValue({
            entitySet: [{
              ...
              name: "C_SalesPlanDimnSelectionTP"
            }]
          }),
          ...
          return {
            getModel: jest.fn().mockImplementation(modelName => {
              if (modelName === "freeStyle") {
                return Application.getInstance().getFreeStyleModelForDialog();
              }
              return {
                setProperty: setPropertyMock,
                setData: jest.fn(),
    ...
    test("should change the entity set path", function () {
      try {
        const event = {
          getSource: jest.fn().mockReturnValue({
        ...
        const freeStyleDialogModel = Application.getInstance().getFreeStyleModelForDialog();
        expect(freeStyleDialogModel.getData()).toMatchSnapshot();
        return Promise.resolve(freeStyleHandlers.onEntitySetPathChange(event)).then(function () {
          expect(freeStyleDialogModel.getData()).toMatchSnapshot();
        });
      } catch (e) {
    ...
    test("Save and Reopen Scenario and should open the existing entity related data", function () {
      try {
    ...
    test("should change the context path", function () {
      try {
        const event = {
          getSource: jest.fn().mockReturnValue({
            getValue: jest.fn().mockReturnValue("C_SalesPlanTP(SalesPlanUUID=guid'fa163e3c-f83d-1ee9-a0a1-8f912f73bf72',IsActiveEntity=false)")
          })
        };
        FreeStyle.createInstance(rootComponent);
        const freeStyleDialogModel = Application.getInstance().getFreeStyleModelForDialog();
        return Promise.resolve(freeStyleHandlers.onContextPathChange(event)).then(function () {
          expect(freeStyleDialogModel.getData()).toMatchSnapshot();
        });
      } catch (e) {
        return Promise.reject(e);
    ...
    test("should apply all the changes", function () {
      try {
        FreeStyle.createInstance(rootComponent);
        const freeStyleDialogModel = Application.getInstance().getFreeStyleModelForDialog();
        freeStyleDialogModel.setProperty("/isEntityPathChanged", true);
        return Promise.resolve(freeStyleHandlers.applyServiceDetails()).then(function () {
          expect(freeStyleDialogModel.getData()).toMatchSnapshot();
...
//# sourceMappingURL=FreeStyle.spec.js.map