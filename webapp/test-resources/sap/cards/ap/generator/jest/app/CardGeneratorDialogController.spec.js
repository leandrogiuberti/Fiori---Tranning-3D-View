...
sap.ui.define(["sap/base/Log", "sap/cards/ap/generator/app/CardGeneratorDialogController", "sap/cards/ap/generator/helpers/FooterActions", "sap/cards/ap/generator/helpers/I18nHelper", "sap/cards/ap/generator/helpers/IntegrationCardHelper", "sap/cards/ap/generator/helpers/Transpiler", "sap/cards/ap/generator/odata/ODataUtils", "sap/cards/ap/generator/pages/Application", "sap/cards/ap/generator/pages/ObjectPage", "sap/cards/ap/generator/utils/CommonUtils", "sap/m/ComboBox", "sap/m/MessageBox", "sap/m/MessageToast", "sap/m/ToggleButton", "sap/m/VBox", "sap/ui/base/Event", "sap/ui/core/Element", "sap/ui/core/Fragment", "sap/ui/core/Item", "sap/ui/core/UIComponent", "sap/ui/core/library", "sap/ui/layout/Splitter", "sap/ui/model/json/JSONModel", "sap/ui/thirdparty/jquery"], function (Log, sap_cards_ap_generator_app_CardGeneratorDialogController, sap_cards_ap_generator_helpers_FooterActions, sap_cards_ap_generator_helpers_I18nHelper, sap_cards_ap_generator_helpers_IntegrationCardHelper, sap_cards_ap_generator_helpers_Transpiler, ODataUtils, sap_cards_ap_generator_pages_Application, sap_cards_ap_generator_pages_ObjectPage, sap_cards_ap_generator_utils_CommonUtils, ComboBox, MessageBox, MessageToast, ToggleButton, VBox, Event, CoreElement, Fragment, Item, UIComponent, sap_ui_core_library, Splitter, JSONModel, jQuery) {
  "use strict";
  ...
  const toggleOffAdvancedPanel = sap_cards_ap_generator_app_CardGeneratorDialogController["toggleOffAdvancedPanel"];
  ...
  describe("handleCriticalityAction", () => {
    ...
    beforeEach(() => {
      ...
      oModel.getProperty = jest.fn(path => {
        const data = {
          "/configuration/groups/0/items/0": {
            name: "to_BusinessPartner",
            navigationProperty: "BusinessPartnerRole"
          }
        };
        return data[path];
      });
  ...
  describe("Validate criticality and UoM, delete trend", () => {
    ...
    test("applyCriticality - Navigation property formatting gets applied correctly", () => {
      ...
      const navEntry = updated.find(entry => entry.name === "SalesOrder");
      ...
      expect(navEntry.navigationalPropertiesForId).toEqual([{
        name: "Amount"
      }]);
  ...
  describe("Card Action Handlers", () => {
    ...
    test("okPressed : should show success message on successful save", function () {
      try {
        ...
        jQueryAjaxSpy.mockImplementation(({
          success
        }) => {
          success();
        });
    ...
    test("onAddedActionTitleChange : Test the handler when added actions title property is changed to an empty value", function () {
      try {
        const addedActions = [{
          title: "",
          titleKey: "",
  ...
  describe("onPropertySelection", () => {
    let setPropertyMock;
    let getPreviewItemsMock = getPreviewItems;
    let windowSpy;
    let coreElementGetElementByIdSpy;
    const oDialogModel = new JSONModel({
      configuration: {
  ...
  describe("updateHeaderNavigationSelection - when navigationValue is of date type.", () => {
    ...
    test("Validate card header to check formatting is applied correctly", () => {
      ...
      expect(setPropertyMock).toHaveBeenNthCalledWith(3, "/configuration/navigationValue", "DraftAdministrativeData/ProcessingStartDateTime");
      ...
      expect(setPropertyMock).toHaveBeenNthCalledWith(5, "/configuration/mainIndicatorStatusKey", "DraftAdministrativeData/ProcessingStartDateTime");
    });
  });
  ...
  describe("handleFormatterUomAction", () => {
    ...
    it("should handle formatter UoM action correctly", () => {
      ...
      expect(setPropertyMock).toHaveBeenNthCalledWith(2, "/configuration/advancedFormattingOptions/targetFormatterProperty", "propertyName/navProp");
      expect(setPropertyMock).toHaveBeenNthCalledWith(3, "/configuration/advancedFormattingOptions/sourceUoMProperty", "propertyName/navProp");
  ...
  describe("Toggle Advanced Settings", () => {
    let coreElementGetElementByIdSpy;
    let removeSplitterContentSpy;
    let insertSplitterContentSpy;
    let getSplitterContentSpy;
    ...
    beforeAll(() => {
      ...
      removeSplitterContentSpy = jest.spyOn(Splitter.prototype, "removeContentArea");
      insertSplitterContentSpy = jest.spyOn(Splitter.prototype, "insertContentArea");
      getSplitterContentSpy = jest.spyOn(Splitter.prototype, "getContentAreas");
    });
    afterAll(() => {
      coreElementGetElementByIdSpy.mockRestore();
    ...
    test("removeContentArea should be called", function () {
      try {
  ...
  describe("setDefaultCardPreview", () => {
    let comboBox;
    let defaultIntegrationCard;
    let adaptiveCardContainer;
    let customIntegrationCard;
    beforeEach(() => {
      comboBox = {
      ...
      const oDialog = {
        getModel: jest.fn().mockReturnValue(new JSONModel({
          hosts: [{
            key: "int-default",
            width: "500px",
            height: "auto"
  ...
  describe("onPropertyFormatting and onFormatTypeSelection", () => {
    ...
    test("should append navigation key to mainIndicatorStatusKey when both are set and '/' not present", function () {
      try {
        // Arrange
  ...
  describe("Set and Reset information state when item is added to preview", () => {
    ...
    test("Resets info state when property exists in preview", function () {
      try {
        const setValueStateMock = jest.fn();
        const setValueStateTextMock = jest.fn();
        const oEvent = {
        ...
        getPreviewItemsMock.mockReturnValue(["net_amount"]);
        return Promise.resolve(CardGeneratorDialogController.onStateIndicatorSelection(oEvent)).then(function () {
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueState", ValueState.None);
    ...
    test("Sets value state to Infromation when property does not exist in preview", function () {
      try {
        const setValueStateMock = jest.fn();
        const setValueStateTextMock = jest.fn();
        const oEvent = {
        ...
        return Promise.resolve(CardGeneratorDialogController.onStateIndicatorSelection(oEvent)).then(function () {
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueState", ValueState.Information);
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueStateText", "The card preview doesn't show this value because the field isn't configured.");
        });
      } catch (e) {
        return Promise.reject(e);
      }
  ...
  describe("Set and Reset information state when navigation property is added to preview", () => {
    ...
    test("Resets info state when navigational property exists in preview", function () {
      try {
        const setValueStateMock = jest.fn();
        const setValueStateTextMock = jest.fn();
        const oEvent = {
        ...
        return Promise.resolve(CardGeneratorDialogController.onStateIndicatorSelection(oEvent)).then(function () {
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueState", ValueState.None);
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueStateText", "");
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/navigationValueState", ValueState.None);
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/navigationValueStateText", "");
        });
      } catch (e) {
        return Promise.reject(e);
      }
    ...
    test("Sets value state to Infromation when property does not exist in preview", function () {
      try {
        ...
        return Promise.resolve(CardGeneratorDialogController.onStateIndicatorSelection(oEvent)).then(function () {
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueState", ValueState.Information);
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/valueStateText", "The card preview doesn't show this value because the field isn't configured.");
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/navigationValueState", ValueState.Information);
          expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/unitOfMeasures/0/navigationValueStateText", "The card preview doesn't show this value because the field isn't configured.");
        });
      } catch (e) {
        return Promise.reject(e);
      }
