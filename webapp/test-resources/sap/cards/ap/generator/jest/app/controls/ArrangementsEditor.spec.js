...
sap.ui.define(["sap/cards/ap/generator/app/controls/ArrangementsEditor", "sap/cards/ap/generator/config/TextArrangementOptions", "sap/cards/ap/generator/helpers/IntegrationCardHelper", "sap/m/Button", "sap/m/ComboBox", "sap/m/CustomListItem", "sap/m/List", "sap/m/Text", "sap/ui/core/Element", "sap/ui/core/library", "sap/ui/model/json/JSONModel", "sap/ui/core/date/Gregorian"], function (ArrangementsEditor, sap_cards_ap_generator_config_TextArrangementOptions, sap_cards_ap_generator_helpers_IntegrationCardHelper, Button, ComboBox, CustomListItem, List, Text, CoreElement, sap_ui_core_library, JSONModel, sap_ui_core_date_Gregorian) {
  "use strict";
  ...
  describe("ArrangementsEditor", () => {
    ...
    test("handleComboBoxEvents: sets valueState and valueStateText to ValueState.None for textArrangements when the property is in preview", () => {
      ...
      expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/textArrangements/0/valueState", ValueState.None);
    ...
    test("handleComboBoxEvents: sets valueState and valueStateText to ValueState.Information for textArrangements when the property is not in preview", () => {
      ...
      const setValueStateTextMock = jest.fn();
      const refreshMock = jest.fn();
      getPreviewItems.mockReturnValue(["so_id"]);
      const arrangementsEditor = {
        getModel: jest.fn().mockImplementation(modelName => {
          if (modelName === "i18n") {
            return {
      ...
      const instance = new ArrangementsEditor(settings);
      instance.handleComboBoxEvents(event, arrangementsEditor, false, true);
      expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/textArrangements/0/valueState", ValueState.Information);
      expect(setPropertyMock).toHaveBeenCalledWith("/configuration/advancedFormattingOptions/textArrangements/0/valueStateText", "The card preview doesn't show this value because the field isn't configured.");
    });
