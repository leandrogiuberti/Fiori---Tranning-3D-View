 ...
 */
import { getSeparatedProperties, updateExpandQuery, updateSelectQuery } from "sap/cards/ap/common/helpers/QueryBuilder";
...
describe("Check formation of header and select query for the card", () => {
	it("should append select query parameters", () => {
		...
		const contentUrlWithSelect = updateSelectQuery(contentUrl, integrationCardManifest as unknown as CardManifest);
		const expected =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?$select=bp_id,created_at,node_key,so_id,currency_code,gross_amount,net_amount,lifecycle_status";
	...
	it("should append only the select query parameters when expand parameters are missing", () => {
		const contentUrl =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?{{parameters._contentSelectQuery}}";
		const integrationCardManifest = {
			_version: "1.15.0",
			"sap.card": {
				extension: "module:sap/manifests/ap/common/extensions/BaseIntegrationCardExtension",
		...
		const contentUrlWithSelect = updateSelectQuery(contentUrl, integrationCardManifest as unknown as CardManifest);
		const contentUrlWithSelectAndExpand = updateExpandQuery(contentUrlWithSelect, integrationCardManifest as unknown as CardManifest);
		const expected =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?$select=bp_id,created_at,node_key,so_id,currency_code,gross_amount,net_amount,lifecycle_status";
	...
	it("should append both select and expand query parameters, even if they are not in the contentUrl", () => {
		...
		const contentUrlWithSelectAndExpand = updateExpandQuery(contentUrlWithSelect, integrationCardManifest as unknown as CardManifest);
		const expected =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?$select=bp_id,created_at,node_key,so_id,currency_code,gross_amount,net_amount,lifecycle_status&$expand=to_Currency,to_BillingStatus,to_LifecycleStatus";
		expect(contentUrlWithSelectAndExpand).toEqual(expected);
	});

	it("should append both select and expand query parameters when both are present and are part of contentUrl", () => {
		const contentUrl =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?{{parameters._contentSelectQuery}}{{parameters._contentExpandQuery}}";
		const integrationCardManifest = {
			_version: "1.15.0",
			"sap.card": {
				extension: "module:sap/manifests/ap/common/extensions/BaseIntegrationCardExtension",
		...
		const contentUrlWthSelect = updateSelectQuery(contentUrl, integrationCardManifest as unknown as CardManifest);
		const contentUrlWthSelectAndExpand = updateExpandQuery(contentUrlWthSelect, integrationCardManifest as unknown as CardManifest);
		const expected =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?$select=bp_id,created_at,to_BillingStatus/Status_Text,node_key,so_id,currency_code,gross_amount,net_amount,lifecycle_status&$expand=to_Currency,to_BusinessPartner,to_BillingStatus,to_LifecycleStatus";
	...
	it("should only add expand parameters when _contentSelectQuery and _headerSelectQuery are empty and both _contentSelectQuery and _contentExpandQuery are part of content url", () => {
		...
		const contentUrlWithSelectAndExpand = updateExpandQuery(contentUrlWithSelect, integrationCardManifest as unknown as CardManifest);
		const expected =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?$expand=to_Currency,to_BusinessPartner,to_BillingStatus,to_LifecycleStatus";
		expect(contentUrlWithSelectAndExpand).toEqual(expected);
	});
	...
	it("should add both expand and select when value exists for both", () => {
		...
		const contentUrlWithSelectAndExpand = updateExpandQuery(contentUrlWithSelect, integrationCardManifest as unknown as CardManifest);
		const expected =
			"C_STTA_SalesOrder_WD_20(node_key=guid'fa163ee4-7bdd-1ee8-b1ff-d3c5a4e5236a',IsActiveEntity=true)?$select=gross_amount,net_amount,tax_amount,to_BillingStatus&$expand=to_BillingStatus($select=status_code,Status_Text),to_LifecycleStatus,to_Currency";
		expect(contentUrlWithSelectAndExpand).toEqual(expected);
	});
});
...
describe("getSeparatedProperties", () => {
	it("Both normal navigation properties and navigation properties with select should be separated", () => {
		const expandQueryParams = [
			"to_BillingStatus($select=status_code)",
			"to_LifecycleStatus",
			"to_Currency",
			"to_BillingStatus($select=status_code",
			"Status_Text)"
	...
	it("Multiple navigation properties with select should be separated", () => {
		const expandQueryParams = [
			"to_BillingStatus($select=status_code",
			"Status_Text)",
			"to_BusinessPartner($select=id",
			"name",
			"address)",
			"to_sales($select=netAmount",
			"totalAmount",
			"taxAmount)"
		];
		const result = getSeparatedProperties(expandQueryParams);
		...
		expect(result).toEqual({
			navigationPropertiesWithSelect: [
				"to_BillingStatus($select=status_code,Status_Text)",
				"to_BusinessPartner($select=id,name,address)",
				"to_sales($select=netAmount,totalAmount,taxAmount)"
			],
