sap.ui.define([
	...
	"sap/ui/vbm/Routes",
	"sap/ui/vbm/Route"
], function (Controller, Spot, Spots, SemanticType, Routes, Route) {
	"use strict";
	return Controller.extend("vbm-regression.tests.54.controller.App", {
		...
		onInit: function () {
			...
			oGeoMap.addVo(new Spots("otherSpots", {
				minSel: "0",
				maxSel: "0",
				items: [
						new Spot("otherSpot", {
			...
			oGeoMap.addVo(new Routes("routes", {
				items: {
					path: "/Routes",
					template: new Route({
						position: '{pos}',
		...
		onSetWarningState: function () {
			...
			for (var nI = 0; nI < myOData.Spots.length; ++nI) {
				myOData.Spots[nI].type = SemanticType.Warning;
			}
			...
			for (var nJ = 0; nJ < otherSpots.length; ++nJ) {
				otherSpots[nJ].setType(SemanticType.Warning);
			}
		},
