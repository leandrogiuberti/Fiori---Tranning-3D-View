...
sap.ui.define([
	...
	return function runRTECommonTests(sEditorType) {
		...
		var fnTestIframeSanitization = async function (assert) {
		...
		QUnit.test("Control Creation and Destruction", async function(assert){ await fnTestControlLifecycle.bind(this)(assert); });
		QUnit.test("Control Creation and Size", async function(assert){await fnTestControlSize.bind(this)(assert); });
		QUnit.test("getNativeApi", async function(assert){ await fnTestNativeApi(assert); });
		QUnit.test("Check InitialValue", async function(assert){ await fnTestInitialValue(assert); });
		QUnit.test("setValue", async function(assert){ await fnTestSetValue(assert); });
		...
		QUnit.test("setValue sanitization", async function(assert){ await fnTestSanitization(assert);});
		QUnit.test("setValue sanitization", async function(assert){ await fnTestIframeSanitization(assert);});
		QUnit.test("setValue sanitization", async function(assert){ await fnTestSanitizationOnTinyMCEChange(assert); });
		QUnit.test("readonly link target modification", async function(assert){ await fnTestLinkTargets(assert);});
		...
		QUnit.test("indexOfCustomButton", function (assert) {
			...
			//Act
			this.oRichTextEditor.addCustomButton(oButton1);
			this.oRichTextEditor.addCustomButton(oButton2);

			// Assert
			index = this.oRichTextEditor.indexOfCustomButton(oButton1);
			assert.ok(oRTEAggregationSpy.calledWithExactly("customButtons", oButton1), "RTE should call indexOfAggregation.");
