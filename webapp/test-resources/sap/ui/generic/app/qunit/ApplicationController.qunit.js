...
sap.ui.define([
		...
		beforeEach: function () {
			...
			this.oModel.getMetaModel = function () {
				return {
					...
					getODataProperty: function (oEntityType, sPropertyName) {
						return oEntityType.property.find(function (oProperty) {
	...
	QUnit.test("Testing _setSelect method", function (assert) {
		// Setting up the entity sets, types and their relations
		var mEntitySets = {
			"SalesOrder": {
				name: "SalesOrder",
				entityType: "SalesOrderType"
			}
		};
		...
		var mEntityTypes = {
			SalesOrderType: {
				...
				associations: {
					"to_ProductCategory": {type: "ProductCategoryType"},
					"to_Supplier": {type: "SupplierType"}
				}
			},
			ProductCategoryType: {
				...
				associations: {
					"to_MainCategory": {type: "MainCategoryType"}
				}
			},
			MainCategoryType: {
		...
		// Preparing the side effect
		var oSideEffect = {
			TargetEntities: [
				{NavigationPropertyPath: "to_Supplier"},
				{NavigationPropertyPath: "to_Dummy"} // Invalid
			],
			TargetProperties: [
				{String: "to_ProductCategory/to_MainCategory/MainCategoryName"},
				{String: "to_Supplier/SupplierName"},
				{String: "Weight"},
				{String: "WeightUnit"},
				{String: "to_Supplier/DummySupplier"}, // Invalid
				{String: "DummyProperty"} // Invalid
			]
		};
		...
		var oController = new ApplicationController(oModel);

		// Act
		oController._setSelect(oSideEffect, mParameters, mRequests, oContext);

		// Assert
		var aExpand = mParameters.readParameters.$expand.split(",");
		var aSelect = mParameters.readParameters.$select.split(",");

		assert.deepEqual(aExpand, [
			"to_Supplier",
			"to_ProductCategory/to_MainCategory"
		], "The $expand parameter should only have the valid navigation properties");

		assert.deepEqual(aSelect, [
			"to_Supplier",
			"to_ProductCategory/to_MainCategory/MainCategoryName",
			"to_Supplier/SupplierName",
			"Weight",
			"WeightUnit"
		], "The $select parameter should only have the valid target properties");
	});
});