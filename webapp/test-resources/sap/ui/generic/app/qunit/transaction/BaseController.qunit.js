...
sap.ui.define([
	...
	QUnit.test("triggerSubmitChanges, strict header", function (assert) {
		var done = assert.async();
		var oController, oModel, bSubmit = false, mParams = { bStrict: true };
		...
		oController.triggerSubmitChanges(mParams).then(function () {
			assert.equal(mParams.changeHeaders.prefer, "handling=strict")
			oController.destroy();
			done();
		});
	});
	...
	QUnit.test("triggerSubmitChanges should not clear timeout if keepDraftMergeTimer is true", function (assert) {
		var done = assert.async();
		var oController, oModel, bSubmit = false, mParams = {};
		...
		oController._oDraftMergeTimer.nTimeoutID = "id";
		mParams.keepDraftMergeTimer = true;
	...
	QUnit.test("hasClientMessages returning messages", function (assert) {
		var done = assert.async();
		var fGetData = Messaging.getMessageModel().getData;
		Messaging.getMessageModel().getData = function () {
			return [{
				processor: {
		...
		oBaseController.hasClientMessages().then(function () {
			assert.ok(false);
			oBaseController.destroy();
			done();
		}, function () {
		...
		Messaging.getMessageModel().getData = fGetData;
	});
	...
	*/
	...
	QUnit.test("triggerSubmitChanges with forceSubmit", function (assert) {
		...
		this.oModel = oModel; // to provide the type-information check

		oModel.metadataLoaded().then(function(){
			this.oModel.create("/SalesOrder",
				{
			...
			var oController = new BaseController(this.oModel);
