sap.ui.define([
	...
	return Controller.extend("singleFile.controller.App", {
		...
		onPressLoadSampleFile: function(event) {
			var view = this.getView();
			var viewer = view.byId("viewer");
			loadModelIntoViewer(viewer, "models/cooper.vds", "vds4");
		},
		...
		onChangeFileUploader: function(event) {
			...
			if (localFile) {
				...
				if (index >= 0 && index < fileName.length - 1) {
					...
					if (sourceType === "vds") {
						// Source type 'vds' is using old DVL rendering engine which is now deprecated.
						// Use 'vds4' for new Threejs based rendering engine
						sourceType = "vds4"
					}
