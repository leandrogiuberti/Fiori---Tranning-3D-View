sap.ui.define([
	"sap/ui/core/mvc/Controller",
	"sap/ui/model/json/JSONModel",
	"sap/ui/comp/sample/smartlink/example_05/formatter",
	"sap/ui/comp/navpopover/LinkData",
	"sap/ui/comp/navpopover/SemanticObjectController",
	"sap/ui/core/Title",
	"sap/ui/layout/form/SimpleForm",
	"sap/m/Text"
], function(Controller, JSONModel, formatter, LinkData, SemanticObjectController, Title, SimpleForm, Text) {
	"use strict";

	return Controller.extend("sap.ui.comp.sample.smartlink.example_05.Example", {

		formatter: formatter,

		onInit: function() {
			this.getView().bindElement("/ProductCollection('1239102')");
		},

		onNavigationTargetsObtainedMainAndContent: function(oNavigationTargets) {
			// new 'navigationTargetsObtainedCallback' handling (since 1.126)
			return Promise.resolve({
				mainNavigationId: "Supplier",
				mainNavigation: new LinkData({
					key: "IDApplicationSpecificFactSheet",
					href: "#/sample/sap.ui.comp.sample.smartlink.factSheetPage/preview",
					text: "Application Specific Fact Sheet",
					target: "_blank"
				}),
				actions: [],
				extraContent: new SimpleForm({
					maxContainerCols: 1,
					content: [
						new Title({
							text: "Detailed information"
						}), new Text({
							text: "By pressing on the links below a new browser window with image of product will be opened."
						})
					]
				})
			});
			// Old navigationTargetsObtained event handling (before 1.126)
			// oParameters.show("Supplier", new LinkData({
			// 	key: "IDApplicationSpecificFactSheet",
			// 	href: "#/sample/sap.ui.comp.sample.smartlink.factSheetPage/preview",
			// 	text: "Application Specific Fact Sheet",
			// 	target: "_blank"
			// }), [], new SimpleForm({
			// 	maxContainerCols: 1,
			// 	content: [
			// 		new Title({
			// 			text: "Detailed information"
			// 		}), new Text({
			// 			text: "By pressing on the links below a new browser window with image of product will be opened."
			// 		})
			// 	]
			// }));
		},

		onNavigationTargetsObtainedMainAndAction: function() {
			return Promise.resolve({
				mainNavigationId: "Supplier",
				mainNavigation: new LinkData({
					key: "IDApplicationSpecificFactSheet",
					href: "#/sample/sap.ui.comp.sample.smartlink.factSheetPage/preview",
					text: "Application Specific Fact Sheet",
					target: "_blank"
				}),
				actions: [
					new LinkData({
						key: "IDApplicationSpecificLink",
						href: "#/sample/sap.ui.comp.sample.smartlink.productPage/preview",
						text: "Application Specific Link",
						target: "_blank"
					})
				]
			});
		},

		onNavigationTargetsObtainedContentAndAction: function() {
			return Promise.resolve({
				actions: [
					new LinkData({
						key: "IDApplicationSpecificLink",
						href: "#/sample/sap.ui.comp.sample.smartlink.productPage/preview",
						text: "Application Specific Link",
						target: "_blank"
					})
				],
				extraContent: new SimpleForm({
					maxContainerCols: 1,
					content: [
						new Title({
							text: "Detailed information"
						}), new Text({
							text: "By pressing on the links below a new browser window with image of product will be opened."
						})
					]
				})
			});
		},

		onNavigationTargetsObtainedMainAndContentAndAction: function() {
			return Promise.resolve({
				mainNavigationId: "Supplier",
				mainNavigation: new LinkData({
					key: "IDApplicationSpecificFactSheet",
					href: "#/sample/sap.ui.comp.sample.smartlink.factSheetPage/preview",
					text: "Application Specific Fact Sheet",
					target: "_blank"
				}),
				actions: [
					new LinkData({
						key: "IDApplicationSpecificLink",
						href: "#/sample/sap.ui.comp.sample.smartlink.productPage/preview",
						text: "Application Specific Link",
						target: "_blank"
					})
				],
				extraContent: new SimpleForm({
					maxContainerCols: 1,
					content: [
						new Title({
							text: "Detailed information"
						}), new Text({
							text: "By pressing on the links below a new browser window with image of product will be opened."
						})
					]
				})
			});
		},

		onNavigationTargetsObtainedOnlyContent: function() {
			return Promise.resolve({
				extraContent: new SimpleForm({
					maxContainerCols: 1,
					content: [
						new Title({
							text: "Detailed information"
						}), new Text({
							text: "By pressing on the links below a new browser window with image of product will be opened."
						})
					]
				})
			});
		},

		onNavigationTargetsObtainedOnlyMain: function() {
			return Promise.resolve({
				mainNavigationId: "Supplier",
				mainNavigation: new LinkData({
					key: "IDApplicationSpecificFactSheet",
					href: "#/sample/sap.ui.comp.sample.smartlink.factSheetPage/preview",
					text: "Application Specific Fact Sheet",
					target: "_blank"
				})
			});
		},

		onNavigationTargetsObtainedNothing: function(oEvent) {
			return Promise.resolve({
				mainNavigationId: "Supplier"
			});
		},

		onNavigationTargetsObtainedOnlyMainNavigationId: function(oEvent) {
			return Promise.resolve({
				mainNavigationId: "Supplier",
				mainNavigation: new LinkData({
					key: "IDApplicationSpecificFactSheet",
					href: "#/sample/sap.ui.comp.sample.smartlink.factSheetPage/preview",
					text: "Application Specific Fact Sheet",
					target: "_blank"
				}),
				actions: [
					new LinkData({
						key: "IDApplicationSpecificLink",
						href: "#/sample/sap.ui.comp.sample.smartlink.productPage/preview",
						text: "Application Specific Link",
						target: "_blank"
					})
				],
				extraContent: new SimpleForm({
					maxContainerCols: 1,
					content: [
						new Title({
							text: "Detailed information"
						}), new Text({
							text: "By pressing on the links below a new browser window with image of product will be opened."
						})
					]
				})
			});
		},

		onBeforePopoverOpens: function(oEvent) {
			var oParameters = oEvent.getParameters();
			setTimeout(function() {
				oParameters.setSemanticAttributes({
					Name: "Power Projector 4713",
					ProductId: "0000001",
					SpecificId: "1234"
				}, oParameters.semanticObject);
				oParameters.open();
			}, 500);
		}
	});
});
