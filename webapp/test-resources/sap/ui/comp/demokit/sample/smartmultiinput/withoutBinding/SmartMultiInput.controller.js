sap.ui.define([
	...
	'sap/m/MessageToast',
	'sap/m/Token'
], function (Controller, MockServer, BindingMode, ODataModel, MessageToast, Token) {
	"use strict";
	...
	return Controller.extend("sap.ui.comp.sample.smartmultiinput.withoutBinding.SmartMultiInput", {
		onInit: function () {
			...
			this.byId("form").getGroups()[0].getGroupElements().forEach(function (oGroupElement) {
				...
				if (oSmartMultiInput.getId().includes("SMIwithInitialTokens")) {
					var createToken = (key, text) => {
						return new sap.m.Token({ key, text });
					};
					//Token which is not the part of MultiInput
					oSmartMultiInput.addAggregation('initialTokens',
						createToken('key1', 'text1')
					);
					//Token which is part of Multiinput
					oSmartMultiInput.addAggregation('initialTokens',
						createToken('CAT1', 'Category 1')
					);
				}
			});
		},
