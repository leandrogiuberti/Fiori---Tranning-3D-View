	...
	return Controller.extend("root.Example", {
		...
		onInit: function() {
			...
			oModel.getMetaModel().loaded().then(function(){
				...
				aColumnKeysOrdered.forEach(function(sColumnKey, iIndex) {
					...
					var bIsSorted = this.oColumnKey2ColumnMap[sColumnKey].getSortOrder() !== "None";

					/** @deprecated As of version 1.120 */
					if (!this.oColumnKey2ColumnMap[sColumnKey].getSorted()) {
						bIsSorted = false;
					}
					...
					if (bIsSorted) {
						aSorter.push(new Sorter(sColumnKey, oColumn.getSortOrder() === coreLibrary.SortOrder.Descending));
					}
		...
		_applySorts: function() {
			...
			aSortItems.forEach(function(oSortItem) {
				...
				if (oSorter) {
					oSorter.bDescending = oSortItem.operation === coreLibrary.SortOrder.Descending;
				} else {
					aSorters.push(new Sorter(oSortItem.columnKey, oSortItem.operation === coreLibrary.SortOrder.Descending));
				}
			}, this);
		...
		onP13nDialogPress: function() {
			this.loadFragment({
				name: "root.PersonalizationDialog"
