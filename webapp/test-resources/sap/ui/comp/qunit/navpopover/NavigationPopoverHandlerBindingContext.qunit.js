...
], function(Log, FakeFlpConnector, NavigationPopoverHandler, Button, Element, MockServer, ODataModel, SimpleForm, Title, VBox, Text, SemanticObjectController, nextUIUpdate) {
	"use strict";
				...
				events: {
					change: function(oEvent) {
						...
						oNavigationPopoverHandler.openPopover().then(function() {
							...
							assert.equal(Element.getElementById(oNavigationPopoverHandler._oPopover._getContentContainer().getExtraContent()).getItems().length, 1);
			...
			events: {
				change: function(oEvent) {
					...
					oNavigationPopoverHandler.openPopover().then(function() {
						// assert
						assert.ok(oNavigationPopoverHandler._oPopover._getContentContainer().getExtraContent());
						var oVBox = Element.getElementById(oNavigationPopoverHandler._oPopover._getContentContainer().getExtraContent());
