...
sap.ui.define([
	...
	QUnit.test("Title of Mainlink: enabled sap.m.Link, text = 'fieldName' value of bindingContext", async function (assert) {
		...
		oNavigationPopoverHandler.openPopover().then(function () {
			...
			assert.equal(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0].text, "valueA");
			assert.ok(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0].href.indexOf("?TestObject#/dummyLink1") > -1);
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getText(), "valueA");
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getHref(), "?TestObject#/dummyLink1");
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getEnabled(), true);
			assert.equal(oNavigationPopoverHandler._oPopover.getAriaLabelledBy(), oNavigationPopoverHandler._oPopover.getContent()[0]._oHeaderArea.getItems()[0].getContent().getId(), "Correct ariaLabelledBy returned for popover.");
	...
	QUnit.test("Title of Mainlink: disabled sap.m.Link, text = 'fieldName' value of bindingContext", async function (assert) {
		...
		oNavigationPopoverHandler.openPopover().then(function () {
			...
			assert.equal(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0].text, "valueA");
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getText(), "valueA");
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getHref(), "");
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getEnabled(), false);
	...
	QUnit.test("Title of Mainlink: enabled sap.m.Link, text = 'Display Fact Sheet'", async function (assert) {
		...
		oNavigationPopoverHandler.openPopover().then(function () {
			...
			assert.ok(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0].href.indexOf("?TestObject#/dummyLink1") > -1);
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getText(), Library.getResourceBundleFor("sap.ui.comp").getText("POPOVER_FACTSHEET"));
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getHref(), "?TestObject#/dummyLink1");
			assert.equal(Element.closestTo(oNavigationPopoverHandler._oPopover._getContentContainer().$().find("a")[0]).getEnabled(), true);
