...
sap.ui.define([
...
], function(MockServer, ODataModel, SmartMicroChart, SmartComparisonMicroChart, SmartBulletMicroChart, SmartHarveyBallMicroChart, SmartAreaMicroChart, SmartLineMicroChart, SmartRadialMicroChart, SmartColumnMicroChart, SmartStackedBarMicroChart, Label, MLibrary, CustomData, Model, microchartLibrary, ComparisonMicroChart, SmartMicroChartBase, Log, Button, Core, nextUIUpdate) {
	"use strict";
	 ...
	 */
	QUnit.test("Check default values of isResponsive", function(assert) {
		assert.ok(!this.oSmartMicroChart._isResponsive());
	});
	...
	QUnit.test("Check chartType delegation to _chart Aggregation", async function(assert) {
		var done = assert.async();
		//Arrange
		this.oSmartMicroChart.attachInitialize(handleInit.bind(this));
		//Act
	...
	QUnit.test("Title, Description and UnitOfMeasure in case of SmartBulletMicroChart", async function(assert) {
		//Arrange
		this.oSmartMicroChart._oChartViewMetadata = {
			chartType: "bullet",
			annotation: {
	...
	QUnit.test("Title, Description and UnitOfMeasure in case of SmartAreaMicroChart", async function(assert) {
		//Arrange
		this.oSmartMicroChart._oChartViewMetadata = {
			chartType: "line",	// AreaChart is mapped to LineChart
			annotation: {
	...
	QUnit.test("Title, Description, UnitOfMeasure and FreeText in case of SmartRadialMicroChart", async function(assert) {
		//Arrange
		this.oSmartMicroChart._oChartViewMetadata = {
			chartType: "donut",
			annotation: {
	...
	QUnit.test("Qualifier is passed correctly to inner chart in case of SmartBulletMicroChart", async function(assert) {
		//Arrange
		var done = assert.async();
		//Arrange
		this.oSmartMicroChart.attachInitialize(handleInit.bind(this));
	 ...
	 * @deprecated Since version 1.62.0
	 */
	QUnit.test("Css class is set when isResponsive", async function(assert) {
		//Arrange
		var done = assert.async();
		//Arrange
		this.oSmartMicroChart.attachInitialize(handleInit.bind(this));
		...
		async function handleInit() {
			this.oSmartMicroChart.setSize(Size.Auto);
	...
	QUnit.test("Setting size property to sap.m.Size.Responsive leads to isResponsive === true", function(assert) {
		...
		this.oSmartMicroChart.setSize(Size.Responsive);
		//Assert
		assert.ok(this.oSmartMicroChart._isResponsive(), "Chart is in Responsive mode for size 'Responsive'");
	});
	...
	QUnit.test("sap.m.Size.Responsive changed to sap.m.Size.L leads to isResponsive === false", function(assert) {
		...
		//Assert
		assert.notOk(this.oSmartMicroChart._isResponsive(), "Chart is in not Responsive mode for size 'L'");
	});
	...
	QUnit.test("isResponsive false sets size Auto", function(assert) {
		...
		this.oSmartMicroChart.setSize(Size.Responsive);
		//Assert
		assert.equal(this.oSmartMicroChart.getSize(), Size.Responsive, "Chart has size Auto for isResponsive false'");
	});
