...
sap.ui.define([
	...
	"sap/ui/qunit/utils/nextUIUpdate"
			 ...
			 JSONModel, Label, BulletMicroChart, microchartLibrary, Button, MLibrary, nextUIUpdate) {
	"use strict";
	...
	*/
	QUnit.test("Height and width not set when isResponsive true", async function(assert) {
		...
		this.oSBMC.setSize(Size.Responsive);
		...
		assert.ok(this.oSBMC.getAggregation("_chart")._isResponsive(), "isResponsive is true on inner chart");
	});
	...
	*/
	QUnit.test("Height and width set when isResponsive false", async function(assert) {
		...
		assert.notOk(this.oSBMC.getAggregation("_chart")._isResponsive(), "isResponsive is false on inner chart");
	});
	...
	*/
	...
	QUnit.test("UnitOfMeasure Association", function(assert) {
		...
		sinon.stub(this.oSBMC, "getModel").returns(this.oSBMC.oMetaModel = new Model());
		this.oSBMC._createChartProvider.apply(this.oSBMC);
