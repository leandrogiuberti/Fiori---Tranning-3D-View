...
sap.ui.define([
	...
	QUnit.test("getDataSourceLabel works as expected", function (assert) {
		...
		// Act
		oGroupElement.getDataSourceLabel();

		// Assert
		assert.strictEqual(oSFGetDataSourceLabelStub.callCount, 1, "Method is called once");
		assert.strictEqual(typeof oGroupElement.getDataSourceLabel, "function", "Protected method is available");
		assert.strictEqual(oGroupElement.getDataSourceLabel(), "MyTestLabel", "Method returns initial empty string value");
	...
	QUnit.test("Precedence (BCP: 2270118842)", function (assert) {
		...
		oGroupElement.addElement(oSF);
		oGroupElement.addCustomData(oPropagated);
	...
	function fieldsInVBoxTest(assert) {
		...
		assert.notOk(Element.getElementById(sHBoxId), "HBox is destroyed");
		...
		assert.notOk(Element.getElementById(sVBoxId), "VBox is destroyed");
		...
		assert.ok(oLabel, "Label still exists");
		assert.equal(oLabel, oGroupElement.getLabel(), "Label sill assigned to GroupElement");
		assert.notOk(Element.getElementById("I1"), "Field1 is destroyed");
		assert.notOk(Element.getElementById("I2"), "Field1 is destroyed");
	...
	function createRemoveFromVBoxTest(assert) {
		...
		var oField3 = Element.getElementById("I3");
		var oGridData = Element.getElementById("GD1");
		var aFields = oGroupElement.getFields();
		var oVBox = aFields[0];
		...
		assert.notOk(Element.getElementById(sVBoxId), "VBox destroyed");
