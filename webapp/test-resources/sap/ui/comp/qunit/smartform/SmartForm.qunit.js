...
sap.ui.define([
	...
	"sap/m/Title",
	// in SmartForm OverflowToolbar is required
	"sap/m/Toolbar",
	"sap/m/ToolbarSpacer",
	...
	var SmartFormValidationMode = CompLib.smartform.SmartFormValidationMode;
	...
	QUnit.test("internal Form", async function(assert) {
		...
		await nextUIUpdate(); // as FormLayout is created only before rendering as not sure if Layout aggregation will be used or not
	...
	function checkButton(assert) {
		...
		assert.notOk(Element.getElementById(sToolbarId), "Toolbar destroyed");
		assert.notOk(Element.getElementById(sSpacerId), "ToolbarSpacer destroyed");
	...
	function checkButtonCustomToolbar(assert) {
		var oCustomToolbar = Element.getElementById("TB1");
		var oToolbar = oSmartForm.getAggregation("toolbar");
		...
		assert.equal(aContent[1].getId(), "B2", "second item is button");
		assert.notOk(Element.getElementById(sSpacerId), "ToolbarSpacer destroyed");
		assert.notOk(Element.getElementById(sSeparatorId), "ToolbarSeparator destroyed");
		assert.notOk(Element.getElementById(sButtonId), "Button destroyed");
	}
	...
	function editTogglableCustomToolbar(assert) {
		...
		assert.equal(aContent[0].getId(), "B1", "first item is button");
		assert.equal(aContent[1].getId(), "B2", "second item is button");
		assert.ok(Element.getElementById(sSpacerId), "ToolbarSpacer not destroyed");
		assert.notOk(Element.getElementById(sSeparatorId), "ToolbarSeparator destroyed");
		assert.notOk(Element.getElementById(sButtonId), "Button destroyed");
	}
	...
	function customToolbarTitle(assert) {
		...
		assert.equal(aContent.length, 2, "CustomToolbar has 2 items");
		assert.equal(aContent[0].getId(), "B1", "first item is button");
		assert.notOk(Element.getElementById(sTitleId), "Title destroyed");
	}
	...
	QUnit.test("destroyGroups", function(assert) {
		...
		assert.equal(oSmartForm.getGroups(), 0, "All Groups removed");
		assert.notOk(Element.getElementById("GE1"), "Group1 is destroyed");
		assert.notOk(Element.getElementById("GE2"), "Group2 is destroyed");
	...
	QUnit.test("use Layout", function(assert) {
		...
		assert.notOk(Element.getElementById("L1"), "Layout is destroyed");
		if (oFormLayout instanceof ResponsiveGridLayout) {
			// if wrong layout these checks will break anyway
	...
	function useHorizontalLayoutWithoutLayout(assert) {
		var oGroup1 = Element.getElementById("G1");
		var oGroupElement1 = Element.getElementById("GE1");
	 ...
	 */
	QUnit.test("UseHorizontalLayout without Layout", async function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		...
		oSmartForm2.placeAt("qunit-fixture");
		await nextUIUpdate(); // as ResponsiveLayout is created only before rendering as not sure if Layout aggregation will be used or not
	...
	QUnit.test("ColumnLayout", async function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		...
		oSmartForm2.placeAt("qunit-fixture");
		await nextUIUpdate(); // as ColumnLayout is created only before rendering as not sure if Layout aggregation will be used or not
	 ...
	 * @deprecated As of version 1.86, replaced by {@link sap.ui.comp.smartform.ColumnLayout ColumnLayout} in the <code>layout</code> aggregation.
	 */
	QUnit.test("ColumnLayout and useHorizontalLayout", async function(assert) {
		var oException;
	 ...
	 * @deprecated As of version 1.86, replaced by {@link sap.ui.comp.smartform.ColumnLayout ColumnLayout} in the <code>layout</code> aggregation.
	 */
	QUnit.test("useHorizontalLayout and ColumnLayout", async function(assert) {
		var oException;
	...
	function expandableTest(assert) {
		...
		assert.notOk(Element.getElementById(sPanelId), "Panel is destroyed");
	...
	function expandableTitleTest(assert) {
		...
		assert.equal(oSmartForm._oForm.getTitle(), null, "Inner form doesn't duplicate the title of the Panel");
	}
	 ...
	 * @deprecated As of version 1.86, replaced by {@link sap.ui.comp.smartform.ColumnLayout ColumnLayout} in the <code>layout</code> aggregation.
	 */
	QUnit.test("editable / Label displayOnly", async function(assert) {
	 ...
	 * @deprecated As of version 1.64, replaced by {@link sap.ui.comp.smartfield.SmartField#checkValuesValidity}
	 */
	QUnit.test("setFocusOnEditableControl", async function(assert) {
	...
	async function eventsChecked(assert) {
		var oField1 = new SmartField("F1", {value: "Text"});
		var oField2 = new SmartField("F2");
		var oField3 = new SmartField("F3");
		var oField4 = new SmartField("F4");
	 ...
	 * @deprecated As of version 1.64, replaced by {@link sap.ui.comp.smartfield.SmartField#checkValuesValidity}
	 */
	QUnit.test("checked", async function(assert) {
		oSmartForm.setCheckButton(true);
		oSmartForm.setEditable(true);
		await asyncToolbarTest(assert, false, true, eventsChecked);
	});

	async function eventsEditToggled(assert) {
		var oToolbar = oSmartForm.getAggregation("toolbar");
		var bEvent = false;
		var bEditable;
	...
	QUnit.test("editToggled", async function(assert) {
		oSmartForm.setEditable(true);
		oSmartForm.setEditTogglable(true);
		await asyncToolbarTest(assert, false, true, eventsEditToggled);
	});

	async function eventsNoEditToggled(assert, fnDone) {
		var oField1 = new SmartField("F1", {value: "Text"});
		var oGroupElement1 = new GroupElement("GE1", {label: "Label", elements: [oField1]});
		var oGroup1 = new Group({groupElements: [oGroupElement1]});
		oSmartForm.addGroup(oGroup1);
	 ...
	 * @deprecated As of version 1.64, replaced by {@link sap.ui.comp.smartfield.SmartField#checkValuesValidity}
	 */
	QUnit.test("no editToggled when error", async function(assert) {
		var oStub = sinon.stub(sap.ui, "require");
		...
		oSmartForm.setEditable(true);
		oSmartForm.setEditTogglable(true);
		await asyncToolbarTest(assert, false, true, eventsNoEditToggled);
		oStub.restore();
	});
