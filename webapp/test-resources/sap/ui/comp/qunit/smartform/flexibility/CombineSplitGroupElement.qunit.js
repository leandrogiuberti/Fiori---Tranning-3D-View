...
sap.ui.define([
	...
	function confirmGroupElementsAreCombined(sExpectedLabel, oUiComponent, oViewAfterAction, assert) {
		assert.strictEqual( oViewAfterAction.byId("new-combined-element").getElements().length, 3,
							"then the groupelement1 contains 3 fields");
		assert.strictEqual( oViewAfterAction.byId("new-combined-element").getLabelText(), sExpectedLabel,
	...
	elementActionTest("Checking the combine action for group elements with special labels", {
		...
		action: {
			...
			parameter: function(oView) {
				return {
					newElementId: oView.createId("new-combined-element"),
	...
	QUnit.module("CombineFields - given RTL mode for CombineFields", {
		beforeEach: function() {
			...
			this.oSmartField3  = new SmartField("field3", {
				textLabel: "Alias Name",
				value: "Name-alias"
			});
			...
			this.oAliasName = new GroupElement("AliasName",{
				elements:[this.oSmartField3]
			});
			this.oGroup = new Group("group", {
				formElements: [
		...
		QUnit.test("when field labels are combined in LTR mode", function(assert) {
			return CombineFieldsChangeHandler.applyChange(this.oChange, this.oGroup, {modifier: JsControlTreeModifier, appComponent: this.oAppComponent})
				.then(function () {
					assert.strictEqual(
		...
		QUnit.test("when field labels are combined 2 times in LTR mode", function(assert) {
			return CombineFieldsChangeHandler.applyChange(this.oChange, this.oGroup, {modifier: JsControlTreeModifier, appComponent: this.oAppComponent})
				.then(function () {
					assert.strictEqual(
						this.oGroup.getGroupElements()[0]._getLabel().getText(),
				...
				.then(ChangesWriteAPI.create.bind(this, {
					changeSpecificData: {
						newElementId: "secondCombinedElement",
						changeType: "combineFields",
						combineElementIds: ["combinedElement", "AliasName"],
						sourceControlId: "AliasName"
					},
					selector: this.oSmartForm
				}))
				.then(function(oSecondChange) {
					return CombineFieldsChangeHandler.applyChange(
						oSecondChange,
						this.oGroup,
				...
				.then(function () {
					...
					var aFields = this.oGroup.getGroupElements()[0].getFields();
					assert.strictEqual(aFields[0].getId(), "field1", "then the first field is positioned correctly");
					assert.strictEqual(aFields[1].getId(), "field2", "then the second field is positioned correctly");
					assert.strictEqual(aFields[2].getId(), "field3", "then the third field is positioned correctly");
				}.bind(this));
		});

		QUnit.test("when field labels are combined 2 times in RTL mode", function(assert) {
			Localization.setRTL(true);
			return CombineFieldsChangeHandler.applyChange(this.oChange, this.oGroup, {modifier: JsControlTreeModifier, appComponent: this.oAppComponent})
				.then(function () {
					assert.strictEqual(
				...
				.then(function () {
					assert.strictEqual(
						this.oGroup.getGroupElements()[0]._getLabel().getText(),
						"Alias Name/Last Name/First Name",
						"then fields have been combined successfully in RTL mode"
					);
					var aFields = this.oGroup.getGroupElements()[0].getFields();
					assert.strictEqual(aFields[0].getId(), "field1", "then the first field is positioned correctly");
