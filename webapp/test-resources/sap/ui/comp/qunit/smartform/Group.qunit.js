...
sap.ui.define([
...
], function(Group, GroupElement, SemanticGroupElement, CustomData, Element, Title, SmartField, GridData) {
	"use strict";
	...
	QUnit.test("destroyGroupElements", function(assert) {
		...
		assert.equal(oGroup.getGroupElements(), 0, "All GroupElements removed");
		assert.notOk(Element.getElementById("GE1"), "GroupElement1 is destroyed");
		assert.notOk(Element.getElementById("GE2"), "GroupElement2 is destroyed");
	});
	...
	QUnit.test("Layout", function(assert) {
		...
		assert.notOk(Element.getElementById("GD1"), "GridData destroyed");
	});

	QUnit.test("clone", function(assert) {
		oGroup.setTitle("Title");
		var oField1 = new SmartField("S1"); // to have visible content in GroupElement
		var oField2 = new SmartField("S2");
		...
		assert.equal(oGroup.getTitle(), "Title", "Original: Title");
		var aCustomData = oGroup.getCustomData();
		...
		assert.equal(oClone.getTitle(), "Title", "Clone: Title");
	 ...
	 * @deprecated Since version 1.48.0, please do not use this property as it does not have any effect on the current layout of the <code>SmartForm</code> control.
	 */
	QUnit.test("_enhanceGroupElementBefore should not call setHorizontalLayoutGroupElementMinWidth when SemanticGroupElement", function(assert) {
		var oGroupElement1 = new GroupElement(),
		oGroupElement2 = new SemanticGroupElement(),
		oSpySetUseHorizontalLayoutGroupElement = this.spy(oGroupElement1, "getUseHorizontalLayout"),
		oSpySetUseHorizontalLayoutSemanticGroupElement = this.spy(oGroupElement2, "getUseHorizontalLayout");

		oGroup.addFormElement(oGroupElement1);
		assert.ok(oSpySetUseHorizontalLayoutGroupElement.calledOnce, "getUseHorizontalLayout called on GroupElement");
		oGroup.addFormElement(oGroupElement2);
		assert.notOk(oSpySetUseHorizontalLayoutSemanticGroupElement.calledOnce, "getUseHorizontalLayout not called on SemanticGroupElement");
