...
sap.ui.define([
...
], function(EventProvider, SortController, Controller, UiTable, UiColumn, Label, JSONModel, MTable, MColumn, coreLibrary, CustomData, ColumnListItem) {
	'use strict';
	...
	QUnit.module("API", {
		beforeEach: function() {
			...
			this.oSortController.getControlDataInitial = function() {
				return {
					sort: {
						sortItems: []
					}
	...
	QUnit.test("with UITable containing sortable columns", function(assert) {
		...
		assert.strictEqual(this.oController._oSettingCurrent.sort.controller._hasSortableColumns(), true);
	});
	...
	QUnit.test("with UITable not containing sortable columns", function(assert) {
		...
		assert.strictEqual(this.oController._oSettingCurrent.sort.controller._hasSortableColumns(), false);
	});
	...
	var fTest03 = function(assert, oSortController, oTable) {
		...
		assert.equal(oTable.getColumns()[0].getSortOrder(), "None");
		...
		oSortController.syncJson2Table({
			sort: {
				sortItems: [
					{
						columnKey: "a",
						operation: coreLibrary.SortOrder.Ascending
					}
				]
			}
		});
		...
		assert.equal(oTable.getColumns()[0].getSortOrder(), "Ascending");
	};
	...
	QUnit.test("test01", function(assert) {
		...
		// act
		this.oSortController._updateInternalModel("a", coreLibrary.SortOrder.Ascending, false);
		...
		assert.equal(this.oSortController.getControlDataBase()["sort"]["sortItems"][0].operation, coreLibrary.SortOrder.Ascending);
		...
		assert.equal(this.oSortController.getControlData()["sort"]["sortItems"][0].operation, coreLibrary.SortOrder.Ascending);

		// act
		this.oSortController._updateInternalModel("a", coreLibrary.SortOrder.Descending, false);

		// assert
		assert.equal(this.oSortController.getControlDataBase()["sort"]["sortItems"].length, 1);
		assert.equal(this.oSortController.getControlDataBase()["sort"]["sortItems"][0].columnKey, "a");
		assert.equal(this.oSortController.getControlDataBase()["sort"]["sortItems"][0].operation, coreLibrary.SortOrder.Descending);

		assert.equal(this.oSortController.getControlData()["sort"]["sortItems"].length, 1);
		assert.equal(this.oSortController.getControlData()["sort"]["sortItems"][0].columnKey, "a");
		assert.equal(this.oSortController.getControlData()["sort"]["sortItems"][0].operation, coreLibrary.SortOrder.Descending);

		// act
		this.oSortController._updateInternalModel("b", coreLibrary.SortOrder.Ascending, false);
		...
		// act
		this.oSortController._updateInternalModel("a", coreLibrary.SortOrder.Ascending, true);
		...
		assert.equal(this.oSortController.getControlDataBase()["sort"]["sortItems"][1].operation, coreLibrary.SortOrder.Ascending);
		...
		assert.equal(this.oSortController.getControlData()["sort"]["sortItems"][1].operation, coreLibrary.SortOrder.Ascending);
	...
	var fTest04 = function(assert, oSortController, oController) {
		...
		oSortController._updateInternalModel("a", coreLibrary.SortOrder.Ascending, false);
		...
		assert.equal(oSortController.getControlDataBase()["sort"]["sortItems"][0].operation, coreLibrary.SortOrder.Ascending);
		...
		assert.equal(oSortController.getControlData()["sort"]["sortItems"][0].operation, coreLibrary.SortOrder.Ascending);
	...
	QUnit.test("SortController: getChangeData with ControlDataInitial", function(assert) {
		// asserts
		var fTest06 = function(assert, oSortController) {
			oSortController.getControlDataInitial = function() {
				return oA;
			};

			assert.deepEqual(oSortController.getChangeData(oA, oA), oA, "no change: {sort} = A");
		};
		// ui5lint-disable no-deprecated-api
		this.oController = new Controller({
			table: this.oTable = new UiTable({
		...
		// ui5lint-enable no-deprecated-api
		fTest06(assert, this.oController._oSettingCurrent.sort.controller, this.oController);
	});
});