...
sap.ui.define([
	...
	QUnit.test("convertFilters should skip filters of type FiscalDate", function(assert) {
		// Arrange
		var oPersonalizationData = {
			filter	: {
				filterItems: [{
					columnKey: "fiscalDate",
					value1: "2022",
					value2: "2023"
				}]
			}
		...
		// Act
		Util.convertFilters(oPersonalizationData, oColumnKey2ColumnMap);

		// Assert
		assert.strictEqual(oPersonalizationData.filter.filterItems[0].value1, "2022", "fiscal date is not converted");
		assert.strictEqual(oPersonalizationData.filter.filterItems[0].value2, "2023", "fiscal date is not converted");

	});

	QUnit.test("convertSelectOptions should skip filters of type FiscalDate", function(assert) {
		// Arrange
		var oPersonalizationData = {
			SelectOptions: [{
		...
		// Act
		Util.convertSelectOptions(oPersonalizationData, oColumnKey2ColumnMap);

		// Assert
		assert.strictEqual(oPersonalizationData.SelectOptions[0].Ranges[0].Low, "2022", "fiscal date is not converted");
		assert.strictEqual(oPersonalizationData.SelectOptions[0].Ranges[0].High, "2023", "fiscal date is not converted");
	});

	QUnit.test("addSortPersistentData should add sorted properties from the metadata", function(assert) {
		var oPersonalizationData = {
			sort: {
				sortItems: [{
					columnKey: "Bukrs",
		...
		// Act
		Util.addSortPersistentData(oPersonalizationData, oDestinationJsonData, [], aMetadata);

		// Assert
		assert.equal(oDestinationJsonData.sort.sortItems.length, 2, "Correct number of sortItems is added");
		assert.equal(oDestinationJsonData.sort.sortItems[1].columnKey, "Budat", "Correct item from metadata is added");
		assert.equal(oDestinationJsonData.sort.sortItems[1].operation, "Ascending", "Correct item from metadata is added");
		assert.equal(oDestinationJsonData.sort.sortItems[1].initiallyNotVisibleColumn, true, "flag for initially visible column is correctly set");
	});
});
