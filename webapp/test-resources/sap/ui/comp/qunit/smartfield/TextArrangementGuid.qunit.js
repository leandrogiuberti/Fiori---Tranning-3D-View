...
sap.ui.define([
...
], function(TextArrangementGuid, Library, ValidateException, Guid) {
	"use strict";
	...
	QUnit.test("it should return the name of the data type class", function(assert) {
		...
		try {
			...
			// assert
			assert.strictEqual(e.message, Library.getResourceBundleFor("sap.ui.core").getText("EnterGuid"));
			oGuid.destroy();
		}
	});
	...
	QUnit.test("it should raise a ValidateException for invalid entry if the validation fails", function(assert) {
		...
		oParseDescriptionOnlyPromise.catch(function(error) {
			var sText = Library.getResourceBundleFor("sap.ui.comp").getText("SMARTFIELD_INVALID_ENTRY");
			assert.ok(true);
	...
	QUnit.test("it should show a description for '00000000-0000-0000-0000-000000000000' if description is not empty", function(assert) {
		// arrange
		var sFormattedValue,
			sGUID = "00000000-0000-0000-0000-000000000000",
			sDescription = "Dummy Description";

		// act
		sFormattedValue = this.oType.formatValue([sGUID, sDescription], "string");

		// assert
		assert.strictEqual(sFormattedValue, sDescription, "The description is shown");
	});

	QUnit.test("it should format value '00000000-0000-0000-0000-000000000000' to empty string when there is no description", function(assert) {
		// arrange
		var sFormattedValue,
			sGUID = "00000000-0000-0000-0000-000000000000";

		// act
		sFormattedValue = this.oType.formatValue([sGUID, undefined], "string");

		// assert
		assert.strictEqual(sFormattedValue, "", "The value is formatted to empty string");
	});
});
