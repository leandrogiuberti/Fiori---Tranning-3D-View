...
"use strict";
	...
	QUnit.test("getType : DateTimeOffset with time zone returning DateTimeWithTimezone when the field is not editable", function(assert) {
		this.oMetaData._oParent.isInEditMode =  function() {
				return false;
		};
		...
		// assert
		assert.strictEqual(oType.getMetadata().getName(), "sap.ui.model.odata.type.DateTimeWithTimezone");
	});

	QUnit.test("getType : DateTimeOffset with time zone returning DateTimeOffset when the field is editable", function(assert) {
		this.oMetaData._oParent.isInEditMode =  function() {
			return false;
		};
	...
	QUnit.test("getType - proper composite type generation in case of fiscal and calendar annotations", function(assert) {
		// Act
		var oProperty = {"property": {"name": "Test", "type": "Edm.String"}},
			oType = this.oMetaData.getType(oProperty, null, null, {composite: true});

		// Assert
		assert.ok(oType.isA("sap.ui.model.CompositeType"), "Composite type should be returned");

		// Act - fiscal type with composite: true
		oProperty = {"property": {"name": "Test", "type": "Edm.String", "com.sap.vocabularies.Common.v1.IsFiscalYearPeriod": {Bool: 'true'}}};
		oType = this.oMetaData.getType(oProperty, null, null, {composite: true});

		// Assert
		assert.ok(oType.isA("sap.ui.model.CompositeType"), "Composite type should be returned");

		// Act - calendar type with composite: true
		oProperty = {"property": {"name": "Test", "type": "Edm.String", "com.sap.vocabularies.Common.v1.IsCalendarYear": {Bool: 'true'}}};
		oType = this.oMetaData.getType(oProperty, null, null, {composite: true});
	...
	QUnit.test("SNOW: DINC0230090 getType - NumericText type returned instead of FiscalDate type", function(assert) {
		// Act - calendar type with composite: false and IsDigitSequence
		var oProperty = {"property": {
			...
			oType = this.oMetaData.getType(oProperty, null, null, {composite: false});

		// Assert
		assert.ok(oType.isA("sap.ui.comp.odata.type.FiscalDate"), "FiscalDate type should be returned");
	});

	QUnit.test("getType - Constraints and FormatOptions are considered for fiscal and calendar type", function(assert) {
		// Act - fiscal type with composite: true
		var oProperty = {"property": {"name": "Test", "type": "Edm.String", "com.sap.vocabularies.Common.v1.IsFiscalYearPeriod": {Bool: 'true'}}},
			oType = this.oMetaData.getType(oProperty, {parseAsString: true}, {maxLength: 2});

		// Assert
		assert.notOk(oType.isA("sap.ui.model.CompositeType"), "Simple type should be returned");
		assert.strictEqual(oType.getFormatOptions().parseAsString, true, "Format options are propagated");
		assert.strictEqual(oType.getConstraints().maxLength, 2, "Constraints are propagated");

		var oProperty = {"property": {"name": "Test", "type": "Edm.String", "com.sap.vocabularies.Common.v1.IsCalendarYear": {Bool: 'true'}}},
			oType = this.oMetaData.getType(oProperty, {parseAsString: true}, {maxLength: 2});
	...
	QUnit.test("getUOMDisplayFormatter for non-currency type with scale type with scale", function(assert) {
		...
		var sMeasure = "1",
			sUnit = "PC",
			sFormattedValue1 = fnFormatter(sMeasure, sUnit, null);
	...
	QUnit.test("getUOMDisplayFormatter for currency", function(assert) {
		...
		var sAmount = "10120.01",
			sCurrency = "*",
			sFormattedValue1 = fnFormatter(sAmount, sCurrency, null),
	...
	QUnit.test("getUOMDisplayFormatter it should mask the currency value", function(assert) {
		...
		var sAmount = "100",
			sCurrency = "EUR",
			sFormattedValue = fnFormatter(sAmount, sCurrency, null);
	...
	QUnit.test("getUOMDisplayFormatter it should format the zero correctly", function(assert) {
		...
		// act
		var sFormattedValue = fnFormatter(vAmount, sCurrency, null);
	...
	QUnit.test("getUoMType for Edm.Decimal", function(assert) {
		...
		// act
		var oType = this.oMetaData.getUoMType(oProperty);
	...
	QUnit.test("getUoMType for Edm.Int16", function(assert) {
		...
		assert.strictEqual(oFormatOptions.emptyString, 0);
		assert.strictEqual(oFormatOptions.showMeasure, false);
		assert.strictEqual(oFormatOptions.parseAsString, false);
		assert.strictEqual(oConstraints.precision, 11);
		assert.strictEqual(oConstraints.scale, 2);
	});

	QUnit.test("getUoMType for Edm.Int32", function(assert) {
	...
	QUnit.test("getUoMType for Edm.Int64", function(assert) {
	...
	QUnit.test("getUoMType for Edm.Byte", function(assert) {
	...
	QUnit.test("getUoMType for Edm.Double", function(assert) {
	...
	QUnit.test("getUoMType for Edm.String", function(assert) {
	...
	QUnit.test('getUoMType() should set the "emptyString" format option to null when ' +
		'the EDM Property is nullable', function(assert) {
	...
	QUnit.test("getUoMType() should return null", function(assert) {

		// act
		var oType = this.oMetaData.getUoMType();
