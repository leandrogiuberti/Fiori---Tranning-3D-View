...
sap.ui.define([
	...
	QUnit.test("it should return the name of the primary data type class", function(assert) {
		...
		// assert
		assert.strictEqual(oPrimaryType.prototype.getName(), "sap.ui.comp.smartfield.type.StringNullable");
	});
	...
	QUnit.test("it should not raise a validate exception for NonNegative values (isDigitSequence)", function(assert) {

		// arrange
		var aValues = ["070004", undefined],
			oRejectSpy = sinon.spy(),
			oSettings = {
				keyField: "ID",
				descriptionField: "Text",
				data: [
			...
			oType = new TextArrangementString(oFormatOptions, oConstraints, oSettings);

		// act
		var bReturn = oType.validateIDAndDescription(aValues, oSettings);
	...
	QUnit.test("it should call the OData model read method", function(assert) {
		...
		var oModelRead = this.stub(oModel, "read").callsFake(function(sPath, mParameters){
			mParameters.settings.success();
		});
		...
		oType.validateValue(aValues).finally(function(){
			// assert
	...
	QUnit.test("TextArrangement.prototype.parseDescriptionOnly should always resolve", function(assert) {
		...
		// act
		oType.parseDescriptionOnly(null, "").then(function(oData){
			// assert
