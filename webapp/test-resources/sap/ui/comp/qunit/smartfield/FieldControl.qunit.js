...
sap.ui.define([
	...
	QUnit.test("_getVisible - UIHidden with path", function(assert) {
		// Arrange
		var oSet, oType, oResult, oMeta, fnGetUIHiddenPathSpy;
		...
		this.oHelper.getProperty(oMeta);
		oMeta.property.property["com.sap.vocabularies.UI.v1.Hidden"] = { "Path" : "UIHidden" };
		fnGetUIHiddenPathSpy = sinon.spy(this.oHelper.oAnnotation, "getUIHiddenPath");
		// Act
		oResult = this.oMetaData._getVisible(oMeta);

		// Assert
		assert.strictEqual(oResult.path()[0], "UIHidden");
		assert.strictEqual(fnGetUIHiddenPathSpy.calledOnce, true, "getUIHiddenPath is called");
		assert.strictEqual(fnGetUIHiddenPathSpy.calledWith(oMeta.property.property), true, "getUIHiddenPath is called with correct arguments");
		assert.strictEqual(oResult.formatter(true), false, "If the UIHidden Path is evaluated to true, the visibility is hidden (false)");
		assert.strictEqual(oResult.formatter(false), true, "If the UIHidden Path is evaluated to false, the visibility is visible (true)");
	});
	...
	QUnit.test("_getEditable - without oEdmProperty", function(assert) {
		var bIsErrorRaised = false,
			oSmartField = sinon.createStubInstance(SmartField),
			oResult = this.oMetaData._getEditable({});

		oSmartField.getBindingPath.returns("");

		// assert
		try {
			oResult.formatter.call(oSmartField, {fieldKey: "fieldContent"});
		} catch (error) {
			bIsErrorRaised = true;
		}

		assert.strictEqual(bIsErrorRaised, false);
	});
	...
	QUnit.test("it should NOT restore the value of the editable property to its default", function(assert) {

		// arrange
		var bEditable = false, // default is true
			oBindingContext = sinon.createStubInstance(Context),
			oSmartField = new SmartField({
			editable: bEditable
		});

		// notice that the binding context is undefined
		this.stub(oSmartField, "getBindingContext").withArgs().returns(oBindingContext);
		this.stub(oSmartField, "getBinding").withArgs("editable").returns({
			isInitial: function() {
				return false;
			}
		});
		this.stub(oSmartField, "getBindingPath").withArgs("editable").returns("");
		var oFieldControl = new FieldControl(oSmartField, this.oHelper);
		var oResult = oFieldControl._getEditable({}, undefined, undefined);
		var vEditable = oResult.formatter.call(oSmartField, null);
		...
		// assert
		assert.strictEqual(oSmartField.getEditable(), bEditable);
	});
	...
	QUnit.test("_getEditable - binding info with formatter", function(assert) {
		...
		oResult.path();
		assert.strictEqual(!oResult.formatter(false), true);
		assert.ok(bFormat);
	});

	QUnit.test("_getEditable - binding info with formatter and first argumnent 'undefined'", function(assert) {
		var oSet, oType, oResult, oMetaData, oBindingInfo, bFormat = false, oMeta;
	...
	QUnit.test("_getEditable - binding info with formatter and first argumnent 'null'", function(assert) {
		var oSet, oType, oResult, oMetaData, oBindingInfo, bFormat = false, oMeta;
		...
		oResult = this.oMetaData._getEditable(oMetaData, oBindingInfo);
		oResult.path();
		assert.strictEqual(oResult.formatter(null), true);
		assert.ok(bFormat);
	});
	...
	QUnit.test("_getEditable - not expanded navigation property", function(assert) {
		// Arrange
		var oSet = this.oHelper.oMeta.getODataEntitySet("Project"),
			oResult,
			aPaths,
		...
		oMeta = {
			entitySet: oSet,
			entityType: this.oHelper.oMeta.getODataEntityType(oSet.entityType),
			path: "Amount"
		};
		this.oHelper.getProperty(oMeta);
		oMeta.navigationPath = "to_Amount"; // Mock navigation path
		...
		// Assert
		assert.strictEqual(aPaths.length, 1, "Only one binding path is added");
		assert.strictEqual(aPaths[0], "to_Amount", "Binding path is targeting the root navigation property");
		assert.strictEqual(
			oResult.formatter(undefined), // Simulate not-expanded navigation returning 'undefined'
			false,
			"When navigation is not expanded the control should be in display mode"
		);
		assert.strictEqual(
			oResult.formatter(null), // Simulate not-expanded navigation returning 'null'
			false,
			"When navigation is not expanded the control should be in display mode"
		);
		assert.strictEqual(
			oResult.formatter({}), // Simulate expanded navigation returning object
			true,
			"When navigation is expanded the control should be in editable mode"
		);
	});
	...
	QUnit.test("field control with complex type (Unit of Measure)", function(assert) {
		...
		var oParent = {
			...
			attachEvent: function() {},
			detachEvent: function() {},
			_shouldSuppressTextAnnotation: function () {return false;},
			_rejectLifecyclePromises: function () {}
		};
		...
		oFactory.bind({mode: oParent.getMode()}).then(function() {
	...
	QUnit.test("getMandatoryCheck - mandatory for Edm.Guid, client check", function(assert) {
		var oProp, oResult, bExc = false;
		oProp = JSON.parse("{\"property\":{\"name\":\"Description\",\"type\":\"Edm.Guid\",\"nullable\":" +
			"\"false\",\"maxLength\":\"80\",\"extensions\":[{\"name\":\"field-control\",\"value\":" +
			"\"Description_FC\",\"namespace\":\"http://www.sap.com/Protocols/SAPData\"},{\"name\":" +
			"\"sortable\",\"value\":\"false\",\"namespace\":\"http://www.sap.com/Protocols/SAPData\"" +
			"},{\"name\":\"filterable\",\"value\":\"false\",\"namespace\":\"http://www.sap.com/Protocols" +
	...
	QUnit.test("getMandatoryCheck should set generic error message when there is not label", function(assert) {
		// Arrange
		var oProperty = JSON.parse("{\"property\":{\"name\":\"Description\",\"com.sap.vocabularies.Common.v1.FieldControl\"" +
				":{\"EnumMember\":\"com.sap.vocabularies.Common.v1.FieldControlType/Mandatory\"},\"type\":\"" +
				"Edm.String\",\"maxLength\":\"80\",\"extensions\":[{\"name\":\"field-control\",\"value\":\"" +
				...
				"SAPData\"}]},\"typePath\":\"Description\",\"extensions\":{\"sap:filterable\":\"false\",\"" +
				"sap:sortable\":\"false\",\"sap:field-control\":\"Description_FC\"}}"),
			fnMandatoryCheck = this.oMetaData.getMandatoryCheck(oProperty);
		...
		try {
			...
			// Assert
			assert.ok(oException.message, oRB.getText("MANDATORY_FIELD_ERROR"), "Exception has generic error message");
		}
	});
	...
	QUnit.test("getMandatoryCheck should set generic error message when there is not label", function(assert) {
		...
		var sLabel = "Some Label",
			oProperty = JSON.parse("{\"property\":{\"name\":\"Description\",\"com.sap.vocabularies.Common.v1.FieldControl\"" +
				":{\"EnumMember\":\"com.sap.vocabularies.Common.v1.FieldControlType/Mandatory\"},\"type\":\"" +
				"Edm.String\",\"maxLength\":\"80\",\"extensions\":[{\"name\":\"field-control\",\"value\":\"" +
				"Description_FC\",\"namespace\":\"http://www.sap.com/Protocols/SAPData\"},{\"name\":\"" +
				"sortable\",\"value\":\"false\",\"namespace\":\"http://www.sap.com/Protocols/SAPData\"" +
				...
				"sap:sortable\":\"false\",\"sap:field-control\":\"Description_FC\"}}"),
			fnMandatoryCheck = this.oMetaData.getMandatoryCheck(oProperty),
			fnStub = sinon.stub(this.oParent, "getComputedTextLabel").returns(sLabel);
		...
		try {
			...
			// Assert
			assert.ok(oException.message, oRB.getText("MANDATORY_FIELD_WITH_LABEL_ERROR", [sLabel]), "Exception has correct error message");
		}
	...
	/**
	 * @deprecated Since 1.32.0 The <code>SmartField</code> control can ignore the proposal.
	 */
