...
sap.ui.define([
	...
	QUnit.test("createValueHelp - enabledMultiSelectionPlugin config as false", function(assert) {
		...
		assert.equal(oFactory._aProviders[0].enabledMultiSelectionPlugin, false);
	...
	QUnit.test("createValueHelp - enabledMultiSelectionPlugin config as true", function(assert) {
		var oMeta, oMetaData, oSet, oType, oFactory, fOnValueListChange, oAnnotation, oControl = {
			setModel: function () {
		...
		assert.equal(oFactory._aProviders[0].enabledMultiSelectionPlugin, true);
	...
	QUnit.test("createValueHelp - enabledMultiSelectionPlugin not config", function(assert) {
		var oMeta, oMetaData, oSet, oType, oFactory, fOnValueListChange, oAnnotation, oControl = {
			setModel: function () {
	...
	QUnit.test("_getDisplayBehaviourConfiguration - the default display behaviour should be 'descriptionAndId' when criticalityQualifier is applied", function(assert) {
		// Arrange
		var sDisplay,
			oFactory,
			oParent = sinon.createStubInstance(SmartField);
		...
		oParent.getConfiguration = function() {
			return {
				getCriticalityQualifier: function() { return "ProductQualifier"; },
				getDisplayBehaviour: function() { return "auto"; } //if not explicitly set as property, getDisplayBehaviour() will always return "auto" as default
			};
		};
