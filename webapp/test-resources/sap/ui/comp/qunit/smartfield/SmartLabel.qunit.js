...
sap.ui.define([
	...
	QUnit.test("it should react on visibility changes of the SmartField control even if the control is NOT " +
			"initially rendered", async function(assert) {
	...
	QUnit.test("method onBeforeRendering should not raise an runtime error if the SmartField is not found", function(assert) {
		// Arrange
		var bIsErrorRaised = false;
		this.oSmartLabel = new SmartLabel();

		sinon.stub(this.oSmartLabel, "_setLabelFor");
		sinon.stub(this.oSmartLabel, "_sSmartFieldId").value("dummy");
		...
		// Assert
		assert.notOk(bIsErrorRaised, "An error was not raised.");
