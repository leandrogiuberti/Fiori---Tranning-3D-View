sap.ui.define([
	...
	return Controller.extend("applicationUnderTest.smartchart_Timezone_UTC.SmartChart", {
		oResults: {
			filterQueryInput: "",
			variantInput: "",
			uiStateInput: "",
		...
		onInit: function() {
			...
			this._oSmartChart = this.getView().byId("smartChart");
			this._oChangeVisibleField = this.getView().byId("changeVisibleField");
			this._sSelectedKey = "";
			this._oSelectedField;

			this.getView().setModel(new ODataModel("/smartchart_Timezone_UTC", true));
		...
		onUpdateResults: function() {
			this.oResults = {
				filterQueryInput: this._printFilters(this._oSmartChart._getChartPersonalisationData().filters),
				variantInput: this._getVariantValue(this._oSmartChart.fetchVariant().filter.filterItems[0]),
				uiStateInput: this._getUiStateValue(this._oSmartChart.getUiState().getSelectionVariant().SelectOptions[0].Ranges[0]),
				filterModelInput: this._getFilterModelValue(this._oSmartChart._oCurrentVariant.filter.filterItems[0]),
				isUsingUI5DateInput: this._getIsUsingUI5DateValue(this._oSmartChart._oCurrentVariant.filter.filterItems[0])
			};
		...
		onReset: function () {
			this._clearInputs();
			this._oSmartChart.applyVariant({});
		...
		onApplyVariant: function (bWithDate) {
			...
			if (sType === "datetime" || sType === "datetimeranges") {
				...
				vDate2 = bWithDate ? new Date(this._oDateTimeToSet1) : this._sDateTimeToSet1;
			}
		...
		onApplyVariantFetchVariant: function () {
			this._clearInputs();
			this._oSmartChart.applyVariant(this._oSmartChart.fetchVariant());
			this.onUpdateResults();
		},

		onSetUiStateGetUiState: function () {
			this._clearInputs();
			this._oSmartChart.setUiState(this._oSmartChart.getUiState());
			this.onUpdateResults();
		},
		...
		_printFilters: function(aFilters) {
			...
			var oDataModelMetadata = this._oSmartChart.getModel().oMetadata,
				oMetadataAnalyser = this._oSmartChart._oChartProvider._oMetadataAnalyser,
				sEntitySet = this._oSmartChart.getEntitySet(),
