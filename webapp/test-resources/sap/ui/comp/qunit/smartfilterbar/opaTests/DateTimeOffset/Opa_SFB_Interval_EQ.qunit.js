...
sap.ui.define([
	...
	opaTest("Check with condition EQ store correct data in UIState", function(Given, When, Then) {
		// Arrange
		Given.iEnsureMyAppIsRunning();
		var aControlName = ["DTOFFSET_INTERVAL"],
			...
			sDateEndResult = sLow.substring(0,19) + ".999Z";
		// Act
		When.iClickShowAll();
		fSetConditionValues(When, aControlName, sViewName, "EQ",[sDate]);
		When.iPressTheFilterGoButton();
		// Assert
		var sResult = "(DTOFFSET_INTERVAL ge datetimeoffset'" + sDateStartResult +
				"' and DTOFFSET_INTERVAL le datetimeoffset'" + sDateEndResult + "')",
			oRange = {
				"Sign": "I",
				"Option": "EQ",
				"Low": sLow,
	...
	opaTest("Check with condition EQ send query with precision", function(Given, When, Then) {
		...
		var aControlName = ["DTOFFSET_INTERVAL", "DTOFFSET_INTERVAL_PRECISION0", "DTOFFSET_INTERVAL_PRECISION1", "DTOFFSET_INTERVAL_PRECISION2", "DTOFFSET_INTERVAL_PRECISION3",
			...
			sLow = new Date(sDate).toJSON(),
			sDateStartResult = sLow.substring(0,19) + "Z",
			sDateEndResult = sLow.substring(0,19);
	...
	opaTest("Saving and restoring variant with condition EQ", function(Given, When, Then) {
		...
		// Act
		var sVariantName = "DTOFFSET_INTERVAL_EQ" + new Date().toISOString();
