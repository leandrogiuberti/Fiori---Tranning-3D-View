sap.ui.define([
	...
	"sap/m/Panel",
	"sap/m/Text"
], function(ODataUtils, ODataModel, Controller, MessageToast, UIState, VariantConverterFrom, Filter, EventProvider, sinon, CustomListItem, Panel, Text) {
	"use strict";

	return Controller.extend("test.app.SmartFilterBar", {
		...
		onInit: function() {
			...
			this._oFilterBarShort = this.getView().byId("smartFilterBarShort");
		...
		registerLogging: function () {
			...
			sinon.stub(EventProvider.prototype, "fireEvent").callsFake(function(sEventId, oParameters, bAllowPreventDefault, bEnableEventBubbling){
				var oContext = this,
					sText,
					oText,
					oCustomListItem;

				if (sEventId === "initialise" ||
					sEventId === "valueListAnnotationLoaded" ||
					sEventId === "initialized") {
						if (oContext.isA("sap.ui.comp.smartfilterbar.SmartFilterBar")) {
							sText = "Source:\n" + oContext.getId() + "\n\nEventId:\n" + sEventId;
							...
							that.byId("events").addItem(oCustomListItem);
					}
				}
...
//				}
		...
		onTimeStampFromChange: function(oEvent){
