...
sap.ui.define([
	...
	QUnit.test("getJSONFromAdditionalConfiguration shall convert the additional configuration into a JS Object", function(assert) {
		...
		assert.strictEqual(oAdditionalConfigurationHelper.controlConfiguration[0].conditionPanelDefaultOperation, null);
	...
	QUnit.test("Default operation should be applied when is valid", function(assert) {
		var oControlConfiguration, oAdditionalConfigurationHelper;

		//Control Configuration
		oControlConfiguration = new ControlConfiguration();
		oControlConfiguration.setKey("key");
		oControlConfiguration.setConditionPanelDefaultOperation(library.valuehelpdialog.ValueHelpRangeOperation.EQ);
		...
		assert.equal(oAdditionalConfigurationHelper.controlConfiguration.length, 1);
		assert.equal(oAdditionalConfigurationHelper.controlConfiguration[0].key, "key");
		assert.strictEqual(oAdditionalConfigurationHelper.controlConfiguration[0].conditionPanelDefaultOperation, library.valuehelpdialog.ValueHelpRangeOperation.EQ);
	});
});
