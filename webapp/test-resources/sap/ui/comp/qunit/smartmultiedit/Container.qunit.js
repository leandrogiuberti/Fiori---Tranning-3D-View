...
sap.ui.define("test.sap.ui.comp.qunit.smartmultiedit.Container", [
	...
	"sap/base/Log"
			 ...
			 Group, GroupElement, Container, Context, Log) {
	"use strict";
	...
	QUnit.test("Returns all fields inside an array", function (assert) {
		...
		assert.equal(this.oContainer._getFields(true).length, 3, "Three fields returned when getFields is passed with true as param");
	});
	...
	QUnit.test("Check for fields when _aFields is null or an empty array", function (assert) {
		//Arrange
		this.oContainer.setLayout(this.oForm);
		this.oContainer._aFields = null;
		//Assert
		assert.equal(this.oContainer._getFields().length, 2, "Valid value returned when _aFields is null");

		//Arrange
		this.oContainer._aFields = [];
		//Assert
		assert.equal(this.oContainer._getFields().length, 2, "Valid value returned when _aFields is an empty array");
	});
		...
		beforeEach: function () {
			...
			this.oStub = sinon.stub(this.oContainer, "getModel").returns({
				...
				createEntry: function() {
					that.oContainer.getPath = function(){
						return "/Employees";
					};
	...
	QUnit.test("Index field of type 'sap.ui.comp.smartmultiedit.Field' at container", function (assert) {
		// Arrange
		this.oMultiEditField._createSmartField();
	...
	QUnit.test("resetContainer", function (assert) {

		//Arrange
		this.oContainer._aFields = [this.oMultiEditField];
		sinon.stub(this.oMultiEditField, "resetField", function(){});
			//dumb binding context
		var oBindingContext = {
			key: "value"
		};
		this.oContainer.setBindingContext(oBindingContext);
		//Act
		this.oContainer.resetContainer();
		//Assert
		assert.equal(oBindingContext, this.oContainer.getBindingContext(), 'resetContainer should not reset binding context');
	});
