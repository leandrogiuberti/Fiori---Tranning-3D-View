...
sap.ui.define([
	...
	QUnit.test("test _getRowCount function", function(assert) {
		...
		oResult = this.oSmartList._getRowCount(true);
		...
		isLengthFinal = false;
		oResult = this.oSmartList._getRowCount(true);
		assert.ok(oResult === undefined, "row count could not be determined correctly");

		isLengthFinal = true;
		oRowBinding.isA = function(str) {
			return str === "sap.ui.model.TreeBinding";
		};
		oResult = this.oSmartList._getRowCount(true);
		assert.ok(oResult === undefined, "row count could not be determined correctly");

		oRowBinding.getCount = function() {
			return 5;
		};
		oResult = this.oSmartList._getRowCount(true);
		assert.equal(oResult, 5, "row count has to be returned correctly");
		oResult = this.oSmartList._getRowCount();
		assert.equal(oResult, 10, "row count has to be returned correctly");
	});
	...
	QUnit.test("test header text features", function(assert) {
		...
		this.oSmartList._createToolbarContent();
		assert.equal(this.oSmartList.getHeader(), sHeaderText, "header text has to be equal");
		assert.equal(this.oSmartList._headerText.getText(), sHeaderText + " (5)", "header text has to contain row count");

		this.oSmartList._getRowCount = function() {
			return 0;
		};
		this.oSmartList._refreshHeaderText();
		assert.equal(this.oSmartList._headerText.getText(), sHeaderText, "header text does not contain row count when it is 0");
	});
	...
	QUnit.test("No $expand when expandFields is empty", function(assert) {
		var done = assert.async();

		this.oSmartList.attachBeforeRebindList(function(oEvent) {
			var oSettings = oEvent.getParameter("bindingParams");

			// No binding is required, only the binding paramters have t be checked
			oSettings.preventListBind = true;

			assert.notOk(Object.keys(oSettings.parameters).includes("expand"), "expand property not defined on binding parameters");

			done();
		});

		assert.ok(this.oSmartList.getExpandFields() === "", "An empty string is returned");

		this.oSmartList._reBindList();
	});
