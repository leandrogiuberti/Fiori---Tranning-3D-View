...
sap.ui.define([
	...
	QUnit.test("Checking the convert method", function(assert) {
		...
		var oFilterBar = {
			...
			_determineFilterItemByName: function(sName) {
				return oFilterItem;
			},
	...
	QUnit.test("Checking the _addRangesAccordingMetaData method with 'multiple'", function(assert) {
		var oControl = {
			isA: function(sClassName){
					return !(sClassName === 'sap.m.MultiInput' || sClassName === 'sap.m.MultiComboBox');
				}
		};
		...
		this.oVariantConverter._addRangesAccordingMetaData(oContent, null, oFilterMetaData, aRanges, oMultiComboBox, "");
		assert.ok(oContent.FIELD);
		...
		this.oVariantConverter._addRangesAccordingMetaData(oContent, null, oFilterMetaData, aRanges, oMultiInput, "");
		assert.ok(oContent.FIELD);
		assert.ok(oContent.FIELD.items);
		...
		oContent = {};
		this.oVariantConverter._addRangesAccordingMetaData(oContent, null, oFilterMetaData, aRanges, oControl, "");
		assert.ok(oContent.FIELD);
		assert.ok(oContent.FIELD.ranges);
		assert.equal(oContent.FIELD.ranges.length, 1);
		assert.ok(oContent.FIELD.ranges[0]);
	...
	QUnit.test("Should handle Empty operation", function (assert) {
		// Arrange
		var oResultOptions,
			sOperation = "EQ",
			sLow = null;
		...
		// Act
		oResultOptions = VariantConverterFrom.convertOption(sOperation, sLow);

		// Assert
		assert.equal(oResultOptions.op, "Empty");
		assert.equal(oResultOptions.v, null);
	...
	QUnit.test("Should handle empty string as key", function (assert) {
		// Arrange
		var oResultOptions,
			sOperation = "EQ",
			sValue = "",
		...
		// Act
		oResultOptions = VariantConverterFrom.convertOption(sOperation, sValue, oMetaData);

		// Assert
		assert.equal(oResultOptions.op, "EQ");
		assert.equal(oResultOptions.v, "");
	...
	QUnit.test("Checking _addRangesAccordingMetaData should not handle MultiComboBox default selected item with empty key as Empty operation", function (assert) {
		// Arrange
		var oFieldContent,
			oContent = {},
			oFilterBar = {},
			...
			oControl = sinon.createStubInstance(MultiComboBox);

		oControl.isA.withArgs('sap.m.MultiComboBox').returns(true);

		// Act
		this.oVariantConverter._addRangesAccordingMetaData(oContent, oFilterBar, oFilterMetaData, aRanges, oControl);
		oFieldContent = oContent["DefectCategory"];

		// Assert
		assert.ok(Array.isArray(oFieldContent.ranges));
		assert.equal(oFieldContent.ranges.length, 0);

		assert.ok(Array.isArray(oFieldContent.items));
		assert.equal(oFieldContent.items.length, 1);
		assert.equal(oFieldContent.items[0].key, "");

		assert.equal(oFieldContent.value, null);
