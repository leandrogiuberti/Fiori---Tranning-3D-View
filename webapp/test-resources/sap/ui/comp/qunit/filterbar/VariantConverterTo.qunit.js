...
sap.ui.define([
				...
				"ranges": [
					{
						...
						"operation": "EQ",
						"keyField": "BLDAT",
						"value1": new Date(2015, 7, 5),
						"value2": "",
						"tokenText": "!(=Aug 5, 2015)"
					}
				]
			}
		};
	...
	QUnit.test("Checking Edm.Int field", function(assert) {
	...
	QUnit.test("Should handle Empty operation", function(assert) {
		...
		// Act
		VariantConverterTo.addRanges(aOutputRanges, aRanges);

		// Assert
		oOutputRange = aOutputRanges[0];
		assert.equal(oOutputRange.Sign, "E");
		assert.equal(oOutputRange.Option, "EQ");
		assert.equal(oOutputRange.Low, "");
		assert.equal(oOutputRange.High, undefined);
	...
	QUnit.test("addRanges method should not add range if the operation is not valid", function (assert) {
		var aOutputRanges = [],
			aRanges = [
				{
					"exclude": true,
					"operation": "non-existing",
					"value1": "value1",
					"value2": "value2"
				}
			];
		...
		// Assert
		assert.equal(aOutputRanges.length, 0, "no ranges should be added");
	});

	QUnit.test("_processP13NDateValue should skip fiscal dates", function (assert) {
		// Act
		VariantConverterTo._processP13NDateValue("0012020", "date", true, true);
