...
sap.ui.define([
	...
	QUnit.test("Shall return an object of export config", function(assert) {
		var oExportCapabilities = this.oTableProvider.getExportCapabilities("UI5Client");

		assert.ok(oExportCapabilities["XLSX"], "Excel export enabled for export type UI5Client");
		assert.notOk(oExportCapabilities["PDF"], "PDF export is not enabled for export type UI5Client");

		this.oTableProvider._bSupportsPDFExport = true;
		var oExportCapabilities = this.oTableProvider.getExportCapabilities("UI5ClientPDF");
		assert.ok(oExportCapabilities["XLSX"], "Excel export enabled for export type UI5ClientPDF");
		assert.ok(oExportCapabilities["PDF"], "PDF export enabled for export type UI5ClientPD");
	});
	...
	QUnit.test("#_isVisible should consider the LineItem UI.Hidden annotation and update visibility of the Field", function(assert) {
		var aBackendMetadata = [
			{
				name: "test",
				type: "Edm.String",
		...
		var oLineItemAnnotation = {
			...
			hidden: {test: true, foo: false, ID: true, test1: false}
		};
		...
		assert.equal(this.oTableProvider._isVisible(aBackendMetadata[0]), false);
		assert.equal(aBackendMetadata[0].visible, false);
		assert.equal(this.oTableProvider._isVisible(aBackendMetadata[1]), true);
		assert.equal(aBackendMetadata[1].visible, true);
		assert.equal(this.oTableProvider._isVisible(aBackendMetadata[2]), false);
		assert.equal(aBackendMetadata[2].visible, false);
		assert.equal(this.oTableProvider._isVisible(aBackendMetadata[3]), false);
		assert.equal(aBackendMetadata[3].visible, false);
	});

	QUnit.test("_isVisible works correctly even without LineItem annotation or UI.Hidden", function(assert) {
		const oProvider = this.oTableProvider;
		const oField = { name: "ID", visible: true };

		/* Without LineItem annotation */
		oProvider._oLineItemAnnotation = undefined;
		assert.ok(oProvider._isVisible(oField), "Field is visible");

		/* Empty LineItem annotation */
		oProvider._oLineItemAnnotation = {};
		assert.ok(oProvider._isVisible(oField), "Field is visible");

		oProvider._oLineItemAnnotation.hidden = { ID : true };
		assert.notOk(oProvider._isVisible(oField), "Field is hidden");
	});
	...
	QUnit.test("getFieldLabel", function(assert) {
		var aBackendMetadata = [
			{
				name: "Property1",
				type: "Edm.String",
				...
				description: "desc",
				visible: true,
				fieldLabel: "Property Label 1"
			}, {
				name: "Property2",
				type: "Edm.String",
				visible: true,
				"com.sap.vocabularies.Common.v1.Label": {
					String: "Property Label 2"
				},
				fieldLabel: "A Property Label"
			}, {
				name: "Property3",
				type: "Edm.String",
				visible: true
		...
		var oBackendNavigationProperty = {
			"name": "Property4/FixedAsset",
			"type": "Edm.String",
			"sap:label": "Subnumber",
			"com.sap.vocabularies.Common.v1.Label": {
				"String": "Subnumber"
		...
		this.oTableProvider._oMetadataAnalyser = sinon.createStubInstance(MetadataAnalyser);
		this.oTableProvider._oMetadataAnalyser.getFieldsByEntitySetName.returns(aBackendMetadata);
		this.oTableProvider._oMetadataAnalyser.extractNavigationPropertyField.returns(oBackendNavigationProperty);

		assert.strictEqual(this.oTableProvider.getFieldLabel("foo", "Property1"), "Property Label 1", "fieldLabel returned");
		assert.strictEqual(this.oTableProvider.getFieldLabel("foo", "Property2"), "Property Label 2", "com.sap.vocabularies.Common.v1.Label returned");
		assert.strictEqual(this.oTableProvider.getFieldLabel("foo", "Property3"), null, "null returned, since no label found");
		assert.strictEqual(this.oTableProvider.getFieldLabel("foo", "Property4/FixedAsset"), "Subnumber", "Navigation property Label returned");
	});

	// customer incident: 0020751294 0000428318 2021
	QUnit.test("for IsFiscalYearWeek, IsFiscalYearQuarter & IsFiscalYearPeriod annotation, the MaxLength should be increased by 1", function(assert) {
		var aBackendMetadata = [
			{
				name: "Property1",
				type: "Edm.String",
		...
		assert.strictEqual(aResult[0].name, "Property1");
		assert.strictEqual(aResult[0].maxLength, "8", "MaxLength is increased by 1 for IsFiscalYearPeriod");

		assert.strictEqual(aResult[1].name, "Property2");
		assert.strictEqual(aResult[1].maxLength, "6", "MaxLength is increased by 1 for IsFiscalYearQuarter");

		assert.strictEqual(aResult[2].name, "Property3");
		assert.strictEqual(aResult[2].maxLength, "7", "MaxLength is increased by 1 for IsFiscalYearWeek");
	});

	QUnit.test("For Edm.DateTimeOffset having Timezone property should be added to the additionalProperty", function(assert) {
		var aBackendMetadata = [
			{
		...
		var aResult = this.oTableProvider.getTableViewMetadata();
		assert.ok(aResult[0].additionalProperty);
		assert.strictEqual(aResult[0].additionalProperty, "TimezoneProperty", "Timezone property path is added to the additionalProperty");
	});

	QUnit.test("Test semanticObjectPath", function(assert) {
		var oBackendMetadata = {
		...
		this.oTableProvider._enrichWithTableViewMetadata(oBackendMetadata);
		assert.strictEqual(oBackendMetadata.semanticObjectPath, "SemanticObjectName", "SemanticObjectPath updated in the FieldMetadata");
		...
		this.oTableProvider._enrichWithTableViewMetadata(oBackendMetadata);
		assert.strictEqual(oBackendMetadata.semanticObjectPath, undefined, "SemanticObjectPath not found hence default value is undefined");
	});
});
