...
sap.ui.define([
	...
	QUnit.test("getConnectedFieldsAnnotation shall return the matching ConnectedFields annotation array", function(assert) {
		var oEntity, aFieldGroupAnnotation, oTestingObjects;
		...
		oEntity = {
			...
			"property": [
				...
				{
					"name": "CriticalitySuccess",
					"type": "Edm.String",
					"nullable": "false",
					"sap:creatable": "false",
					"sap:updatable": "true",
					"sap:label": "Criticality",
					"com.sap.vocabularies.Common.v1.Label": {
						"String": "Criticality"
					}
				}
			...
			"com.sap.vocabularies.UI.v1.ConnectedFields": {
				"Label": {
					"String": "Semantically connected field(No Qualifier)"
				},
				"Template": {
					"String": "{Name}"
				},
			...
			"com.sap.vocabularies.UI.v1.ConnectedFields#IsSimulatedIndicator": {
				"Label": {
					"String": "Semantically connected field"
				},
				"Template": {
					"String": "{Name} / {Surname}"
				},
		...
		aFieldGroupAnnotation = this.oMetadataAnalyser.getConnectedFieldsAnnotation("EmployeesNamespace.Employee");

		assert.ok(aFieldGroupAnnotation);
		assert.strictEqual(aFieldGroupAnnotation.length, 2, "getConnectedFieldsAnnotation reads the correct count of the ConnectedField annotations.");

		var oTestingObjects = [
			{fields: ["Name"], groupName: undefined, groupLabel: "Semantically connected field(No Qualifier)", criticalityField: "Name", criticalityFieldPath: "CriticalityWarning"},
			{fields: ["Name", "Surname"], groupName: "IsSimulatedIndicator", groupLabel: "Semantically connected field", criticalityField: "Surname", criticalityFieldPath: "CriticalitySuccess"}
		];

		for (let i = 0; i < aFieldGroupAnnotation.length; i++) {
			const oAnnotation = aFieldGroupAnnotation[i],
				sCriticalityFieldName = oTestingObjects[i].criticalityField;


			assert.strictEqual(oAnnotation.groupName, oTestingObjects[i].groupName, "Qualifier is correctly read.");
			assert.strictEqual(oAnnotation.groupLabel, oTestingObjects[i].groupLabel, "Label is correctly read.");
			assert.ok(oAnnotation.criticality[sCriticalityFieldName], "Criticality of a field is correctly read.");
			assert.strictEqual(oAnnotation.criticality[sCriticalityFieldName].path, oTestingObjects[i].criticalityFieldPath, "Criticality path of a field is read correctly.");

			assert.deepEqual(oAnnotation.fields, oTestingObjects[i].fields, "Fields are correctly read.");
		}

		this.oMetadataAnalyser._oMetaModel.getODataEntityType.returns(null);
		aFieldGroupAnnotation = this.oMetadataAnalyser.getConnectedFieldsAnnotation("EmployeesNamespace.Employee");
		assert.ok(aFieldGroupAnnotation);
		assert.strictEqual(aFieldGroupAnnotation.length, 0, "getConnectedFieldsAnnotation reads the correct count of the ConnectedField annotations.");
	});
