...
(function() {
	...
	sap.ui.define([
		...
		QUnit.test("Shall return the proper UI5 ValueState based on CriticalityType/Int32 value", function(assert) {
			...
			// String type should also be considered to return the correct Int32 value
			sState = CriticalityMetadata.getCriticalityState("0");
			assert.strictEqual(sState, "None");
			sState = CriticalityMetadata.getCriticalityState("1");
			assert.strictEqual(sState, "Error");
			sState = CriticalityMetadata.getCriticalityState("2");
			assert.strictEqual(sState, "Warning");
			sState = CriticalityMetadata.getCriticalityState("3");
			assert.strictEqual(sState, "Success");
		});
		...
		QUnit.test("Shall return the proper UI5 Icon based on CriticalityType/Int32 value", function(assert) {
			...
			sIcon = CriticalityMetadata.getCriticalityIcon("0");
			assert.strictEqual(sIcon, null);
			sIcon = CriticalityMetadata.getCriticalityIcon("1");
			assert.strictEqual(sIcon, "sap-icon://error");
			sIcon = CriticalityMetadata.getCriticalityIcon("2");
			assert.strictEqual(sIcon, "sap-icon://alert");
			sIcon = CriticalityMetadata.getCriticalityIcon("3");
			assert.strictEqual(sIcon, "sap-icon://sys-enter-2");
		});
