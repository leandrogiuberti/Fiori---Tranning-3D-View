sap.ui.require([
	...
	"sap/ui/core/Element"
		...
		Device, JSONModel, typeDouble, typeOdataString, typeInt16, typeSingle, typeDecimal, typeOdataDate, typeOdataDateTimeOffset, typeOdataTime, typeOdataDateTime,
		typeOdataBoolean, typeDate, typeString, typeTime, typeBoolean, Title, mLibrary, TextArea, Token, Tokenizer, MultiInput, MessageToast, Label, Input,
		SearchField, Switch, List, StandardListItem, ResponsivePopover, CheckBox, RadioButtonGroup, RadioButton, Page, App, VBox, ColumnListItem, SimpleForm, jQuery, Formatting, CoreLib, Element){
	"use strict";
	...
	var oTokenParser = new TokenParser({defaultOperation: ValueHelpRangeOperation.EQ});
	oTokenParser.associateInput(theTokenInput);
	oTokenParser.addKeyField({ key: "name", label: "Name", type: "string", oType: new typeString() });
	...
	theTokenInput.attachValueHelpRequest(function() {
		...
		if (useExternalTable.getSelected()) {
			...
			CoreLib.load('sap.ui.table', {async: true}).then(function() {
				sap.ui.require([
//						"sap/ui/table/Table"
//						"sap/m/Table"
...
//				MessageToast.show("Tokens= " + sTokens);
		...
		if (multipleKeyFields.getSelected()) {
			oValueHelpDialog.setRangeKeyFields([{
				...
				type: "string",
				//maxLength: "10",
				typeInstance: new typeOdataString({}, {
					maxLength: 10
				})
			}, {
				key: "Int16",
		...
		var oFilterBar = new FilterBar({
			...
			search: function(oEvent) {
				...
				if (this.getBasicSearch()) {
					sMsg = Element.getElementById(this.getBasicSearch()).getValue();
	...
	var oSwitch = new Switch({ customTextOn : "none Gregorian", customTextOff : "Gregorian", change: function(oEvent) {
	...
	var oTextArea = new TextArea({rows: 20, cols: 150});
