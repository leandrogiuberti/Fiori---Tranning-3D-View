sap.ui.define([
    ...
    "sap/ui/core/Element",
    "../locationColor"
], function (Mobile, Element, locationColor) {
    ...
    var oAddCmpBarButton = new sap.m.Button({
        text: "Add Comparison Bar", press: function (oE) {
            oGenericTileData.data.push({ title: fnSelectRegion(), value: fnNumerInRange(-100, 100), color: fnSelectColor() })
            oGenericTileModel.checkUpdate();
        }
    });

    var oRemCmpBarButton = new sap.m.Button({
        text: "Remove Comparison Bar", press: function (oE) {
            oGenericTileData.data.splice(oGenericTileData.data.length - 1);
            oGenericTileModel.checkUpdate();
        }
    });

    var oWideSwith = new sap.m.Switch({
        state: true, customTextOff: "Normal", customTextOn: "Wide", change: function (oE) {
            var oCmpChart = Element.getElementById("comp-chart-r-__clone0");
            var bState = oE.getParameter("state");
            if (bState) {
