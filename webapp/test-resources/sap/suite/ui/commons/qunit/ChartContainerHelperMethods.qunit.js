sap.ui.define([
	...
	"sap/base/Log",
		...
		SegmentedButtonItem, Control, ToolbarSpacer, sinon, nextUIUpdate, Popover, Dialog, ResponsivePopover, Log) {
	"use strict";
	...
	QUnit.test("_chartChange changes the chart properly", function(assert) {
		...
		assert.ok(this.oChartContainer._oActiveChartButton instanceof SegmentedButtonItem, "_oActiveChartButton is an sap.m.Button");
	...
	QUnit.test("_adjustIconsDisplay adds all _aCustomIcons to content toolbar", function(assert) {
		...
		assert.ok(spyAddItem.called, "_addContentToolbar called");
		assert.ok(spyAddItem.calledTwice, "_addContentToolbar called twice");
		assert.ok(stubAddContentToolbar.called, "_addContentToolbar called");
