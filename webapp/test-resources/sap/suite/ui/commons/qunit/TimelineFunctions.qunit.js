sap.ui.define([
...
], function (suiteLibrary, Timeline, UI5Date) {
	"use strict";

	var TimelineGroupType = suiteLibrary.TimelineGroupType;

	QUnit.module("TimelineFunctions");

	QUnit.test("Date diff: Year 2016 - 2014", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Year, UI5Date.getInstance(2014, 1, 1), UI5Date.getInstance(2016, 1, 1));
		assert.equal(diff, 2, " 2016 - 2014 = 2 years");
	});

	QUnit.test("Date diff: Year 2016 - 2016", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Year, UI5Date.getInstance(2016, 1, 1), UI5Date.getInstance(2016, 1, 1));
		assert.equal(diff, 0, "2016 - 2016 = 0 years");
	});

	QUnit.test("Date diff: Month 2016/5 - 2016/9", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Month, UI5Date.getInstance(2016, 5, 1), UI5Date.getInstance(2016, 9, 1));
		assert.equal(diff, 4, "2016/5 - 2016/9 = 4 months");
	});

	QUnit.test("Date diff: Month 2010/11 - 2011/0", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Month, UI5Date.getInstance(2010, 11, 1), UI5Date.getInstance(2011, 0, 1));
		assert.equal(diff, 1, "2010/11 - 2011/0 = 1 month");
	});

	QUnit.test("Date diff: Quarter 2010/11 - 2011/0", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Quarter, UI5Date.getInstance(2010, 11, 1), UI5Date.getInstance(2011, 0, 1));
		assert.equal(diff, 1, "2010/11 - 2011/0 = 1 quarter");
	});

	QUnit.test("Date diff: Quarter 2010/11 - 2010/11", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Quarter, UI5Date.getInstance(2010, 11, 1), UI5Date.getInstance(2010, 11, 1));
		assert.equal(diff, 0, "2010/11 - 2010/11 = 0 quarters");
	});

	QUnit.test("Date diff: Quarter 2010/10 - 2010/11", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Quarter, UI5Date.getInstance(2010, 10, 1), UI5Date.getInstance(2010, 11, 1));
		assert.equal(diff, 0, "2010/10 - 2010/11 = 0 quarters");
	});

	QUnit.test("Date diff: Quarter 2016/8 - 2017/0", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Quarter, UI5Date.getInstance(2016, 8, 1), UI5Date.getInstance(2017, 0, 1));
		assert.equal(diff, 2, "2016/8 - 2017/0 = 2 quarters");
	});

	QUnit.test("Date diff: Quarter 2016/0 - 2016/10", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Quarter, UI5Date.getInstance(2016, 0, 1), UI5Date.getInstance(2016, 10, 1));
		assert.equal(diff, 3, "2016/0 - 2016/10 = 3 quarters");
	});

	QUnit.test("Date diff: Day 2016/0/0 - 2016/0/1", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Day, UI5Date.getInstance(2016, 0, 0), UI5Date.getInstance(2016, 0, 1));
		assert.equal(diff, 1, "2016/0/0 - 2016/0/1 = 1 day");
	});

	QUnit.test("Date diff: Day 2015/12/31 - 2016/0/1", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Day, UI5Date.getInstance(2015, 11, 31), UI5Date.getInstance(2016, 0, 1));
		assert.equal(diff, 1, "2015/12/31 - 2016/0/1 = 1 day");
	});

	QUnit.test("Date diff: Day 2015/12/31 - 2016/0/1", function (assert) {
		var diff = Timeline.prototype._fnDateDiff(TimelineGroupType.Day, UI5Date.getInstance(2015, 0, 1), UI5Date.getInstance(2016, 0, 1));
	...
	QUnit.test("Date add - 10 years round up with owerflow", function (assert) {
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			oMaxDate = UI5Date.getInstance(2025, 5, 5),
	...
	QUnit.test("Test _setRangeFilter for Custom Range & Months", function (assert) {
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			oMaxDate = UI5Date.getInstance(2017, 5, 5),
		...
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			oMaxDate = UI5Date.getInstance(2040, 5, 1),
	...
	QUnit.test("Date add - 10 years round up within range", function (assert) {
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			...
			oTimeline = buildTimeline(oMinDate, oMaxDate, TimelineGroupType.Year),
			oDate = oTimeline._fnAddDate(10, "UP");
		assert.equal(oDate.getTime(), UI5Date.getInstance(2027, 11, 31, 23, 59, 59).getTime(),
	...
	QUnit.test("Date add - 10 years round down", function (assert) {
		...
		assert.equal(oDate.getTime(), UI5Date.getInstance(2027, 0, 1).getTime(),
	...
	QUnit.test("Date add - 10 months round up", function (assert) {
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			...
			oTimeline = buildTimeline(oMinDate, oMaxDate, TimelineGroupType.Month),
			oDate = oTimeline._fnAddDate(10, "UP");
		assert.equal(oDate.getTime(), UI5Date.getInstance(2018, 3, 30, 23, 59, 59).getTime(),
	...
	QUnit.test("Date add - 10 months round down", function (assert) {
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			...
			oTimeline = buildTimeline(oMinDate, oMaxDate, TimelineGroupType.Month),
			oDate = oTimeline._fnAddDate(10, "DOWN");
		assert.equal(oDate.getTime(), UI5Date.getInstance(2018, 3, 1).getTime(),
	...
	QUnit.test("Date add - 1 quarter round down", function (assert) {
		...
		assert.equal(oDate.getTime(), UI5Date.getInstance(2017, 6, 1).getTime(),
	...
	QUnit.test("Date add - 1 quarter round down", function (assert) {
		...
		assert.equal(oDate.getTime(), UI5Date.getInstance(2017, 8, 30, 23, 59, 59).getTime(),
	...
	QUnit.test("Date add - 0 quarter round up", function (assert) {
		...
		assert.equal(oDate.getTime(), UI5Date.getInstance(2017, 5, 30, 23, 59, 59).getTime(),
	...
	QUnit.test("Date add - 365 days round up", function (assert) {
		...
		assert.equal(oDate.getTime(), UI5Date.getInstance(2018, 5, 5, 23, 59, 59).getTime(),
	...
	QUnit.test("Date add - 1 days round down", function (assert) {
		var oMinDate = UI5Date.getInstance(2017, 5, 5),
			...
			oTimeline = buildTimeline(oMinDate, oMaxDate, TimelineGroupType.Day),
			oDate = oTimeline._fnAddDate(1, "DOWN");
		assert.equal(oDate.getTime(), UI5Date.getInstance(2017, 5, 6).getTime(),
	...
	QUnit.test("Test _calculateRangeTypeFilter for days", function (assert) {
		var oMinDate = UI5Date.getInstance(2016, 0, 1),
			oMaxDate = UI5Date.getInstance(2016, 0, 5),
	...
	QUnit.test("Test _calculateRangeTypeFilter for Months", function (assert) {
		var oMinDate = UI5Date.getInstance(2016, 0, 1),
			oMaxDate = UI5Date.getInstance(2016, 3, 1),
			oTimeline = buildTimeline(oMinDate, oMaxDate),
			sType = oTimeline._calculateRangeTypeFilter();
	...
	QUnit.test("Test _calculateRangeTypeFilter for Quarter", function (assert) {
		var oMinDate = UI5Date.getInstance(2016, 0, 1),
			oMaxDate = UI5Date.getInstance(2016, 7, 1),
			oTimeline = buildTimeline(oMinDate, oMaxDate),
			sType = oTimeline._calculateRangeTypeFilter();
	...
	QUnit.test("Test _calculateRangeTypeFilter for Year", function (assert) {
		var oMinDate = UI5Date.getInstance(2016, 0, 1),
			oMaxDate = UI5Date.getInstance(2018, 0, 5),
			oTimeline = buildTimeline(oMinDate, oMaxDate),
			sType = oTimeline._calculateRangeTypeFilter();
