sap.ui.define([
...
], function (jQuery, SimpleShape, SimpleShapeRenderer, suiteLibrary, StubsFactory,
						 HtmlElement, Utils, sinon, sinon_qunit, createAndAppendDiv, nextUIUpdate) {
	"use strict";

	var FillingType = suiteLibrary.statusindicator.FillingType;
	...
	*/
	...
	QUnit.test("Test set initial and runtime values using SemanticColor", function (assert) {
		// Given
		var oShape1 = new SimpleShape({
			fillColor: "Good",
			strokeWidth: 1,
		...
		// When
		oShape2.setFillColor("Error");
		oShape2.setStrokeWidth(2);
		oShape2.setStrokeColor("Information");

		// Then
		assert.equal(oShape1.getFillColor(), "Good", "FillColor is correctly set to Good");
		assert.equal(oShape2.getFillColor(), "Error", "FillColor is correctly set to Error");
		assert.equal(oShape1.getStrokeWidth(), 1, "Value of strokeWidth is correctly set to 1");
		assert.equal(oShape2.getStrokeWidth(), 2, "Value of strokeWidth is correctly set to 2");
		assert.equal(oShape1.getStrokeColor(), "Error", "Value of strokeColor is correctly set to Error");
		assert.equal(oShape2.getStrokeColor(), "Information", "Value of strokeColor is correctly set to Information");
	});

	/**
		* @deprecated Since version 1.120
	*/
	QUnit.test("Test html structure", async function (assert) {
	...
	QUnit.test("Test html structure using SemanticColor", async function (assert) {
		this.oSimpleShape = new SimpleShape({
			fillColor: "Sequence1"
		});
		var oAnimationPropertiesResolver = StubsFactory.createDummyPropertiesResolver(true);
		this.oSimpleShape._injectAnimationPropertiesResolver(oAnimationPropertiesResolver);
		...
		var oShapeElement = oRootElement.childNodes[1];
		assert.equal(oShapeElement.tagName, "rect");
		assert.ok(oShapeElement.getAttribute("class").includes("fillSemanticColorCritical"));
		assert.equal(Utils.getUrlId(oShapeElement.getAttribute("mask")), this.oSimpleShape.getId() + "-mask");
		assert.equal(oShapeElement.getAttribute("stroke-width"), 0);
	...
	QUnit.test("Test custom viewBox in html structure", async function (assert) {
		this.oSimpleShape = new SimpleShape();
	...
	*/
	QUnit.test("Update DOM Test ", function (assert) {
		...
		var getDisplayedFillColorStub = this.sandbox.stub(oSimpleShape,'getDisplayedFillColor').returns("red");
		...
		assert.ok(simpleShapeUpdateObject.attr.calledWith("fill", "red"), "fill is set to correct color");
		assert.ok(findNodes.attr.calledWith("offset", 0.5), "attr  called with right arguments");
	});

	QUnit.test("Update DOM Test using SemanticColor", function (assert) {
		// Given
		var oSimpleShape = new SimpleShape({
			fillingType: FillingType.Linear
		});
		...
		var getDisplayedFillColorStub = this.sandbox.stub(oSimpleShape,'getDisplayedFillColor').returns("Good");
		var simpleShape$Stub = this.sandbox.stub(oSimpleShape,'$');
		var findNodes = {
			attr: this.sandbox.stub()
		};
		...
		assert.ok(simpleShapeUpdateObject.find.calledWith("stop"), "find called with right arguments");
		assert.ok(getDisplayedFillColorStub.called, "getDisplayedFillColor called");
		assert.ok(simpleShapeUpdateObject.addClass.calledWith("fillSemanticColorGood"), "fill is set to correct color");
		assert.ok(findNodes.attr.calledWith("offset", 0.5), "attr  called with right arguments");
	});
	...
	QUnit.test("Check usage of polygon mask", async function (assert) {
		this.oSimpleShape = new SimpleShape({
			fillingType: FillingType.Linear,
			fillingAngle: 10
		});
	...
	function testFillingUnderAngleSupportedByGradient(testing, expected) {
		...
		QUnit.test("Filling under " + fillingAngle + "\u00B0 angle should use gradient filling", async function (assert) {
