sap.ui.define([
...
], function (suiteLibrary, Rectangle, Parameters, StubsFactory, Utils, sinon, nextUIUpdate, ThemingUtil) {
	"use strict";

	var FillingType = suiteLibrary.statusindicator.FillingType,
		oSemanticColorType = suiteLibrary.SemanticColorType,
		FillingDirectionType = suiteLibrary.statusindicator.FillingDirectionType;
	...
	function prepareRectangle(oRectangle){
		...
		oRectangle.setStrokeColor("Good");
		oRectangle.setFillColor("Error");
	...
	function prepareRectangleSemantic(oRectangle){
		oRectangle.setX(20);
		oRectangle.setY(10);
		oRectangle.setRx(50);
		oRectangle.setRy(55);
		...
		oRectangle.setHeight(200);
		oRectangle.setStrokeWidth(2);
		oRectangle.setStrokeColor("Sequence2");
		oRectangle.setFillColor("Sequence1");
		oRectangle.setFillingType(FillingType.Linear);
		oRectangle._injectAnimationPropertiesResolver(StubsFactory.createDummyPropertiesResolver(true));
		return oRectangle;
	}
	...
	*/
	QUnit.test("Changing values", function (assert) {
		...
		assert.equal(oRectangle.getStrokeColor(), "Good", "Value of StrokeColor is red");
		assert.equal(oRectangle.getFillColor(), "Error", "Value of FillColor is black");
	...
	QUnit.test("Changing values using SemanticColor", function (assert) {
		// Given
		var oRectangle = new Rectangle();
		oRectangle = prepareRectangleSemantic(oRectangle);
		// Then
		assert.equal(oRectangle.getX(), 20, "Value of x is 20");
		assert.equal(oRectangle.getY(), 10, "Value of y is 10");
		assert.equal(oRectangle.getRx(), 50, "Value of Rx is 50");
		...
		assert.equal(oRectangle.getHeight(), 200, "Value of Height is 200");
		assert.equal(oRectangle.getStrokeWidth(), 2, "Value of StrokeWidth is 2");
		assert.equal(oRectangle.getStrokeColor(), "Sequence2", "Value of StrokeColor is red");
		assert.equal(oRectangle.getFillColor(), "Sequence1", "Value of FillColor is black");
	...
	*/
	...
	QUnit.test("Test of Rendering using SemanticColor", async function (assert) {
		...
		oRectangle.setFillColor("Critical");
		oRectangle.setStrokeColor("Sequence1");
		...
		assert.equal(oRectangleElement.getAttribute("height"), 200, "Value of height is 200");
		assert.equal(oRectangleElement.getAttribute("stroke-width"), 0, "Value of strokeWidth is 0");
		assert.ok(oRectangleElement.getAttribute("class").includes("strokeSemanticColorSequence1"), "Value of stroke has been converted according to theme");
		assert.ok(oRectangleElement.getAttribute("class").includes("fillSemanticColorCritical"), "Value of fill has been determined by resolver");
