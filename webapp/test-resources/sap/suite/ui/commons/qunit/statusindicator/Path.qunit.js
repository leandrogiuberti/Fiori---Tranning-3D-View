sap.ui.define([
...
], function (Path, suiteLibrary, StubsFactory, Parameters, sinon, nextUIUpdate) {
	"use strict";
	...
	*/
	QUnit.test("Test html structure with HEX", async function (assert) {
	...
	QUnit.test("Test html structure with SemanticColor", async function (assert) {
		var sPath1D = "M 10 29.27 c -5.5 0 -10 -4.89 -10 -10.9";
		var oPath1 = new Path({
			d: sPath1D,
			fillingType: FillingType.Linear,
			fillColor: "Sequence1",
			strokeColor: "Sequence2"
		});
		oPath1._injectAnimationPropertiesResolver(StubsFactory.createDummyPropertiesResolver(true));
		this.sandbox.stub(oPath1, "_getDisplayedGradientOffset").returns(20);
		...
		assert.ok(oPathElement.getAttribute("class").includes("strokeSemanticColorSequence2"), "Value of stroke has been converted according to theme");
		assert.equal(oPathElement.getAttribute("stroke-width"), 0);
		assert.ok(oPathElement.getAttribute("class").includes("fillSemanticColorCritical"), "Value of fill has been determined by resolver");
