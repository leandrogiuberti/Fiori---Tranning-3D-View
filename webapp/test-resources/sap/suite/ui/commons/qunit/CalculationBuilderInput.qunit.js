sap.ui.define([
	...
	"sap/ui/core/Lib"
], function (jQuery, CalculationBuilder, CalculationBuilderItem, CalculationBuilderVariable, createAndAppendDiv, nextUIUpdate, CoreLib) {
	"use strict";
	...
	QUnit.test("Parsing string to items", async function (assert) {
	...
	QUnit.test("Test inserting items from Input Toolbar", async function (assert) {
	...
	QUnit.test("Find current span", async function (assert) {
		var oInput = this.oCalculationBuilder._oInput,
			sExpression = "Roun",
			$span;
	...
	QUnit.test("Suggestion test", async function (assert) {
		var oCalculationBuilder = this.oCalculationBuilder,
			oInput = oCalculationBuilder._oInput,
			aSuggestionListData;
	...
	QUnit.test("Parsing string to items extended ", async function (assert) {
		var oInput = this.oCalculationBuilder._oInput;
	...
	QUnit.test("Empty custom Literal ", async function (assert) {
		this.oCalculationBuilder.setAllowStringLiterals(true);
	...
	QUnit.test("Text only ", async function (assert) {
	...
	QUnit.test("Replacement of nbsp; ", async function (assert) {
		await render(this.oCalculationBuilder);
	...
	QUnit.test("Text only Mode aria-label check", async function (assert) {
		var fnDone = assert.async(),
			aAriaValues = [oResourceBundle.getText("CALCULATION_BUILDER_COMMA_ARIA_LABEL"), oResourceBundle.getText("CALCULATION_BUILDER_MINUS_ARIA_LABEL")];
		this.oCalculationBuilder.setLayoutType("TextualOnly");
		this.oCalculationBuilder.setShowInputToolbar(true);

		var afterRenderDelegate = {
			onAfterRendering: function () {
				this.oCalculationBuilder.getDomRef().querySelectorAll(".sapCalculationBuilderInputToolbarButtons .sapMBtnContent").forEach(function (elem) {
					var sElementAria = elem.getAttribute("aria-label") 
					if ( sElementAria && aAriaValues.includes(sElementAria)) {
						assert.ok("Correct aria-label is set" + elem.getAttribute("aria-label"));
					}
				});
				this.oCalculationBuilder.removeEventDelegate(afterRenderDelegate);
				fnDone();
			}.bind(this)
		};
		this.oCalculationBuilder.addEventDelegate(afterRenderDelegate);
		await render(this.oCalculationBuilder);
