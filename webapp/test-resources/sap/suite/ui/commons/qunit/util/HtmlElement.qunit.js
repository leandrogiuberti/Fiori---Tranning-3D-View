sap.ui.define("sap.suite.ui.commons.qunit.util.HtmlElement", [
	...
	QUnit.test("Only strings and numbers allowed as styles.", function (assert) {
		...
		assert.deepEqual(element._mAttributes.style, styles, "Styles set incorrectly as string.");
		styles = {color: 10};
		element.addStyle(name, 10);
		assert.deepEqual(element._mAttributes.style, styles, "Styles set incorrectly as number.");
	});
