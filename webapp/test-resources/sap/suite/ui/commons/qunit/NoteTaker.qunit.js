...
sap.ui.define([
	...
	var noteTaker2Feeder = Element.getElementById("NT2-feeder");
	...
	QUnit.test("Default config test", function(assert) {
		var noteTaker2 = Element.getElementById("NT2");
		var card;
	...
	QUnit.module("sap.suite.ui.commons.NoteTaker. Test overriden methods for cards aggregation", {
		beforeEach: function() {
			...
			Element.getElementById("NT_AGGR1").destroy();
		}
	});
	...
	QUnit.test("Check add method", function(assert) {
		...
		var noteTakerAddTest = Element.getElementById("NT_AGGR1");
		noteTakerAddTest.addCard(addedcard);
	...
	QUnit.module("Process delete card. No data binding", {
		...
		afterEach: function() {
			Element.getElementById("NT1_DELETE_NO_BINDING").destroy();
		}
	});
	QUnit.test("Delete card", function(assert) {
		var isCalled = false;
		var taker = Element.getElementById("NT1_DELETE_NO_BINDING");
	...
	QUnit.module("Process delete card. With Data binding", {
		...
		afterEach: function() {
			Element.getElementById("NT1_DELETE_BINDING").destroy();
		}
	});

	QUnit.test("Delete card. With databinding", function(assert) {
		var isCalled = false;
		var taker = Element.getElementById("NT1_DELETE_BINDING");
		taker.attachDeleteCard(function(oEvent) {
			isCalled = true;
			//            var title =  oEvent.getParameter("title");
	...
	QUnit.module("Filtering buttons rendering", {
		...
		afterEach: function() {
			var oNoteTaker = Element.getElementById("NT1");
