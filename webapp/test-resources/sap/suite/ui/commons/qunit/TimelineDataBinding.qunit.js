sap.ui.define([
...
], function (jQuery, TestUtils, TimelineItemRenderer, createAndAppendDiv, UI5Date, nextUIUpdate) {
	"use strict";
	...
	QUnit.test("Simple binding test.", async function (assert) {
		...
		assert.equal($items.length, 3, "3 items must be rendered.");
		for (var i = 0; i < 3; i++) {
			var $item = $items.eq(i);
	...
	QUnit.test("Binding with parts and formatter.", async function (assert) {
		var timeline = TestUtils.buildTimeline(data, { sortOldestFirst: true, showIcons: false }, {
			dateTime: {
				parts: ["dateParts"],
				formatter: function (parts) {
					return UI5Date.getInstance(parts[0], parts[1], parts[2]);
				}
			},
			userName: {
	...
	QUnit.test("Sorting with complex binding.", async function (assert) {
	...
	QUnit.test("Search on complex binding.", async function (assert) {
	...
	QUnit.test("Search with no results.", async function (assert) {
		var oTimeline = TestUtils.buildTimeline(data, { sortOldestFirst: true, showIcons: false }, {
			dateTime: "{dateTime}",
			userName: "{fullName}",
			text: "{text}"
		...
		oSearchField.fireSearch();
		await nextUIUpdate();
		assert.equal(oTimeline.$().find(".sapSuiteUiCommonsTimelineItem").length,
	...
	QUnit.test("Check the Previous Filters on New Search.", async function (assert) {
		var oTimeline = TestUtils.buildTimeline(data, { sortOldestFirst: true, showIcons: false }, {
			dateTime: "{dateTime}",
			userName: "{fullName}",
			text: "{text}"
		...
		var sSearchedTerm = "First1";
		var oSearchField = oTimeline.getHeaderBar().getContent().filter(function (oItem) {
			return TestUtils.stringEndsWith(oItem.getId(), "-searchField");
		});
		oSearchField = oSearchField[0];
		assert.equal(oTimeline.aPrevFilters.length, 0, "Previous Filters are empty before first search.");

		oSearchField.setValue(sSearchedTerm);
		oSearchField.fireSearch();
		await nextUIUpdate();

		var prevFilter = oTimeline.aPrevFilters[0].aFilters[0].aFilters[0];
		assert.equal(prevFilter.aFilters[0].oValue1, sSearchedTerm, "Previous Filters have the last searched term.");

		oTimeline.destroy();
	});

	QUnit.test("Timeline Sort Icon - for 1 content", async function (assert) {
	...
	QUnit.test("Data model change in double sided mode.", async function (assert) {
		...
		assert.equal($wrongItems.length, 0, "0 wrong items must be rendered.");
		...
		assert.equal($oddItems.length, 2, "2 odd items must be rendered.");
		assert.equal($evenItems.length, 1, "1 even items must be rendered.");
	...
	QUnit.test("Data model adding timelineitem in double sided mode.", async function (assert) {
		...
		myData[3] = {
			dateTime: UI5Date.getInstance(2015, 0, 1),
		...
		var $evenItems = oTimeline.$().find(".sapSuiteUiCommonsTimelineItemEven");
		assert.equal($oddItems.length, 2, "2 odd items must be rendered.");
		assert.equal($evenItems.length, 2, "2 even items must be rendered.");
	...
	QUnit.test("Data model change on TimelineItem event userNameClicked.", async function (assert) {
		var myData = JSON.parse(JSON.stringify(data));
		var newUserName = "newUserName";
		var oTimeline = TestUtils.buildTimeline(myData, { sortOldestFirst: true }, {
			...
			userNameClicked: async function (oEvent) {
				var oModel = oTimeline.getModel();
				oModel.setProperty("/Items/0/fullName", newUserName);
				await nextUIUpdate();
	...
	QUnit.test("Data model change on Timeline grow event.", async function (assert) {
		var myData = jQuery.extend(true, [], data);
		myData[3] = {
			dateTime: UI5Date.getInstance(2015, 0, 1),
			fullName: "First4 Last4",
		...
		var $oddItems = oTimeline.$().find(".sapSuiteUiCommonsTimelineItemOdd");
		var $evenItems = oTimeline.$().find(".sapSuiteUiCommonsTimelineItemEven");
		assert.equal($oddItems.length, 1, "1 odd items must be rendered.");
		assert.equal($evenItems.length, 1, "1 even items must be rendered.");
