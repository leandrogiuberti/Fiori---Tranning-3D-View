...
sap.ui.define([
	...
	QUnit.test("Default mode re-rendering", function (assert) {
		...
		function checkIfOk() {
			...
			assert.ok(getComputedStyle(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").get(0)).outline.indexOf(oColor.sapUiContentFocusStyle), "Focus Style applied.");
			assert.ok(getComputedStyle(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").get(0)).outline.indexOf(oColor.sapUiContentFocusColor), "Focus Color applied.");
			assert.ok(getComputedStyle(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCanvasInnerContainer").get(0)).outline.indexOf(oColor.sapUiContentFocusWidth), "Focus Width applied.");
		}
		...
		var afterRenderDelegate = {
			onAfterRendering: function () {
				if (that.oImageEditor && that.oImageEditor.$()) {
					...
					that.oImageEditor.getDomRef().querySelectorAll(".sapSuiteUiCommonsImageEditorCropItemRectangle:not(.sapSuiteUiCommonsImageEditorCropItemEllipse)").forEach(function (oElement) {
						assert.ok(window.getComputedStyle(oElement).display != "none", "rectangle item visible");
					});
					that.oImageEditor.getDomRef().querySelectorAll(".sapSuiteUiCommonsImageEditorCropItemEllipse:not(.sapSuiteUiCommonsImageEditorCropItemRectangle)").forEach(function (oElement) {
						assert.notOk(window.getComputedStyle(oElement).display != "none", "ellipse item not visible");
					});
				}
				if (count === 1) {
		...
		var afterRenderDelegate = {
			onAfterRendering: function () {
				if (that.oImageEditor && that.oImageEditor.$()) {
					...
					assert.ok(that.oImageEditor.$().find(".sapSuiteUiCommonsImageEditorCropInnerRectangle").is(":visible"), "crop area visible");
					that.oImageEditor.getDomRef().querySelectorAll(".sapSuiteUiCommonsImageEditorCropItemRectangle:not(.sapSuiteUiCommonsImageEditorCropItemEllipse)").forEach(function (oElement) {
						assert.notOk(window.getComputedStyle(oElement).display != "none", "rectangle item not visible");
					});
					that.oImageEditor.getDomRef().querySelectorAll(".sapSuiteUiCommonsImageEditorCropItemEllipse:not(.sapSuiteUiCommonsImageEditorCropItemRectangle)").forEach(function (oElement) {
						assert.ok(window.getComputedStyle(oElement).display != "none", "ellipse item visible");
					});
