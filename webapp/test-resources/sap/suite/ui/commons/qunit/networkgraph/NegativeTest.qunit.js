sap.ui.define([
...
], function (GraphTestUtils, createAndAppendDiv, LayeredLayout, SwimLaneChainLayout, nextUIUpdate) {
	"use strict";
	...
	QUnit.test("Line leading to nowhere fails.", async function (assert) {
		var oData = {
				nodes: [{key: 0}, {key: 1}],
				lines: [{from: 0, to: 1}, {from: 1, to: 2}]
			};
		assert.expect(1);
		await GraphTestUtils.checkGraphDataFiresEvent(assert, oData, "failure");
	});

	QUnit.test("Line leading from nowhere fails.", async function (assert) {
		var oData = {
				nodes: [{key: 0}, {key: 1}],
	...
	QUnit.test("Node without proper key fails.", async function (assert) {
		var oData = {nodes: [{key: -1}, {}, {key: 1}]};
		assert.expect(1);
		await GraphTestUtils.checkGraphDataFiresEvent(assert, oData, "failure");
	});

	QUnit.test("Group without proper key fails.", async function (assert) {
		var oData = {nodes: [{key: 0, group: 1}], groups: [{key: -1}, {}, {key: 1}]};
		assert.expect(1);
		await GraphTestUtils.checkGraphDataFiresEvent(assert, oData, "failure");
	});

	QUnit.test("Node belonging to nonexistent group fails.", async function (assert) {
		var oData = {
	...
	QUnit.test("Nodes with duplicit ids fail.", async function (assert) {
		var oData = {
	...
	QUnit.test("Groups with duplicit ids fail.", async function (assert) {
		var oData = {
			nodes: [{key: 0, group: 0}, {key: 1}, {key: 2, group: 1}],
			lines: [{from: 0, to: 1}, {from: 1, to: 2}],
	...
	QUnit.test("Groups without nodes are silently ignored.", async function (assert) {
		var oData = {
	...
	QUnit.test("Group with nonexistent parent group fails.", async function (assert) {
		var oData = {
	...
	QUnit.test("Hierarchical groups are rejected by Layered layout algorithm.", async function (assert) {
	...
	QUnit.test("Hierarchical groups are rejected by Swim lane layout algorithm.", async function (assert) {
		var oGraph = fnSimpleHierarchicalGroupsGraph();
		assert.expect(1);
	...
	QUnit.test("Duplicit lines should be processed and have different paths.", async function (assert) {
		var oData = {
	...
	QUnit.test("Graph initialized with completely empty data shouldn't fail and should turns out ready.", async function (assert) {
