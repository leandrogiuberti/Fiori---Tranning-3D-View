sap.ui.define([
...
], function (GraphTestUtils, suiteLibrary, TestLayout, ForceBasedLayout, ForceDirectedLayout, SwimLaneChainLayout, TwoColumnsLayout,
			 KlayWrapper, D3Wrapper, createAndAppendDiv, nextUIUpdate) {
	"use strict";

	var Orientation = suiteLibrary.networkgraph.Orientation;
	...
	QUnit.test("Layout failure is fired by graph when layout fails.", async function (assert) {
		...
		await GraphTestUtils.checkGraphFiresEvent(assert, oGraph, "failure");
		await nextUIUpdate();
	});

	// default value changed from undefined -> 0
	QUnit.skip("Coordinates of nodes are tested after layouting is done.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({nodes: [{key: 0}]});
	...
	QUnit.test("Coordinates of groups are tested after layouting is done.", async function (assert) {
	...
	QUnit.test("Coordinates of line sources are tested after layouting is done.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
			nodes: [{key: 0}, {key: 1}],
	...
	QUnit.test("Coordinates of line targets are tested after layouting is done.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
			nodes: [{key: 0}, {key: 1}],
			lines: [{from: 0, to: 1}, {from: 1, to: 0}]
		});
	...
	//For the fix of DINC0068037
	QUnit.test("checking console errors if Coordinates of line sources/targets are not present after layouting is done.", function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
		  nodes: [
			{key: 0}, {key: 1}
		  ],
		...
		// Stub the getSource and getTarget methods of the oLine object
		var oLines = oGraph.getLines();
		var getSourceStub = sinon.stub(oLines[0], 'getSource');
		var getTargetStub = sinon.stub(oLines[0], 'getTarget');
		getSourceStub.returns(undefined);
		getTargetStub.returns(undefined);
		...
		var oLayoutAlgorithm = oGraph.getLayoutAlgorithm();

		oLayoutAlgorithm._stretchLinesToCircleNodeAxes();

		// Check for console errors
		assert.ok(oConsoleErrorSpy.notCalled, "There is no console error");

		// Restore the stubbed methods
		getSourceStub.restore();
		getTargetStub.restore();
		oConsoleErrorSpy.restore();
	  });


	QUnit.test("Coordinates of line bend points are tested after layouting is done.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
			nodes: [{key: 0}, {key: 1}],
			lines: [{from: 0, to: 1}, {from: 1, to: 0}]
		});
	...
	QUnit.test("Preset node coordinates are overriden by Klay layout.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
	...
	QUnit.test("Left-to-right: Three chained nodes with three respective groups are aligned horizontally and with uniform distances.", async function (assert) {
	...
	QUnit.test("Top-to-bottom: Three chained nodes with three respective groups are aligned vertically and with uniform distances.", async function (assert) {
		var oGraph = fnGetGroupCollapseGraph(),
			fnDone = assert.async();
	...
	QUnit.test("Right-to-left: Single node within single group are rendered at the same coordinates as for Left-to-right.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("Lines between nodes reach at least their border, and end/start within appropriate groups.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, title: "Zero"},
	...
	QUnit.test("Forced based algorithm renders 2 nodes correctly.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0},
					{key: 1}
	...
	// Skipped for integration to go on, version af1b7b9 of D3ForceWorker.js works everywhere,
	// but not no Hudson Voter, see build #10326
	QUnit.test("Forces based algorithm ignores groups.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0},
					{key: 1, group: "A"}
			...
			fnAssert = function () {
				assert.ok(oGraph.$("groups").children().length === 0, "No groups should be rendered.");
			},
			fnDone = assert.async();
	...
	QUnit.test("Forces directed algorithm ignores groups.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0},
					{key: 1, group: "A"}
	...
	QUnit.test("Swimlane algorithm keeps equal height of all groups when oriented from left to right.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("Swimlane algorithm keeps equal height of all groups when oriented from right to left.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: "A"}, {key: 1, group: "A"}, {key: 2, group: "B"}
				],
	...
	QUnit.test("Swimlane algorithm keeps equal widths of all groups when oriented from top to bottom.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: "A"}, {key: 1, group: "A"}, {key: 2, group: "B"}
				],
	...
	QUnit.test("Swimlane algorithm keeps equal widths of all groups when oriented from bottom to top.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: "A"}, {key: 1, group: "A"}, {key: 2, group: "B"}
				],
	...
	QUnit.test("Swimlane algorithm manages all types of line tracing for vertical lanes.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("Swimlane algorithm manages all types of line tracing for horizontal lanes.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: 1}, {key: 1, group: 2}, {key: 2, group: 3}, {key: 3, group: 1}, {key: 4, group: 3}, {key: 5, group: 1}
				],
	...
	QUnit.test("Swimlane algorithm is transparent to different ordering of groups than is their ordering in the data set.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("Swimlane algorithm put nipples between collapsed groups.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("Swimlane algorithm handles various root-less cycles properly.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("Swimlane algorithm is generated for groups which is collapsed and does not have any node.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
	...
	QUnit.test("TwoColumns algorithm works properly.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: "left"}, {key: 1, group: "right"},
				...
				groups: [
					// Top groups
					{key: "left", title: "left", icon: "sap-icon://arrow-left"},
					{key: "right", title: "right", icon: "sap-icon://arrow-right"},
					// Sub-groups
					{key: 2, parentGroupKey: "left"}, {key: 3, parentGroupKey: "right"},
					{key: 4, parentGroupKey: 2}, {key: 5, parentGroupKey: 3}
				]
			}),
			fnAssert = function () {
				assert.ok(true, "Didn't fail, bingo.");
				assert.ok(oGraph.getDomRef().querySelector(".sapSuiteUiCommonsNetworkTopGroupDelimiter"), "Delimiter Created.");
				assert.ok(oGraph.getDomRef().querySelector(".sapSuiteUiCommonsNetworkTopGroup"), "Left Group Created.");
				assert.ok(oGraph.getDomRef().querySelector(".sapSuiteUiCommonsNetworkTopGroupRight"), "Right Group Created.");
				assert.ok(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkTopGroupHeader").length, "Left and Right Group Header are Created.");
				assert.ok(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkGraphIcon").length, "Left and Right Group Icons are Created.");
				assert.ok(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkGraphIcon")[0], "Left Icon is Created.");
				assert.ok(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkGraphIcon")[1], "Right Icon is Created.");
				assert.ok(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkGroupHeaderTitle").length, "Left and Right Group Title are Created.");
				assert.equal(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkGroupHeaderTitle")[0].innerHTML, "left", "Left Title is correct.");
				assert.equal(oGraph.getDomRef().querySelectorAll(".sapSuiteUiCommonsNetworkGroupHeaderTitle")[1].innerHTML, "right", "Right title is correct.");
			},
			fnDone = assert.async();
	...
	QUnit.test("TwoColumns algorithm accepts only lines going from one column to the other.", async function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
				nodes: [
					{key: 0, group: 0},
					{key: 1, group: 1},
	...
	QUnit.test("Re-setting line's bend, in TwoColumns algorithm, to initial after reaching to the opposite group's x-axis", function (assert) {
		var oGraph = GraphTestUtils.buildGraph({
			nodes: [
				{key: 0, group: 2}, {key: 1, group: 3},
				{key: 2, group: 3}, {key: 3, group: 3},
				{key: 4, group: 3}, {key: 5, group: 3},
				{key: 6, group: 3}, {key: 7, group: 3},
				{key: 8, group: 3}, {key: 9, group: 3},
				{key: 10, group: 3}, {key: 11, group: 3}
			],
			lines: [
				{from: 0, to: 1},
				{from: 0, to: 3},
		...
		oGraph.setLayoutAlgorithm(new TwoColumnsLayout());
		var allNodes = oGraph.getNodes();
		allNodes.forEach(function(oNode){
			var allLines = oNode.getChildLines();
			allLines.forEach(function(oLine){
				var allBends = oLine.getBends();
				if (allBends.length == 0){
					assert.ok(1,"Line doesn't conatins bends.");
				} else {
					allBends.forEach(function(bend){
						assert.ok(bend.getX() <= oLine.getToNode()._oGroup.getX(), "bend is not crossing the opposite group's border.");
					});
				}
			});
		});
