sap.ui.define([
	...
	QUnit.test("Preselecting, selecting and unselecting of nodes keep correct state and DOM presence, " +
		"and also fires expected events with correct parameteres.", async function (assert) {
		var oData = {
				nodes: [
					{key: 0, selected: true},
	...
	QUnit.test("Selection change event should be fired with correct parameters.", async function (assert) {
		...
		oGraph.attachGraphReady(async function () {
			await GraphTestUtils.runSyncActionAssertChain(oGraph, mSyncChain, fnDone, false);
		});
