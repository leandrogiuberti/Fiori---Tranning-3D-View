function ProductPush(aRequests, oMockserver, AjaxHelper) {
   ...
   //Get Contact details
	aRequests.push({
		method: 'GET',
		path: /(STTA_C_MP_Product)\(([^/\?#]+)\)\/(to_AllEmployeeContacts(.+)\))/,
		response: function (oXhr, sEntitySet, sProductID, sEmployeeId) {
			var oDraftResponse, oSiblingEntity, sSiblingEntityPath, oCorrespondingSiblingEntityObject;
			var sEmployeeNo = sEmployeeId.split("'")[1];
			var sJsonFilesModulePath = "STTA_MP/localService";
			var sJsonFilesUrl = sap.ui.require.toUrl(sJsonFilesModulePath);
			oDraftResponse = AjaxHelper.sjax({
				url: sJsonFilesUrl + "/SEPMRA_I_Employee.json",
				dataType: "json"
			});
			var oDraftData = {};
			if (oDraftResponse.data) {
				...
				for (var i = 0; i < oDraftResponse.data.length; i++) {
					oObject = oDraftResponse.data[i];
					if (oObject.Employee == sEmployeeNo) {
						oDraftResponse.data[i].__metadata = {
							"id": sRootUriRelative + "SEPMRA_I_Employee('" + sEmployeeNo +
								"')",
							"type": "STTA_PROD_MAN.SEPMRA_I_EmployeeType",
							"uri": sRootUriRelative + "SEPMRA_I_Employee('" + sEmployeeNo +
								"')"
						};
