	...
	function (Utils, deepExtend, AjaxHelper, MessageBox) {
		...
		function fnSave(aRequests, oMockServer) {
			...
			aRequests.push({
				...
				response: function (oXhr, SalesOrder) {
					var sSalesOrderID = SalesOrder.split("'")[1].split("'")[0];
					//We throw 412 error message for Sales Order IDs 500000011 and 500000012
					if ((sSalesOrderID === "500000011" || sSalesOrderID === "500000012") && oXhr.requestHeaders.Prefer === 'handling=strict'){
						oXhr.respondJSON(412, {
							"preference-applied": "handling=strict"
						}, JSON.stringify({
							"error": {
						...
						if (oResponse) {
							//Here for the SalesOrderID 500000012, sap-message header will be sent in the response to mimick the 412 duplicate message handling scenario
							if (sSalesOrderID === "500000012") {
							...
							} else if (sSalesOrderID === "500000011") {//Here for Sales Order ID 500000011, we throw a different 412 warning message than earlier
								oXhr.respondJSON(200, {
									"sap-message":
										JSON.stringify({
											"code": "002",
											"message": "412 code sent Warning Transient Message different than earlier",
											"severity": "warning",
											"type": "warning",
											"transition": true,
										})
