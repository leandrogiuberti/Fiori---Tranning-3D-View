...
function(UIComponent, ReuseComponentSupport) { "use strict";
	...
	return UIComponent.extend("STTA_MP.implementingComponents.productCanvas.Component", {
		metadata: {
			manifest: "json",
			library: "STTA_MP.implementingComponents.productCanvas",
			properties: {
		...
		stStart: function(oModel, oBindingContext, oExtensionAPI) {
			...
			oComponentModel.setProperty("/sPath", oExtensionAPI.getNavigationController().getCurrentKeys()[2]);
		...
		stRefresh: function(oModel, oBindingContext) {
			...
			oComponentModel.setProperty("/sPath", oNavigationController.getCurrentKeys()[2]);
