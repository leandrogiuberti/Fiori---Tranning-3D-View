...
function(UIComponent, ReuseComponentSupport) { "use strict";
	...
	return UIComponent.extend("analytics2.implementingComponents.productCanvas.Component", {
		metadata: {
			manifest: "json",
			library: "analytics2.implementingComponents.productCanvas",
			properties: {
				/* Standard properties for reuse components */

				/* UI mode across fiori applications so the component knows in what mode the application is running
		...
		stStart: function(oModel, oBindingContext, oExtensionAPI) {
			...
			oComponentModel.setProperty("/costElementText", oExtensionAPI.getNavigationController().getCurrentKeys()[1]);
		...
		stRefresh: function(oModel, oBindingContext) {
			...
			oComponentModel.setProperty("/costElementText", oNavigationController.getCurrentKeys()[1]);
