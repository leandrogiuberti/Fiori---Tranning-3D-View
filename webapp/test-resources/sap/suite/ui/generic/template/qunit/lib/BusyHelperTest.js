...
sap.ui.define(["testUtils/sinonEnhanced", "sap/ui/model/json/JSONModel", "sap/suite/ui/generic/template/lib/BusyHelper", "sap/suite/ui/generic/template/lib/MessageUtils"
	...
	var oDeregisterForMessageNavigationSpy;

	function fnGeneralSetup(){
		fnNavigationEnded = null;
		oSandbox = sinon.sandbox.create();
		oRemoveTransientMessageSpy = oSandbox.stub(oTemplateContract.oApplicationProxy, "removeTransientMessages");
		oHandleTransientMessageSpy = oSandbox.stub(MessageUtils, "handleTransientMessages", function(){
			return Promise.resolve();
		});
		oDeregisterForMessageNavigationSpy = oSandbox.spy(oTemplateContract.oApplicationProxy, "deregisterForMessageNavigation");
		oBusyHelper = new BusyHelper(oTemplateContract);
	}

	function fnCheckInitialBusyDelayValue(assert) {
		assert.strictEqual(oBusyHelper.getBusyDelay(), 0, "Busy Helper delay 0");
	}
	...
	QUnit.module("Test setting busy via setBusyReason", {
	...
	QUnit.test("Test that setting to busy works", function(assert) {
		...
		fnCheckInitialBusyDelayValue(assert);
		...
		assert.ok(!oDeregisterForMessageNavigationSpy.called, "Message navigation must not have been affected when removing a not existing busy reason");
		...
		assert.ok(oDeregisterForMessageNavigationSpy.calledOnce, "Message navigation must have been removed");
	...
	QUnit.module("Test setting busy via setBusy", {
		beforeEach: fnGeneralSetup,
		afterEach: fnGeneralTeardown
	});
	...
	QUnit.module("Complex scenarios", {
		beforeEach: fnGeneralSetup,
		afterEach: fnGeneralTeardown
	});
