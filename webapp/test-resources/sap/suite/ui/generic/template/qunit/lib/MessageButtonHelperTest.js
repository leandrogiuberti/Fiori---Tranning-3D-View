...
sap.ui.define(["testUtils/sinonEnhanced",
	...
	function fnGeneralSetup(){
		...
		oCommonUtils.getDialogFragment = function(sFragment){
			...
			return Promise.resolve(oMessagePopover);
		};
		oMessagePopover.getBinding = function(){
			delete oMessagePopover.getBinding;
			return oItemBinding;
		};
		oItemBinding.getAllCurrentContexts = function(){
		...
		oTemplPriv.setProperty = function(sProperty, oValue){
			if (sProperty !== "/generic/messageButtonInfo"){
	...
	function setMessagesForPopover(aMessages) {
		...
		for (var i in aMessages) {
			if (aMessages[i] ==="Error"){
				aMessagesShownInPopover.push({ getObject: function(){
					return { type: "Error" }; 
				}});
	...
	function checkTemplPrivContent(assert, iLength, severity, iSeverityCount){
		...
		if (iLength) {
			if (severity === "Negative") {
				assert.strictEqual(oTemplPriv.getProperty("/generic/messageButtonInfo/label"), iSeverityCount ? ("" + iSeverityCount) : "", "number of severity messages must be correct");
			}
			assert.strictEqual(oTemplPriv.getProperty("/generic/messageButtonInfo/severity"), severity, "severity of messages must be correct");
			assertIsText(assert, oTemplPriv.getProperty("/generic/messageButtonInfo/tooltip"), iLength === 1 ? "MESSAGE_BUTTON_TOOLTIP_S" : "MESSAGE_BUTTON_TOOLTIP_P", iLength);
		}
	}
	...
	QUnit.test("Test that class can be instantiated", function(assert) {
		...
		setTimeout(function(){
			...
			assert.ok(oSetModelSpy.calledWith(sap.ui.getCore().getMessageManager().getMessageModel(), "msg"), "Message model must have been set for MessagePopover");
			...
			assert.strictEqual(oTemplPriv.getProperty("/generic/messageButtonInfo/count"), 0, "0 messages must be available in the beginning");
			assertIsText(assert, oTemplPriv.getProperty("/generic/messageButtonInfo/tooltip"), "MESSAGE_BUTTON_TOOLTIP_P", 0);
...
/*	todo: Revitalize this coding with the new filter logic
	...
	QUnit.module("registerMessageFilterProvider", {
		beforeEach: function(){
			testableHelper.startTest();
			fnFilterAnalyzer = testableHelper.observeConstructor(Filter);
			fnGeneralSetup();
