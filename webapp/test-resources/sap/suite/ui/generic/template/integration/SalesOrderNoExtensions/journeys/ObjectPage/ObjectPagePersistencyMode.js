sap.ui.define(["sap/ui/test/opaQunit"],
	function (opaTest) {
		...
		opaTest("Starting the FCL app and loading the OP", function (Given, When, Then) {
			Given.iStartMyAppInDemokit("sttasalesordernoext", "manifestFCL", null, { width: "1400", height: "600" });
		...
		opaTest("Open another record from LR and check the Object page header pin status is persisted", function (Given, When, Then) {
			When.onTheGenericObjectPage
				.iSelectSectionOrSubSectionByName("Sales Order Items");
			When.onTheGenericListReport
		...
		opaTest("Check the tab selection and table personalisation changes are persisted", function (Given, When, Then) {
			Then.onTheGenericObjectPage
		...
		opaTest("Open another record from LR and check the table variant selection is persisted", function (Given, When, Then) {
			When.onTheGenericListReport
		...
		opaTest("Close the OP and open another record and check the Object page header pin status is not persisted", function (Given, When, Then) {
            When.onTheGenericObjectPage
                .iSelectSectionOrSubSectionByName("Sales Order Items")
				.and
				.iCloseTheObjectPage();
		...
		opaTest("Starting the app and loading the OP with Paginator button", function (Given, When, Then) {
			Given.iStartMyAppInDemokit("sttasalesordernoext", "manifestVariantManagementOnObjectPage", { "bWithChange": true });
			When.onTheGenericListReport
				.iExecuteTheSearch()
				.and
		...
		opaTest("Check discard draft popup appears on navigating via paginator button when set to restricted in manifest", function (Given, When, Then) {
			When.onTheGenericObjectPage
		...
		opaTest("Move to a different object using paginator button and check the Object page header pin status is persisted", function (Given, When, Then) {
			When.onTheGenericObjectPage
				.iSelectSectionOrSubSectionByName("Sales Order Items");
			When.onTheGenericObjectPage
				.iNavigateUpOrDownUsingObjectPageHeaderActionButton("NavigationDown");
		...
		opaTest("Move to a different object using paginator button and check the table variant selection is persisted", function (Given, When, Then) {
			When.onTheGenericObjectPage
				.iNavigateUpOrDownUsingObjectPageHeaderActionButton("NavigationDown");
			Then.onTheGenericObjectPage
				.theObjectPageHeaderTitleIsCorrect("500000004");
		...
		opaTest("Starting the app and loading the OP having table without variant management", function (Given, When, Then) {
			Given.iStartMyAppInDemokit("sttasalesordernoext", "manifestNavDraftPopup", { "bWithChange": true });
			When.onTheGenericListReport
				.iExecuteTheSearch()
				.and
				.iNavigateFromListItemByFieldValue({ Field: "SalesOrder", Value: "500000002" });
		...
		opaTest("Move to a different object using paginator button and check the table personalisation changes are persisted", function (Given, When, Then) {
			When.onTheGenericObjectPage
				.iSelectSectionOrSubSectionByName("Sales Order Items");
			When.onTheGenericObjectPage
				.iNavigateUpOrDownUsingObjectPageHeaderActionButton("NavigationDown");
