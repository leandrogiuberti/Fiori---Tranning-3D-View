	...
	function (opaTest) {
		...
		QUnit.module("Sales Order Item Aggregation - MultiSelectForTreeTable: Single Select", function () {
			...
			opaTest("Starting the app and loading data", function (Given, When, Then) {
				// arrangements
				Given.iStartMyAppInSandbox("STTASOWD20-STTASOWD20,SalesOrder-itemaggregation#SalesOrder-itemaggregation", "manifestTreeTable");
			...
			opaTest("Checking the FE rendered column properties on the Tree Table", function (Given, When, Then) {
				When.onTheListReportPage
					.iLookAtTheScreen()
				Then.onTheListReportPage
					.iCheckControlPropertiesById("sAction::STTA_C_SO_ItemAggr", { "visible": true, "sortProperty": "", "filterProperty": "Category", "showFilterMenuEntry": false, "width": "15rem" })
					.and
					.iCheckControlPropertiesById("sSemanticObject::STTASOWD20:::sAction::STTASOWD20", { "visible": true, "sortProperty": "ProductId", "filterProperty": "", "showFilterMenuEntry": false, "width": "15rem" })
					.and
					.iCheckControlPropertiesById("sProperty::SalesOrderId:::sSemanticObject::STTASOWD20:::sAction::STTASOWD20", { "visible": true, "sortProperty": "SalesOrderId", "filterProperty": "SalesOrderId", "showFilterMenuEntry": false, "width": "9rem" })
					.and
					.iCheckControlPropertiesById("sTarget:::40com.sap.vocabularies.UI.v1.DataPoint:23Progress", { "visible": true, "sortProperty": "to_Product/Width", "filterProperty": "to_Product/Width", "showFilterMenuEntry": false, "width": "6.875rem" });
			});

			opaTest("External navigation from Tree Table - Dynamic values for Semantic Object and Action", function (Given, When, Then) {
				When.onTheGenericListReport
		...
		QUnit.module("Sales Order Item Aggregation - MultiSelectForTreeTable: Multi Select with Limit and OP direct edit", function () {
			...
			opaTest("Check the Edit icon visibility on the Tree table on LR", function (Given, When, Then) {
				Then.onTheListReportPage
					.iCheckTheEditIconVisibilityOnNthRowOfTable(1, true, "TreeTable-_tab1");
			});

			opaTest("Check the Edit icon not visible for the non editable records", function (Given, When, Then) {
				Then.onTheListReportPage
					.iCheckTheEditIconVisibilityOnNthRowOfTable(4, false, "TreeTable-_tab1")
			});
			...
			opaTest("Click on Save and check the navigation to LR", function (Given, When, Then) {
				...
				Then.onTheGenericListReport
					.theListReportPageIsVisible();
				Then.onTheListReportPage
					.iShouldSeeTheNavigatedRowHighlightedInUITables(0, true, "TreeTable-_tab1")
					.and
					.iCheckRenderedColumnTextOnNthRowOfTable(1, [4], ["Test"], "TreeTable-_tab1", "treeTable");
			});

			opaTest("Navigate to another OP and check the OP is opened in edit mode", function (Given, When, Then) {
				When.onTheListReportPage
					.iNavigateUsingUITable(2, "TreeTable-_tab1");
				Then.onTheGenericObjectPage
					.theObjectPageIsInEditMode();
			});
			...
			opaTest("Click on Cancel and check the navigation to LR", function (Given, When, Then) {
				...
				Then.onTheGenericListReport
					.theListReportPageIsVisible();
				Then.onTheListReportPage
					.iShouldSeeTheNavigatedRowHighlightedInUITables(1, true, "TreeTable-_tab1");
			});
