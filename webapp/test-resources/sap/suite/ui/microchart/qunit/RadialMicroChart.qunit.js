...
sap.ui.define([
	...
	"sap/ui/core/Lib",
	"sap/ui/core/Theming"
], function(nextUIUpdate, jQuery, RadialMicroChart, RadialMicroChartRenderer, Button, FlexBox, TooltipBase, mobileLibrary, Text, JSONModel, GenericTile, TileContent, microchartLibrary, TestUtils, KeyCodes, Mobile, Core, CoreLib, Theming) {
	"use strict";

	var Size = mobileLibrary.Size;
	var oRb = CoreLib.getResourceBundleFor("sap.suite.ui.microchart");
	var ValueColor = mobileLibrary.ValueColor;
	var oMicroChartColorType = microchartLibrary.MicroChartColorType;
	...
	*/
	QUnit.test("Core initialization check - core and theme", function(assert) {
		...
		//Assert
		assert.ok(oChart._bThemeApplied, "Rendering variable has been correctly set.");
		assert.ok(this.oSpyReady.calledOnce, "Method Core.ready has not been called.");
		assert.ok(this.oSpyHandleCoreInitialized.calledOnce, "Method _handleCoreInitialized has been called once.");
		assert.ok(this.oStubAttachThemeApplied.calledOnce, "Method Core.attachThemeChanged has been called once.");
		assert.ok(this.oSpyHandleThemeApplied.calledOnce, "Method _handleThemeApplied has been called once.");
	});
	...
	QUnit.test("Controls in different sizes rendered", function(assert) {
		//Size XS
		assert.ok(document.getElementById("radialMicroChartXS"), "Control was rendered");
		...
		//SizeS
		assert.ok(document.getElementById("radialMicroChartS"), "Control was rendered");
		...
		//Size M
		assert.ok(document.getElementById("radialMicroChartM"), "Control was rendered");
		...
		//Size L
		assert.ok(document.getElementById("radialMicroChartL"), "Control was rendered");
		...
		//Size Auto
		assert.ok(document.getElementById("radialMicroChartAuto"), "Control was rendered");
		...
		//Size Responsive
		assert.ok(document.getElementById("radialMicroChartResponsive"), "Control was rendered");
		...
		assert.ok(document.getElementById("radialMicroChartDefault"), "Control was rendered");
	...
	QUnit.test("RadialMicroChart.prototype._getAltHeaderText", function(assert){
		var oConsoleErrorSpy = this.spy(console, "error");
		this.oRadialMicroChart._getAltHeaderText();
	...
	QUnit.test("Control rendered", function(assert) {
		...
		//Act
		//Assert
		assert.ok(document.getElementById("radialMicroChart"), "Control was rendered");
		...
		var a$CircleElementsInRadialChartDom = this.oRadialMicroChart.$().find("circle");
		assert.equal(a$CircleElementsInRadialChartDom.length, 3, "3 circles were rendered, Background, 2xBorder");
		var a$PathElementsInRadialChartDom = this.oRadialMicroChart.$().find("path");
	...
	QUnit.test("Text Color test", function(assert) {
		...
		//Assert
		assert.equal(sColor, "sapSuiteRMCTextColorNeutral", "Color class for 'Neutral' was correct if nothing was configured (default)");
		...
		//Assert
		assert.equal(sColor, "sapSuiteRMCTextColorNeutral", "Color class for 'Neutral' was correct");
		...
		sColor = RadialMicroChartRenderer._getTextColorClass(this.oRadialMicroChart);
		//Assert
		assert.equal(sColor, "sapSuiteRMCTextColorGood", "Color class for 'Good' was correct");
		...
		sColor = RadialMicroChartRenderer._getTextColorClass(this.oRadialMicroChart);
		//Assert
		assert.equal(sColor, "sapSuiteRMCTextColorError", "Color class for 'Error' was correct");
		...
		sColor = RadialMicroChartRenderer._getTextColorClass(this.oRadialMicroChart);
		//Assert
		assert.equal(sColor, "sapSuiteRMCTextColorCritical", "Color class for 'Critical' was correct");
	});
	...
	*/
	QUnit.test("Property valueColor test for multiple datatypes", async function(assert) {
		//Arrange
	...
	QUnit.test("Property valueColor test for multiple valid datatypes", async function(assert) {
		//Arrange
		this.oRadialMicroChart.setValueColor("Good");
		//Act
		await nextUIUpdate();
		//Assert
		assert.equal(this.oRadialMicroChart.getValueColor(), "Good", "Applying hex value to property works");

		//Arrange
		this.oRadialMicroChart.setValueColor("Error");
		//Act
		await nextUIUpdate();
		//Assert
		assert.equal(this.oRadialMicroChart.getValueColor(), "Error", "Applying color string value to property works");

		//Arrange
		this.oRadialMicroChart.setValueColor("Critical");
		//Act
		await nextUIUpdate();
		//Assert
		assert.equal(this.oRadialMicroChart.getValueColor(), "Critical", "Applying enum value to property works");

		//Arrange
		this.oRadialMicroChart.setValueColor("Sequence1");
		//Act
		await nextUIUpdate();
		//Assert
		assert.equal(this.oRadialMicroChart.getValueColor(), "Sequence1", "Applying enum value as string to property works");
	});
	 ...
	 */
	QUnit.test("GenerateTextContent for 0%", function (assert) {
		var sText = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(sText,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_TEXT", [0]),"correct text shown when percentage is equal to 0%");
		assert.equal(!sText,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_SYMBOL", [0]),"correct text shown when percentage is equal to 0");
		this.oRadialMicroChart.setPercentage(0);
		this.oRadialMicroChart.setShowPercentageSymbol(false);
		var control = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(control,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_SYMBOL", [0]),"correct text shown when percentage is equal to 0");
	  });
	 ...
	 * @deprecated since 1.123.0
	 */
	QUnit.test("GenerateTextContent for 100%", function (assert) {
		var sText = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(sText,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_TEXT", [100]),"correct text shown when showPercentageSymbol is equal to true");
		this.oRadialMicroChart.setPercentage(100);
		this.oRadialMicroChart.setShowPercentageSymbol(false);
		var control = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(control,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_SYMBOL", [100]),"correct text shown when showPercentageSymbol is equal to false");
	  });
	 ...
	 * @deprecated since 1.123.0
	 */
	QUnit.test("GenerateTextContent for percentage < 0%", function (assert) {
		var sText = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(sText,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_TEXT", [0]),"correct text shown when showPercentageSymbol is equal to true");
		this.oRadialMicroChart.setPercentage(-1);
		this.oRadialMicroChart.setShowPercentageSymbol(false);
		var control = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(control,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_SYMBOL", [0]),"correct text shown when showPercentageSymbol is equal to false");
	  });
	 ...
	 * @deprecated since 1.123.0
	 */
	QUnit.test("GenerateTextContent for percentage > 100%", function (assert) {
		var sText = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(sText,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_TEXT", [100]),"correct text shown when showPercentageSymbol is equal to true");
		this.oRadialMicroChart.setPercentage(150);
		this.oRadialMicroChart.setShowPercentageSymbol(false);
		var control = RadialMicroChartRenderer._generateTextContent(this.oRadialMicroChart);
		assert.equal(control,this.oRadialMicroChart._oRb.getText("RADIALMICROCHART_PERCENTAGE_SYMBOL", [100]),"correct text shown when showPercentageSymbol is equal to false");
	  });
	...
	QUnit.test("Control rendered", function(assert) {
		...
		var oSVG = this.oRadialMicroChart.$().find("svg");
	...
	QUnit.test("No data rendered when no property given", function(assert) {
		assert.ok(document.getElementById("radialMicroChart"), "Control was rendered");
		var a$NoDataElement = this.oRadialMicroChart.$().find(".sapSuiteUiMicroChartNoData");
	...
	QUnit.test("Minimum sizes handling", async function(assert) {
	...
	QUnit.test("Minimum width reached", async function(assert) {
	...
	QUnit.test("Width exceeded", async function(assert) {
		this.oFlexBox.setHeight("65px");
	...
	QUnit.test("Render outside label", async function(assert) {
	...
	QUnit.test("Size after rendering needs to be the same as parent element.", async function(assert) {
		//Arrange
	...
	QUnit.test("Right alignment", async function(assert) {
	...
	QUnit.test("Center and left alignment", async function(assert) {
	...
	QUnit.test("Neutral color text", function(assert) {
		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCInsideLabel").hasClass("sapSuiteRMCTextColorNeutral"), true, "Inside label has neutral color");
		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCOutsideLabel").hasClass("sapSuiteRMCTextColorNeutral"), true, "Outside label has neutral color");
	});

	QUnit.test("Critical color text", async function(assert) {
		this.oRadialMicroChart.setValueColor(ValueColor.Critical);

		await nextUIUpdate();

		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCInsideLabel").hasClass("sapSuiteRMCTextColorCritical"), true, "Inside label has critical color");
		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCOutsideLabel").hasClass("sapSuiteRMCTextColorCritical"), true, "Outside label has critical color");
	});

	QUnit.test("Error color text", async function(assert) {
		this.oRadialMicroChart.setValueColor(ValueColor.Error);

		await nextUIUpdate();

		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCInsideLabel").hasClass("sapSuiteRMCTextColorError"), true, "Inside label has error color");
		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCOutsideLabel").hasClass("sapSuiteRMCTextColorError"), true, "Outside label has error color");
	});

	QUnit.test("Good color text", async function(assert) {
		this.oRadialMicroChart.setValueColor(ValueColor.Good);

		await nextUIUpdate();

		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCInsideLabel").hasClass("sapSuiteRMCTextColorGood"), true, "Inside label has good color");
		assert.equal(this.oRadialMicroChart.$().find(".sapSuiteRMCOutsideLabel").hasClass("sapSuiteRMCTextColorGood"), true, "Inside label has good color");
	});
	...
	*/
	...
	QUnit.test("Test Aria-label and Aria-labelledBy superseding", async function(assert) {
		...
		this.oRadialMicroChart.addAriaLabelledBy(new Text());
		await nextUIUpdate();
		...
		this.oRadialMicroChart.addAriaLabelledBy(new Button());
		await nextUIUpdate();
	...
	QUnit.test("attachPress called", function(assert) {
		...
		sinon.spy(this.oRadialMicroChart, "invalidate");
		this.oRadialMicroChart.addEventDelegate({
			onAfterRendering: function () {
				//Assert
				assert.ok(this.oRadialMicroChart.invalidate.calledOnce, "rerender was calledOnce on oRadialMicroChart on detachPress/detachEvent");
	...
	QUnit.test("attachEvent called with non press", function(assert) {
		...
		//Assert
		assert.equal(this.oRadialMicroChart.invalidate.calledOnce, false, "rerender was not calledOnce on oRadialMicroChart since other event then press was attached");
	...
	QUnit.test("detachPress called", function(assert) {
		...
		oDelegateFirstRendering = {
			onAfterRendering: function () {
				...
				this.oRadialMicroChart.removeEventDelegate(oDelegateFirstRendering);
				fnDone();
			}.bind(this)
		};
		this.oRadialMicroChart.addEventDelegate(oDelegateFirstRendering);
		//Act
