...
sap.ui.define([
	...
	"sap/ui/core/Theming",
	"sap/ui/core/Control"
], function(nextUIUpdate, jQuery, InteractiveDonutChart, InteractiveDonutChartSegment, FlexBox, mobileLibrary, Device, Label, microchartLibrary, Log, IntervalTrigger, Mobile, Core, Theming, Control) {
	"use strict";
	...
	QUnit.module("Control initialization core and theme checks", {
		beforeEach: function() {
			...
			this.oSpyControlInvalidate = sinon.spy(Control.prototype, "invalidate");
		},
		afterEach: function() {
			...
			this.oSpyControlInvalidate.restore();
		}
	});
	...
	*/
	QUnit.test("Core initialization check - no core, no theme", function(assert) {
		...
		assert.ok(this.oSpyControlInvalidate.called, "The control is invalidated once the theme is changed");
	});
	...
	*/
	...
	QUnit.test("InteractiveDonutChart can be instantiated", function(assert) {
		...
		assert.equal(InteractiveDonutChart.AREA_HEIGHT_INTERACTIVE_MINVALUE, 44, "Minimum interactive area is 44px");
	});
	...
	QUnit.test("Method _getSegmentColor mixes the colors with the right factor for getting a valid segment color", function(assert) {
		...
		for (var i = 0; i < iSegmentsCount; i++) {
			oDonutRenderer._getSegmentColor(i, iSegmentsCount, this.oChart);
		}
		//Assert
	...
	QUnit.module("Legend contrasts tests", {
		beforeEach: async function() {
			this.oChart = new InteractiveDonutChart({
				segments : [
					new InteractiveDonutChartSegment(),
	...
	QUnit.test("Border applied to selected segment of legend when in non focused state", function(assert) {
		//Arrange
		var oEventDown = {
			preventDefault : function(){},
			stopImmediatePropagation: function(){}
		...
		this.oChart.onsapdown(oEventDown);
		//Assert
		assert.equal(window.getComputedStyle(document.querySelector('.sapSuiteIDCLegendSegmentSelected'))['border'], '1px solid rgb(8, 84, 160)', "The border has been applied");
	});
});
