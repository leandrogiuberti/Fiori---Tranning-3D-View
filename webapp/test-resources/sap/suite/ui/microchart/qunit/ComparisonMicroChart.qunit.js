...
sap.ui.define([
...
], function(nextUIUpdate, jQuery, ComparisonMicroChart, ComparisonMicroChartData, mobileLibrary, Device, JSONModel, Button, ColumnListItem, Table, Text, GenericTile, TileContent, FlexBox, TooltipBase, microchartLibrary, TestUtils, Mobile, Core, CoreLib,Theming,QUtils,Localization, Parameters) {
	"use strict";
	...
	*/
	...
	QUnit.module("Rendering tests for no data placeholder inside genericTile", {
		beforeEach: async function() {
			this.tileContent = new TileContent({
				content: new ComparisonMicroChart({
				})
			});
	...
	QUnit.test("No data rendered when no data given", function(assert) {
		//Arrange
		var originalPx = window.getComputedStyle(document.getElementsByClassName('sapMTileCntContent')[0]).getPropertyValue('width');
		var reducedPx = (parseInt(originalPx, 10) - 32) + 'px';
		assert.equal(window.getComputedStyle(document.getElementsByClassName('sapMTileCntContent')[0]).getPropertyValue('height'), window.getComputedStyle(document.getElementsByClassName('sapSuiteCpMCChartContent')[0]).getPropertyValue('height'), "The height of the chart is set to 100%");
		assert.equal(window.getComputedStyle(document.getElementsByClassName('sapSuiteCpMCChartContent')[0]).getPropertyValue('width'), reducedPx, "The height of the chart is set to 100%");
	});
	...
	QUnit.module("Border checks", {
		beforeEach: async function() {
			this.normalizeHexColor = function(hex) {
				if (hex.match(/^#[\da-f]{3}$/)) {
					return hex.replace(/[\da-f]/g, "$&$&");
				}
				return hex;
			}
			...
			this.Parameters_getAsync = function (key, oElement) {
	...
	QUnit.test("Chart border rendered for 0 value in chart bar",function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_horizon", async function() {
		var borderColor = await this.Parameters_getAsync("sapContent_ForegroundBorderColor");

		var rgbColor = this.hexToRgb(borderColor);

		var boxShadow = [
			rgbColor + " -1px 0px 0px 0px inset",
			rgbColor + " 1px 0px 0px 0px inset",
			rgbColor + " 0px 1px 0px 0px inset",
			rgbColor + " 0px -1px 0px 0px inset"
		].join(", ");
			assert.equal(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, boxShadow, "The border has been added successfully for horizon");
			done();
		}.bind(this));
	});
	...
	QUnit.test("Chart border rendered for 0 value in chart bar", function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_horizon_dark", async function() {
		var borderColor = await this.Parameters_getAsync("sapContent_ForegroundBorderColor");
		var rgbColor = this.hexToRgb(borderColor);
		...
		var boxShadow = [
			...
			assert.ok(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, "The box shadow has been added");
			assert.equal(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, boxShadow, "The border has been added successfully for horizon dark");
			done();
		}.bind(this));
	});
	...
	QUnit.test("Chart border rendered for 0 value in chart bar", function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_horizon_hcw", async function() {
		var borderColor = await this.Parameters_getAsync("sapContent_ForegroundBorderColor");
		...
		var boxShadow = [
			...
			assert.ok(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, "The box shadow has been added");
			assert.equal(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, boxShadow, "The border has been added successfully for horizon high contrast white");
			done();
		}.bind(this));
	});
	...
	QUnit.test("Chart border rendered for 0 value in chart bar", function(assert) {
		//Assert
		var done = assert.async();
		this.applyTheme("sap_horizon_hcb", async function() {
		var borderColor = await this.Parameters_getAsync("sapContent_ForegroundBorderColor");
		...
		var boxShadow = [
			...
			assert.ok(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, "The box shadow has been added");
			assert.equal(window.getComputedStyle(document.getElementsByClassName("sapSuiteCpMCChartBarNegNoValue")[0]).boxShadow, boxShadow, "The border has been added successfully for horizon high contrast black");
			done();
		}.bind(this));
	});
