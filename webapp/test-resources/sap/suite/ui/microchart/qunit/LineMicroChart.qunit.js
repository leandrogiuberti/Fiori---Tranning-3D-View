...
sap.ui.define([
	...
	*/
	...
	QUnit.test("Function _getQualitativeColorClass", function (assert) {
		var aClasses = [];
		for (var i = 1; i <= 12; i++) {
			//Act
			aClasses.push(this.oRenderer._getQualitativeColorClass(i, "border-color"));
			aClasses.push(this.oRenderer._getQualitativeColorClass(i, "background-color"));
			aClasses.push(this.oRenderer._getQualitativeColorClass(i));
		}

		for (var i = 0; i < aClasses.length; i++) {
			var iSequenceIndex = Math.floor(i / 3) + 1;
			var sExpectedValue;

			if (i % 3 === 0) {
				sExpectedValue = "sapSuiteLMCBorderSequence" + iSequenceIndex;
			} else if (i % 3 === 1) {
				sExpectedValue = "sapSuiteLMCBackgroundSequence" + iSequenceIndex;
			} else {
				sExpectedValue = "sapSuiteLMCStrokeSequence" + iSequenceIndex;
			}

			assert.strictEqual(aClasses[i], sExpectedValue, "Class set correctly");
		}
	});
	...
	*/
	...
	QUnit.test("Lines/Points are not colored when EmphasizedPoints are used - composite hex chart colors", async function (assert) {
		...
		if (this.oRenderer._getQualitativeColor) {
		...
		if (!bColorSet) {
			oSvgLines.each(function() {
				assert.ok(jQuery(this).attr("class").indexOf("sapSuiteLMCStrokeSequence"), "class is added correctly");
				assert.notOk(jQuery(this).attr("style"), "above / below color is not used");
			});

			oPoints.each(function() {
				assert.ok(jQuery(this).attr("class").indexOf("sapSuiteLMCBackgroundSequence1"), "class is added correctly");
				assert.ok(jQuery(this).attr("style").indexOf("background-color:#333333") < 0, "below color is not used");
			});
		}
	});
	...
	*/
	QUnit.test("EmphasizedPoint fall back to Neutral if property color is incorrect", function (assert) {
		...
		return TestUtils.getThemeColor("sapUiChartPaletteQualitativeHue1").then(async function(sFallbackColor) {
			...
			if (!bColorSet) {
				var aPoints = this.oChart.$().find(".sapSuiteLMCPoint").filter(function() {
					return this.classList.contains("sapSuiteLMCBackgroundSequence1");
				});
			}
			//Assert
	...
	QUnit.test("Lines/Points fall back to Neutral if property color is incorrect - single incorrect chart color", function (assert) {
		...
		return TestUtils.getThemeColor("sapUiChartPaletteQualitativeHue1").then(async function(sFallbackColor) {
			...
			if (!bColorSet) {
				...
				var oSvgLines = this.oChart.$().find(".sapSuiteLMCSvgElement line").not(".sapSuiteLMCLineThreshold").filter(function() {
					return this.classList.contains("sapSuiteLMCStrokeSequence1");
				});
			}
			//Assert
	...
	*/
	...
	QUnit.module("Lines and Points Alignment when ancestor has center aligned style", {
		beforeEach: async function() {
			this.sTextAlign = window.getComputedStyle(document.getElementById("qunit-fixture"))['text-align'];
			this.oChart = new LineMicroChart("mChart", {
				leftBottomLabel: "June 1",
				rightBottomLabel: "June 30",
				leftTopLabel: "0M",
			...
			document.getElementById("qunit-fixture").style.textAlign = "center";
			await nextUIUpdate();

		},
		afterEach: function () {
			document.getElementById("qunit-fixture").style.textAlign = this.sTextAlign;
			this.oChart.destroy();
			this.oChart = null;
		}
	});

	QUnit.test("Lines and Points should be aligned when ancestor has center aligned style", function(assert) {	
		const oDomRef = this.oChart.getDomRef();
		const oSvgBox = oDomRef.querySelector(".sapSuiteLMCSvgCanvas").querySelector("svg").getBoundingClientRect();
		const oDivBox = oDomRef.querySelector(".sapSuiteLMCSvgCanvas").querySelector("div").getBoundingClientRect();
		
		assert.equal(oSvgBox.left, oDivBox.left, `Lines and Points are aligned correctly`);
	});

	QUnit.test("Chart height shouldn't increase beyond its container", function(assert) {	
		const aVerticalAlignmentContainer = document.querySelectorAll(".sapSuiteLMCVerticalAlignmentContainer");
		let bResult = true;
		for (const oVerticalAlignmentContainer of aVerticalAlignmentContainer) {
			const iVerticalAlignmentContainerHeight = oVerticalAlignmentContainer.getBoundingClientRect().height;
			let iTotalChildrenHeight = 0;
			for (const child of oVerticalAlignmentContainer.children) {
				iTotalChildrenHeight += child.getBoundingClientRect().height;
			}
			if(!(iVerticalAlignmentContainerHeight >= iTotalChildrenHeight)){
				bResult = false;
				break;
			}
		}
		assert.ok( bResult, `chart height doesn't increase`);
	});
