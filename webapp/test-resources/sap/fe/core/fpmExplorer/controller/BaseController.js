	...
	function (
		...
		return Controller.extend("sap.fe.core.fpmExplorer.controller.BaseController", {
			...
			applyAppSettings: function () {
				...
				if (frame && frame.contentWindow && frame.contentWindow.sap) {
					...
					if (jsonModel) {
						...
						// Insertion of the rule for the spacing of the pin symbol in the example
						if (frame.contentDocument && frame.contentDocument.head) {
							var style = frame.contentDocument.createElement("style");
							style.type = "text/css";
							style.appendChild(
								frame.contentDocument.createTextNode(".sapFDynamicPageHeaderPinButton { margin-left: 0.25rem !important; }")
							);
			...
			_onRouteMatched: async function (event) {
				...
				if (!oSample) {
					// redirect to a sample that is configured in the navigationRedirect model, or to default if not found
					this._handleRedirect(sSampleKey);
				}
			...
			/**
			 * Handles redirect logic for invalid or non-existent sample keys.
			 * Checks the navigation redirect model for configured redirects and falls back to default.
			 * @param {string} sSampleKey The sample key that was not found
			 * @private
			 */
			_handleRedirect: function (sSampleKey) {
				try {
					// Get or create the navigation redirect model
					var oNavigationRedirectModel = this.getOwnerComponent().getModel("navigationRedirect");
					if (!oNavigationRedirectModel) {
						// Create the model if it doesn't exist
						this.getOwnerComponent().setModel(navigationRedirect, "navigationRedirect");
						oNavigationRedirectModel = navigationRedirect;
					}

					var aRedirects = oNavigationRedirectModel.getProperty("/redirects") ?? [];
					var oRedirect = aRedirects.find(function (redirect) {
						return redirect.key === sSampleKey;
					});

					if (oRedirect && oRedirect.target) {
						// Navigate to the configured redirect target
						var aParts = oRedirect.target.split("/");
						if (aParts.length >= 2) {
							this.getRouter().navTo(aParts[0], {
								sample: aParts[1],
								subSample: aParts[2] // Optional third part
							});
						} else {
							// Single part target, navigate to topic
							this.getRouter().navTo(oRedirect.target);
						}
					} else {
						// No redirect found, navigate to default home/overview
						this.getRouter().navTo("default");
					}
				} catch (error) {
					Log.error("Error in redirect handling", error);
					// Fallback to overview in case of any errors
					this.getRouter().navTo("default");
				}
			}
		});
