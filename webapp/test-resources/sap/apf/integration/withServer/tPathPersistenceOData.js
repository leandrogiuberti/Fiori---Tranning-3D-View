...
sap.ui.define([
	...
	QUnit.module('Path serialization/deserialization via SerializationMediator', {
		beforeEach : function(assert) {
			...
			function odataRequestWrapper(oInject, oRequest, fnSuccess, fnError, oBatchHandler) {
				...
				function success(data, response) {
					try {
						...
						var oMessage = utils.checkForTimeout(response);
						var oError = {};
				...
				function error(oError) {
					try {
						var oMessage = utils.checkForTimeout(oError);
			...
			this.api = new ApfApi(component, {
				constructors : { MessageHandler : MessageHandler,
					UiInstance : UiInstance 
				},
				functions: {
					odataRequest: odataRequestWrapper
				},
				probe : Probe
			});
			this.fnOriginalAjax = jQuery.ajax;
			this.messageHandler.setMessageCallback(function(){});
			helper.replacePathsInAplicationConfiguration(this.fnOriginalAjax);
		...
		selectionOnSecondStep : function() {
			...
			oRepresentation.emulateSelectionStrategy(SelectionStrategy.coCode1000Cust1001_1002);
