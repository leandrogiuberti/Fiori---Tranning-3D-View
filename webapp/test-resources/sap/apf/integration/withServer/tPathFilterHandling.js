...
sap.ui.define([
	...
	"sap/apf/testhelper/config/configurationForIntegrationTesting",
], function(
	_userData,
	oAuthTestHelper,
	helper,
	...
	configurationForIntegrationTesting
) {
	'use strict';
	...
	function commonSetupAsPromise(oContext) {
		...
		return createComponentAsPromise(oContext, 
	...
	QUnit.module('Path with 3 steps', {
		...
		defineFilterOperators : function() {
			Object.assign(this, this.startFilter.getOperators());
		},
	...
	QUnit.test("Single selection in 2nd step", function(assert) {
		...
		function startTest(oStep) {
			...
			this.step2.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.oneCompanyCode);
	...
	QUnit.test("Filter mapping from country to customers without preserving country filter", function(assert) {
		...
		function startTest(oStep) {
			...
			this.step1.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.oneCountry);
			this.oApi.updatePath(callbackForUpdatePath.bind(this));
		}
	});
	...
	QUnit.test("Selection on first step exists, selection for another step created", function(assert) {
		...
		function startTestSetup(oStep) {
			...
			this.step1.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.threeCompanyCodes);
	...
	QUnit.test("Path contains steps with selection, new selection on first step, ", function(assert) {
		...
		function startTest(oStep) {
			...
			this.step1.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.oneCompanyCode);
			this.oApi.updatePath(callbackForUpdatePath.bind(this));
		}
		function startTestSetup(oStep) {
			if (oStep !== this.step3) {
			...
			this.step1.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.threeCompanyCodes);
			this.oApi.setActiveStep(this.step2);
			this.step2.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.twoCompanyCodes);
			this.oApi.updatePath(startTest.bind(this));
		}
	});
	QUnit.test("No initial selection on 1st step - delete step in the middle of analysis path - change on data of last step", function(assert) {
		...
		function startTestSetup(oStep) {
			...
			this.step1.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.threeCompanyCodes);
			this.step2.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.twoCompanyCodes);
			this.step3.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.oneCompanyCode);
			this.oApi.updatePath(startTest.bind(this));
		}
	});
	QUnit.test("Delete step in the middle of analysis path, that has initial selection - no effect on data of last step", function(assert) {
		...
		function startTestSetup(oStep) {
			...
			this.step1.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.threeCompanyCodes);
			this.step3.getSelectedRepresentation().emulateSelectionStrategy(SelectionStrategy.twoCompanyCodes);
			this.oApi.updatePath(startTest.bind(this));
		}
	});
