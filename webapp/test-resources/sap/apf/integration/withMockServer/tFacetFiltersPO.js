			...
			assertions : {
				iShouldSeeSelFacetFilterListsValuesFromCompanyCode : function() {
					return this.waitFor({
						...
						controlType : "sap.m.FacetFilter",
						success : function(oFacetFilter) {
							Opa.getContext().selectedCompanyCodeCount = oFacetFilter[0].getLists()[2].getSelectedItems().length;
							Opa.getContext().selectedCompanyCodeKey = oFacetFilter[0].getLists()[2].getSelectedItems()[0].getKey();
							Opa5.assert.strictEqual(oFacetFilter[0].getLists()[2].getSelectedItems().length, 1, "One Company Code is Selected from the list");
						}
					});
				},
				iShouldSeeTheLineItemsWithFacetfilterCompanyCodes : function() {
					return this.waitFor({
						...
						check : function(oTable) {
							...
							Opa5.assert.strictEqual(companyCodes[0], Opa.getContext().selectedCompanyCodeKey, "Representation is filtered with select company code values");
							return (companyCodes.length === Opa.getContext().selectedCompanyCodeCount) ? true : false;
						},
						success : function() {
							Opa5.assert.ok(true, "Data in chart is filtered as per values selected in the Company code facet filter");
						},
				...
				iShouldSeeChartWithFacetfilterTimeFrame : function(sFilterType) {
					return this.waitFor({
						...
						success : function(lineChart) {
							...
							if (sFilterType) {
								if (sFilterType === "From_date") {
									Opa5.assert.strictEqual(oChart.getModel().getData().data[0].YearMonth, "201311", "Chart loaded with selected From Date of Facet Filter");
								} else if (sFilterType === "To_date") {
									Opa5.assert.strictEqual(oChart.getModel().getData().data[oChart.getModel().getData().data.length - 1].YearMonth, "201311", "Chart loaded with selected To Date of Facet Filter");
								}
							}
						}
					});
