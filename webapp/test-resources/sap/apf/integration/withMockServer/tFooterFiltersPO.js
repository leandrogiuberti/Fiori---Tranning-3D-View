			...
			assertions : {
				iShouldSeeReportingCurrencyDialog : function() {
					return this.waitFor({
						...
						success : function() {
							Opa5.assert.ok(true, "Currency Select Dialog Opened on UI");
						}
					});
				},
				iShouldSeeChangeInChart : function() {
					return this.waitFor({
						...
						success : function(chart) {
							Opa5.assert.equal(chart[0].getDataset().getMeasures()[0].getName(), "Revenue (EUR)", "Chart loaded is in currency EUR");
						}
					});
				},
				iShouldSeeExchangeRateDialog : function() {
					return this.waitFor({
						...
						success : function() {
							Opa5.assert.ok(true, "Exchange Rate Settings Dialog opened on UI");
						}
					});
				},
				iShouldSeeValuesInExchangeRateDialog : function() {
					return this.waitFor({
						...
						success : function(oDialog) {
							...
							Opa5.assert.strictEqual(sExchangeRateType.getText(), "M - Standard translation at average rate", "M - Standard translation at average rate is selected Exchange Rate Type");
							Opa5.assert.strictEqual(sExchangeRateDate.getText(), "Dynamic Date", "Dynamic date is selected Date Type");
						}
					});
				},
