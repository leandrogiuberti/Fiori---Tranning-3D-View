...
], function(Opa5, PropertyStrictEquals, apfIntegrationTestsPO) {
	"use strict";
	...
	var apfScenarioCommonPO = apfIntegrationTestsPO.extend("apf.tAPFScenarioCommonPO", {
		...
		iSelectAnItemFromSelectDialog : function(sName, sValue) {
			return this.waitFor({
				...
				success : function(aListItems) {
					...
					sap.ui.getCore().applyChanges();
					Opa5.assert.ok(true, "I selected " + sValue + " from the list");
				}
			});
		},
		...
		iStoreTheSelectedChartAndTitle : function() {
			return this.waitFor({
				...
				success : function(chartToolbar) {
					Opa5.getContext().selectedChartType = chartToolbar[0].getCharts()[0].getVizType();
					Opa5.getContext().selectedChartTitle = chartToolbar[0].getCharts()[0].getVizProperties().title.text;
				},
		...
		iClickonObjectHeader : function() {
			return this.waitFor({
				...
				success : function(header) {
					...
					sap.ui.getCore().applyChanges();
					Opa5.assert.ok(true, "Clicked on Analysis Path toolbar");
				}
			});
		},
		iClickOnToolbarItem : function(index) {
			return this.waitFor({
				...
				success : function() {
					...
					sap.ui.getCore().applyChanges();
					Opa5.assert.ok(true, "Clicked on " + _getAnalysisPathToolbarButtons(index) + " button from Analysis Path toolbar");
				}
			});
		},
		...
		iShouldSeeSelectedChart : function(chartType) {
			return this.waitFor({
				...
				success : function(chartToolbar) {
					Opa5.assert.strictEqual(chartToolbar[0].getCharts()[0].getVizType(), chartType, chartType + " is active chart in the Chart container");
					Opa5.assert.ok(chartToolbar[0].getCharts()[0].getVizProperties(), "Viz Properties Exists");
				},
				errorMessage : "Did not find the Chart"
			});
		},
		iAssertAnalysisPathWithSteps : function(steps) {
			return this.waitFor({
				...
				success : function() {
					Opa5.assert.ok(true, "An Analysis step is added to Analysis Path");
				},
