...
], function(Opa5, apfIntegrationTestsPO, apfScenarioCommonPO) {
	"use strict";
				...
				iStoreFirstRecordInTable : function() {
					return this.waitFor({
						...
						success : function(chartToolbar) {
							...
							Opa5.getContext().tableData = table.getModel().getData().tableData[0].CustomerName;
						}
					});
				},
			...
			assertions : {
				iShouldSeeTheTableRepresentation : function() {
					return this.waitFor({
						...
						success : function() {
							Opa5.assert.ok(true, "Main chart representation is switched to alternate representation - table");
						}
					});
				},
				iShouldSeeViewSettingsDialog : function() {
					return this.waitFor({
						...
						success : function() {
							Opa5.assert.ok(true, "View Settings Dialog is opened on UI");
						}
					});
				},
				iShouldSeeTheChartDataSorted : function() {
					return this.waitFor({
						...
						success : function(chartToolbar) {
							...
							var bIsSorted = ((this.myGlobalVariable.oJQuery(tr[0]).find("td:eq(1)").text()).charAt(0) !== (Opa5.getContext().tableData).charAt(0)) ? true : false ;
							Opa5.assert.ok(bIsSorted, "Table Representation is sorted as per sort order and property selected in View Settings Dialog");
						}
					});
				}
			}
