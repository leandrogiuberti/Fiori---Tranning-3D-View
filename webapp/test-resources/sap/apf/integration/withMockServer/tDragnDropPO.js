			...
			assertions : {
				iShouldSeeTheStepsSwapped : function() {
					return this.waitFor({
						...
						success : function() {
							Opa5.assert.ok(true, "Second Step is swapped to first position and vice versa");
						}
					});
				},
				iShouldSeeActiveChart : function(chartType) {
					return this.waitFor({
						...
						success : function(chartToolbar) {
							...
							Opa5.assert.equal(chart.getVizType(), chartType, message);
							Opa5.assert.ok(chart.getVizProperties(), "Viz Properties Exists");
						}
					});
				}
			}
