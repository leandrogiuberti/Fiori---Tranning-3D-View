...
sap.ui.define([
	...
	function commonSetup(oContext, assert) {
		...
		var inject = {
				constructors : {
					...
					ResourcePathHandler : ResourcePathHandlerDouble
				}
		};
		ResourcePathHandlerDouble.prototype.loadConfigFromFilePath = function() {
		...
		createComponentAsPromise(oContext, 
	...
	QUnit.test("Path with 3 steps without selection, change selection and test filter propagation", function(assert) {
		...
		oRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_1002);
		...
		oRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001);
		this.oApi.updatePath(updatePathCallback);
		...
		oRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_2_1004);
		this.oApi.updatePath(updatePathCallback);
	...
	QUnit.test("Callback for update indicates refresh", function(assert) {
		...
		this.oStep1.getSelectedRepresentation().emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_1002);
	...
	QUnit.test("Path with first step with selection and 2 further steps with initial 'all' selection, change selection and test filter update", function(assert) {
		...
		oFirstStepRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_2_1004);
		this.oApi.updatePath(updatePathCallback);
		var aDataOfFirstStepRepresentationAfterSelectionChange = aSteps[0].getSelectedRepresentation().aDataResponse;
		assert.equal(aDataOfFirstStepRepresentationAfterSelectionChange.length === aDataOfFirstStepRepresentationBeforeSelectionChange.length, true, "No change on the first step");
		var aDataOfLastStepRepresentationAfterSelectionChange = aSteps[2].getSelectedRepresentation().aDataResponse;
	...
	QUnit.test("Path with first step with selection and 2 further steps with a selection, change selection and test filter update", function(assert) {
		...
		oFirstStepRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001);
		this.oApi.updatePath(updatePathCallback);
		var aDataOfFirstStepRepresentationAfterSelectionChange3 = aSteps[0].getSelectedRepresentation().aDataResponse;
		assert.equal(aDataOfFirstStepRepresentationAfterSelectionChange3.length === aDataOfFirstStepRepresentationBeforeSelectionChange.length, true, "No change on the first step");
		var aDataOfLastStepRepresentationAfterSelectionChange3 = aSteps[2].getSelectedRepresentation().aDataResponse;
	...
	QUnit.test("Path with first step with selection and 2 further steps with a selection, delete step in middle, which has a selection and test filter update", function(assert) {
		...
		oFirstStepRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coArea1000_2000);
		this.oApi.updatePath(updatePathCallback);
		var aDataOfFirstStepRepresentationAfterSelectionChange = aSteps[0].getSelectedRepresentation().aDataResponse;
		assert.equal(aDataOfFirstStepRepresentationAfterSelectionChange.length === aDataOfFirstStepRepresentationBeforeSelectionChange.length, true, "No change on the first step");
		var aDataOfLastStepRepresentationAfterSelectionChange = aSteps[2].getSelectedRepresentation().aDataResponse;
		...
		oRepr2.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coArea1000project1001_1002);
		this.oApi.updatePath(updatePathCallback);
		...
		var bContains = helper.isContainedInArray(aExpectedData, aDataOfLastStepRepresentationAfterSelectionChange);
	...
	QUnit.module('PFH Representation change in combination with path update', {
		...
		selectTwo : function(oStep) {
			this.setSelection(oStep, RepresentationDouble.SelectionStrategy.coCode1000Cust1001_1002);
		},
		selectFour : function(oStep) {
			this.setSelection(oStep, RepresentationDouble.SelectionStrategy.coCode1000Cust1001_2_1004);
		},
		selectNothing : function(oStep) {
			this.setSelection(oStep, RepresentationDouble.SelectionStrategy.nothing);
		},
		selectIndices : function(oStep) {
			var oSelectedRepresentation = oStep.getSelectedRepresentation();
			oSelectedRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.indicesOfSelectedData);
		},
		afterEach : function() {
			this.oCompContainer.destroy();
		}
	...
	QUnit.test('Set data is called in binding  after update when switching representations', function(assert) {
		...
		oReprOfSecondStep.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_2_1004);
		var o1stReprOfThirdStep = oThirdStep.getSelectedRepresentation();
		o1stReprOfThirdStep.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_1002);
		this.oApi.updatePath(updatePathCallback);
		...
		oReprOfSecondStep.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_1002);
		this.oApi.updatePath(updatePathCallback);
	...
	QUnit.test("Step in the middle changes to empty selection", function(assert) {
		...
		oSecondStepRepresentation.emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.undefinedSelection);
		this.oApi.updatePath(updatePathCallback);
	...
	var bChangeBehaviour = false;
	QUnit.module('PFH Errorhandling', {
		beforeEach : function(assert) {
			var that = this;
			var done = assert.async();
			var PatchedRequestDouble = function(oInject, oConfig) {
				bChangeBehaviour = false;
				var oMessageHandler = oInject.instances.messageHandler;
				var oRequest = new RequestDouble(oInject, oConfig);
	...
	QUnit.test("Update fails on existing path", function(assert) {
		...
		bChangeBehaviour = true;
		aSteps[0].getSelectedRepresentation().emulateSelectionStrategy(RepresentationDouble.SelectionStrategy.coCode1000Cust1001_2_1004);
	...
	QUnit.module("Path with different steps", {
		beforeEach : function(assert) {
			...
			ResourcePathHandlerDouble.prototype.loadConfigFromFilePath = function() {
				this.oMessageHandler.loadConfig(this.getApfMessages(), true);
				this.oMessageHandler.loadConfig(this.getAppMessages());
				var oConfiguration = sampleConfig.getSampleConfiguration("twoFilterMappingSteps");
				this.oCoreApi.loadAnalyticalConfiguration(oConfiguration);
			};
			createComponentAsPromise(this, {
