...
sap.ui.define([
...
], function(coreConstants, ProxyTextHandlerForLocalTexts, SamplePropertyFiles, utils, cloudFoundryUtils, proxyModule, MessageObject, Filter,
			deepExtend, isEmptyObject, jQuery, sinon) {
	'use strict';
	...
	QUnit.test("WHEN successfully creating an Application", function (assert) {
		...
		var applicationContinuation = function (data, metadata, messageObject) {
			...
			assert.strictEqual(isEmptyObject(metadata), true, "THEN Metadata is empty");
	...
	QUnit.test("WHEN successfully creating an Analytical Configuration", function (assert) {
		...
		var expectedConfig = deepExtend({}, analyticalConfiguration);
		var requestBody = {
		...
		var response = {
				analyticalConfigurations:
					[
					 ...
					 textFile: {
						 inDevelopmentLanguage: SamplePropertyFiles.getPropertyFile("dev", requestUrlParameters.application)
					 }
		};
		var expectedResult = [{
			AnalyticalConfiguration: "BBBBBBBB05550175E10000000BBBBBBB",
	...
	QUnit.test("WHEN successfully reading an Analytical Configuration list but the returned data is an empty object", function (assert) {
		...
		var callback = function (data, metadata, messageObject) {
			...
			assert.strictEqual(isEmptyObject(data), true, "THEN data is an empty object due to error");
			assert.deepEqual(this.spyBuildErrorMessage.getCall(0).args[0], jqXHR200, "THEN jqxhr has been provided");
		...
		var responseFromServer = {
				analyticalConfigurations:
					[
					 ...
					 textFile: {
						 inDevelopmentLanguage: SamplePropertyFiles.getPropertyFile("dev", requestUrlParameters.applicationId)
					 }
		};
	...
	QUnit.test("WHEN _initText is called", function(assert){
		...
		var propertyFileString = SamplePropertyFiles.getPropertyFile("dev", applicationId);
	...
	QUnit.test("WHEN _updateRemoteTextFile is called successfully for import from file", function (assert) {
		...
		var inputDataContract = {
				...
				textFileLanguage: coreConstants.developmentLanguage,
				serializedTextFile: JSON.stringify(SamplePropertyFiles.getPropertyFile("dev", testData.applicationId)),
			...
			var responseFromServerSuccess = {
					analyticalConfigurations:
						[
						 ...
						 textFile: {
							 inDevelopmentLanguage: SamplePropertyFiles.getPropertyFile("dev", testData.applicationId)
						 }
			};
			var callCountAjax = 0;
			var ajaxHandler = {
	...
	QUnit.module("Do change operations in batch - aka textpool cleanup No.4", {
		beforeEach: function () {
			...
			this.returnFromCreateTextFileOfApplication = SamplePropertyFiles.getPropertyFile("dev", testData.applicationId);
