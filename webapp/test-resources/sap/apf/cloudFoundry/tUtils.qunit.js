sap.ui.define([
	...
	function(Component, cloudFoundryUtils, MessageObject, testConfig) {
		'use strict';
		...
		QUnit.module("cloudFoundryUtils.resolveUri", {
			beforeEach: function() {
				...
				this.oComponentPath = sap.ui.require.toUrl("sap/ui/core/");
		...
		QUnit.test("WHEN resolve relative URL", function(assert) {
			...
			assert.strictEqual(cloudFoundryUtils.resolveUri(this.oCoreApi, "test?q=12&c=asdf"), this.oComponentPath + "test?q=12&c=asdf", "THEN path with query is correctly resolved");
			assert.strictEqual(cloudFoundryUtils.resolveUri(this.oCoreApi, "test#abc"), this.oComponentPath + "test#abc", "THEN path with fragment is correctly resolved");
		});
