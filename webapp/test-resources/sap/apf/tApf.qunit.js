...
sap.ui.define([
	...
	QUnit.test('Filter method types', function(assert) {
		assert.equal(coreConstants.filterMethodTypes.selectionAsArray, 'saa', 'Selection as arrray exists');
		assert.equal(coreConstants.filterMethodTypes.filter, 'f', 'Filter object exists');
	});
	...
	QUnit.module('Core Api with Configuration Factory - getConfigurationObjectById', {
		beforeEach : function(assert) {
			var done = assert.async();
			var that = this;
			this.configurationTemplate = sampleConfig.getSampleConfiguration('addTwoFacetFilters');
			ResourcePathHandlerDouble.prototype.loadConfigFromFilePath = function() {
				this.oMessageHandler.loadConfig(this.getApfMessages(), true);
				this.oMessageHandler.loadConfig(this.getAppMessages());
			...
			var MetadataStub = function() {
				...
				this.isInitialized = function() {
					return utils.createPromise(this);
				};
			};
			var inject = {
					constructors : {
	...
	QUnit.module('Filter creation', {
		beforeEach : function(assert) {
			var done = assert.async();
			createComponentAsPromise(this, { stubAjaxForResourcePaths : true, doubleUiInstance : true}).done(function(){
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test('createFilter works', function(assert) {
		...
		assert.equal(oFilter instanceof Filter, true, 'Filter object returned');
	});
	...
	function setupWithDatajsInjection(context, assert) {
		...
		ResourcePathHandlerDouble.prototype.loadConfigFromFilePath = function() {
			this.oMessageHandler.loadConfig(this.getApfMessages(), true);
			this.oMessageHandler.loadConfig(this.getAppMessages());
			var oConfigurationTemplate = sampleConfig.getSampleConfiguration();
	...
	QUnit.module("GetPropertyValuesOfExternalContext", {
		beforeEach : function() {
			...
			var component = defineDummyComponent();
			this.api = new Api(component, { constructors : { ExternalContext : ExternalContext }});
		}
	});
	...
	QUnit.test("WHEN getPropertyValuesOfExternalContext is called", function(assert){
		...
		function prepareFilterOfExternalContext(context) {
			...
			context.filterOfExternalContext = new CoreFilter(messageHandler, "SAPClient", "EQ", "777");
			context.filterOfExternalContext.addAnd(new CoreFilter(messageHandler, "Prop1", "EQ", "999"));
		}
	...
	QUnit.test("WHEN getPropertyValuesOfExternalContext with more complex Filter is called", function(assert){
		...
		function prepareFilterOfExternalContext(context) {
			var messageHandler = new MessageHandler();
			context.filterOfExternalContext = new CoreFilter(messageHandler, "SAPClient", "EQ", "777");
			var filterProp1 = new CoreFilter(messageHandler, "Prop1", "EQ", "999");
			filterProp1.addOr(new CoreFilter(messageHandler, "Prop1", "LE", "888"));
	...
	QUnit.test("WHEN api is created AND fatal error occurs during instantiation", function(assert){
		...
		assert.ok( result instanceof Text, "THEN text object as supplement for APF LAYOUT is returned in startup");
	});
	QUnit.test("WHEN api is created AND fatal error occurs during startApf", function(assert){
		...
		var fnResourcePathHandler = function(inject) {
			inject.instances.coreApi.loadMessageConfiguration(messageDefinition, true);
	...
	QUnit.module("Inject provided", {
		beforeEach : function() {
			this.externalContextSpy = sinon.spy(StartParameter);
			...
			var TApfComponent = BaseComponent.extend("sap.apf.test.tApfComponent", {
				getInjections : that.spyInject
			});
			this.oCompContainer = new TApfComponent();
		},
		afterEach : function() {
			this.oCompContainer.destroy();
		}
	...
	QUnit.test("and getInjections set THEN injected constructor should be called", function (assert) {
		assert.ok(this.externalContextSpy.called);
	...
	QUnit.test("WHEN callback onBeforeStartApfCallback is active", function(assert){
		...
		createComponentAsPromise(context, config).done(function() {	
			done();
		});
		var corePromise = context.oComponent.getProbe().corePromise;
