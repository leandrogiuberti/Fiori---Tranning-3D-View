...
sap.ui.define([
	...
	QUnit.module("Facet Filter value formatter for list values", {
		beforeEach : function() {
			this.formatterDouble = sinon.stub(formatter.prototype, "getFormattedValue", getFormattedValueStub);
			//oCoreApi stub
			var oCoreApi = {
			...
			this.oFacetFilterValueFormatter = new FacetFilterValueFormatter(oUiApi, oCoreApi);
		},
		afterEach: function(){
	...
	QUnit.test("Global export exists and is the same as the AMD export", function(assert) {
		var globalExport = ObjectPath.get("sap.apf.ui.utils.FacetFilterValueFormatter");
		assert.strictEqual(typeof globalExport, "function")
		assert.strictEqual(globalExport, FacetFilterValueFormatter);
	});
});
