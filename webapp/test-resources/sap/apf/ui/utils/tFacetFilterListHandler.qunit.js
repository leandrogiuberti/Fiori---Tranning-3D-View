...
sap.ui.define([
	...
	function commonSetup(context){
		...
		context.ffListHandler = new FacetFilterListHandler(oCoreApi, oUiApi, context.oConfiguredFilter);
	}
	...
	QUnit.test("List has correct properties", function(assert){
		...
		assert.strictEqual(facetFilterList.getMode(), ListMode.SingleSelectMaster, "List is not in MultiSelect mode");
	...
	QUnit.test("List is multiselect", function(assert){
		...
		assert.strictEqual(facetFilterList.getMode(), ListMode.MultiSelect, "List is in MultiSelect mode");
	});
	...
	/**
	 * @deprecated the use of global names is deprecated
	 */
	QUnit.test("Global export exists and is the same as the AMD export", function(assert) {
		var globalExport = ObjectPath.get("sap.apf.ui.utils.FacetFilterListHandler");
		assert.strictEqual(typeof globalExport, "function")
		assert.strictEqual(globalExport, FacetFilterListHandler);
	});
});
