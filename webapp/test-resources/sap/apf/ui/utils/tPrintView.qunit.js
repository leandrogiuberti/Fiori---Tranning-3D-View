...
sap.ui.define([
	...
	function _getStep(oGlobalApi) {
		...
		var dummyContentForChart = new LineChart(interfaceProxy, parameter);
	...
	QUnit.test("When header for first page is created", function(assert) {
		...
		//assert
		assert.strictEqual(oHeaderForFirstPage instanceof HorizontalLayout, true, "then horizontal layout is created to print header first page");
		assert.strictEqual(oHeaderForFirstPage.getContent().length, 3, "then header layout has 3 contents");
		assert.strictEqual(oHeaderForFirstPage.getContent()[0] instanceof Text, true, "then header layout's first content is an instance of text");
		assert.strictEqual(oHeaderForFirstPage.getContent()[1] instanceof Text, true, "then header layout's second content is an instance of text");
		assert.strictEqual(oHeaderForFirstPage.getContent()[2] instanceof Text, true, "then header layout's third content is an instance of text");
	...
	QUnit.test("When filters has been formatted and prepared to print", function(assert) {
		...
		//assert
		assert.strictEqual(oFiltersLayout instanceof VerticalLayout, true, "then vertical layout is created for filters");
		assert.strictEqual(oFiltersLayout.getContent().length, 2, "then filter layout has 2 contents");
		assert.strictEqual(oFiltersLayout.getContent()[0] instanceof Text, true, "then filter layout's first content is an instance of text");
		assert.strictEqual(oFiltersLayout.getContent()[1] instanceof Text, true, "then filter layout's second content is an instance of text");
	...
	QUnit.test("When steps has been prepared to print", function(assert) {
		...
		//assert
		assert.strictEqual(oPrintLayoutForEachStep instanceof VerticalLayout, true, "then vertical layout is created for steps");
		assert.strictEqual(oPrintLayoutForEachStep.getContent().length, 2, "then step layout has 2 contents");
		assert.strictEqual(oPrintLayoutForEachStep.getContent()[0] instanceof HTML, true, "then step layout's first content is an instance of html");
		assert.strictEqual(oPrintLayoutForEachStep.getContent()[1] instanceof VerticalLayout, true, "then step layout's second content is an instance of vertical layout");
