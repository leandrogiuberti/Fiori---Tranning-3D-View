...
sap.ui.define([
	...
	function _getStepsStubOfTableRep() {
         var titleControl = new Title({
				level : TitleLevel.H1
			}).addStyleClass("sapUiTinyMarginBegin").addStyleClass("sapUiTinyMarginTop");
		 titleControl.oParent = new HBox();
		 titleControl.oParent.addItem(new Title());
		 titleControl.oParent.addItem(new HBox());
       return [ {
	...
	QUnit.module("Print Model Tests", {
		beforeEach : function(assert) {
			...
			createUiApiAsPromise().done(function(oApi) {
				...
				printModel = new PrintModel(oInject);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When printing table representation", function(assert) {
		...
		assert.strictEqual(oTable instanceof Table, true, "table Representation for print");
		//cleanup
	...
	QUnit.module("Print with smartFilterbar with parameters", {
		beforeEach : function() {
			...
			this.printModel = new PrintModel(oInject);
		}
	});
	...
	QUnit.test("With parameter and filter", function(assert) {
		...
		this.filters.push(new UI5Filter({
			path: "Country",
			operator: "EQ",
			value1: "DE"
		}));
