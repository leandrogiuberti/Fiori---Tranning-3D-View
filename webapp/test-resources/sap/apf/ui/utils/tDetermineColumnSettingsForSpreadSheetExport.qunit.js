sap.ui.define(
	...
	function (determineColumnSettingsForSpreadSheetExport, exportLibrary) {
		"use strict";
		...
		QUnit.test("WHEN no metadata is supplied", function (assert) {
			var result = determineColumnSettingsForSpreadSheetExport("decimalProperty");
		...
		QUnit.test("WHEN property has type Edm.Int32", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("integerProperty", metadata);
			assert.equal(result.type, EdmType.Number, "THEN number type returned");
		...
		QUnit.test("WHEN property has type Edm.Decimal AND no sap:unit", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("decimalProperty", metadata);
			assert.equal(result.type, EdmType.Number, "THEN number type returned");
		...
		QUnit.test("WHEN property has type Edm.Decimal AND sap:unit", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("decimalProperty", metadata);
			assert.equal(result.type, EdmType.Currency, "THEN currency type returned");
			assert.equal(result.scale, 3, "THEN scale as expected");
			assert.equal(result.precision, 12, "THEN precision as expected");
		});
		QUnit.test("WHEN property has type Edm.DateTime AND no display format as date", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("dateProperty", metadata);
			assert.equal(result.type, EdmType.DateTime, "THEN date time type returned");
		});
		...
		QUnit.test("WHEN property has type Edm.DateTime AND display format as date", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("dateProperty", metadata);
			assert.equal(result.type, EdmType.Date, "THEN date type returned");
		});
		QUnit.test("WHEN curency code", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("currency", metadata);
			assert.equal(result.type, EdmType.String, "THEN string type returned");
		...
		QUnit.test("WHEN Edm.String", function (assert) {
			...
			var result = determineColumnSettingsForSpreadSheetExport("stringProperty", metadata);
			assert.equal(result.type, EdmType.String, "THEN string type returned");
