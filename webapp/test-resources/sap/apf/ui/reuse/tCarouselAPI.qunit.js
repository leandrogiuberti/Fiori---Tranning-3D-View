...
sap.ui.define([
	...
	'sap/ui/thirdparty/sinon'
			...
			Control, Element, MvcController, MvcView, CustomListItem, WrappedChartWithCornerTexts, sapLog, deepExtend, sinon) {
	'use strict';
	...
	QUnit.module("Context createStep - Given a stubbed core instance and a step gallery controller", {
		beforeEach: async function (assert) {
			...
			that.view = await MvcView.create({
				viewName: "module:sap/apf/ui/fixture/stub.view",
				controller: await MvcController.create({
	...
	QUnit.module("Context createStep - Given an analysisPathController with a stubbed view", {
		...
		beforeEach: async function (assert) {
			...
			that.analysisPathController = await MvcController.create({
				name: "sap.apf.ui.reuse.controller.analysisPath"
			});
			that.analysisPathView = await MvcView.create({
				viewName: "module:sap/apf/ui/fixture/stub.view",
				controller: that.analysisPathController,
				id: "sap.apf.ui.fixture.stub-ID",
				viewData: {
			...
			that.analysisPathView.getCarouselView = function () {
				return {
					getController: function () {
						return mocked;
					}
	...
	QUnit.module("Given a stubbed Carousel and a coreApi", {
		...
		beforeEach: function (assert) {
			...
			that.someChart2 = deepExtend({}, that.someChart1);
			that.someChartN = deepExtend({}, that.someChart1);
			...
			MvcView.create({
				...
				sapLog.error("*** TEST: MvcView.create: : " + error);
				done();
			});
		},
		afterEach: function () {
			...
			subView = Element.getElementById('idViewStubbedCarousel--idAddAnalysisStepButtonInFooter');
			if (subView) {
				subView.destroy();
			}
			subView = Element.getElementById('idViewStubbedCarousel--idTestPathButtonInFooter');
			if (subView) {
				subView.destroy();
			}
			subView = Element.getElementById('idViewStubbedCarousel--idDeleteStepButtonInFooter');
			if (subView) {
				subView.destroy();
			}
			subView = Element.getElementById('idViewStubbedCarousel--idMoveStepUpButtonInFooter');
			if (subView) {
				subView.destroy();
			}
			subView = Element.getElementById('idViewStubbedCarousel--idMoveStepDownButtonInFooter');
			if (subView) {
				subView.destroy();
			}
	...
	* s-s-a remove 3rd: 2nd gets active
	...
	//prep
		...
		that.spy._setBusyOnCustomListItem.restore();
		that.spy._setBusyOnCustomListItem = sinon.stub(that.carouselController, "_setBusyOnCustomListItem");
	...
	QUnit.test("GIVEN new WrappedChartWithCornerTexts - When calling setHighlighted", function(assert) {
		...
		var oClonedChart = Object.assign(new Control(), {
			setHeight: function(){},
			setWidth: function(){}
		});
