sap.ui.define([
...
], function(Device, XMLView, nextUIUpdate){
	"use strict";
	...
	async function commonSetup(context, data){
		context.oCoreApi = {
		...
		context.filterDialogView = await XMLView.create({
	...
	function attachAssertsOnClose(assert, context){
		...
		context.filterDialog.attachAfterClose(function(){
			...
			//assert.strictEqual(context.filterDialogView.bIsDestroyed, false, "View is closed");
			done();
		});
	}
	QUnit.module("Basic Interaction",{
		beforeEach: function(){
			...
			return commonSetup(this, this.modelData);
		},
		afterEach: function(){
	...
	QUnit.module("Warning and Information Icon",{
		beforeEach: function(){
			...
			return commonSetup(this, modelData);
		},
		afterEach: function(){
			this.filterDialog.destroy();
			this.filterDialogView.destroy();
