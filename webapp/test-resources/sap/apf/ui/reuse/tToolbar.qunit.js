...
sap.ui.define([
	...
	], function(createUiApiAsPromise, NavigationHandler, MessageHandler, AjaxStub, mobileLibrary, StandardListItem, Element, nextUIUpdate, sinon) {
	'use strict';
	...
	function dialogWithEmptyTitleFound() {
		let maxCount = 40;
		return new Promise(function(resolve, reject) {
			function check() {
				const dialog = getDialogByEmptyTitle();
				if ( dialog ) {
					resolve(dialog);
					return;
				}
	...
	QUnit.test("WHEN stable ids are defined", function(assert){
		...
		listWithIdAndTitleKey.forEach(function(idAndKey){
			...
			assert.ok(listItem instanceof StandardListItem, "THEN list item with id exists");
	...
	QUnit.test('given an unsaved path,when clicking "New" button from toolbar,then clicking "Ok" button of new dialog then clicking "Cancel" button of save dialog', function(assert) {
		...
		assert.strictEqual(dialog.getType(), DialogType.Message,"Dialog is of message Type");
		//act
		pressOkButtonOfDialog("newPath", self, assert, function() {
			...
			assert.strictEqual(saveAnalysisPathDialog.getType(), DialogType.Message,"Dialog is of message Type");
	...
	QUnit.test('given a saved path,when clicking "Open" button from toolbar', async function(assert) {
		//arrangement
		var self = this; // eslint-disable-line no-invalid-this
		var pathName = "analysisPathName";
		var layoutView = self.oGlobalApi.oUiApi.getLayoutView().getController();
		...
		//assert
		const selectPathDialog = await dialogWithEmptyTitleFound();
		assert.ok(selectPathDialog, "then selectPathDialog exists");
		assert.equal(selectPathDialog.isOpen(), true, "then selectPathDialog is in open state");
		pressCancelButtonOfDialogByEmptyTitle(assert, async function() {
			const selectPathDialog = await dialogWithEmptyTitleFound();
			//assert
			assert.strictEqual(selectPathDialog, undefined, "then 'Select Analysis Path' dialog does not exist");
		});
	...
	QUnit.test('given a saved path,when clicking "Open" button again from toolbar after closing select path gallery dialog', async function(assert) {
		//arrangement
		...
		//assert
		let selectPathDialog = await dialogWithEmptyTitleFound();
		assert.ok(selectPathDialog, "then selectPathDialog exists");
		...
		assert.equal(selectPathDialog.isOpen(), true, "then selectPathDialog is in open state");
		pressCancelButtonOfDialogByEmptyTitle(assert, async function() {
			selectPathDialog = await dialogWithEmptyTitleFound(); 
			//assert
			assert.strictEqual(selectPathDialog, undefined, "then 'Select Analysis Path' dialog does not exist");
	...
	QUnit.test('given a saved path,when performing two single clicks on "Open" button from toolbar', async function(assert) {
		//arrangement
		var self = this; // eslint-disable-line no-invalid-this
		var pathName = "analysisPathName";
		var layoutView = self.oGlobalApi.oUiApi.getLayoutView().getController();
		...
		//assert
		const selectPathDialogOnFirstClick = await dialogWithEmptyTitleFound();
		...
		const selectPathDialogOnSecondClick = await dialogWithEmptyTitleFound();
	...
	QUnit.test('given already saved path, when clicking "Save As" button from toolbar and then clicking "Yes" button of "Overwrite AnalysisPath" dialog ', function(assert) {
		...
		pressOkButtonOfDialog("save-analysis-path", self, assert, function() {
			...
			//assert
			assert.ok(overwritePathDialog, "when ok then overwrite Ananlysis path dialog exists");
			assert.strictEqual(overwritePathDialog.getType(), DialogType.Message,"Dialog is of message Type");
	...
	/*DELETE ANALYSISPATH */
	QUnit.test('given a saved/unsaved path,when clicking "Delete" button from toolbar', async function(assert) {
		//arrangement
		var done = assert.async();
		var self = this; // eslint-disable-line no-invalid-this
		var pathName = "analysisPathName";
		...
		var pSelectPathDialog = self.toolbarController.oPathGalleryDialog["deleteAnalysisPath"];

		//assert
		assert.ok(pSelectPathDialog, "then selectPathDialog has been requested");
		var selectPathDialog = (await pSelectPathDialog).getContent()[0];
		assert.ok(selectPathDialog, "then selectPathDialog exists");
		assert.equal(selectPathDialog.isOpen(), true, "then selectPathDialog is in open state");
		selectPathDialog.attachAfterClose(function() {
			...
			assert.strictEqual(selectPathDialog.bIsDestroyed, true, "Delete analysis path view is destroyed");
			done();
		});
		selectPathDialog.getBeginButton().firePress();
		await nextUIUpdate();
	});
	QUnit.test('When calling destroy function', async function(assert) {
		var self = this; // eslint-disable-line no-invalid-this
		var pathName = "analysisPathName";
		var layoutView = self.oGlobalApi.oUiApi.getLayoutView().getController();
		layoutView.oSavedPathName.setText(pathName);
		...
		var selectPathDialog = (await self.toolbarController.oPathGalleryDialog["deleteAnalysisPath"]).getContent()[0];
		selectPathDialog.getBeginButton().firePress();
		await nextUIUpdate();
		//action
