...
sap.ui.define([
	...
	function _createCommonSetup(oSmartFilterBarConfiguration, stubs, context) {
		...
		createComponentAsPromise(context, { stubAjaxForResourcePaths : true }).done(async function(){
			//setup for mock server to instantiate OData model for SFB
			context.oMockServer = new MockServer({
				rootUri : "/foo/"
			});
			var sUrl = sap.ui.require.toUrl("sap/apf/testhelper/mockServer/metadata/smartFilterBar.xml");
			context.oMockServer.simulate(sUrl, {
				'sMockdataBaseUrl' : sap.ui.require.toUrl("sap/apf/testhelper/mockServer/metadata/"),
				'bGenerateMissingMockData' : true
			});
			context.oMockServerWithSapSystem = new MockServer({
				rootUri : "/foo;o=myERP/"
			});
			...
			context.oSmartFilterBarView = await View.create({
	...
	QUnit.test("When smart filter bar view is loaded", function(assert) {
		...
		assert.strictEqual(oSmartFilterBar.getModel() instanceof ODataModel, true, "THEN model instance of v2 odata model");
		assert.strictEqual(oSmartFilterBar.getModel().getDefaultCountMode(), CountMode.None, "THEN expected count mode");
	});
