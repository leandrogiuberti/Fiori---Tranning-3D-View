...
sap.ui.define([
	...
	"sap/apf/testhelper/stub/activeStepStub",
	"sap/apf/testhelper/helper",
	"sap/apf/testhelper/stub/ajaxStub",
	"sap/apf/testhelper/interfaces/IfUiInstance"
], function(deepExtend, CoreControl, Element, Fragment, VerticalLayout, jQuery, createUiApiAsPromise, activeStepStub) {
	"use strict";
	...
	var getAnalysisPathStub = function() {
		var control = CoreControl.extend("hugo", {
			        renderer: null
		});
		...
		analysisPath.getCarouselView = function() {
			...
			carousel.getStepView = function(x) {
				var stepView = {};
				stepView.oThumbnailChartLayout = new VerticalLayout();
	...
	QUnit.module('When drawing chart container with Chart representation with selection', {
		beforeEach : function(assert) {
			var activeStep = activeStepStub.prototype;
	...
	QUnit.test("When pressing selected property count link", async function(assert) {
		var oController = this.oStepContainerView.getController();
		sinon.spy(oController, "openSelectionDisplayDialog");
		oController.handlePressSelectedPropertyCountLink();
		assert.strictEqual(oController.openSelectionDisplayDialog.callCount, 1, "openSelectionDisplayDialog called exactly once");
		await oController.openSelectionDisplayDialog.returnValues[0]; // wait for the dialog to be loaded
		var selectionDisplayDialog = Fragment.byId("idSelectionDisplayFragment", "idSelectionDisplayDialog");
		...
		oController.openSelectionDisplayDialog.restore();
	});
	...
	QUnit.test("When creating toggle representation with Key", function(assert) {
		var oController = this.oStepContainerView.getController();
		var requiredDimensions = activeStepStub.prototype.getActiveStepAlternateRep().getSelectedRepresentation().getParameter().dimensions;
		var oRepresentation = oController.oView.oViewData.oCoreApi.getActiveStep().getSelectedRepresentation();
	...
	QUnit.module("Draw StepContent", {
				...
				getSelectedRepresentation : function() {
					return {
						...
						getMainContent : function() {
							return new CoreControl("ChartContent");
						},
		...
		afterEach : function() {
			...
			Element.getElementById("stepList")?.destroy(); // created by Carousel
		}
	});
	...
	QUnit.test("Step has changed", function(assert) {
		var controller = this.oStepContainerController;
		//change active step
		this.activeStep = deepExtend({}, this.activeStep);
		controller.drawStepContent(false);
		var chartContainer = controller.byId("idChartContainer");
		var chartContent = chartContainer.getContent()[0].getContent();
		assert.strictEqual(controller.byId("idSelPropertyAndCount").getText(), "selectedProperty (1) ", "SelectedProperty link in toolbar is set (toolbar is drawn)");
