sap.ui.define([
	...
	function createApi(property) {
		...
		return {
			createFilter : function() {
				return new Filter(messageHandler);
			},
	...
	QUnit.module("UI5 Charthelper with Time Series Chart", {
		beforeEach : function() {
			...
			this.oTimeAxisConvertor = new TimeAxisDateConverter();
			var formatter = new FormatterStub();
			this.oChartDataSetHelper = new ChartDataSetHelper(formatter, this.oTimeAxisConvertor);
			var metadata = {
	...
	QUnit.module("UI5 Charthelper with Time Series Chart and Conversion of YYYYMMDD field", {
		beforeEach : function() {
			...
			var formatter = new FormatterStub();
			this.oChartDataSetHelper = new ChartDataSetHelper(formatter, this.oTimeAxisConvertor);
			this.oRepresentationFilterHandler = new RepresentationFilterHandler(createApi(), this.parameters, this.oTimeAxisConvertor);
			this.metadata = {
				getPropertyMetadata : getPropertyMetadata
			};
		...
		assertExtendedDataResponseIsPrepared : function(helper, assert) {
			...
			for(i = 0; i < data.length; i++) {
				...
				expectedDate = utils.convertFiscalYearMonthDayToDateString(referenceData[i].YearMonthDay);
