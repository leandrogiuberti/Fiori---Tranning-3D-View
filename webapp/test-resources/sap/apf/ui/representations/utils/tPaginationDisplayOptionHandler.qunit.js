...
sap.ui.define([
	...
	QUnit.module("Pagination Display option handler", {
		beforeEach : function(assert) {
			...
			createUiApiAsPromise().done(function(oApi) {
				...
				oRepresentationHelper = representationHelper.prototype;
				oMetadata = _getsampleMetadata();
				aDataResponse = sampleService.get('sampleData');
				...
				oPaginationDisplayOptionHandler = new PaginationDisplayOptionHandler();
				done();
			});
		},
		afterEach: function(){
	...
	QUnit.module("Display for key property is different from filter property (treeTable)", {
		beforeEach : function(assert) {
			this.paginationDisplayHandler = new PaginationDisplayOptionHandler();
			...
			this.convertToExternalFormatStub = sinon.stub(utils, "convertToExternalFormat", function(sSelectionDisplayText, propertyMetadataInstance){
			...
			this.propertyMetadataStub = sinon.stub(MetadataProperty, "constructor", function(propertyMetadata){
				assert.strictEqual(propertyMetadata, "propertyMetadata", "PropertyMetadata correctly handed over to MetadataProperty class");
