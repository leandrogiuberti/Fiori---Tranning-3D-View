...
sap.ui.define([
...
], function(MockServer, exportToGlobal, HashTable, jQuery) {
	'use strict';
	...
	function getApplicationRoot() {
		var sApfLocation = sap.ui.require.toUrl("sap/apf/");
		var karmaBaseIndex = sApfLocation.indexOf("/base");
		var sPath = null;
		if (karmaBaseIndex === 0) { // Karma
			return "base/test/uilib/sap/apf/";
		}
		if (window.location.pathname.indexOf("/resources") !== -1){ // resources precedes test-resources
			sPath = window.location.pathname.replace("/resources", "/test-resources");
		} else {
			var sHref = jQuery(window.location).attr('href');
			sPath = sHref.replace(window.location.protocol + "//" + window.location.host, "");
		}
	...
	/*BEGIN_COMPATIBILITY*/
	exportToGlobal("sap.apf.testhelper.mockServer", staticMethods);
	/*END_COMPATIBILITY*/
