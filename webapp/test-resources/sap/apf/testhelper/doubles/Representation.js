sap.ui.define([
	...
	/**
	 * @alias sap.apf.testhelper.doubles.SelectionStrategy
	 */
	...
	exportToGlobal("sap.apf.testhelper.doubles.SelectionStrategy", SelectionStrategy);

	/**
	 * @alias sap.apf.testhelper.doubles.RequestOptionStrategy
	 */
	var RequestOptionStrategy = {
		nothing : 'nothing',
		top : 'top',
		topAndSkip : 'topSkip',
		skip : 'skip',
	...
	exportToGlobal("sap.apf.testhelper.doubles.RequestOptionStrategy", RequestOptionStrategy);

	/**
	 * @class
	 * @param oApi - required methods interface to APF
	 * @param oParameter - parameters for the representation
	 * @alias sap.apf.testhelper.doubles.Representation
	 */
	function Representation(oApi, oParameter) {
		...
		var oParametersOfSetData = {
			...
			metadata : undefined,
			sSelectionStrategy : oParameter.sSelectionStrategy || SelectionStrategy.nothing,
			sRequestOptionStrategy : oParameter.sRequestOptionStrategy || RequestOptionStrategy.nothing
		};
		...
		this.getSelectionAsArray = function() {
			...
			switch (oParametersOfSetData.sSelectionStrategy) {
				...
				case SelectionStrategy.coCode1000Cust1001_2_1004:
				...
				case SelectionStrategy.nothing:
					return [];
				case SelectionStrategy.all:
					...
					return aIndices;
				case SelectionStrategy.undefinedSelection:
					return undefined;
				case SelectionStrategy.indicesOfSelectedData:
					return this.aIndicesOfSelectedData;
				case SelectionStrategy.allMinus1:
					aIndices = [];
					for(i = 2; i < oParametersOfSetData.dataResponse.length - 7; i++) {
						aIndices.push(i);
					}
					return aIndices;
				case SelectionStrategy.oneCountry:
		...
		this.getRequestOptions = function() {
			switch (oParametersOfSetData.sRequestOptionStrategy) {
				case RequestOptionStrategy.nothing:
					return {};
				case RequestOptionStrategy.top:
					return {
						paging : {
							top : 10
						}
					};
				case RequestOptionStrategy.topAndSkip:
					return {
						paging : {
							top : 20,
							skip : 10
				...
				case RequestOptionStrategy.skip:
					return {
						paging : {
							skip : 10
						}
					};
				case RequestOptionStrategy.inlineCount:
					return {
						paging : {
							inlineCount : true
						}
					};
				case RequestOptionStrategy.all:
					return {
						paging : {
							top : 20,
							skip : 10,
				...
				case RequestOptionStrategy.orderbyPropertyTwo:
					return {
						orderby : 'PropertyTwo'
					};
				case RequestOptionStrategy.multiOptionsPagingAndOrderbyPropertyOne:
					return {
						paging : {
							top : 20,
							skip : 10
				...
				case RequestOptionStrategy.multiOptionsPagingAndOrderbyPropertyTwo:
					return {
						paging : {
							top : 20,
							skip : 10
		...
		if (this.type.toLowerCase().search("initial") > -1) {
			_sFilterMethodType = coreConstants.filterMethodTypes.startFilter;
		} else {
			_sFilterMethodType = coreConstants.filterMethodTypes.selectionAsArray;
		}
		/**
		...
		this.adoptSelection = function(oSource) {
			if (bAdoptionSucceeds) {
				this.emulateSelectionStrategy(SelectionStrategy.indicesOfSelectedData);
		...
		this.getMainContent = function() {
			...
			self.line = new VizFrame({
				...
				// TODO: is a dataset needed? self.dataset is not a proper dataset, just raw data.
	 ...
	 * TODO seems not to be used; should anyhow be replaced by a call to sap.ui.require.toUrl()
	 */
	exportToGlobal("sap.apf.testhelper.doubles.getPathToRepresentationInterface", function() {
	...
	Representation.prototype = new RepresentationInterface();

	// export SelectionStrategy as static member of Representation
	Representation.SelectionStrategy = SelectionStrategy;
	// export RequestOptionStrategy as static member of Representation
	Representation.RequestOptionStrategy = RequestOptionStrategy;

	exportToGlobal("sap.apf.testhelper.doubles.Representation", Representation);
