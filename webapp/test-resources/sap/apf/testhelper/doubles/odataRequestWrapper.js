sap.ui.define([
	...
	var nGuidCounter = 0;
	/**
	 * @alias sap.apf.testhelper.doubles.odataRequestWrapper
	 */
	function odataRequestWrapper(oMessageHandler, oRequest, fnSuccess, fnError, oBatchHandler) {
		var memberOfAnalysisPathUrl = coreConstants.entitySets.analysisPath + '(';
		if (oRequest.requestUri.search("apf.xsodata") > -1) {
			...
			odataRequestWrapper(oMessageHandler, oRequest, fnSuccess, fnError, oBatchHandler);
		}
		function delete_(oRequest) {
			var sId = extractUrlParam();
			var aPathData = JSON.parse(Storage.get("path"));
			...
			Storage.put("path", JSON.stringify(aPathData));
			var oData = {};
			var oResponse = {
				body : "",
	...
	exportToGlobal("sap.apf.testhelper.doubles.odataRequestWrapper", odataRequestWrapper);
