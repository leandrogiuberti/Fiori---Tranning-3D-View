sap.ui.define([
...
], function(MessageObject, messageDefinition, exportToGlobal, Hashtable){

	/**
	 * @description Constructor, simply clones the MessageHandler
	 * @alias sap.apf.testhelper.doubles.MessageHandler
	 */
	function doubleMessageHandler() {
		'use strict';
		...
		this.spyPutMessage = function() {
			...
			this.putMessage = function(oMessageObject) {
				...
				if (this.spyResults.putMessage) {
					if (Array.isArray(this.spyResults.putMessage)) {
	...
	exportToGlobal("sap.apf.testhelper.doubles.MessageHandler", doubleMessageHandler);
