sap.ui.define([
	...
	QUnit.test("function contained in array 1", function(assert) {
		...
		var bContained = helper.isContainedInArray(aContained, aData);
	...
	beforeEach : function() {
			this.helper = helper.createIsolation().initPathPersistenceProxy();
		},
	afterEach : function() {
	...
	beforeEach : function() {
			QUnit.stop();
			helper.doubleCheckAndMessaging();
			this.helper = helper.createIsolation().initLogicalSystemRequestProxy().initCoreUriGenerator();
			this.oAuthTestHelper = new AuthTestHelper(function() {
	...
	afterEach : function() {
			...
			helper.resetDoubleCheckAndMessaging();
		}
	});
	...
	*/
	...
	QUnit.test("Injection of URL Parameters",function(assert) {
		var oUriParams = utils.getUriParameters();
