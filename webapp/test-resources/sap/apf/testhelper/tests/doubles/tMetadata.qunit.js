sap.ui.define([
	...
	function commonSetup_tMData(oContext) {
		var oIfMessageHandler = new MessageHandlerDouble();
		...
		var oInject = {
			...
			instances: {
				messageHandler : oIfMessageHandler
			}
		};
		oContext.oMetadata = new MetadataDouble(oInject, "");
	}
	...
	QUnit.test('Get edm-type for property', function(assert) {
		assert.deepEqual(this.oMetadata.getPropertyMetadata(), {
			dataType: {}
		});
	...
	QUnit.test('Get parameter entity set key properties for entity type', function(assert) {
		assert.deepEqual(this.oMetadata.getParameterEntitySetKeyProperties(), []);
	...
	QUnit.test('Annotate properties of entity type with filter required attribute', function(assert) {
		...
		var oEntityTypeAnnotations = this.oMetadata.getEntityTypeAnnotations('entityTypeWithFilterRequiredAnnotations');
		assert.ok(oEntityTypeAnnotations.requiresFilter !== undefined && oEntityTypeAnnotations.requiresFilter === "true", 'Entity type requires filter');
		assert.equal(oEntityTypeAnnotations.requiredProperties,'myProp1','One property expected as required for the entity type');
	});
