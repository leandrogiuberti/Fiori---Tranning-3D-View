...
sap.ui.define([
	...
	/**
	 * @alias sap.apf.testhelper.baseClass.baseVizFrameChartRepresentation
	 */
	var baseVizFrameChartRepresentation = {
		run: run
	};

	function getParametersWithRequiredFilters() {
		return representationHelper.prototype.selectableRepresentationDataWithDimension();
	}
	...
	function run(Chart, fnModifyParameters) {
		baseUI5ChartRepresentation.run(Chart);

		fnModifyParameters = fnModifyParameters || identity;

		function setupChart(parameters, callback) {
			baseUI5ChartRepresentation.setupChart(parameters, Chart, callback);
		}
		...
		QUnit.module("BaseVizFrameChartRepresentation specification - Chart inheritance", {
			beforeEach: function(assert) {
				...
				createUiApiAsPromise().done(function(api) {
					...
					testEnv.oBase = new BaseVizFrameChartRepresentation(interfaceProxy, requiredParameter);
			...
			beforeEach: function(assert) {
				...
				setupChart(fnModifyParameters(getParametersWithRequiredFilters()), function(setup) {
					...
					testEnv.formatterForMeasureSpy =
						sinon.spy(BaseUI5ChartRepresentation.prototype, "getFormatStringForMeasure");
					testEnv.selectionFormatterSpy =
						sinon.spy(BaseUI5ChartRepresentation.prototype, "attachSelectionAndFormatValue");
					testEnv.setFormatOnChartSpy =
						sinon.spy(BaseVizFrameChartRepresentation.prototype, "setFormatStringOnChart");
		...
		QUnit.test("When calling getMainContent", function(assert) {
			...
			assert.ok(BaseObject.isObjectA(mainContent, "sap.viz.ui5.controls.VizFrame"), "then result is a VizFrame");
	...
	exportToGlobal("sap.apf.testhelper.baseClass.baseVizFrameChartRepresentation", baseVizFrameChartRepresentation);
