...
sap.ui.define([
...
], function(exportToGlobal, representationHelper, createUiApiAsPromise, sampleService, ImportedBaseUI5ChartRepresentation, RepresentationInterfaceProxy) {
	"use strict";

	/**
	 * @alias sap.apf.testhelper.baseClass.baseUI5ChartRepresentation
	 */
	var baseUI5ChartRepresentation = {
			run: run,
	...
	function getSampleDataForCharts() {
		// see testhelper/odata/sampleDataForCharts.json
		return sampleService.get('sampleDataForCharts');
	}
	function getSmallSampleData() {
		...
		return sampleService.get('smallSampleData');
	}
		...
		QUnit.module("BaseUI5ChartRepresentation specification - Chart inheritance", {
			beforeEach: function(assert) {
				...
				createUiApiAsPromise().done(function(api) {
					...
					testEnv.oBase = new ImportedBaseUI5ChartRepresentation(interfaceProxy, requiredParameter);
	...
	exportToGlobal("sap.apf.testhelper.baseClass.baseUI5ChartRepresentation", baseUI5ChartRepresentation);
