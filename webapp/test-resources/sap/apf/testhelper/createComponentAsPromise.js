...
sap.ui.define([
	...
	'require'
], function(UiInstance, ComponentDouble, ajaxWithAdjustedResourcePathsAndApplicationConfigurationPatch, exportToGlobal, require) {
	'use strict';
	...
	function determinePathOfApplicationConfiguration(configuration) {
		...
		if (configuration.noLoadingOfApplicationConfig) {
			...
			return require.toUrl("./config/applicationConfiguration.json");
		} 
		return configuration.path;
	}

	/*BEGIN_COMPATIBILITY*/
	exportToGlobal("sap.apf.testhelper.createComponentAsPromise", createComponentAsPromise);

	// TODO this export does not match the package and seems not to be used anywhere. Remove? 
	exportToGlobal("sap.apf.testhelper.doubles.createComponentAsPromise", createComponentAsPromise);
	/*END_COMPATIBILITY*/
