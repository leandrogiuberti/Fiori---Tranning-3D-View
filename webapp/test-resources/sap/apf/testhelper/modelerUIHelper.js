...
sap.ui.define([
...
], function(mockServerWrapper, ModelerCoreInstance, CoreInstance, MetadataFactory,
			coreConstants, modelerConstants, exportToGlobal, startParameter, apfUtils,
			modelerHelper, testHelper, messageHandler, authTestHelper, sampleConfiguration,
			ColumnChart, BarChart, PieChart, BubbleChart, Table, XMLView){
	"use strict";
	...
	var module = {
		getInstance : function() {
			...
			modelerHelper.createApplicationHandler(this, async function(appHandler) {
				//register the UI callback for message handling after having modeler core api
				var oMessageHandlerView = await XMLView.create({
	...
	/*BEGIN_COMPATIBILITY*/
	exportToGlobal("sap.apf.testhelper.modelerUIHelper", module);
	/*END_COMPATIBILITY*/
