...
sap.ui.define([
	...
	function commonSetup(testEnv, configurationOfLogicalSystem) {
		...
		testEnv.oCoreApi = new CoreApiDouble({
			instances : {
		...
		testEnv.oCoreApi.getXsrfToken = function() {
			return utils.createPromise('dummyToken');
		};
		...
		testEnv.oCoreApi.getCumulativeFilter = function() {
			...
			var filter = new CoreFilter(testEnv.oMessageHandler, "SAPClient", "eq", "999");
			filter = filter.addAnd(new CoreFilter(testEnv.oMessageHandler, "PropertyOfNoInterest", "eq", "333"));
			deferred.resolve(filter);
			return deferred.promise();
		};
		testEnv.persistence = new Persistence.constructor({
			instances: {
				messageHandler : testEnv.oMessageHandler,
	...
	QUnit.test('createPath()', function(assert) {
		...
		function callbackCreate() {
			...
			var expectedSerializableApfState = deepExtend({}, this.serializableApfState);
	...
	QUnit.test('modifyPath()', function(assert) {
		...
		function callbackModify() {
			...
			expectedSerializableApfState = deepExtend({}, this.serializableApfState);
