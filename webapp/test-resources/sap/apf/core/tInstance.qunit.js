sap.ui.define([
	...
	QUnit.test("WHEN isUsingCloudFoundryProxy is injected", function(assert){
		...
		var spyAnalysisPathProxy = sinon.spy(AnalysisPathProxy);
		...
		new Instance.constructor({
				...
				constructors : {
					AnalysisPathProxy : spyAnalysisPathProxy,
					ResourcePathHandler : ResourcePathHandlerStub
				},
				functions: {
					getComponentName : function() { return "Comp1"; },
	...
	QUnit.test("WHEN manifests are injected", function(assert) {
		var spyResourcePathHandler = sinon.spy(ResourcePathHandler);
		var spyConfigurationFactory = sinon.spy(ConfigurationFactory);
		var oMessageHandler = new MessageHandler();
		...
		this.oInstance = new Instance.constructor({
			manifests : manifests,
			constructors: {
				ConfigurationFactory: spyConfigurationFactory,
				ResourcePathHandler: spyResourcePathHandler
			},
			instances : {
				startParameter : startParameter,
				messageHandler : oMessageHandler
	...
	QUnit.module("SmartFilterBar default filter values", {
		beforeEach : function() {
			...
			this.coreInstance = new Instance.constructor({
				...
				constructors: {
					SFBControlConfiguration: function(controlConfig) {
						return controlConfig;
					},
					SFBSelectOption: function(selectOption) {
						return selectOption;
					},
				},
				functions : {
