...
sap.ui.define([
	...
	function commonSetupOdataProxy(assert, context, statusCode, statusText) {
		...
		context.odataProxy = new OdataProxy({
	...
	QUnit.test("Read all Configurations of an Application", function(assert) {
		assert.expect(5);
		var oFilter = new CoreFilter(this.messageHandler, "Application", "eq", "applicationGuid");
	...
	QUnit.module("M: Error handling of OdataProxy", {
		beforeEach : function(assert) {
			var messageHandler = new MessageHandlerDouble().spyPutMessage();
			var coreApi = {
				check : function(oBooleExpr) {
					messageHandler.check(oBooleExpr);
				},
			...
			this.odataProxy = new OdataProxy({
				serviceRoot : "/serviceRoot",
				entityTypes : {
	...
	QUnit.test("M: Read 2 collections with filter in batch", function(assert) {
		...
		var batchRequests = [];
		var oFilterForApplication = new CoreFilter(this.messageHandler, "Application", "eq", "applictionId");
		var oFilterForTexts = new CoreFilter(this.messageHandler, "Language", "eq", "zy");
	...
	QUnit.test("M: Read collections asynchronously", function(assert) {
		...
		var batchRequest = [{
			entitySetName : "configuration",
			filter : new CoreFilter(this.messageHandler, "Application", "eq", "applictionId")
		}];
