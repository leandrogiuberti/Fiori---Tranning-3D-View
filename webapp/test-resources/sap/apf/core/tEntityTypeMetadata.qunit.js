sap.ui.define([
...
], function(Hashtable, MessageHandlerDouble, MetadataDouble, EntityTypeMetadata) {
	"use strict";
	...
	QUnit.test('wrong argument for entitytype', function(assert) {
		assert.throws(function() {
			new EntityTypeMetadata(this.oIfMessageHandler, 1);
		}, "Error successfully thrown due to wrong creation");
	});
	QUnit.test('wrong argument for metadata', function(assert) {
		assert.throws(function() {
			new EntityTypeMetadata(this.oIfMessageHandler, "entitytype", {});
		}, "Error successfully thrown due to wrong creation");
	});
	QUnit.module('Get property metadata', {
		beforeEach : function(assert) {
			this.oIfMessageHandler = new MessageHandlerDouble();
			this.oIfMessageHandler.raiseOnCheck();
			this.oMetadata = new MetadataDouble({
				instances : {
			...
			this.oEntityTypeOneMetadata = new EntityTypeMetadata(this.oIfMessageHandler, 'entityTypeOne', this.oMetadata);
			this.oEntityTypeTwoMetadata = new EntityTypeMetadata(this.oIfMessageHandler, 'entityTypeTwo', this.oMetadata);
