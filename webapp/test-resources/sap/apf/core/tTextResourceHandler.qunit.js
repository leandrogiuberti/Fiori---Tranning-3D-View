...
sap.ui.define([
	...
	QUnit.module('TRH -- GetText', {
		beforeEach : function(assert) {
			...
			this.createStubs4TextResourceHandler = function(key, value) {
				...
				that.stubResourceBundleCreate = sinon.stub(ResourceBundle, "create", function(settings) {
				...
				that.stubUI5collectOriginInfo = sinon.stub(Supportability, "collectOriginInfo", function() {
					return undefined;
				});
			...
			this.unstubAll = function() {
				if (that.xStubbed) {
					...
					that.stubUI5collectOriginInfo.restore();
					that.stubResourceBundleCreate.restore();
				}
			};
			this.initTextResourceHandlerAsPromise = function() {
				var deferred = jQuery.Deferred();
				var applicationMessageTextBundle = helper.adjustUrl("/apf-test/test-resources/sap/apf/resources/i18n/applicationMessages.properties");
				var apfUiTextBundle = helper.adjustUrl("/apf-test/test-resources/sap/apf/resources/i18n/apfUi.properties");
				var applicationUiTextBundle = helper.adjustUrl("/apf-test/test-resources/sap/apf/resources/i18n/test_texts.properties");			
	...
	QUnit.test('GIVEN stub WHEN getTextNotHtmlEncoded input string invalid THEN return error 3001 ""', function(assert) {
		...
		this.initTextResourceHandlerAsPromise().done(function(){
			...
			assert.ok(this.stubResourceBundleCreate.called, "tried to retrieve key");
	...
	QUnit.test('GIVEN stub WHEN getTextHtmlEncoded called THEN text retrieved from jQuery.sap.resources AND jQuery.sap.encodeHTML() called', function(assert) {
		...
		this.initTextResourceHandlerAsPromise().done(function(){
			...
			assert.ok(this.stubResourceBundleCreate.called);
			done();
		}.bind(this));
	});
	...
	QUnit.test("GIVEN label & stub WHEN call getTextHtmlEncoded THEN HTML encoded text", function(assert) {
		...
		this.initTextResourceHandlerAsPromise().done(function(){
			...
			assert.equal(encodeXML(text), sEncodedText, "result encoded");
			done();
		}.bind(this));
	});
	...
	QUnit.test("Get Text for the initial text key", function(assert) {
		...
		this.oTextHandler.loadTextElements(textElements);
		assert.equal(this.oTextHandler.getTextNotHtmlEncoded(coreConstants.textKeyForInitialText), "", "Empty Text expected");
	});
	...
	QUnit.module('Resource model', {
		beforeEach : function(assert) {
			...
			this.stubResourceBundleCreate = sinon.stub(ResourceBundle, "create", function(settings) {
				return new Promise(function(resolve){
		...
		afterEach : function() {
			this.stubResourceBundleCreate.restore();
		}
	});
	QUnit.test("sap/base/i18n/ResourceBundle.prototype.getText shall be called with undefined parameters", function(assert){
		assert.expect(3);
	...
	QUnit.test("WHEN texts are registered", function(assert){
		// Note: the double apostrophe acts as an indicator whether formatMessage is called or not
		// (formatMessage reduces the double apostrophs to a single one)
		this.oTextHandler.registerTextWithKey("registeredText", "I''m registered");

		const text = this.oTextHandler.getTextNotHtmlEncoded("registeredText", []);
		assert.strictEqual(text, "I''m registered", "THEN formatMessage is not called when no parameters are given");

		const textWithParameters = this.oTextHandler.getTextNotHtmlEncoded("registeredText", [1]);
		assert.strictEqual(textWithParameters, "I'm registered", "THEN formatMessage is called when parameters are given");
	});
});
