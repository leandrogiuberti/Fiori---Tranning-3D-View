...
sap.ui.define([
	...
	"sap/apf/testhelper/doubles/configurationFactory",
	...
	function commonSetupBinding(oContext, afterGetFilter) {
		...
		oContext.oSampleConfiguration = sampleConfig.getSampleConfiguration();
	...
	QUnit.module('Binding template1 & representation info', {
		beforeEach : function(assert) {
			...
			sinon.stub(this.oCoreApi, "createRepresentation", function() {
		...
		afterEach : function(assert) {
			this.oCoreApi.createRepresentation.restore();
		}
	});
	...
	QUnit.test('alternateRepresentationType correctly passed to constructor of Representation', function(assert) {
		...
		this.oConfigurationFactory.loadConfig(this.oSampleConfiguration);
		sinon.stub(this.oCoreApi, "createRepresentation", function(oApi, oParameter) {
	...
	QUnit.test('GetSelectedRepresentation on empty array', function(assert) {
		...
		assert.strictEqual(oSelectedRepresentation, undefined, "undefined returned");
	});
	...
	QUnit.module("Representation instantiation", {
		beforeEach : function(assert) {
			this.oMessageHandler = new MessageHandlerDouble().doubleCheckAndMessaging();
			this.RepresentationStub = function(oApi, oParameter) {
				return sinon.stub(new RepresentationInterface(oApi, oParameter));
			};
		},
		afterEach : function(assert) {
		},
		setupBinding : function(RepresentationStub) {
			RepresentationStub = RepresentationStub || this.RepresentationStub;
			this.oSampleConfiguration = sampleConfig.getSampleConfiguration();
			...
			this.oConfigurationFactory = new ConfigurationFactoryDouble({
				instances : {
					messageHandler : this.oMessageHandler,
					coreApi : this.oCoreApi
				}
			...
			this.oConfigurationFactory.loadConfig(this.oSampleConfiguration);
			sinon.stub(this.oCoreApi, "createRepresentation", function(oApi, oParameter) {
				return new RepresentationStub(oApi, oParameter);
			});
			this.oBinding = this.oConfigurationFactory.createBinding("bindingTemplate1");
		}
	});
	QUnit.test('WHEN a representation is created THEN the requried filters are given to the constructor', function(assert) {
		var oOriginalRequiredFilters = sampleConfig.getSampleConfiguration().bindings[0].requiredFilters;
		this.setupBinding(function(oApi, oParameter) {
	...
	QUnit.test('WHEN a representation is created THEN the parameters are given to the constructor', function(assert) {
		var property;
		var oOriginalParameters = sampleConfig.getSampleConfiguration().bindings[0].representations[0].parameter;
		this.setupBinding(function(oApi, oParameter) {
	...
	QUnit.module('Representation change', {
		beforeEach : function(assert) {
			...
			this.oConfigurationFactory.loadConfig(this.oSampleConfiguration);
			this.oBinding = this.oConfigurationFactory.createBinding("bindingTemplate1");
			this.aDataResponse = SampleServiceData.getSampleServiceData("EntityType1").data;
	...
	QUnit.module("Convert sort to orderby", {
		beforeEach : function(assert) {
			...
			var that = this;
			this.oConfigurationFactory.loadConfig(this.oSampleConfiguration);
			sinon.stub(this.oCoreApi, "createRepresentation", function(oApfApi, oParameter) {
	...
	function setupForExitInjection(context, exitFunction) {
		...
		context.oConfigurationFactory.loadConfig(sampleBinding());
			sinon.stub(context.oCoreApi, "createRepresentation", function(oApfApi, oParameter) {
	...
	QUnit.test("WHEN not injected afterGetFilter", function(assert) {
		...
		assert.equal(filterResult instanceof CoreFilter, true, "THEN getFilter unchanged");
	});
	QUnit.test("WHEN afterGetFilter is injected", function(assert) {
		...
		assert.strictEqual(filterResult.filter instanceof CoreFilter, true, "THEN getFilter is modified by running afterGetFilter");
	...
	QUnit.test("WHEN afterGetFilter is injected", function(assert) {
		...
		setupForExitInjection(this, function(oFilter, selectedRepresentation, oCoreApi, oBindingConfig, oContextInfo) {
			...
			assert.ok(selectedRepresentation instanceof RepresentationInterface, "THEN selected representation is passed to exit");
			assert.ok(oCoreApi instanceof CoreApiDouble, "THEN oCoreApi is passed to exit");
	...
	QUnit.module('Binding template1 & setData to representation', {
		beforeEach : function(assert) {
			...
			commonSetupBinding(this);
			this.oConfigurationFactory.loadConfig(this.oSampleConfiguration);
			sinon.stub(this.oCoreApi, "createRepresentation", function(api, configuration) {
	...
	QUnit.module('Required Filter Options', {
		beforeEach : function(assert) {
			...
			commonSetupBinding(this);
			this.oConfigurationFactory.loadConfig(this.oSampleConfiguration);
			sinon.stub(this.oCoreApi, "createRepresentation", function(oApi, oParameters) {
