...
sap.ui.define([
	...
	function commonSetup(oContext, inject, useAsyncPromises) {
		...
		oContext.Filter = CoreFilter;
		var odataRequestWrapper = (inject && inject.functions && inject.functions.odataRequestWrapper) || importedOdataRequestWrapper;
		defineFilterOperators();
		var oMessageHandler = (inject && inject.instances && inject.instances.messageHandler) || new MessageHandlerDouble().doubleCheckAndMessaging();
		...
		oContext.oMetadataDouble = new MetadataDouble({
			instances: {
				messageHandler : oMessageHandler
			},
			constructors : {
	...
	function setupCoreAndMessageHandling(oContext, bSaveConstructors, bFilterReductionIsActive, inject) {
		var oMessageHandler = new (inject && inject.constructors && inject.constructors.MessageHandler || MessageHandlerDouble)().doubleCheckAndMessaging().supportLoadConfigWithoutAction();
		oContext.oMessageHandler = oMessageHandler;
		if (!oContext.oCoreApi) {
			oContext.oCoreApi = new CoreApiDouble({
				instances : {
		...
		oContext.oInject.instances.fileExists = new FileExists();
		oContext.oInject.functions.checkForTimeout = checkForTimeout;
		oContext.oInject.functions.isUsingCloudFoundryProxy = function() { return false; };
		var oTextResourceHandler = new TextResourceHandler(oContext.oInject);
		...
		oContext.oRPH = new ResourcePathHandler(oContext.oInject);
		var sConfigPath = sap.ui.require.toUrl('sap/apf/testhelper/config/' + helper.determineApplicationConfigName());
	...
	QUnit.module('Missing service definition during create request', {
		beforeEach : function(assert) {
			...
			var inject = {
					functions : {
						ajax : ajaxWithAdjustedResourcePathsAndApplicationConfigurationPatch
					}
			};
	...
	QUnit.module('WHEN sendInBatch returns odata error', {
		beforeEach : function() {
			...
			messageHandler.loadConfig(messageDefinition, true);
	...
	QUnit.test("WHEN error message is returned", function(assert){
		...
		var oFilter = new CoreFilter(this.oMessageHandler, 'prop1', this.EQ, 'val1');
		oFilter.addAnd('prop2', this.EQ, 'val2');
		oFilter.addAnd('prop3', this.EQ, 'val3');
		oFilter.addAnd('prop4', this.EQ, 'val4');
	...
	QUnit.module('Given sap.apf.activateFilterReduction is deactivated', {
		beforeEach : function(assert) {
			...
			this.filter = new CoreFilter(this.oMessageHandler, oFilter);
	...
	QUnit.module('Given activateFilterReduction AND(OR(1,2,1),OR(1,2,42))', {
		beforeEach : function(assert) {
			...
			var left = new CoreFilter(this.oMessageHandler).addOr('A', this.EQ, '1').addOr('A', this.EQ, '2').addOr('A', this.EQ, '1');
			var right = new this.Filter(this.oMessageHandler).addOr('A', this.EQ, '1').addOr('A', this.EQ, '2').addOr('A', this.EQ, '42');
			this.filter = new CoreFilter(this.oMessageHandler).addAnd(left).addAnd(right);
	...
	QUnit.module('Passing metadata to datajs', {
		beforeEach : function(assert) {
			...
			this.oCoreApi = new CoreInstance.constructor(oInject);
		},
	...
	QUnit.test('datajs parses date', function (assert) {
		...
		var request = new Request(oInject, {
		...
		var oFilter = new CoreFilter(this.oMessageHandler);
	...
	QUnit.module('Selection validation request', {
		beforeEach : function(assert) {
			...
			this.filter = new CoreFilter(this.oMessageHandler, 'prop1', this.EQ, 'val1');
			this.request = new Request(this.oInject, {
				id : "requestForTesting",
				service : 'dummy',
				entityType : 'entityTypeFilterable',
	...
	QUnit.test('sendGetInBatch', function(assert) {
		var selectionFilter =  new CoreFilter(this.oMessageHandler, 'prop2', this.EQ, 'val2');
		...
		assert.equal(typeof this.buildUriArguments[1][8], "function", 'Format function in second get request');
	...
	QUnit.test('Filter reduction', function(assert) {
		var selectionFilter =  new CoreFilter(this.oMessageHandler, 'prop1', this.EQ, 'val1');
		var selectionValidationRequest = {
	...
	QUnit.test("WHEN first odata call fails", function(assert){
		...
		function callSendGetInBatchAgain() {
			...
			var oFilter = new CoreFilter(that.oMessageHandler, 'prop1', that.EQ, 'val1');
			that.request.sendGetInBatch(oFilter, function() {
