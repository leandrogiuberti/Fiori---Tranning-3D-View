...
sap.ui.define([
	...
	MessageHandlerStub.prototype = new IfMessageHandler();
	MessageHandlerStub.prototype.constructor = MessageHandlerStub;
	...
	function commonCoreSetup(oContext, inject) {
		oContext.oMessageHandler = new MessageHandlerStub();
	...
	function commonSetup(oContext, inject) {
		...
		oContext.oInject = {
			instances: {
				...
				fileExists : new ((inject && inject.constructors && inject.constructors.FileExists) || FileExists)()
			},
			functions : {
				isUsingCloudFoundryProxy : function() {
					return false;
				},
				checkForTimeout : (inject && inject.functions && inject.functions.checkForTimeout) || checkForTimeout,
				initTextResourceHandlerAsPromise : (inject && inject.functions &&  inject.functions.initTextResourceHandlerAsPromise) || utils.createPromise
			},
			corePromise : oContext.corePromise
		};
		oContext.resPathHandler = new ResourcePathHandler(oContext.oInject);
		//------------
	...
	QUnit.test("GIVEN jQuery.ajax stubbed such that it returns a json WHEN loadConfigFromFilePath() is called with file name THEN parser returns correct properties", function(assert) {
		...
		var sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.applicationMessageDefinitionLocation);
		var sExpectedPath = "ZZZ2.json";
		assert.ok(sPath.indexOf(sExpectedPath) > -1, "expected path of applicationMessageDefinitionLocation ");
		sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.applicationMessageTextBundle);
		sExpectedPath = "zzz.properties";
		assert.ok(sPath.indexOf(sExpectedPath) > -1, "expected path of applicationMessageTextBundle ");
		sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.apfUiTextBundle);
		sExpectedPath = "qqq.properties";
		assert.ok(sPath.indexOf(sExpectedPath) > -1, "expected path of apfUiTextBundle ");
		sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.applicationUiTextBundle);
		sExpectedPath = "uuu.properties";
		assert.ok(sPath.indexOf(sExpectedPath) > -1, "expected path of applicationUiTextBundle ");
		sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.analyticalConfigurationLocation);
	...
	QUnit.test("GIVEN jQuery.ajax stubbed such that it returns a json WHEN loadConfigFromFilePath() is called with file name THEN parser returns persistence config", function(assert) {
		...
		var oPersistenceConfigurationExpected = deepExtend({}, this.applConfigJSON.applicationConfiguration.persistence);
		oPersistenceConfigurationExpected.path.entitySet = coreConstants.entitySets.analysisPath;
	...
	QUnit.test("GIVEN ajax stub and applConfig.json WHEN appl config has missing persistence service THEN putMessage is called with missing analytical config", function(assert) {
		...
		var sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.analyticalConfigurationLocation);
	...
	QUnit.module("Error handling during loading of application message configuration", {
		beforeEach : function() {
				...
				this.messageHandler.loadConfig(messageDefinition);
				...
				this.resPathHandler = new ResourcePathHandler(inject);
	...
	QUnit.test("WHEN loadApplicationConfig is called", function(assert) {
		var done = assert.async();
		this.resPathHandler.loadConfigFromFilePath(sap.ui.require.toUrl("sap/apf/testhelper/config/minimalApplicationConfiguration.json"));
		this.corePromise.done(function(){
			var sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.applicationMessageTextBundle);
	...
	QUnit.test("WHEN loadAnalyticalConfigurationAndRessources is called", function(assert) {
		...
		this.corePromise.done(function(){
			...
			var sPath = this.resPathHandler.getResourceLocation(coreConstants.resourceLocation.applicationUiTextBundle);
	...
	QUnit.test("WHEN manifest with missing path persistence is loaded AND isUsingCloudFoundryProxy returns true", function(assert) {
		...
		this.loadManifestAndResourcePathHandler(false, false, true);
		assert.ok(isEmptyObject(this.oMessageHandler.spyResults), "THEN no Message");
