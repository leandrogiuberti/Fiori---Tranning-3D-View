...
sap.ui.define([
...
], function(jQuery, coreConstants, LayeredRepositoryProxy, MessageHandler, CoreFilter, Hashtable, parseTextPropertyFile, StartParameter, utils, sampleConfig, MessageHandlerDouble, IfCoreApi) {
	'use strict';
	...
	function createProxy(messageHandler, Connector) {
		...
		return new LayeredRepositoryProxy({}, inject);
	}
			...
			var Connector = function() {
				this.upsert = function(namespace, filename, type, layer, content, fileType) {
					...
					if (that.numberOfCallsForUpsert === 0) {
						...
						assert.ok(utils.isValidGuid(texts[0].TextElement), "Text has a text element");
	...
	QUnit.test("Create Application, create Texts and update application", function(assert) {
		...
		function callbackFromCreateText1(text, metadata, messageObject) {
			assert.ok(utils.isValidPseudoGuid(text.TextElement), "The text element guid was returned and is valid");
		...
		function callbackFromCreateApplication(applicationData, metadata, messageObject) {
			var text1 = {
				...
				Language : coreConstants.developmentLanguage,
			...
			assert.ok(utils.isValidPseudoGuid(applicationData.Application), "The application guid was returned and is valid");
	...
	QUnit.test("Update analytical application", function(assert) {
		this.Application = utils.createPseudoGuid(32);
		assert.expect(4);
		this.config = {
	...
	QUnit.module("Read collection of applications", {
		beforeEach : function(assert) {
			var that = this;
			var appGuid1 = utils.createPseudoGuid();
			var appGuid2 = utils.createPseudoGuid();
			that.applicationListResponse = [ getApplicationTemplate(appGuid1, "k1"), getApplicationTemplate(appGuid2, "k2")];
			that.application = utils.createPseudoGuid();
			that.configuration = utils.createPseudoGuid();
			var messageHandler = new MessageHandlerDouble().doubleCheckAndMessaging();
	...
	QUnit.test("Read Collection in Batch", function(assert) {
		...
		var filterApplication = new CoreFilter(this.messageHandler, 'Application', 'eq', this.application);
		var filterLanguage = new CoreFilter(this.messageHandler, 'Language', 'eq', coreConstants.developmentLanguage);
	...
	QUnit.module("Create configuration with and without ATO", {
		beforeEach : function(assert) {
			...
			var Connector = function() {
				...
				var metadataRead = new Hashtable(messageHandler);
				...
				this.upsert = function(namespace, filename, type, layer, content, fileType, changeList) {
					...
					if (callsToUpsert === 1) {
						...
						assert.ok(utils.isValidGuid(content.configHeader.Application), "Configuration has application in header");
						...
						assert.ok(utils.isValidGuid(content.configHeader.AnalyticalConfiguration), "Configuration has guid in header");
	...
	QUnit.test("Create Configuration without ATO", function(assert) {
		...
		function callbackFromCreateConfiguration(configurationData, metadata, messageObject) {
			assert.ok(utils.isValidPseudoGuid(configurationData.AnalyticalConfiguration), "THEN the Analytical Configuration Guid is returned");
		...
		var analyticalConfiguration = sampleConfig.getSampleConfiguration();
		var config = {
			AnalyticalConfigurationName : "configForTesting",
			Application : utils.createPseudoGuid(32),
	...
	QUnit.test("Server Error when creating Configuration without ATO", function(assert) {
		var applicationId = utils.createPseudoGuid();
		function callbackFromCreateConfiguration(configurationData, metadata, messageObject) {
	...
	QUnit.test("Server Error in lrep meta data access when creating Configuration without ATO", function(assert) {
		var applicationId = utils.createPseudoGuid(32);
		function callbackFromCreateConfiguration(configurationData, metadata, messageObject) {
	...
	QUnit.test("Create Configuration With Guid (imported configuration)", function(assert) {
		...
		var config = {
			AnalyticalConfigurationName : "configForTesting",
			Application : utils.createPseudoGuid(32),
			AnalyticalConfiguration : utils.createPseudoGuid(32),
			SerializedAnalyticalConfiguration : JSON.stringify(analyticalConfiguration)
		};
		function callbackFromCreateConfiguration(configurationData, metadata, messageObject) {
	...
	QUnit.module("Update and delete configuration", {
		beforeEach : function(assert) {
			...
			this.Application = utils.createPseudoGuid();
			this.AnalyticalConfiguration = utils.createPseudoGuid();
			var Connector = function() {
				var callsToUpsert = 0;
	...
	QUnit.test("Update configuration without ATO", function(assert) {
		...
		var serializedConfig = JSON.stringify(sampleConfig.getSampleConfiguration());
	...
	QUnit.test("Applications with configurations exist", function(assert) {
		...
		var appGuid1 = utils.createPseudoGuid();
		var appGuid2 = utils.createPseudoGuid();
		var configuration1OfApp1 = utils.createPseudoGuid();
		var configuration2OfApp1 = utils.createPseudoGuid();
		var configuration1OfApp2 = utils.createPseudoGuid();
	...
	QUnit.module("Delete application and configuration", {
		beforeEach : function(assert) {
			...
			that.Application = utils.createPseudoGuid(32);
			that.AnalyticalConfiguration = utils.createPseudoGuid(32);
	...
	QUnit.test("Updating and reading texts", function(assert) {
		...
		var filterApplication = new CoreFilter(this.messageHandler, 'Application', 'eq', that.config.Application);
		var filterLanguage = new CoreFilter(this.messageHandler, 'Language', 'eq', coreConstants.developmentLanguage);
		var textData = {
		...
		function callbackFromCreateText(textData) {
			if (textData && utils.isValidPseudoGuid(textData.TextElement)) {
		...
		beforeEach : function(assert) {
			...
			var Connector = function() {
				...
				this.send = function(url, method, data, options) {
					...
					var serializedConfiguration = sampleConfig.getSampleConfiguration();
					if (options.complete) {
				...
				this.getStaticResource = function(sNamespace, sName, sType, bIsRuntime) {
					...
					var serializedConfiguration = JSON.stringify(sampleConfig.getSampleConfiguration());
	...
	QUnit.test("With VENDOR layer option and without select list", function(assert) {
		...
		function callbackFromReadEntity(data, metadata, messageObject) {
			assert.deepEqual(data.SerializedAnalyticalConfiguration, JSON.stringify(sampleConfig.getSampleConfiguration()));
			assert.equal(messageObject, undefined, "There weren't any errors during readEntity");
		...
		beforeEach : function(assert) {
			...
			var Connector = function() {
				...
				this.upsert = function(sNamespace, sName, sType, layer, content, fileType) {
					...
					var parseResult = parseTextPropertyFile(content, { instances : { messageHandler : messageHandler }});
	...
	QUnit.test("Delete Texts in batch (textpool cleanup)", function(assert) {
		...
		batchRequestsNone.push({
			...
			inputParameters : [ {
				...
				name : 'Language',
				value : coreConstants.developmentLanguage
			} ]
		});
		batchRequestsNone.push({
			method : "DELETE",
