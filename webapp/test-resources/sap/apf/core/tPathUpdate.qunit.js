sap.ui.define([
	...
	"sap/apf/utils/utils"
], function(coreConstants, Path, CoreFilter, Filter, CoreApiDouble, MessageHandlerDouble) {
	'use strict';
	...
	function commonSetup(testEnv) {
		testEnv.oMessageHandler = new MessageHandlerDouble().doubleCheckAndMessaging();
		testEnv.coreApi = new CoreApiDouble({
			instances : {
				messageHandler : testEnv.oMessageHandler
			}
		}).doubleCumulativeFilter();
		testEnv.coreApi.storeApfState = function() {};
		testEnv.path = new Path.constructor({
			instances : {
				messageHandler : testEnv.oMessageHandler,
	...
	QUnit.module('Step determine Filter and callback', {
		beforeEach : function(assert) {
			commonSetup(this);
			this.coreApi.doubleCumulativeFilter().setCumulativeFilter(new CoreFilter(this.oMessageHandler, "OldCumulativeFilter", "eq", "1"));
		...
		StepDouble: function(context, id, assert){
			...
			this.determineFilter = function(oCumulatedFilter, callbackToPath) {
				callbackToPath(this.getFilter(), new CoreFilter(context.oMessageHandler, "NewCumulativeFilter", "eq", "1"));
			};
		},
		...
		StepDouble: function(context, id, assert){
			...
			this.determineFilter = function(oCumulatedFilter, callbackToPath) {
				setTimeout(function(){
					...
					callbackToPath(new CoreFilter(context.oMessageHandler));
				},1);
			};
		}
	});
	...
	QUnit.module('Interleave semantic path update', {
		beforeEach : function(assert) {
			...
			this.path = new Path.constructor({
				instances : {
					messageHandler : this.oMessageHandler,
	...
	QUnit.module('Filter merge', {
		...
		preparePathWithStartFilter : function() {
			var oFilter = new Filter(this.oMessageHandler);
			var oExpression;
			var eq = coreConstants.FilterOperators.EQ;
			oExpression = {
				name : "propStart",
			...
			this.oStep1.filterFromSelection = new CoreFilter(this.oMessageHandler, 'prop1', eq, 'val1');
			this.oStep2.filterFromSelection = new CoreFilter(this.oMessageHandler, 'prop2', eq, 'val2');
			this.path.addStep(this.oStep1, function() {
			});
			this.path.addStep(this.oStep2, function() {
			});
	...
	QUnit.test('Three steps last step gets correct filter after update', function(assert) {
		...
		this.oStep1.filterFromSelection = new CoreFilter(this.oMessageHandler, 'prop1', eq, 'val1');
		this.oStep2.filterFromSelection = new CoreFilter(this.oMessageHandler, 'prop2', eq, 'val2');
		this.oStep3.filterFromSelection = new CoreFilter(this.oMessageHandler, 'prop3', eq, 'val3');
		this.coreApi.setCumulativeFilter(new CoreFilter(this.oMessageHandler));
		this.path.addStep(this.oStep1, function() {
		});
		this.path.addStep(this.oStep2, function() {
		});
	...
	QUnit.test('Start filter and first step get correct filter after update', function(assert) {
		...
		var filter0 = new CoreFilter(this.oMessageHandler, 'propStart', eq, 'valStart');
		var filter1 = new CoreFilter(this.oMessageHandler, 'prop1', eq, 'val1');
		var filter2 = new CoreFilter(this.oMessageHandler, 'prop2', eq, 'val2');
	...
	QUnit.test('Empty path', function(assert) {
		var expectedFilter = new CoreFilter(this.oMessageHandler);
		this.path.getCumulativeFilterUpToActiveStep().done(function(cumulativeFilterUpToActiveStep) {
