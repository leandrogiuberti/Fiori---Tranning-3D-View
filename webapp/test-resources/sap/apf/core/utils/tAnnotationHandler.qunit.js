sap.ui.define([
	...
	function createManifests() {
		...
		var baseManifest = {};
		return {
			localRoot: sap.ui.require.toUrl(manifest["sap.app"].id.replace(/\./g, "/")),
			manifest : manifest,
	...
	QUnit.module("Annotation files are defined in manifest",{
		beforeEach : function () {
			...
			this.localRoot = manifests.localRoot;
			this.inject = {
				functions : {
	...
	QUnit.test("Get absolute and local annotation as relative uri", function(assert){
		...
		assert.deepEqual(uris, ["/sap/datasource/with/local/annotation/Annotation.xml", this.localRoot + "/local/annotation.xml"], "THEN absolute and local annotation is found in manifest AND relative url is transformed in absolute URL");
	});
	...
	QUnit.test("Get local annotation WHEN no uri is defined", function(assert){
		...
		assert.deepEqual(uris, [this.localRoot + "/only/local/annotation.xml"], "THEN local annotation is found in manifest");
	});
