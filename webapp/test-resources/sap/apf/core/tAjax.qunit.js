...
sap.ui.define([
	...
	function setupMessageHandler(testEnv) {
		testEnv.messageHandler = new MessageHandler();
		testEnv.messageHandler.loadConfig(messageDefinition, true);
	...
	QUnit.test('GIVEN an AJAX stub simulating a NON existing resource WHEN send simple request THEN error function are called', function(assert) {
		var spyAJAX = sinon.stub(jQuery, "ajax").callsFake(function(object) {
			var deferred = jQuery.Deferred();
		...
		ajax(this.ajaxInjectAndSettings); //<<< unit under test
