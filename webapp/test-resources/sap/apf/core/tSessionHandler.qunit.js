sap.ui.define([
...
], function(SessionHandler, uriGenerator, utils, MessageHandlerDouble, SessionHandlerStubbedAjax, extend, jQuery) {
	'use strict';
	function commonSetup(testEnv) {
		testEnv.inject = testEnv.inject || {}; 
		testEnv.inject.instances = testEnv.inject.instances || {};
		testEnv.inject.instances.messageHandler = new MessageHandlerDouble().doubleCheckAndMessaging().spyPutMessage();
		testEnv.sessionHandler = new SessionHandlerStubbedAjax(testEnv.inject);
			...
			this.inject = {
				functions : {
					...
					deserializeApfState : function(serializedState) {
						...
						this.spyDeserializeArgumente = extend({}, serializedState);
	...
	QUnit.test('Serialization function is only called if injected', function(assert) {
		var sessionHandler = new SessionHandler({
			instances : {
				messageHandler : new MessageHandlerDouble().doubleCheckAndMessaging()	
			},
			functions : {}
		});
		assert.expect(0);
	...
	QUnit.module("GET xsrf token WHEN sap-system is set", {
		beforeEach : function(assert) {
			...
			var inject = {
					instances : {				
						messageHandler : new MessageHandlerDouble().doubleCheckAndMessaging().spyPutMessage(),
						coreApi : {
							getUriGenerator : function() {
								return uriGenerator;
							},
			...
			this.sessionHandler = new SessionHandler(inject);
