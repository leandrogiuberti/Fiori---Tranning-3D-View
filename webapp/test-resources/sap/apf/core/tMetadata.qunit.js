...
sap.ui.define([
	...
	QUnit.module('Basic functionality', {
		beforeEach : function(assert) {
			this.oIfMessageHandler = new MessageHandlerDouble();
			this.oIfMessageHandler.raiseOnCheck().spyPutMessage();
			this.coreApi = new IfCoreApi();
			var annotationHandler = createDefaultAnnotationHandler();
			mockServer.activateGenericMetadata();
			...
			this.metadataForAnalyticalConfig = new Metadata(oInject, "/sap/hba/r/apf/core/odata/modeler/AnalyticalConfiguration.xsodata");
			this.oMetadata = new Metadata(oInject, "/some/path/dummy.xsodata");
		},
	...
	QUnit.module('Metadata for Entity Sets without aggregation semantics', {
		beforeEach : function(assert) {
			...
			this.coreApi = new IfCoreApi();
			var annotationHandler = createDefaultAnnotationHandler();
			mockServer.activateGatewayWithEntitySetWithOutAggretation();
			var oInject = {
					instances : {
						messageHandler : this.oIfMessageHandler,
						coreApi : this.coreApi,
			...
			this.metadata = new Metadata(oInject, "/cds/ZI_ANA_FLIGHT_CDS");
		},
		afterEach : function() {
			mockServer.deactivate();
		}
	...
	QUnit.module('Metadata  without annotation', {
		beforeEach : function(assert) {
			...
			this.coreApi = new IfCoreApi();
			var annotationHandler = createDefaultAnnotationHandler();
			mockServer.activateGenericMetadataWithoutAnnotations();
			this.oInject = {
					instances : {
						messageHandler : this.oIfMessageHandler,
						coreApi : this.coreApi,
			...
			this.oMetadata = new Metadata(this.oInject, "/some/path/dummy.xsodata");
		},
		afterEach : function(assert) {
			mockServer.deactivate();
		}
	...
	QUnit.test('Invalid service puts message with fatal error', function(assert) {
		assert.expect(1);
		var done = assert.async();
		var oMetadata = new Metadata(this.oInject, "invalidService");
	...
	beforeEach : function() {
		...
		var oInject = {
			...
			this.metadata = new Metadata(oInject, "/sap/opu/odata/sap/ZJH_4APF_005_SRV"); 
	...
	QUnit.module('Metadata for Navigation from Parameter entity set to results set', {
		beforeEach : function(assert) {
			...
			this.oMetadata = new Metadata(oInject, "/sap/mm/ZAPF_Q002_SRV");
		},
		afterEach : function(assert) {
			mockServer.deactivate();
		}
	...
	QUnit.module('Errorhandling for empty Metadata document', {
		beforeEach : function(assert) {
			...
			this.coreApi = new IfCoreApi();
			var annotationHandler = createDefaultAnnotationHandler();
			mockServer.activateEmptyMetadata();
			var oInject = {
					instances : {
						messageHandler : this.oIfMessageHandler,
						coreApi : this.coreApi,
			...
			this.oMetadata = new Metadata(oInject, "/sap/empty/empty.xsodata");
		},
		afterEach : function(assert) {
			mockServer.deactivate();
		}
	...
	QUnit.test("Test, whether annotations from annotation handler are taken over", function(assert) {
		...
		var metadata = new Metadata(oInject, "/some/path/to/service/empty.xsodata");
		assert.ok(metadata);
	});
	...
	QUnit.module('annotation name mapping', {
		beforeEach : function(assert) {
			...
			this.metadata = new Metadata(this.oInject, "/some/path/to/service/empty.xsodata");
		}
	});
	...
	QUnit.module('Hierarchy Service', {
		beforeEach : function(assert) {
			var done = assert.async();
			this.messageHandler = new MessageHandlerDouble();
			this.messageHandler.doubleCheckAndMessaging();
			this.coreApi = new IfCoreApi();
			var annotationHandler = createDefaultAnnotationHandler();
			mockServer.activateHierarchyMetadata();
			this.oInject = {
					instances : {
			...
			this.oMetadata = new Metadata(this.oInject, "/some/path/hierarchy.xsodata");
	...
	QUnit.test('getHierarchyAnnotationsForProperty() with non existing entitySet', function(assert) {
		...
		this.oMetadata.isInitialized().done(function(metadata){
			...
			assert.ok(result instanceof MessageObject, "MessageObject returned");
	...
	QUnit.test('getHierarchicalEntitySets() with non-hierarchical service', function(assert) {
		...
		var done = assert.async();
		mockServer.activateGenericMetadata();
		var oMetadata = new Metadata(this.oInject, "/some/path/dummy.xsodata");
	...
	QUnit.test('getHierarchicalEntitySets() with broken hierarchical service', function(assert) {
		assert.expect(1);
		var done = assert.async();
		mockServer.activateBrokenHierarchyMetadata();
		var oMetadata = new Metadata(this.oInject, "/some/path/brokenHierarchy.xsodata");
		oMetadata.isInitialized().done(function(metadata){
			var result = metadata.getHierarchicalEntitySets();
			assert.strictEqual(result.length, 0, "No hierarchical EntitySet found");
			done();
	...
	QUnit.module('Metadata for Entity Sets with unexpected entity set name', {
		beforeEach : function(assert) {
			...
			this.coreApi = new IfCoreApi();
			var annotationHandler = createDefaultAnnotationHandler();
			mockServer.activateUnexpectedEntitySetName();
			this.oInject = {
					instances : {
						messageHandler : this.oIfMessageHandler,
						coreApi : this.coreApi,
	...
	QUnit.test('get properties of entity set with type in its name', function(assert) {
		...
		var oMetadata = new Metadata(this.oInject, "/unexpected/unexpectedEntitysetName.xsodata");
		oMetadata.isInitialized().done(function(metadata){
