...
QUnit.module('Message Handler', {
	beforeEach : function(assert) {
		this.currentLogLevel = Log.getLevel();
		Log.setLevel(Log.Level.INFO);
		this.oMessageHandler = new MessageHandler();
		this.getLogEntries = Log.getLogEntries;
	},
	afterEach : function(assert) {
		Log.setLevel(this.currentLogLevel);
	},
	...
	createMessageObjectWithPreviousMessages : function() {
		...
		for(i = 0; i < 12; i++) {
			oLast = this.oMessageHandler.createMessageObject({
				code : coreConstants.message.code.errorInMessageDefinition,
				aParameters : [],
				oCallingObject : this
			});
...
QUnit.test("WHEN loadConfig used without reset THEN duplicate error message", function(assert) {
	function assertMessageForDuplicateCode(oMessageObject) {
		...
		assert.equal(sCode, coreConstants.message.code.errorExitTriggered);
		sCode = oMessageObject.getPrevious().getCode();
		assert.equal(sCode, coreConstants.message.code.errorInMessageDefinition);
	...
	try {
		...
		assert.equal(oError.message.search(coreConstants.message.code.suppressFurtherException) > -1, true, "Correct exception thrown");
	}
});
...
QUnit.test('Log all messages to console already during createMessageObject', function(assert) {
	this.oMessageHandler = new MessageHandler(true);
	var nLogEntriesBeforeMessage = this.getLogEntries().length;
	this.oResourcePathHandler = new ResourcePathHandlerDouble({
		instances : {
			messageHandler : this.oMessageHandler
...
QUnit.test('Creation of message with many previous messages - only 10 are displayed and handled!', function(assert) {
	...
	assert.ok(sMessage.search(coreConstants.message.code.errorInMessageDefinition) == -1, "not more than 10 messages");
...
QUnit.test("Check method with special code", function(assert) {
	...
	var fnCallback = function(oMessageObject) {
		...
		assert.equal(bContainsText, true, "correct message text");
		assert.equal(sSeverity, coreConstants.message.severity.error, "Error expected");
	};
	this.oResourcePathHandler = new ResourcePathHandlerDouble({
		instances : {
			messageHandler : this.oMessageHandler
