...
sap.ui.define([
...
], function(ViewValidator, modelerUIHelper, Token, View, ViewType, JSONModel) {
	'use strict';
	...
	QUnit.module("Validator apis ", {
		beforeEach : async function(assert) {
			var oSampleView = await View.create({
				type: ViewType.XML,
				definition : sampleViewXML
			});
			...
			oViewValidator = new ViewValidator(oSampleView);
		},
		afterEach : function() {
	...
	QUnit.test('When new oViewValidator is created', async function(assert) {
		// arrange
		var oViewValidator2;
		var oSampleView2 = await View.create({
			type: ViewType.XML,
			definition : sampleViewXML
		});
		oViewValidator2 = new ViewValidator(oSampleView2);
