...
sap.ui.define([
...
], function(modelerConstants, propertyTypeOrchestration, StepPropertyMetadataHandler, deepExtend, extend, Device, Controller, View, ViewType, nextUIUpdate, jQuery, modelerUIHelper) {
	'use strict';
	...
	async function _placeViewAt(oRepresentationSortPropertyView) {
	...
	async function _instantiateRepresentationSortPropertyView(oParentStepObject, assert, bHasMultipleSortProperty) {
		...
		var oRepresentationSortPropertyController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationSortPropertyType"
		});
		...
		oRepresentationSortPropertyView = await View.create({
	...
	function _commonSpiesForTest(oParentStepObject) {
		oStepPropertyMetadataHandler = new StepPropertyMetadataHandler(oModelerInstance.modelerCore, oParentStepObject);
	...
	QUnit.module("Sort property for a Representation - one sort property and no top N available on the parent step", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepresentationSortPropertyView = await _instantiateRepresentationSortPropertyView(oModelerInstance.unsavedStepWithFilterMapping, assert, false);
	...
	QUnit.test("When a sort row is added", async function(assert) {
		//arrange
		...
		await _placeViewAt(oRepresentationSortPropertyView);
		//assert
		assert.strictEqual(spyFireEvent.getCall(0).args[0], modelerConstants.events.ADDPROPERTY, "then ADDPROPERTY is fired with correct first parameter when sort property is changed");
	...
	QUnit.module("Sort property for a Representation - multiple sort properties", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepresentationSortPropertyView = await _instantiateRepresentationSortPropertyView(oModelerInstance.unsavedStepWithFilterMapping, assert, true);
				spyFireEvent = sinon.spy(oRepresentationSortPropertyView, "fireEvent");
				done();
			});
		},
	...
	QUnit.module("Sort property for a Representation - with top N available on the parent", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepresentationSortPropertyView = await _instantiateRepresentationSortPropertyView(oModelerInstance.unsavedStepWithoutFilterMapping, assert, false);
				spyFireEvent = sinon.spy(oRepresentationSortPropertyView, "fireEvent");
				done();
			});
		},
	...
	QUnit.module("Sort property for representation - Validate previously selected values", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepresentationSortPropertyView = await _instantiateRepresentationSortPropertyView(oModelerInstance.unsavedStepWithFilterMapping, assert);
				spyFireEvent = sinon.spy(oRepresentationSortPropertyView, "fireEvent");
				done();
			});
		},
	...
	async function browserTestsCommonSetup(assert, browserName, browserVersion){
		if (browserVersion){
		...
		oRepresentationSortPropertyView = await _instantiateRepresentationSortPropertyView(oModelerInstance.unsavedStepWithFilterMapping, assert, false);
		await oRepresentationSortPropertyView.loaded();
	...
	/** @deprecated As of version 1.87, IE is no longer supported */
	QUnit.test("When Sort Property view is initialized on Internet Explorer 11", async function(assert) {
		var oRepSortPropertyView = await browserTestsCommonSetup(assert,"ie", 11);
	...
	QUnit.test("When Property view is initialized on Internet Explorer version below 11 (IE10)", async function(assert) {
		var oRepSortPropertyView = await browserTestsCommonSetup(assert,"ie", 10);
	...
	QUnit.module("Given a RepresentationSortPropertySortType controller", {
		beforeEach: async function(assert){
			...
			this.sortPropertyTypeController = await Controller.create({
				name: "sap.apf.modeler.ui.controller.representationSortPropertyType"
			});
			...
			extend(this.sortPropertyTypeController, stubObj);
			this.spies = {
