...
sap.ui.define([
	...
	async function _instantiateView(parameter, sRepresentationType, oRepresentationHandlerStub, bAscending, assert, bHasNoLongTitle) {
		...
		var oView;
		that.oPreviewContentController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.previewContent"
		});
	...
	QUnit.module("Given a Column Chart", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oPreviewContentView = await _instantiateView(parameter);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("when content preview is requested", function(assert) {
		...
		assert.ok(BaseObject.isObjectA(oMainChart.getItems()[0], "sap.viz.ui5.controls.VizFrame"), "then main chart is drawn on main chart holder.");
	...
	QUnit.module("Given a Column Chart with no assigned dimension properties", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oModelerInstance = modelerInstance;
				that.oPreviewContentView = await _instantiateView(parameter, "sap.apf.ui.representations.columnChart", oRepresentationHandlerStub, false, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.module("Given a Table Representation", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oPreviewContentView = await _instantiateView(parameter, "sap.apf.ui.representations.table", oRepresentationHandlerStub, true, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("when content preview is requested", function(assert) {
		...
		var titleControl = oTable.getExtension().find((oCtrl) => oCtrl.isA("sap.m.HBox"));
		//assert
	...
	QUnit.module("Preview Content Tests for step with no long title", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oModelerInstance = modelerInstance;
				that.oPreviewContentView = await _instantiateView(parameter, "sap.apf.ui.representations.columnChart", oRepresentationHandlerStub, false, assert, true);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.module("Preview Content Tests for step with long title as null", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oPreviewContentView = await _instantiateView(parameter,"sap.apf.ui.representations.columnChart", oRepresentationHandlerStub, false, assert, false);
				done();
			});
		},
		afterEach : function() {
