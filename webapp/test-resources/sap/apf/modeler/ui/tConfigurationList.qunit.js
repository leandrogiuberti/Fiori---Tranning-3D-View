...
sap.ui.define([
	...
	async function _instantiateView(oConfigurationListController) {
		...
		// wait for the controller to initialize its sub views
		await oConfigurationListController.whenInitialized();
		return oView;
	}
	...
	async function _updateConfigList() {
	...
	async function _setupForSwitchFromNewConfig(assert) {
		//arrangement
		...
		oConfigurationListView.getController().configurationHandler.loadConfiguration(newConfigId, async function(configurationEditor) {
			...
			await oConfigurationListView.getController().updateSubView(oParams);
			...
			//action
			oSubView =  await oConfigurationListView.getController()._getSubView();
	...
	async function _setupForUnsavedConfig() {
		//arrangement
		var oParams;
	 ...
	 async function _setupForSavedConfig() {
		//arrangement
		var oParams;
	...
	async function _setupForCategoryAssignment() {
		var oSubView;
	...
	async function _pressButtonOnMandatoryPopup(assert, buttonNumber, continuation) {
	...
	QUnit.module("Configuration List Unit Tests", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				await that.oConfigurationListController.whenInitialized();
	...
	QUnit.test("onInit Test", async function(assert) {
		this.oConfigurationListController.onInit();
		await this.oConfigurationListController.whenInitialized();
	...
	QUnit.test("updateSubView Test", async function(assert) {
		var oParams = {
			name : "configuration",
			arguments : {
				appId : "test App Id",
		...
		await this.oConfigurationListController.updateSubView(oParams);
		const SubView = await this.oConfigurationListController._getSubView();
	    assert.equal(SubView.getViewName(), "sap.apf.modeler.ui.view.configuration", "Proper subview is loaded in the detail area");
	});
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press YES on reset mandatory fields and NO on loss of data popup to save current configuration(saved) before switch", async function(assert) {
		const done = assert.async();
		await oConfigurationListView.getController().oTreeInstance.expandToLevel(0);
		//arrangement
	...
	QUnit.test("Test reset of mandatory fields, when switching between configurations, press CANCEL on loss of data popup when switching between different configurations", async function(assert) {
		...
		oSubView.byId("idConfigTitle").setValue("change title");
		oModelerInstance.configurationEditorForSavedConfig.setIsUnsaved();
		var previousSelectedItem = await oConfigurationListView.getController().oTreeInstance.getSelectedItem();
		oConfigurationListView.getController().oTreeInstance.previousSelectedItem = previousSelectedItem;
		oConfigurationListView.getController().oTreeInstance.setSelectedItem(oConfigurationListView.getController().oTreeInstance.getItems()[2]);
		var currentSelectedItem = oConfigurationListView.getController().oTreeInstance.getSelectedItem();
		if (previousSelectedItem.getId()!== currentSelectedItem.getId()) {
	...
	QUnit.module("NavBack (Cancel or back button)", {
		beforeEach : async function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				await this.oConfigurationListController.updateSubView({ //create subView (which is used to check mandatory state)
					arguments : {
	...
	QUnit.test("Mandatory fields are not filled, old configuration is restored and saved", async function(assert) {
		var configurationView =  await this.oConfigurationListController._getSubView();
		...
		var mandatoryFieldsPopup = await oConfigurationListView.getDependents()[0];
		...
		var lossOfDataPopup = await oConfigurationListView.getDependents()[1];
	...
	QUnit.test("Set Display Text and Visibility", async function(assert) {
		this.controller.onInit();
		await this.controller.whenInitialized();
	...
	QUnit.test("Set correct text", async function(assert) {
	...
	QUnit.test("Set publish visible (not on cf)", async function(assert) {
	...
	QUnit.test("Set publish visible (on cf)", async function(assert) {
		var spySetVisible = sinon.spy();
		this.byId = function(name) {
			return {
				setTitle : function() {},
	...
	QUnit.test("Handle publish press (not on cf)", async function(assert) {
		this.isUsingCloudFoundryProxy = false;
		this.controller.onInit();
		await this.controller.whenInitialized();
	...
	QUnit.test("Handle publish press (on cf)", async function(assert) {
		this.isUsingCloudFoundryProxy = true;
		this.controller.onInit();
		await this.controller.whenInitialized();
		this.controller.handlePublishPress();
