...
sap.ui.define([
...
], function(modelerConstants, Controller, View, ViewType, nextUIUpdate, jQuery, modelerUIHelper) {
	'use strict';
	...
	async function _instantiateView(sId, assert) {
		...
		var oVHRController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.facetFilterVHR"
		});
		var spyOnInit = sinon.spy(oVHRController, "onInit");
		// eslint-disable-next-line new-cap
		oVHRView = await View.create({
	...
	QUnit.module("For a Facetfilter with existing VHR", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oVHRView = await _instantiateView(oModelerInstance.facetFilterUnsaved, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When source is changed and there are no common entities between new and old source", function(assert) {
		...
		//assert
		assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.facetFilter.UPDATEPROPERTIES), true, "then updateFFProperties is fired when source is changed");
