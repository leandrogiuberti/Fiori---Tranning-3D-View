...
sap.ui.define([
...
], function(Table, Fragment, coreLibrary, Controller, View, ViewType, nextUIUpdate, jQuery, modelerUIHelper) {
	'use strict';
	...
	/*async function getDialogByTitleKey(key, context) {
		await nextUIUpdate();
	...
	QUnit.module("Import of configuration and text properties file", {
		beforeEach : async function(assert) {
			...
			var context = this;
			var oImportFilesController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.importFiles"
			});
			...
			modelerUIHelper.getModelerInstance(async function(oModelerInstance) {
				...
				oImportFilesView = await View.create({
	...
	QUnit.test("When only .json file is added in uploader and upload button of the dialog is clicked", async function(assert) {
		//arrange
	...
	QUnit.test("When only .properties file is added in uploader and upload button of the dialog is clicked", async function(assert) {
		//arrange
		_getDialogAndItsElements();
	...
	QUnit.test("When both .json and .properties file is added in uploader and upload button of the dialog is clicked", async function(assert) {
		//arrange
		_getDialogAndItsElements();
		var jsonFileUploaderSpy = sinon.spy(jsonFileUploader, "upload");
		jsonFileUploader.setValue("Test.json");
	...
	QUnit.test("When clicking on Cancel button of import files dialog", async function(assert) {
		//action
	...
	QUnit.test("When uploading configuration and when configuration already exists and then clicking on cancel button", function(assert) {
		...
		reader.addEventListener("load", function() {
			nextUIUpdate().then(async () => {
				assert.strictEqual(Fragment.load.callCount, 1, "Fragment.load was called exactly once");
				await Fragment.load.returnValues[0]; // wait for the fragment to be fully loaded
	...
	QUnit.test("When uploading configuration file and both application and configuration do not exist", async function(assert) {
		//arrange
		var context = this;
		var done = assert.async();
	...
	QUnit.test("When uploading configuration file and configuration already exists", async function(assert) {
		//arrange
		var context = this;
		var done = assert.async();
		...
		reader.addEventListener("load", function() {
			...
			assert.strictEqual(Fragment.load.callCount, 1, "Fragment.load was called exactly once");
			Fragment.load.returnValues[0].then(async () => {
				assert.strictEqual(Fragment.load.callCount, 1, "Fragment.load was called exactly once");
				await Fragment.load.returnValues[0]; // wait for the fragment to be fully loaded
	...
	QUnit.test("When uploading configuration and when configuration already exists and on overwriting the existing configuration", async function(assert) {
		//arrange
		var context = this;
		var done = assert.async();
		var importConfigurationStub = sinon.stub(oImportFilesView.getViewData().oCoreApi, "importConfiguration", _importConfigurationStub);
	...
	QUnit.test("When uploading configuration and when configuration already exists and on not overwriting existing configuration and providing a title for new config", async function(assert) {
		//arrange
		var context = this;
		var done = assert.async();
		var importConfigurationSpy = sinon.stub(oImportFilesView.getViewData().oCoreApi, "importConfiguration", _importConfigurationStub);
		...
		reader.addEventListener("load", function() {
			assert.strictEqual(Fragment.load.callCount, 1, "Fragment.load was called exactly once");
			Fragment.load.returnValues[0].then(() => {
				var oOverwriteDialog = oImportFilesView.getDependents()[0];
				oOverwriteDialog.getContent()[0].getItems()[1].setSelectedButton(oOverwriteDialog.getContent()[0].getItems()[1].getButtons()[1]);
				oController.handleChangeForOverwriteConfigOptions();
	...
	QUnit.test("When importing configuration and when configuration already exists and on not overwriting existing configuration and not providing title for new config", async function(assert) {
		//arrange
		var context = this;
		var done = assert.async();
		var importConfigurationSpy = sinon.stub(oImportFilesView.getViewData().oCoreApi, "importConfiguration", _importConfigurationStub);
