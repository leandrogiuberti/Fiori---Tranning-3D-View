...
sap.ui.define([
...
], function(RepresentationBasicDataHandler, ViewValidator, SortDataHandler,
			ModelerConstants, UiControllerRepresentation,
			ModelerCoreRepresentation, ElementContainer,
			RepresentationTypesHandler, fnRepresentationTypes,
			Hashtable, utilsUtils, modelerUIHelper,
			Bar, coreLibrary, View, ViewType, nextUIUpdate, jQuery, sinon
){
	'use strict';
	...
	async function createdView(fn) {
		sinon.spy(View, "create");
		const result = fn();
		if ( View.create.callCount !== 1 ) {
			throw new Error("View.create not called");
		}
		await View.create.returnValues[0];
		View.create.restore();
		return result;
	}
	...
	function createStepPropertyMetadataHandlerStub(context, isHierarchicalStep) {
		return function() {
			return {
				meta : "data",
				oStep : {
	...
	async function _instantiateView(context, assert, isHierarchicalStep) {
		...
		var oView = await View.create({
			...
			viewData : {
				...
				StepPropertyMetadataHandler: createStepPropertyMetadataHandlerStub(context, isHierarchicalStep)
			}
		});
		// also wait for the full initialization of the controller
		await oView.getController().promiseControllerIsCreated;
		return oView;
	}
	...
	QUnit.test("When validating Chart Type Selection and all mandatory fields are filled", async function(assert){
		var done = assert.async();
		var that = this;
		this.bValidationState = true;
		this.oRepresentationView = await _instantiateView(this, assert, false);
		this.oRepresentationView.getController().promiseControllerIsCreated.then(function(){
			setTimeout(function(){
				assert.strictEqual(that.oRepresentationView.byId("idChartType").getValueState(), ValueState.None, "then chartType selection control value state is set as default onInit");
				that.bValidationState = false;
				that.oRepresentationView.getController().handleChangeForChartType(_createEvent("BubbleChart"));
				setTimeout(function(){
					assert.strictEqual(that.oRepresentationView.byId("idChartType").getValueState(), ValueState.Error, "then chartType selection control value state is set to error on handleChangeforChartType");
					done();
				}, 1000);
			}, 1000);
		});
	});
	QUnit.test("When validating Chart Type Selection and not all mandatory fields are filled", async function(assert){
		var that = this;
		var done = assert.async();
		that.bValidationState = false;
		that.oRepresentationView = await _instantiateView(that, assert, undefined);
		...
		that.oRepresentationView.getController().promiseControllerIsCreated.then(function(){
			setTimeout(function(){
				assert.strictEqual(that.oRepresentationView.byId("idChartType").getValueState(), ValueState.Error, "then chartType selection control value state is set as default onInit");
				that.bValidationState = true;
				that.oRepresentationView.getController().handleChangeForChartType(_createEvent("BubbleChart"));
				setTimeout(function(){
					assert.strictEqual(that.oRepresentationView.byId("idChartType").getValueState(), ValueState.None, "then chartType selection control value state is set as default on handleChangeforChartType");
					done();
				}, 1000);
			}, 1000);
		});
	});
	QUnit.test("When getValidationState is called and all mandatory fields are filled", async function(assert){
		var done = assert.async();
		this.bValidationState = true;
		this.oRepresentationView = await _instantiateView(this, assert, false);
		this.oRepresentationView.getController().promiseControllerIsCreated.then(function(){
	...
	QUnit.test("When getValidationState is called and not all mandatory fields are filled", async function(assert){
		var done = assert.async();
	...
	QUnit.module("Given an existing Column Chart Representation", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oRepresentationView = await _instantiateView(that, assert);
	...
	QUnit.test("Test handlePreviewButtonPress for representation ColumnChart", async function(assert) {
		//action
		await createdView(() => {
	...
	QUnit.module("Given a new Representation", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				that.oRepresentationView = await _instantiateView(that, assert);
				that.representationController = that.oRepresentationView.getController();
				sinon.spy(RepresentationBasicDataHandler.prototype, "instantiateBasicDataAsPromise");
				done();//Start the test once modeler instance is got and setup is done
			});
			that.oRepresentation = null;
		},
		afterEach : function() {
			RepresentationBasicDataHandler.prototype.instantiateBasicDataAsPromise.restore();
			modelerUIHelper.destroyModelerInstance();
			spyOnGetText.restore();
	...
	QUnit.test("Test handleChangeForChartType for representation Bubble Chart", async function(assert) {
		//arrange
		...
		await nextUIUpdate();
		//asserts
		assert.strictEqual(RepresentationBasicDataHandler.prototype.instantiateBasicDataAsPromise.callCount, 1);
		await RepresentationBasicDataHandler.prototype.instantiateBasicDataAsPromise.returnValues[0];
		setTimeout(() => {
			assert.ok(this.oRepresentationView.byId("idChartType"), "then chart type box exists");
			assert.ok(this.oRepresentationView.byId("idChartTypeLabel"), "then chart type label box exists");
			assert.ok(this.representationController.byId("idCornerTextsVBox"), "then corner text VBox is available");
	...
	QUnit.test("Test handleChangeForChartType for Table representation", async function(assert) {
		//arrange
		var aStepCategories = this.oModelerInstance.configurationEditorForUnsavedConfig.getCategoriesForStep(this.oRepresentationView.getViewData().oParentStep.getId());
		//action
	...
	QUnit.test("Test handlePreviewButtonPress for representation", async function(assert) {
		//action
		await createdView(() => {
			this.representationController.handlePreviewButtonPress();
		});
	...
	QUnit.test("Test handlePreviewButtonPress for table representation", async function(assert) {
		//action
		await createdView(() => {
			this.representationController.handlePreviewButtonPress();
		});
	...
	QUnit.module("When tree table representation is initilized with display options label as text", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				_commonSpiesInBeforeEach(that.oRepresentation, that.oModelerInstance);
				that.oRepresentationView = await _instantiateView(that, assert, true);
				that.representationController = that.oRepresentationView.getController();
				done();//Start the test once modeler instance is got and setup is done
			});
		},
	...
	async function execTestSpec(assert, testEnv, testSpec){
		var done = assert.async();
		testEnv.aDimensions = testSpec.dimensions;
		testEnv.aMeasures = testSpec.measures;
		testEnv.oRepresentationView = await _instantiateView(testEnv, assert);
	...
	QUnit.test("When creating the Representation view, initialized with 0 measure and 0 dimension", async function(assert) {
		await execTestSpec(assert, this, {
			dimensions: [],
			measures: [],
			text: "Combination: [] x []"
		});
	});
	QUnit.test("When creating the Representation view, initialized with 0 measure and 1 dimension", async function(assert) {
		await execTestSpec(assert, this, {
	...
	QUnit.test("When creating the Representation view, initialized with 1 measure and 0 dimension", async function(assert) {
		await execTestSpec(assert, this, {
				dimensions: [],
				measures: ["property4"],
	...
	QUnit.test("When creating the Representation view, initialized with 1 measure and 1 dimension", async function(assert) {
		await execTestSpec(assert, this, {
				dimensions: ["property2"],
				measures: ["property4"],
	...
	QUnit.test("When Representation is initialized with two measure properties and 0 dimensions", async function(assert) {
		await execTestSpec(assert, this, {
			dimensions: [],
	...
	QUnit.test("When Representation is initialized with two measure properties and one or more dimension properties", async function(assert) {
		await execTestSpec(assert, this, {
			dimensions: ["property2"],
			measures: ["property4", "property5"],
			text: "Combination: 1 x 1"
	...
	QUnit.test("When Representation is initialized with three or more measure properties and no dimensions", async function(assert) {
		await execTestSpec(assert, this, {
			dimensions: [],
	...
	QUnit.test("When Representation is initialized with one or more dimension properties and three or more measure properties", async function(assert) {
		await execTestSpec(assert, this, {
			dimensions: ["property2"],
			measures: ["property4", "property5", "property6"],
			text: "Combination: 1 x 1"
	...
	QUnit.module("Given the list of APF representation types and a getText stub", {
		beforeEach : function(assert) {
			...
			this.aRepresentationTypes = fnRepresentationTypes();
			this.coreApi = {
	...
	QUnit.module("Given the list of APF representation types", {
		beforeEach : function(assert) {
			var aRepresentationTypes = fnRepresentationTypes();
			var coreApi = {
				getText: function(key, aNames) {
					return "";
				}
