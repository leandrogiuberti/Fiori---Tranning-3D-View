...
sap.ui.define([
	...
	async function _placeViewAt(oRepLegendView) {
	...
	async function _instantiateView(sId, assert, sProperty, oStep) {
		var oLegendsController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationLegend"
		});
		...
		oRepLegendView = await  View.create({
			...
			viewData : {
				...
				sPropertyType : modelerConstants.propertyTypes.LEGEND
			}
		});
	...
	QUnit.module("For a Representation with property type of Legend with property type as none", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepLegendView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, "None", oModelerInstance.unsavedStepWithoutFilterMapping);
	...
	QUnit.test("When add icon of legend is pressed", async function(assert) {
		//arrange
		var propertyInformationList = [{
			sProperty: "property1",
			sKind: 'legend',
		...
		await _placeViewAt(oRepLegendView);
	...
	QUnit.module("For a Representation with property type of Legend with defined property type", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepLegendView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, "property1", oModelerInstance.unsavedStepWithoutFilterMapping);
				done();
			});
		},
		afterEach : function() {
