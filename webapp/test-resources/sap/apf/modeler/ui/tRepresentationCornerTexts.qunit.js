...
sap.ui.define([
	...
	async function _instantiateView(oParentObject, assert) {
		var oRepCornerTextsController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationCornerTexts"
		});
		var spyOnInit = sinon.spy(oRepCornerTextsController, "onInit");
		oRepCornerTextsView = await View.create({
	...
	QUnit.module("For an already saved representation, when corner texts from step are available and few corner text on representation are available", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepCornerTextsView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When suggestions are triggered for right lower corner text", function(assert) {
		...
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.REPRESENTATION_CORNER_TEXT;
	...
	QUnit.module("For a new representation, when few corner texts from step are available", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				oModelerInstance = modelerInstance;
				spyOnGetText = sinon.spy(oModelerInstance.modelerCore, "getText");
				oRepCornerTextsView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[1], assert);
				done();
			});
		},
		afterEach : function() {
