...
sap.ui.define([
...
], function(utils, RepresentationTypesHandler, Input, Item, Controller, View, ViewType, nextUIUpdate, jQuery, modelerUIHelper, modelerConstants, textPoolHelper){
	'use strict';
	...
	async function _placeViewAt(oRepMeasureView) {
	...
	async function _instantiateView(sId, assert, oStep) {
		var oMeasuresController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationMeasure"
		});
		...
		oRepMeasureView = await View.create({
			...
			viewData : {
				...
				sPropertyType : modelerConstants.propertyTypes.MEASURE
			}
		});
	...
	QUnit.module("For a Representation with property type of Measure", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepMeasureView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, oModelerInstance.unsavedStepWithoutFilterMapping);
	...
	QUnit.test("When add icon of measure is pressed", async function(assert) {
		//arrange
		...
		await _placeViewAt(oRepMeasureView);
	...
	QUnit.test("When add icon of measure is pressed - when already one measure exists", async function(assert) {
		//arrange
		this.stubbedPropertyOrchestration.getPropertyInformationList =  function(){
			return [{
				sProperty: "property4",
	...
	QUnit.module("Show measure display options for combinationCharts", {
		beforeEach : async function(assert) {
			...
			this.representationMeasureController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.representationMeasure"
			});
		},
		instantiateView : async function(){
			this.view = await View.create({
				viewName : "sap.apf.modeler.ui.view.propertyType",
				type : ViewType.XML,
				viewData : this.viewData,
				controller : this.representationMeasureController
			});
			// View.create makes a deep clone of its options, therefore update viewData after create
			this.viewData = this.view.getViewData();
			this.view.placeAt("testArea");
			await nextUIUpdate();
			return this.view;
		},
	...
	QUnit.test("Display Option for non-CombinationChart", async function(assert) {
		var isCombinationChartStub = sinon.stub(this.viewData.oRepresentationTypeHandler, "isCombinationChart", function(){
			return false;
		});
		var view = await this.instantiateView();
	...
	QUnit.test("Display Option for CombinationChart when measure is mandatory", async function(assert) {
	...
	QUnit.test("Display Option for CombinationChart when measure is not mandatory", async function(assert) {
		var view = await this.instantiateView();
	...
	QUnit.test("When Display option is already set in the configuration to bar", async function(assert) {
	...
	QUnit.test("When Display option is already set in the configuration to line", async function(assert) {
	...
	QUnit.test("When Display option is already set but chart is not a combination chart (representation switch)", async function(assert) {
		this.viewData.oParentObject.measureOption = "line";
		var isCombinationChartStub = sinon.stub(this.viewData.oRepresentationTypeHandler, "isCombinationChart", function(){
			return false;
		});
		await this.instantiateView();
		assert.strictEqual(this.viewData.oParentObject.measureOption, undefined, "MeasureOptions is removed in the configuration");
		isCombinationChartStub.restore();
	});
	QUnit.test("When Display option is changed from line to bar", async function(assert) {
		this.viewData.oParentObject.measureOption = "line";
		var view = await this.instantiateView();
		this.assertCommonDisplayOptionType(assert);
	...
	QUnit.test("When Display option is changed from bar to line", async function(assert) {
		this.viewData.oParentObject.measureOption = "bar";
		var view = await this.instantiateView();
		this.assertCommonDisplayOptionType(assert);
