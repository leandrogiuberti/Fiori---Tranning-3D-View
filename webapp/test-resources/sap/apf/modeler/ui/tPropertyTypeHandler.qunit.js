...
sap.ui.define([
...
], function(RepresentationTypesHandler, fnRepresentationTypes, propertyTypeOrchestrationModule, uiModelerConstants, utils, VBox, Controller, View, ViewType, nextUIUpdate, jQuery, modelerUIHelper){
	'use strict';
	var oModelerInstance, oPropTypeHandlerView;
	async function _instantiateView(sId, assert, context) {
		var oPropTypeHandlerController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.propertyTypeHandler"
		});
		...
		oPropTypeHandlerView = await View.create({
		...
		await oPropTypeHandlerView.getController().initPromise;
	...
	QUnit.module("For a Representation property type handler", {
		beforeEach: function (assert) {
			...
			modelerUIHelper.getModelerInstance(async function (modelerInstance) {
				...
				oPropTypeHandlerView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, context);
				done();
			});
		},
		afterEach: function () {
	...
	QUnit.test("When add icon is pressed", async function(assert) {
		//arrange
		var that = this;
		const localID = (sID) => sID.includes("--") ? sID.slice(sID.lastIndexOf("--") + 2) : sID;
		const id = "iddimensionsView0";
		const id2 = "iddimensionsView1";
		//var done = assert.async();
		...
		assert.expect(15);
		// setTimeout(async ()=>{
		//assert.strictEqual(View.create.callCount, 1, "View.create was called exactly once");
		//await View.create.returnValues[0]; // wait for the new view to be fully loaded
		...
		assert.ok([this.representationId, "Step-6-Representation-1"].includes(this.spy.getConsumableAndAvailablePropertiesAsPromise.getCall(0).args[0]), "Then getConsumableAndAvailablePropertiesAsPromise was called with expected parameters"); // async -> necessary to avoid togglers (order is unknown)
		assert.ok([this.representationId, "Step-6-Representation-1"].includes(this.spy.getConsumableAndAvailablePropertiesAsPromise.getCall(1).args[0]), "Then getConsumableAndAvailablePropertiesAsPromise was called with expected parameters"); // async -> necessary to avoid togglers (order is unknown)
		assert.ok([this.representationId, "Step-6-Representation-1"].includes(this.spy.getConsumableAndAvailablePropertiesAsPromise.getCall(2).args[0]), "Then getConsumableAndAvailablePropertiesAsPromise was called with expected parameters"); // async -> necessary to avoid togglers (order is unknown)
		...
		// it's only a deepEqual as View.create deepClones its options...
		assert.deepEqual(this.spy.getConsumableAndAvailablePropertiesAsPromise.getCall(0).args[2], this.oStepPropertyMetadataTypeHandlerStub, "Then getConsumableAndAvailablePropertiesAsPromise was called with expected parameters");
		...
		const expectedIDs = [id, id2].sort();
		const actualIDs = [
			localID(this.spy.addPropertyTypeReference.getCall(0).args[0]),
			localID(this.spy.addPropertyTypeReference.getCall(1).args[0])
		].sort();
		assert.deepEqual(actualIDs, expectedIDs, "Then addPropertyTypeReference was called with expected parameters"); // async -> necessary to avoid togglers (order is unknown)
		assert.strictEqual(oPropTypeHandlerView.getController().byId("idPropertyTypeVBox").getItems().length, 2, "then 2 dimensions vbox of propertyTypeHandler view is created");
		assert.strictEqual(oPropTypeHandlerView.getController().nCounter, 2, 'then two propertyTypeHandler views are created for dimensions');
		//done();
		//}, 2000);
	...
	QUnit.test("When property is removed", function(assert) {
		...
		//assert
		assert.expect(1);
		assert.strictEqual(spyFireEvent.calledWith(uiModelerConstants.events.REMOVEPROPERTYFROMPARENTOBJECT), true, "then REMOVEPROPERTYFROMPARENTOBJECT event is fired");
	});
	QUnit.module("Initialization Promise", {
		beforeEach : async function() {
			...
			context.stepType = "";
			this.oPropTypeHandlerView = await View.create({
				viewName : "sap.apf.modeler.ui.view.propertyTypeHandler",
				type : ViewType.XML,
				viewData : {
					oViewDataForPropertyType : {
						...
						oRepresentationTypeHandler : new RepresentationTypesHandler(fnRepresentationTypes())
					},
					aPropertiesToBeCreated : [ {
						sContext : "xAxis",
						sProperty : "property1",
