...
sap.ui.define([
	...
	async function _instantiateView(sId, assert, oStep) {
		var oPropertiesController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationHierarchyProperty"
		});
		...
		oRepHierarchyPropertyView = await View.create({
			...
			viewData : {
				...
				sPropertyType : modelerConstants.propertyTypes.HIERARCHIALCOLUMN
			}
		});
	...
	QUnit.module("For a Representation with property type of hierarchical Property with display option text", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepHierarchyPropertyView = await _instantiateView(oModelerInstance.firstHierarchicalStep.getRepresentations()[0], assert, oModelerInstance.firstHierarchicalStep);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.module("For a Representation with property type of hierarchical Property with display option key", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepHierarchyPropertyView = await _instantiateView(oModelerInstance.firstHierarchicalStep.getRepresentations()[2], assert, oModelerInstance.firstHierarchicalStep);
				done();
			});
		},
		afterEach : function() {
