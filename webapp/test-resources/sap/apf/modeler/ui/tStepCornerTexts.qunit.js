...
sap.ui.define(
	...
	function(textPoolHelper, modelerUIHelper, Input, CustomData, Item, Controller, View, ViewType, nextUIUpdate, jQuery) {
		"use strict";
		...
		async function _instantiateView(oParentObject, assert) {
			var oStepCornerTextsController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.stepCornerTexts"
			});
			var spyOnInit = sinon.spy(oStepCornerTextsController, "onInit");
			oStepCornerTextsView = await View.create({
		...
		QUnit.module("For an already saved step", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					...
					oStepCornerTextsView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping, assert);
					done();
				});
			},
			afterEach : function() {
		...
		QUnit.test("When left upper corner text is changed", async function(assert) {
			var done = assert.async();
		...
		QUnit.test("When right upper corner text is changed", async function(assert) {
			var done = assert.async();
			var textHasBeenResolved = jQuery.Deferred();
			//arrange
			var spyOnConfigEditorSetIsUnsaved = sinon.spy(oStepCornerTextsView.getViewData().oConfigurationEditor, "setIsUnsaved");
		...
		QUnit.test("When left lower corner text is changed", async function(assert) {
			//arrange
			var spyOnConfigEditorSetIsUnsaved = sinon.spy(oStepCornerTextsView.getViewData().oConfigurationEditor, "setIsUnsaved");
		...
		QUnit.test("When right lower corner text is changed", async function(assert) {
			//arrange
			var spyOnConfigEditorSetIsUnsaved = sinon.spy(oStepCornerTextsView.getViewData().oConfigurationEditor, "setIsUnsaved");
		...
		QUnit.test("When suggestions are triggered for right lower corner text", function(assert) {
			...
			var oTranslationFormat = textPoolHelper.TranslationFormatMap.STEP_CORNER_TEXT;
		...
		QUnit.module("For a new step", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					oModelerInstance = modelerInstance;
					spyOnGetText = sinon.spy(oModelerInstance.modelerCore, "getText");
					oStepCornerTextsView = await _instantiateView(oModelerInstance.unsavedStepWithFilterMapping, assert);
					done();
				});
			},
			afterEach : function() {
