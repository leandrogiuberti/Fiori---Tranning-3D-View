...
sap.ui.define([
	...
	async function _instantiateView(sId, assert) {
		var oCategoryController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.category"
		});
	...
	QUnit.module("Category Unit Tests - Existing category", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				categoryView = await _instantiateView(oModelerInstance.categoryIdUnsaved, assert);
	...
	QUnit.test("When Category view is loaded", async function(assert) {
		// arrange
	...
	QUnit.test("When editing the category title", function(assert) {
		...
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.CATEGORY_TITLE;
		// act
	...
	QUnit.module("Category Unit Tests - New category", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				spyOnGetOfTextPool = sinon.spy(oTextPool, "get");
				spyOnSetTextOfTextPool = sinon.spy(oTextPool, "setTextAsPromise");
				categoryView = await _instantiateView("newCategory0", assert);
				done();//Start the test once modeler instance is got and setup is done
			});
		},
		afterEach : function() {
