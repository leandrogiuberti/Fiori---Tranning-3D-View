...
sap.ui.define([
	...
	function createSortPropertyTypeController() {
		return Promise.resolve(new SortPropertyTypeController());
	}
	...
	QUnit.module("Given an orchestration and a sortPropertyType instance and a stub view", {
		beforeEach : function(assert) {
			...
			that.createSortPropertyTypeContext = async function (initialSelectedProperty){
				...
				var innerContext = {
					...
					sortPropertyTypeController : await createSortPropertyTypeController(),
				...
				extend(innerContext.sortPropertyTypeController, innerContext.stubForSortPropertyTypeController);
				that.spies.push({
	...
	QUnit.test("Given 2 selectable properties, one selected, WHEN change selection to the other", async function(assert) {
		// arrange
		assert.expect(4+5+3);
		var that = this;
		var done = assert.async();
		var inputContractTranslatedKey = "hugo";
		var context = await that.createSortPropertyTypeContext("otto");
	...
	QUnit.test("Given 2 selectable properties, one selected, WHEN change selection to 'None'", async function(assert) {
		// arrange
		assert.expect(9+3);
		var that = this;
		var done = assert.async();
	...
	QUnit.test("Given 2 selectable properties, one selected, WHEN change selection to not-available property", async function(assert) {
		// arrange
		assert.expect(6);
		var that = this;
		var done = assert.async();
	...
	QUnit.test("Given 2 selectable properties, and 2 rows, WHEN change selection in 1st row to 'None'", async function(assert) {
		// arrange
		assert.expect(16+3);
		var that = this;
		var done = assert.async();
		var row_1 = await that.createSortPropertyTypeContext("otto");
		that.oPropertyTypeOrchestration.addPropertyTypeReference(row_1.view.getId(), {sProperty: row_1.selectedTestKey}, "Any", row_1.view);
		var row_2 = await that.createSortPropertyTypeContext("hugo");
	...
	QUnit.test("Given 2 selectable properties, and 2 rows, one pre-selected with None, WHEN change selection in 2nd row to available property", async function(assert) {
		// arrange
		assert.expect(16);
		var that = this;
		var done = assert.async();
		var row_1 = await that.createSortPropertyTypeContext("otto");
		that.oPropertyTypeOrchestration.addPropertyTypeReference(row_1.view.getId(), {sProperty: row_1.selectedTestKey}, "Any", row_1.view);
		var row_2 = await that.createSortPropertyTypeContext(row_1.view.getController().oTextReader("none"));
		that.oPropertyTypeOrchestration.addPropertyTypeReference(row_2.view.getId(), {sProperty: row_2.selectedTestKey}, "Any", row_2.view);
	...
	QUnit.test("Given 1 sortPropertyType, WHEN calling the UI handler for change of direction", async function(assert) {
		// arrange
		assert.expect(2);
		var that = this;
		...
		var row_1 = await that.createSortPropertyTypeContext("true");
	...
	QUnit.test("When removing a sort row", async function(assert) {
		// arrange
		var done = assert.async();
		assert.expect(6);
		var that = this;
		var row_1 = await that.createSortPropertyTypeContext("any");
		// Act
