...
sap.ui.define([
	...
	QUnit.module("Toolbar unit tests", {
		beforeEach : async function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(oModelerInstance) {
				...
				that.oConfigurationView = await View.create({
				...
				that.oGetViewStub = function() {
					...
					obj.byId = function(param) {
						if (param === "idConfigDetailData") {
							return new Grid({
								content : [ that.oConfigurationView ]
							});
						}
					};
				...
				that.oTreeInstance = new APFTree();
				...
				that.toolbarView = await View.create({
				...
				// View.create makes a deep clone of its options, therefore update oConfigListInstance after create
				that.oConfigListInstance = that.toolbarView.getViewData().oConfigListInstance
	...
	QUnit.test("Test enable disable add menu items", async function(assert) {
		const addMenu = await Fragment.load({
			type: "XML",
			id: "idAddMenuFragment",
			name: "sap.apf.modeler.ui.fragment.addMenu",
			controller: this
		});
		const aAddMenuItems = addMenu.getItems();
		...
		oSelectedTreeNodeDetails = {
			nodeObjectType : modelerConstants.configurationObjectTypes.CONFIGURATION
		};
		this.toolbarController._enableDisableAddMenuItems(oSelectedTreeNodeDetails, addMenu);
		...
		oSelectedTreeNodeDetails = {
			nodeObjectType : modelerConstants.configurationObjectTypes.FACETFILTER
		};
		this.toolbarController._enableDisableAddMenuItems(oSelectedTreeNodeDetails, addMenu);
		...
		oSelectedTreeNodeDetails = {
			nodeObjectType : modelerConstants.configurationObjectTypes.CATEGORY
		};
		this.toolbarController._enableDisableAddMenuItems(oSelectedTreeNodeDetails, addMenu);
		...
		oSelectedTreeNodeDetails = {
			nodeObjectType : modelerConstants.configurationObjectTypes.NAVIGATIONTARGET
		};
		this.toolbarController._enableDisableAddMenuItems(oSelectedTreeNodeDetails, addMenu);
		...
		oSelectedTreeNodeDetails = {
			nodeObjectType : modelerConstants.configurationObjectTypes.STEP
		};
		this.toolbarController._enableDisableAddMenuItems(oSelectedTreeNodeDetails, addMenu);
		assert.equal(aAddMenuItems[0].getEnabled() && aAddMenuItems[1].getEnabled() && aAddMenuItems[2].getEnabled() && aAddMenuItems[3].getEnabled() && aAddMenuItems[4].getEnabled(), true,
				"If selected node is a step, all menu items are enabled in add menu");
		oSelectedTreeNodeDetails = {
			nodeObjectType : modelerConstants.configurationObjectTypes.REPRESENTATION
		};
		this.toolbarController._enableDisableAddMenuItems(oSelectedTreeNodeDetails, addMenu);
		assert.equal(aAddMenuItems[0].getEnabled() && aAddMenuItems[1].getEnabled() && aAddMenuItems[2].getEnabled() && aAddMenuItems[3].getEnabled() && aAddMenuItems[4].getEnabled(), true,
	...
	QUnit.test("Test handleAddExistingStepPress", async function(assert) {
		var oAddButtonStub = {
			getSource : function() {
				return new Button();
			}
		...
		await this.toolbarController._pAddMenu;
		...
		await this.toolbarController._pAddExistingStepDialog;
		...
		var existingStepDialog = Element.getElementById(jQuery(".sapMDialog")[0].getAttribute("id"));
	...
	QUnit.test("Test _handleExistingStepDialogOK", function(assert) {
		...
		var existingStepList = new StandardListItem();
	...
	QUnit.test("Test handlePressAddButton", async function(assert) {
		...
		await this.toolbarController._pAddMenu; // wait for the addMenu fragment tobe fully loaded
		assert.equal(this.toolbarController.addMenu.isOpen(), true, "Add menu is open");
		oButton.destroy();
	});
	//Existing step tests are covered separately
	QUnit.test("Test handleAddMenuItemPress", async function(assert) {
		var oAddButtonStub = {
			getSource : function() {
				return new Button();
			}
		...
		this.toolbarController._handleAddMenuItemPress(oEvtStub);
		this.oConfigListInstance.oTreeInstance.expandToLevel(3);
		assert.equal(this.oConfigListInstance.oTreeInstance.getItems()[20].getTitle(), "< " + this.oModelerInstance.modelerCore.getText("newNavigationTarget") + " >", "The new node has the text <New Navigation Target>");
		this.oConfigListInstance.oTreeInstance.setSelectionOnTree("/aConfigDetails/0/configData/2/navTargets/2");
	});
	...
	QUnit.test("Test handlePressDeleteButton for configuration", async function(assert) {
		...
		await this.toolbarController._pConfirmationDialog;
	...
	QUnit.test("Test handlePressDeleteButton for facet filter", async function(assert) {
		var oDeleteButtonStub = {
			getSource : function() {
				return new Button();
			}
	...
	QUnit.test("Test handlePressDeleteButton for category", async function(assert) {
		var oDeleteButtonStub = {
			getSource : function() {
				return new Button();
			}
	...
	QUnit.test("Test handlePressDeleteButton for step", async function(assert) {
		var oDeleteButtonStub = {
			getSource : function() {
				return new Button();
			}
	...
	QUnit.test("Test handlePressDeleteButton for representation", async function(assert) {
		var oDeleteButtonStub = {
			getSource : function() {
				return new Button();
			}
	...
	QUnit.test("Test handlePressDeleteButton for navigation target", async function(assert) {
		var oDeleteButtonStub = {
			getSource : function() {
				return new Button();
			}
