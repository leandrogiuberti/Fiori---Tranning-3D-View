...
sap.ui.define(
	...
	function(modelerUIHelper, utils, Controller, View, ViewType, nextUIUpdate) {
		...
		async function _placeViewAt(oSFBView) {
		...
		async function _instantiateView(assert) {
			var oView;
			var oSFBRequestController = await Controller.create({
		...
		QUnit.module("For an existing smart filter", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					...
					oSFBRequestView = await _instantiateView(assert);
					done();
				});
			},
			afterEach : function() {
			...
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					...
					spyOnGetAllEntitySetsExceptParameterEntitySets = sinon.stub(oModelerInstance.configurationEditorForUnsavedConfig, "getAllEntitySetsExceptParameterEntitySets", function() {
						return utils.createPromise([ "entitySet2" ]); //entitySet1 is no more available
					});
					oSFBRequestView = await _instantiateView(assert);
					done();
				});
