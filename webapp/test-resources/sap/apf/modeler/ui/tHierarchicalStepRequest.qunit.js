	...
	"sap/ui/qunit/utils/nextUIUpdate"
], function(modelerUIHelper, utils, coreLibrary, Controller, View, ViewType, nextUIUpdate) {
		'use strict';
		...
		async function _instantiateView(sId, assert) {
			var oStepRequestController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.hierarchicalStepRequest"
			});
		...
		async function _placeViewAt(oStepRequestView) {
		...
		QUnit.module("When request option is instantiated for a hierarchical step", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					...
					oStepRequestView = await _instantiateView(oModelerInstance.firstHierarchicalStep, assert);
		...
		QUnit.test("When hierarchical step request is initialized", async function(assert) {
			//arrange
			await _placeViewAt(oStepRequestView);
		...
		QUnit.test("When a service which does not have hierarchical entity is entered", function(assert) {
			// TODO test is designed to be async (it uses two promise callbacks), but when monitoring the 
			// asynv behavior with assert.async(), then the test hangs. Obviously, the first promise
			// does not resolve as expected. Therefore adding the missing async monitoring only as comments
			// var done = assert.async();
			//arrange
			...
			oStepRequestView.getController().handleChangeForSourceAsPromise(_createEvent(oStepRequestView.byId("idSource"))).done(function() {
				oStepRequestView.getController().getAllEntitiesAsPromise("hierarchicalService2").done(function(aAllEntities) {
					...
					assert.strictEqual(oStepRequestView.byId("idSource").getValueState(), ValueState.Error, "then the value state of service input is Error");
		...
		QUnit.module("When request option is instantiated for a hierarchical step which has a required filter", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					oModelerInstance = modelerInstance;
					_commonSpiesInBeforeEach();
					oStepRequestView = await _instantiateView(oModelerInstance.hierarchicalStepWithFilter, assert);
					oStepRequestController = oStepRequestView.getController();
					spyOnResetEntityAndProperties = sinon.spy(oStepRequestController, "resetEntityAndProperties");
					done();
				});
		...
		QUnit.test("When hierarchical step request with required filter is initialized", async function(assert) {
			//arrange
			await _placeViewAt(oStepRequestView);
			//assert
		...
		QUnit.module("When the enetity, hierarchy property, non-hierarchy properties and selctable property are not available because of metadata unavailibility", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					...
					oStepRequestViewWithNotAvailable = await _instantiateView(oModelerInstance.hierarchicalStepWithFilterForNotAvailable, assert);
					done();
				});
			},
			afterEach : function() {
