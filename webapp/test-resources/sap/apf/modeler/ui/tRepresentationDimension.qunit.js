...
sap.ui.define([
...
], function(modelerConstants, textPoolHelper, utils, Input, Item, Controller, View, ViewType, nextUIUpdate, jQuery, modelerUIHelper){
	'use strict';
	...
	async function _placeViewAt(oRepDimensionView) {
	...
	async function _instantiateView(sId, assert, sProperty, oStep, bMandatory, sContext) {
		var aValidatorFields = [];
		var oDimensionsController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationDimension"
		});
		...
		oRepDimensionView = await View.create({
			...
			viewData : {
				...
				sPropertyType : modelerConstants.propertyTypes.DIMENSION
			}
		});
	...
	QUnit.module("For a Representation with mandatory property type of Dimension", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepDimensionView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, "property1", oModelerInstance.unsavedStepWithoutFilterMapping, true);
	...
	QUnit.test("When add icon of mandatory dimension is pressed", async function(assert) {
		//arrange
		...
		await _placeViewAt(oRepDimensionView);
	...
	QUnit.test("When add icon of mandatory dimension is pressed - when already one dimension exists", async function(assert) {
		//arrange
		var spyFireEvent = sinon.spy(oRepDimensionView, "fireEvent");
		var spyFireEventOnFocusOfAdd = sinon.spy(oRepDimensionView.byId("idAddPropertyIcon"), "fireEvent");
		oRepDimensionView.getViewData().oPropertyTypeState.getViewAt = function() {
	...
	QUnit.module("For a Representation with non-mandatory property type of Dimension, in a legend context", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepDimensionView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, "property1", oModelerInstance.unsavedStepWithoutFilterMapping, false, "legend");
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.module("For a Representation with non-mandatory property type of Dimension", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepDimensionView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, "property1", oModelerInstance.unsavedStepWithoutFilterMapping, false);
				var stubbedPropertyOrchestration = {
					getPropertyInformationList: function(){
						return [];
					},
	...
	QUnit.module("For a Representation with non-mandatory property type of Dimension set to None", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepDimensionView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[0], assert, "None", oModelerInstance.unsavedStepWithoutFilterMapping, false);
				var stubbedPropertyOrchestration = {
					getPropertyInformationList: function(){
						return [{
							sProperty: "property1",
	...
	QUnit.module("For a Representation with property type of mandatory Dimension and when text property is removed from the step level", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepDimensionView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[1], assert, "property2", oModelerInstance.unsavedStepWithoutFilterMapping, true);
				done();
			});
		},
		afterEach : function() {
