...
sap.ui.define([
...
], function(utils, Element, Fragment, Controller, View, ViewType, nextUIUpdate) {
	"use strict";
	 ...
	 * @param assert
	 */
	async function _instantiateView(context, settings, assert) {
		var oConfigurationController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.configuration"
		});
		...
		var textPool = {
				setTextAsPromise : function() {
					return utils.createPromise(utils.createPseudoGuid());
				}
		};
	...
	function _pressButtonOnFilterOptionChangeDialog(assert, buttonNumber, continuation) {
		var filterOptionChangeDialog = Fragment.byId("idFilterOptionChangeFragment", "idDialogWithTwoButtons");
		filterOptionChangeDialog.getButtons()[buttonNumber].firePress();
		assert.strictEqual(Element.getElementById("idFilterOptionChangeFragment--idDialogWithTwoButtons"), undefined, "then filter option dialog is destroyed");
		continuation();
	}
	...
	QUnit.module("Configuration Unit Tests - New configuration", {
		beforeEach : async function(assert) {
			...
			this.configurationView = await _instantiateView(this, settings, assert);
		},
		afterEach : function() {
			_destroyResources(this);
		}
	});
	QUnit.test("When configuration page is initialised", async function(assert) {
		//arrange
		await nextUIUpdate();
		// assert
	...
	QUnit.test("When changing filter option type to smart filter and press Continue in the filter option change dialog", async function(assert) {
		// arrange
		...
		assert.strictEqual(Fragment.byId("idFilterOptionChangeFragment", "idDialogWithTwoButtons").isOpen(), true, "then filter option change dialog is open");
		// act on dialog
	...
	QUnit.test("When changing filter option type to smart filter and press Cancel in the filter option change dialog", async function(assert) {
		// arrange
		this.configurationView.byId("idFilterTypeRadioGroup").setSelectedButton(this.configurationView.byId("smartFilterBar"));
		// act
		await loadedFragment(() => {
	...
	QUnit.test("When changing filter option type to none and press Continue in the filter option change dialog", async function(assert) {
		// arrange
	...
	QUnit.test("When changing filter option type to none and press Cancel in the filter option change dialog", async function(assert) {
		// arrange
		this.configurationView.byId("idFilterTypeRadioGroup").setSelectedButton(this.configurationView.byId("none"));
		// act
		await loadedFragment(() => {
	...
	QUnit.test("When changing filter option type to none and press escape in the filter option change dialog", async function(assert) {
	...
	QUnit.test("When changing filter option type to smart filter and press escape in the filter option change dialog", async function(assert) {
		var done = assert.async();
		// arrange
		this.configurationView.byId("idFilterTypeRadioGroup").setSelectedButton(this.configurationView.byId("smartFilterBar"));
		// act
