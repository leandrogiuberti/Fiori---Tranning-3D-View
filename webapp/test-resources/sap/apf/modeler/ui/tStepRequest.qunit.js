...
sap.ui.define([
	...
	var StepEvents = modelerConstants.events.step;
	...
	async function _instantiateView(sId, assert) {
		var oStepRequestController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.stepRequest"
		});
		...
		oStepRequestView = await View.create({
	...
	QUnit.module("For an existing step", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oStepRequestView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When source is changed and there are no common entities between new and old source", function(assert) {
		...
		assert.strictEqual(spyOnFireEvent.calledWith(StepEvents.SETDATAREDUCTIONSECTION), true, "then event to set data reduction is fired");
		assert.strictEqual(spyOnFireEvent.calledWith(StepEvents.SETVISIBILITYOFFILTERMAPPINGFIELDS), true, "then event to set visibility of filter mapping fields is fired");
		assert.strictEqual(spyOnFireEvent.calledWith(StepEvents.UPDATEFILTERMAPPINGFIELDS), true, "then event to update the filter mapping fields is fired");
	});
	...
	QUnit.test("When source is changed to an invalid service", function(assert) {
		...
		oStepRequestView.getViewData().oConfigurationEditor.registerServiceAsPromise("test11").done(function(result) {
			assert.strictEqual(oStepRequestView.byId("idSource").getValueState(), ValueState.None, "then the value state of service input is None");
		...
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				spyOnGetAllEntitySetsOfService = sinon.stub(oModelerInstance.configurationEditorForUnsavedConfig, "getAllEntitySetsOfServiceAsPromise", function() {
					return utils.createPromise([ "entitySet2", "entitySet3", "entitySet4" ]); //entitySet1 is no more available
				});
				oStepRequestView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping, assert);
				done();
			});
		...
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				spyOnGetAllPropertiesOfEntitySet = sinon.stub(oModelerInstance.configurationEditorForUnsavedConfig, "getAllPropertiesOfEntitySetAsPromise", function() {
					return utils.createPromise([ "property1", "property2", "property4", "property1Text", "property3Text" ]); //property 3 is not available due to change in metadata
				});
				oStepRequestView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping, assert);
				done();
			});
	...
	QUnit.module("Async loading of existing StepRequest", {
		beforeEach : async function(assert) {
			...
			var configurationEditor = {
				getAllEntitySetsOfServiceAsPromise : function(service){
					if(service === "TestService"){
						return utils.createPromise(["EntitySet1", "EntitySet2"], 1);
					}
				},
				getAllPropertiesOfEntitySetAsPromise : function(service, entitySet){
					if(service === "TestService" && entitySet === "EntitySet2"){
						return utils.createPromise(["property1", "selectableProperty"], 1);
					}
				}
			};
			var stepObject = {
			...
			var stepPropertyMetadataHandler = {
				getEntityTypeMetadataAsPromise : function(){
					return utils.createPromise(entityTypeMetadata, 1);
				},
			...
			this.oStepRequestController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.stepRequest"
			});
			View.create({
				viewName : "sap.apf.modeler.ui.view.requestOptions",
