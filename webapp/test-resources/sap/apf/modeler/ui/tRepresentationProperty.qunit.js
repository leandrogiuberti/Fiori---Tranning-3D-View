...
sap.ui.define([
...
], function(modelerUIHelper, modelerConstants, textPoolHelper, utils, deepExtend, Input, Device, Item, Controller, View, ViewType, nextUIUpdate, jQuery) {
	"use strict";
	...
	async function _placeViewAt(oRepPropertyView) {
	...
	async function _instantiateView (sId, assert, oStep, isHierarchicalStep, context) {
		var oPropertiesController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.representationProperty"
		});
		...
		oRepPropertyView = await View.create({
			...
			viewData : {
				...
				sPropertyType : modelerConstants.propertyTypes.PROPERTY
			}
		});
	...
	QUnit.module("For a Representation with property type of Property", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepPropertyView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[2], assert, oModelerInstance.unsavedStepWithoutFilterMapping, undefined, that);
	...
	QUnit.test("When suggestions are triggered for property label text", function(assert) {
		...
		sinon.stub(textPoolHelper, "SuggestionTextHandler", function() {
			return oSuggestionTextHandler;
		});
		var spyOnManageSuggestionTexts = sinon.spy(oSuggestionTextHandler, "manageSuggestionTexts");
		var oTranslationFormat = textPoolHelper.TranslationFormatMap.REPRESENTATION_LABEL;
		//action
		oRepPropertyView.getController().handleChangeForLabelText();
	...
	QUnit.test("When add icon of property is pressed", async function(assert) {
		//arrange
		var propertyInformationList = [{
			sProperty: "property1",
			sKind: 'column',
		...
		await _placeViewAt(oRepPropertyView);
		oRepPropertyView.getController().setNextPropertyInParentObject();
		//assert
		_commonTestForSetDetailData(assert, "property1");
		assert.strictEqual(oRepPropertyView.byId("idPropertyType").getSelectedKey(), "property1", "then value for property type is set as property1");
		assert.deepEqual(oRepPropertyView.byId("idPropertyType").getModel().getData(), oModelForPropertyType, "then model for property type is set");
		assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.ADDPROPERTY), true, "then ADDPROPERTY event is fired");
		assert.strictEqual(spyFireEventOnFocusOfAdd.calledWith(modelerConstants.events.SETFOCUSONADDICON), true, "then SETFOCUSONADDICON event is fired");
	...
	QUnit.test("When _shallAddPropertyBeHandled returns true", function (assert) {
		...
		oRepPropertyView.byId("idAddPropertyIcon").firePress(); // system under test
		// verify
		assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.ADDPROPERTY), true, "then ADDPROPERTY event is fired for further processing the pressing of add property icon");
	});
	QUnit.test("When _shallAddPropertyBeHandled returns false", function (assert) {
		...
		oRepPropertyView.byId("idAddPropertyIcon").firePress(); // system under test
		// verify
		assert.strictEqual(spyFireEvent.calledWith(modelerConstants.events.ADDPROPERTY), false, "then ADDPROPERTY event is not fired and the pressing of add property icon is not processed");
	});
	...
	QUnit.module("Treetable representation with property type property", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oRepPropertyView = await _instantiateView(oModelerInstance.firstHierarchicalStep.getRepresentations()[0], assert, oModelerInstance.firstHierarchicalStep, true, that);
				that.stubbedPropertyOrchestration = {
					getPropertyInformationList: function(){
						return [];
					},
	...
	QUnit.test("When add icon of property is pressed for hierarchical step", async function(assert) {
		//arrange
		var propertyInformationList = [{
	...
	async function browserTestsCommonSetup(assert, context, browserName, browserVersion){
		var done = assert.async();
		if (browserVersion){
		...
		Device.browser.name = browserName;
		oRepPropertyView = await _instantiateView(oModelerInstance.unsavedStepWithoutFilterMapping.getRepresentations()[2], assert, oModelerInstance.unsavedStepWithoutFilterMapping, undefined, context);
	...
	QUnit.module("Browser specific functionality", {
		beforeEach : function(assert){
			var done = assert.async();
			this.oBrowserSettings = deepExtend({}, Device.browser);
		...
		afterEach : function(){
			Device.browser = this.oBrowserSettings;
			_commonCleanUpsInAfterEach();
		}
	});
	QUnit.test("When Property view is initialized on Internet Explorer 11", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "ie", 11);
	...
	QUnit.test("When Property view is initialized on Internet Explorer version below 11 (IE10)", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "ie", 10);
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
	QUnit.test("When Property view is initialized on Internet Explorer version below 11 (IE10.1)", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "ie", 10.1);
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
	QUnit.test("When Property view is initialized on Google Chrome", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "cr");
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
	QUnit.test("When Property view is initialized on Mozilla Firefox", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "ff");
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
	QUnit.test("When Property view is initialized on Edge Browser", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "ed");
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
	QUnit.test("When Property view is initialized on Safari", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "sf");
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
	QUnit.test("When Property view is initialized on Android Browser", async function(assert) {
		oRepPropertyView = await browserTestsCommonSetup(assert, this, "an");
		assert.strictEqual(oRepPropertyView.byId("idAriaPropertyForBasicDataGroup").getText(), "", "then text for aria label is not set");
	});
});