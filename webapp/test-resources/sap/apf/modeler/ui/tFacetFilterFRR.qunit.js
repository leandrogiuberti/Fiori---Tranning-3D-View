...
sap.ui.define([
	...
	], function(FacetFilterController, modelerConstants, modelerUIHelper, Controller, View, ViewType, jQuery) {
	'use strict';
	...
	async function _instantiateView(sId, assert) {
		var oFRRController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.facetFilterFRR"
		});
		var spyOnInit = sinon.spy(oFRRController, "onInit");
		oFRRView = await View.create({
	...
	QUnit.module("For a facet filter with existing FRR", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oFRRView = await _instantiateView(oModelerInstance.facetFilterUnsaved, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When source is changed and there are no common entities between new and old source", function(assert) {
		...
		assert.deepEqual(spyOnRemoveSelectPropForValueHelp.calledWith("property3"), true, "then existing select property of filter resolution is removed");
		setTimeout(function(){
