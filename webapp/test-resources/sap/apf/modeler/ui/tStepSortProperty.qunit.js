...
sap.ui.define([
	...
	async function _placeViewAt(oStepSortPropertyView) {
	...
	async function _instantiateStepSortPropertyView(oParentObject, assert, bHasMultipleSortProperty) {
		...
		var oStepSortPropertyController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.stepSortPropertyType"
		});
		...
		oStepSortPropertyView = await View.create({
	...
	function _commonSpiesForTest() {
		oStepPropertyMetadataHandler = new StepPropertyMetadataHandler(oModelerInstance.modelerCore, oModelerInstance.unsavedStepWithoutFilterMapping);
	...
	QUnit.module("Sort property for step - One sort property", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oStepSortPropertyView = await _instantiateStepSortPropertyView(oModelerInstance.unsavedStepWithoutFilterMapping, assert, false);
	...
	QUnit.test("When a sort row is added", async function(assert) {
		...
		await _placeViewAt(oStepSortPropertyView);
		//assert
	...
	QUnit.module("Sort property for step - Multiple sort properties", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oStepSortPropertyView = await _instantiateStepSortPropertyView(oModelerInstance.unsavedStepWithoutFilterMapping, assert, true);
				spyFireEvent = sinon.spy(oStepSortPropertyView, "fireEvent");
				done();
			});
		},
	...
	QUnit.module("Sort property for step - When properties are not available", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oStepSortPropertyView = await _instantiateStepSortPropertyView(oModelerInstance.unsavedStepWithoutFilterMapping, assert);
				spyFireEvent = sinon.spy(oStepSortPropertyView, "fireEvent");
				done();
			});
		},
