...
sap.ui.define(
	...
	function(modelerUIHelper, utils, Controller, View, ViewType, nextUIUpdate) {
		"use strict";
		...
		async function _instantiateView(sId, assert) {
			var oNavTargetContextmappingController = await Controller.create({
				name : "sap.apf.modeler.ui.controller.navTargetContextMapping"
			});
		...
		QUnit.module("For a navigation target with existing context mapping service", {
			beforeEach : function(assert) {
				...
				modelerUIHelper.getModelerInstance(async function(modelerInstance) {
					...
					oNavTargetContextmappingView = await _instantiateView(oModelerInstance.firstNavigationtarget, assert);
					done();
				});
			},
			afterEach : function() {
