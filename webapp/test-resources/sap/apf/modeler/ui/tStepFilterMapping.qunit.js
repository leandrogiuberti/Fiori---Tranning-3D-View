...
sap.ui.define([
	...
	async function _instantiateView(oStep, assert) {
		var oStepFilterMappingController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.stepFilterMapping"
		});
		var spyOnInit = sinon.spy(oStepFilterMappingController, "onInit");
		oStepFilterMappingView = await View.create({
	...
	QUnit.module("For a step with existing filter mapping service", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				oStepFilterMappingView = await _instantiateView(oModelerInstance.unsavedStepWithFilterMapping, assert);
				done();
			});
		},
		afterEach : function() {
	...
	QUnit.test("When source selected from value help in the input field", async function(assert) {
		//arrange
		sinon.spy(View, "create");
		//action
		oStepFilterMappingView.byId("idSource").fireValueHelpRequest();
		assert.strictEqual(View.create.callCount, 1, "View.create has been called exactly once");
		...
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				spyOnGetAllEntitySetsOfServiceWithGivenProperties = sinon.stub(oModelerInstance.configurationEditorForUnsavedConfig, "getAllEntitySetsOfServiceWithGivenPropertiesAsPromise", function() {
					return utils.createPromise([ "entitySet2", "entitySet3", "entitySet4" ]); //entitySet10 is no more available
				});
				oStepFilterMappingView = await _instantiateView(oModelerInstance.unsavedStepWithFilterMapping, assert);
				done();
			});
	...
	QUnit.module("For a step with existing existing filter mapping service - Validate previously selected properties", {
		beforeEach : function(assert) {
			...
			modelerUIHelper.getModelerInstance(async function(modelerInstance) {
				...
				spyOnGetAllPropertiesOfEntitySet.withArgs("testService4", "entitySet10").returns(utils.createPromise([ "property3", "property13" ]));
				spyOnGetAllPropertiesOfEntitySet.withArgs("testService4", "entitySet12").returns(utils.createPromise([ "property5", "property6", "property7" ]));
				spyOnGetAllPropertiesOfEntitySet.withArgs("testService4", "entitySet4").returns(utils.createPromise([ "property1", "property2", "property6", "property8" ]));
				spyOnGetAllPropertiesOfEntitySet.withArgs("testService4", "entitySet5").returns(utils.createPromise([ "property3", "property9" ]));
				oStepFilterMappingView = await _instantiateView(oModelerInstance.unsavedStepWithFilterMapping, assert);
				done();
			});
		},
	...
	async function commonSetupHelper (context, targetProperties, displayOption, labelKey, serviceNotDefined, entitySetNotDefined){
		context.oStep = commonStepSetup(targetProperties, labelKey, displayOption, serviceNotDefined, entitySetNotDefined);
		context.oStepFilterMappingController = await Controller.create({
			name : "sap.apf.modeler.ui.controller.stepFilterMapping"
			});
		context.oStepFilterMappingView = await View.create({
			viewName : "sap.apf.modeler.ui.view.requestOptions",
			type : ViewType.XML,
	...
	function commonStepSetup (targetProps, labelKey, displayOption, serviceNotDefined, entitySetNotDefined) {
		var messageHandler = new MessageHandlerDouble().raiseOnCheck().spyPutMessage();
		var inject = {
			instances : {
	...
	QUnit.module("For a step with existing filter mapping service: Filter Mapping Target Property display options", {
		commonSetup : async function(assert, targetProperties, displayOption, labelKey, serviceNotDefined, entitySetNotDefined){
			await commonSetupHelper(this, targetProperties, displayOption, labelKey, serviceNotDefined, entitySetNotDefined);
	...
	QUnit.test("When Service is not defined", async function(assert){
		await this.commonSetup(assert, [], undefined, undefined, true);
	...
	QUnit.test("When Entity Set is not defined", async function(assert){
		await this.commonSetup(assert, [], undefined, undefined, undefined, true);
		testDefaultUIState(assert, this, true);
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
		assert.strictEqual(displayOptionsElement.getEnabled(), false, "then the display options list is disabled");
	});
	QUnit.test("When Target Property is empty", async function(assert){
		await this.commonSetup(assert, []);
		testDefaultUIState(assert, this, true);
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
		assert.strictEqual(displayOptionsElement.getEnabled(), false, "then the display options list is disabled");
	});
	QUnit.test("Target Property is not empty and has no text property", async function(assert){
		await this.commonSetup(assert, ["propertyWithoutText"]);
		testDefaultUIState(assert, this, false);
	});
	QUnit.test("Target Property has no text property but text was selected as label display option", async function(assert){
		await this.commonSetup(assert, ["propertyWithoutText"], "text");
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
	...
	QUnit.test("Target Property has no text property but key and text was selected as label display option", async function(assert){
		await this.commonSetup(assert, ["propertyWithoutText"], "keyAndText");
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
	...
	QUnit.test("Target Property is not empty and has text property", async function(assert){
		await this.commonSetup(assert, ["propertyWithText"]);
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
		assert.strictEqual(displayOptionsElement.getSelectedItem().getKey(), "key", "Selected item in list is Key");
		assert.strictEqual(displayOptionsElement.getSelectedItem().getText(), "text : key", "Selected item in list is Key");
		assert.strictEqual(displayOptionsElement.getItemAt(0), displayOptionsElement.getSelectedItem(), "First item in list in the key item");
	...
	QUnit.test("Target Property has text property and text was selected as label display option", async function(assert){
		await this.commonSetup(assert, ["propertyWithText"], "text");
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
	...
	QUnit.test("Target Property has text property and key and text was selected as label display option", async function(assert){
		await this.commonSetup(assert, ["propertyWithText"], "keyAndText");
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
	...
	QUnit.test("Target Property has text property and label display option changed from key and text to key", async function(assert){
		await this.commonSetup(assert, ["propertyWithText"], "keyAndText");
		var displayOptionsElement = this.oStepFilterMappingView.byId("idOptionalLabelDisplayOptionType");
	...
	QUnit.module("For a step with existing filter mapping service: Filter Mapping Target Property label field", {
		commonSetup : async function(assert, targetProperties, labelKey, displayOption, serviceNotDefined, entitySetNotDefined) {
			await commonSetupHelper(this, targetProperties, labelKey, displayOption, serviceNotDefined, entitySetNotDefined);
		},
		afterEach : function() {
			this.oStepFilterMappingView.destroy();
		}
	});
	QUnit.test("When Service is undefined", async function(assert){
		await this.commonSetup(assert, [], undefined, undefined, true);
	...
	QUnit.test("When Entity Set is undefined", async function(assert){
		await this.commonSetup(assert, [], undefined, undefined, undefined, true);
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
		var targetPropertyElement = this.oStepFilterMappingView.byId("idOptionalRequestField");
	...
	QUnit.test("When Target Property is not set", async function(assert){
		await this.commonSetup(assert, []);
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
		var targetPropertyElement = this.oStepFilterMappingView.byId("idOptionalRequestField");
	...
	QUnit.test("When Target Property is set", async function(assert){
		await this.commonSetup(assert, ["property"]);
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
	...
	QUnit.test("When Target Property is set and label text is changed", async function(assert){
		await this.commonSetup(assert, ["property"], "labelText1");
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
	...
	QUnit.test("When Target Property is changed", async function(assert){
		await this.commonSetup(assert, ["property1"], "property1", "key");
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
		var targetPropertyElement = this.oStepFilterMappingView.byId("idOptionalRequestField");
	...
	QUnit.test("When Source is cleared", async function(assert){
		await this.commonSetup(assert);
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
	...
	QUnit.test("When EntitySet is changed", async function(assert){
		await this.commonSetup(assert, ["property"]);
		var labelTextElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabelText");
		var labelElement = this.oStepFilterMappingView.byId("idOptionalSelectedPropertyLabel");
