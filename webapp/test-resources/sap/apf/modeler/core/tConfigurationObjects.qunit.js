...
sap.ui.define([
	...
	'use strict';
	var measureDisplayOptions = coreConstants.representationMetadata.measureDisplayOptions;
	var displayOptions = coreConstants.representationMetadata.labelDisplayOptions;
	...
	QUnit.module("Serialization", {
		beforeEach : function() {
			...
			this.editor = new ConfigurationEditor("apf1972-tempId", {
				instances : {
					messageHandler : this.messageHandler,
			...
			this.configurationObjects = new ConfigurationObjects(this.inject);
		}
	});
	QUnit.test("WHEN serializeCategory THEN validates", function(assert) {
		var factory = new ConfigurationObjects(this.inject);
	...
	QUnit.test("serializeConfiguration filter option 'facetFilter'", function(assert) {
		this.editor.setFilterOption({facetFilter : true});
		var obj = this.configurationObjects.serializeConfiguration(this.editor);
		assert.ok(Array.isArray(obj.facetFilters), "Property facetFilters is empty array");
	});
	...
	QUnit.module("TopN", {
		beforeEach : function() {
			...
			this.editor.setApplicationTitle("appTitleTextKey");
			this.factory = new ConfigurationObjects(this.inject);
	...
	QUnit.test("ConfigurationFactory Registry", function(assert) {
		...
		var factory = new ConfigurationFactory(inject);
		...
		assert.equal(this.registry instanceof Hashtable, true, "probe receives hashtable");
	});
	QUnit.test("GIVEN class RegistryWrapper - inject to ConfigurationFactory", function(assert) {
		var inject = {
			...
			constructors : {
				RegistryProbe : RegistryWrapper
			}
		};
		var factory = new ConfigurationFactory(inject);
		assert.equal(factory.getRegistry() instanceof RegistryWrapper, true, "WHEN Inject Registry Wrapper to ConfigurationFactory THEN getRegistry is defined");
	...
	QUnit.test("test getItem .. GIVEN RegistryWrapper", function(assert) {
		...
		var hashtable = new Hashtable(this.messageHandler);
		hashtable.setItem(step.id, step);
		hashtable.setItem(category.id, category);
		var registry = new RegistryWrapper(hashtable);
	...
	QUnit.test('Get facetFilters returns indicator for empty facet filter array', function(assert) {
		var hashtable = new Hashtable(this.messageHandler);
		hashtable.setItem(coreConstants.existsEmptyFacetFilterArray, true);
		var registry = new RegistryWrapper(hashtable);
	...
	function commonProducer(){
		...
		this.RegistryProducer = function() {
			var hash = new Hashtable(that.messageHandler);
			var configuration = "";
			var producer = new ConfigurationEditor("apf1972-producer", that.editorInject);
			/**
	...
	QUnit.module("Map to design time", {
		beforeEach : function() {
			...
			this.messageHandler = new MessageHandlerDouble().raiseOnCheck().spyPutMessage();
			this.configurationObjects = new ConfigurationObjects({
				instances : {
					messageHandler : this.messageHandler,
					textPool : {
			...
			this.configurationEditor = new ConfigurationEditor("apf1972-tempId", this.editorInject);
			this.configurationEditor.registerServiceAsPromise = function(service) {
				
				return utils.createPromise(false);
			};
	...
	QUnit.test("Deserialize Step", function(assert) {
		...
		this.configurationEditor.registerServiceAsPromise = function(service) {
			...
			return utils.createPromise(null);
		};
		this.configurationObjects.mapToDesignTimeAsPromise(this.registry, this.configurationEditor).then(function(){
	...
	QUnit.test('Deserialize empty facet filter array', function(assert) {
		var done = assert.async();
		var registryWrapper;
		var registry = new Hashtable(this.messageHandler);
		registry.setItem('applicationTitle', {});
		registry.setItem(coreConstants.existsEmptyFacetFilterArray, true);
		registryWrapper = new RegistryWrapper(registry);
	...
	QUnit.module("Serializable configuration object", {
		beforeEach : function() {
			...
			var producer = new ConfigurationEditor("apf1972-producer", this.editorInject);
	...
	QUnit.test("GIVEN a serializable configuration object", function(assert) {
		...
		var expectedResult = [  "textKeyOfApplicationTitle",
		                        ...
		                        "labelKey-1100"];
		var textKeys = ConfigurationObjects.getTextKeysFromConfiguration(this.configuration);
		assert.deepEqual(textKeys, expectedResult, "WHEN ConfigurationObjects.getTextKeysFromConfiguration() THEN the right text keys are returned");
	});
	...
	QUnit.test("WHEN deepDataCopy", function(assert) {
		...
		var result = ConfigurationObjects.deepDataCopy(input);
		...
		assert.equal(ConfigurationObjects.deepDataCopy(func), func, "Function attribute is tranferred directly to copy result");
	});
	QUnit.module("Texts", {
		beforeEach : function() {
			var that = this;
