...
sap.ui.define([
	...
	function getXsrfToken() {
		...
		return utils.createPromise("Dummy");
	}
	...
	QUnit.test("T1: Subsequent calls return non-empty array", function(assert) {
		...
		var coreInstance = new ModelerCoreInstance({
	...
	QUnit.module("M2: Configuration Modeler", {
		...
		afterEach : function(assert) {
			if (isMockServerActive) {
				...
				modelerHelper.removeApplication(this, assert);
			}
		},
		appA : {
	...
	QUnit.test("T4: Delete saved configuration", function(assert) {
		...
		function callbackRemoveConfiguration(id, metadata, messageObject) {
			...
			modelerHelper.createApplicationHandler(that, function(appHandler) {
	...
	QUnit.module("M3: Configuration Modeler with test helper: Create and save configuration and reload with new configuration editor", {
		beforeEach : function(assert) {
			...
			modelerHelper.createConfigurationEditorWithSave(isMockServerActive, that, that.appA, callbackForSetup, callbackAfterSave);
		...
		afterEach : function(assert) {
			if (isMockServerActive) {
				coreConstants.developmentLanguage = '';
				mockServer.deactivate();
			} else {
				modelerHelper.removeApplication(this, assert);
			}
	...
	QUnit.test("Open saved configuration with filter option 'none'", function(assert) {
		...
		var done = assert.async();
		var that = this;
		modelerHelper.createConfigurationEditorWithSave(isMockServerActive, this, this.appA, callbackForSetup, callbackAfterSave);
		function callbackForSetup() {
			that.configurationEditor.setApplicationTitle("testAppTitleKey");
	...
	QUnit.test("T9: Open saved configuration in runtime and test, whether the steps and texts are available", function(assert) {
		...
		var oStartParameter = new StartParameter(componentDouble);
		...
		var sUrl = helper.determineTestResourcePath() + "/modeler/integration/applicationConfig.json";
	...
	QUnit.module("M4: Configuration Modeler with test helper: Create and save configuration, export configuration and texts to file", {
		beforeEach : function(assert) {
			...
			this.coreApi = new CoreInstance.constructor({
				instances: {
			...
			modelerHelper.createConfigurationEditorWithSave(isMockServerActive, that, that.appA, callbackForSetup, callbackAfterSave);
			if (isMockServerActive) {
				this.isValidGuid = utils.isValidGuid;
				utils.isValidGuid = function(guid) {
					return true;
				};
			}
			function callbackForSetup() {
		...
		afterEach : function(assert) {
			if (isMockServerActive) {
				coreConstants.developmentLanguage = '';
				mockServer.deactivate();
				utils.isValidGuid = this.isValidGuid;
			} else {
				modelerHelper.removeApplication(this, assert);
			}
		},
	...
	QUnit.test("T1: Export saved configuration and texts to file", function(assert) {
		...
		assert.equal(configuration.configHeader.UI5Version, Global.version, "Right UI5 version");
	...
	QUnit.test("T3: Import configuration - application id and configuration id does not exist", function(assert) {
		...
		var done = assert.async();
		var that = this;
		var applicationOfThisTest = helper.generateGuidForTesting(); // Needs to be a "GUID": the server will check for /[0-9A-F]{32}/
		var configurationOfThisTest = helper.generateGuidForTesting(); // Needs to be a "GUID": the server will check for /[0-9A-F]{32}/
	...
	QUnit.test("T4: Import configuration - application id exists, configuration id does not exist", function(assert) {
		...
		this.configId = helper.generateGuidForTesting(); // Needs to be a "GUID": the server will check for /[0-9A-F]{32}/
	...
	QUnit.module("M5: Import texts", {
		beforeEach : function(assert) {
			coreConstants.developmentLanguage = 'NN';
			...
			if (isMockServerActive) {
				...
				var done = assert.async();
				this.authTestHelper = new AuthTestHelper(done, function() {
					done();
				});
			}
		},
		...
		doCreateApplication : function(applicationGuid, oMessageHandler, assert) {
			...
			getXsrfToken().done(function(sXsrfToken){
				...
				odataRequestWrapper(oInject, oRequest, function(oData, oResponse) {
		...
		function localPreparations() {
			...
			that.doDeleteApplication(that.applicationId, that.messageHandler).done(function(){
				...
				that.doCreateApplication(that.applicationId, that.messageHandler, assert).done(function(){
					...
					that.modelerInstance = new ModelerCoreInstance(persistenceConfiguration, inject);
					deferred.resolve();
				});
			});
			return deferred.promise();
	...
	QUnit.module("M6: Integration Test for Memorize", {
		beforeEach : function(assert) {
			...
			var injectForConfObjects = {
					instances : {
						persistenceProxy : new OdataProxy(this.persistenceConfiguration, injectForPersistenceProxy),
						messageHandler : this.messageHandler
					},
					constructors : {
						Hashtable : Hashtable
			...
			this.configurationObjects = new ConfigurationObjects(injectForConfObjects);
		},
		afterEach : function(assert) {
			if (isMockServerActive) {
				coreConstants.developmentLanguage = '';
	...
	QUnit.test("T3: GIVEN Two saved configurations with two unused text keys WHEN getUnusedTextKeys ", function(assert) {
		...
		textPool.setTextAsPromise("Unused text 1", {
			...
			textPool.setTextAsPromise("Unused text 2", {
				...
				var modelerInstance = new ModelerCoreInstance(that.persistenceConfiguration);
	...
	QUnit.test("All Services finished registering after load", function(assert){
		...
		function executeTest(){
			var configEditor = new ConfigurationEditor(that.config, that.inject, function(){
				setTimeout(function(){
