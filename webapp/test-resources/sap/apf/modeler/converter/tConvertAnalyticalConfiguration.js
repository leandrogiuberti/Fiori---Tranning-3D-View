...
sap.ui.define([
	...
	QUnit.module("Setup " + WCAType, {
		...
		beforeEach : function(assert) {
			...
			that.getTextFormatFromPath = function(path) {
				...
				switch (path) {
				case ".steps.title":
					resultValue = textPoolHelper.TranslationFormatMap.STEP_TITLE;
					break;
				case ".steps.longTitle":
					resultValue = textPoolHelper.TranslationFormatMap.STEP_LONG_TITLE;
					break;
				case ".steps.thumbnail.leftLower":
					resultValue = textPoolHelper.TranslationFormatMap.STEP_CORNER_TEXT;
					break;
				...
				case ".categories.label":
					resultValue = textPoolHelper.TranslationFormatMap.CATEGORY_TITLE;
					break;
				case ".facetFilters.label":
					resultValue = textPoolHelper.TranslationFormatMap.FACETFILTER_LABEL;
					break;
			...
			this.mapTextsFromRessourceBundleToTextPool = function(textKeyObjects) {
				var that = this,
					result = new Hashtable(this.messageHandler),
					uniqueTextKeyObjects = [],
					ht = new Hashtable(this.messageHandler),
					htPath = new Hashtable(this.messageHandler),
					textElements, i;

				// make textKeyObjects unique and extract unique pathes
				textKeyObjects.forEach(function(textKeyObject) {
					if (textKeyObject.key !== CoreConstants.textKeyForInitialText && !ht.hasItem(textKeyObject.key)) {
			...
			var url1 = "test-resources/sap/apf/modeler/converter/" + WCAType + "ApplicationConfiguration.json",
				url2 = "test-resources/sap/apf/modeler/converter/" + WCAType + "AnalyticalConfiguration.json",
				url3 = "test-resources/sap/apf/modeler/converter/" + WCAType + "Application.properties";
			...
			utils.migrateConfigToCategoryStepAssignment(this.analyticalConfiguration, {
				instances : {
			...
			that.authTestHelper = new AuthTestHelper(done, function() {
				modelerHelper.createConfigurationEditor(that, that.appA, that.serviceRoot, function() {
					done();
				}, assert, done);
			});
		},
	...
	QUnit.test("Convert " + WCAType + " config", function(assert) {
		...
		that.modelerCore.getConfigurationHandler(that.applicationCreatedForTest, function(configurationHandler, messageObject) {
			...
			assert.ok(true, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			// save <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
			textPool.setText(that.analyticalConfiguration.appDescription, textPoolHelper.TranslationFormatMap.APPLICATION_TITLE).done(function(textKey){
