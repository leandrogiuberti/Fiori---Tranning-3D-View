...
sap.ui.define([
	...
	"sap/apf/utils/utils",
	"./ModelerIntegrationConfigForImportTesting",
	"./TesthelperConfigConfigurationForIntegrationTesting"
], function(
	MessageHandler,
	Hashtable,
	utils,
	configForTesting, // currently undefined
	ConfigurationForIntegrationTesting // currently undefined
) {
	'use strict';
	...
	QUnit.module("Migrate analytical configuration", {
		beforeEach : function(assert) {
			...
			this.loadFile = function(filename) {
				var url = "test-resources/sap/apf/modeler/migration/" + filename;
				var returnValue;
				jQuery.ajax({
					url : url,
					dataType : "json",
			...
			this.migrate = function(filename) {
				...
				if (oConfig instanceof Array) {
					oConfig.forEach(function(oTableEntry) {
						...
						utils.migrateConfigToCategoryStepAssignment(oConfForMigration, that.inject);
						oTableEntry.SerializedAnalyticalConfiguration = JSON.stringify(oConfForMigration);
					});
				} else {
					utils.migrateConfigToCategoryStepAssignment(oConfig, this.inject);
				}
