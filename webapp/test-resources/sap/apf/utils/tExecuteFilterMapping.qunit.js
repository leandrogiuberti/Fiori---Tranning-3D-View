sap.ui.define([
...
], function(MessageObject, executeFilterMapping, MessageHandlerDouble, Filter, UtilsFilter){
	"use strict";
	...
	QUnit.test('WHEN executeFilterMapping with one target property and succeeding mapping request THEN the right result filter is returned', function(assert) {
		var that = this;
		assert.expect(3);
		var oFilter1 = new Filter(this.oMessageHandler, "targetProperty1", this.EQ, "A");
		var oFilter2 = new Filter(this.oMessageHandler, "targetProperty1", this.EQ, "C");
		var oExpectedFilter = new Filter(this.oMessageHandler, oFilter1).addOr(oFilter2);
	...
	QUnit.test('WHEN executeFilterMapping with with two target properties and succeeding mapping request THEN the right result filter is returned', function(assert) {
		...
		assert.expect(3);
		var oFilter1 = new Filter(this.oMessageHandler, "targetProperty1", this.EQ, "A");
		var oFilter2 = new Filter(this.oMessageHandler, "targetProperty2", this.EQ, "B");
		var oFilter3 = new Filter(this.oMessageHandler, "targetProperty1", this.EQ, "C");
		var oFilter4 = new Filter(this.oMessageHandler, "targetProperty2", this.EQ, "D");
		var oFilterAnd1 = new Filter(this.oMessageHandler, oFilter1).addAnd(oFilter2);
		var oFilterAnd2 = new Filter(this.oMessageHandler, oFilter3).addAnd(oFilter4);
		var oExpectedFilter = new Filter(this.oMessageHandler, oFilterAnd1).addOr(oFilterAnd2);
		function fnCallback(oMappedFilter, oMessageObject, oResponseData) {
			assert.ok(!oMessageObject, "no message object is returned");
			assert.equal(oMappedFilter.toUrlParam(), oExpectedFilter.toUrlParam(), "the right filter is returned to the callback from executeFilterMapping");
			assert.deepEqual(oResponseData, that.oResponse.data, "Callback is called with responseData");
