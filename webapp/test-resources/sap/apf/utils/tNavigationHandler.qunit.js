...
sap.ui.define([
	...
	"sap/ui/model/Filter"
], function(NavigationHandler, MessageHandlerDouble, ushellHelper, CoreFilter, UtilsFilter, HashChanger, MessageObject, Ui5Filter){
	'use strict';
	function commonSetup(testEnv, navigationTargets, navigationTargetsAssignedToActiveStep, buildComplexCummulativeFilter, FilterReduction, getCumulativeFilterFunction) {
		testEnv.messageHandler = new MessageHandlerDouble().doubleCheckAndMessaging().spyPutMessage();
	...
	QUnit.test("WHEN navigateToApp is called", function(assert) {
		...
		var expectedCumulatedFilterWithoutFilterReduction = 
			...
			var id = ushellHelper.spys.setData.selectionVariant.SelectionVariantID;
	...
	QUnit.test("SelectionVariant if filterreduction is turned off", function (assert) {
		...
		var id = selectionVariant.SelectionVariantID;
		var expect = {
			SelectionVariantID: id,
			Parameters: [],
			SelectOptions: [{
