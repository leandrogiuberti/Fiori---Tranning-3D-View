...
sap.ui.require([
	...
	function authenticate(connector, assert, furtherTask) {
		...
		var promise = connector.send('/sap/bc/lrep/content/sap/apf/dt', "GET", {}, 
				{ username : userData.lrep.user, password : userData.lrep.password });
	...
	function createLayeredRepositoryProxy(context) {
		...
		context.proxy = new LayeredRepositoryProxy({}, injectForPersistenceProxy);
	}
	...
	QUnit.test("WHEN reading all applications in CUSTOMER layer", function(assert) {
		...
		function callbackFromReadAllApplications(applications, metadata, messageObject) {
			...
			applications.forEach(function(application) {
				assert.ok(utils.isValidPseudoGuid(application.Application), application.Application + " application guid is returned in correct format");
	...
	QUnit.test("Create Analytical Application, Configuration, Texts with Connector", function(assert) {
		...
		function callbackFromUpdateAnalyticalConfiguration(metadata, messageObject) {
			...
			var filterApplication = new CoreFilter(that.messageHandler, 'Application', 'eq', that.expectedApplication);
			var filterLanguage = new CoreFilter(that.messageHandler, 'Language', 'eq', CoreConstants.developmentLanguage);
		...
		function updateAnalyticalConfiguration(config) {
			...
			function callbackFromCreate(result) {
				assert.ok(utils.isValidPseudoGuid(result.TextElement), "THEN GUID for text element is returned");
			}
			for(var i = 0; i < 3; i++) {
				data = {
		...
		function callbackFromReadEntityOfConfiguration(configurationData, metadata, messageObject) {
			...
			var expectedConfig = sampleConfiguration.getSampleConfiguration();
		...
		function createAnalyticalConfiguration(Application) {
			...
			var analyticalConfiguration = sampleConfiguration.getSampleConfiguration();
			var config = {
	...
	QUnit.test("CREATE AND READ analytical Configuration in VENDOR LAYER and copy to CUSTOMER layer", function(assert){
		...
		function callbackFromCreateVendorConfiguration(configurationData, metadata, messageObject) {
			...
			assert.ok(configurationData && configurationData.AnalyticalConfiguration && utils.isValidPseudoGuid(configurationData.AnalyticalConfiguration), "THEN the Analytical Configuration Guid is returned");
	...
	QUnit.module('Import Configuration from VENDOR layer', {
		beforeEach : function(assert) {
			...
			function prepareVendorConfiguration (done) {
				...
				function callbackFromCreateVendorConfiguration(configurationData, metadata, messageObject) {
					...
					var filterApplication = new CoreFilter(that.messageHandler, 'Application', 'eq', that.VendorApplication);
					var filterLanguage = new CoreFilter(that.messageHandler, 'Language', 'eq', CoreConstants.developmentLanguage);
					filterLanguage.addAnd(filterApplication);
		...
		initProxy : function(context, assert) {
				...
				var persistenceServiceRoot = CoreConstants.modelerPersistenceServiceRoot;
				...
				Object.setPrototypeOf(inject.instances.component, UIComponent.prototype);
				inject.constructors = {
						PersistenceProxy : LayeredRepositoryProxy
				};
				inject.functions = {
				...
				context.coreApi = new ModelerCoreInstance(persistenceConfiguration, inject);
	...
	function createTextPropertyFile(applicationId, messageHandler) {
		...
		var parseResult = parseTextPropertyFile(textString, { instances : { messageHandler : messageHandler}});
		var textElements = parseResult.TextElements;
		var textsTable = new Hashtable(messageHandler);
