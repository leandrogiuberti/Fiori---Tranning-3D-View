...
function (MockServer, StandardConfiguration, SFBConfiguration, HierarchyConfiguration, deepExtend, jQuery, Storage, require) {
	'use strict';
	...
	QUnit.test("Given * content in session storage, When getAllConfigurationIdsInStorage", function(assert) {
		function createConfigInStorage(configId1) {
			...
			var localStorage = new Storage(Storage.Type.session);
	...
	QUnit.test("GET without any URL parameters", function(assert) {
		jQuery.ajax({
			...
			async: false,
			success: function() {
				assert.ok(true, "query was successful");
			},
			error: function() {
				assert.ok(false, "request failed unexpectedly");
			}
		});
	});
	...
	QUnit.test("GET aggregated query with inlinecount", function(assert) {
		jQuery.ajax({
			...
			success: function(data) {
				...
				assert.notStrictEqual(data.d.__count, undefined,
					"then the inline count is present");

				assert.strictEqual(data.d.results.length, data.d.__count,
	...
	QUnit.test("GET aggregated query with paging and inlinecount", function(assert) {
		var dataWithoutPaging;
		jQuery.ajax({
			dataType: "json",
			url: _mockServer.constants.DEMOKIT_SERVICE_URL + "RevenueQuery(P_Currency=%27USD%27)/Results?$select=Country&$inlinecount=allpages",
			async: false,
			success: function(data) {
				dataWithoutPaging = data;
			},
			error: function() {
				assert.ok(false, "request failed unexpectedly");
			}
		...
		assert.notStrictEqual(dataWithoutPaging.d.results.length, dataWithPaging.d.results.length,
			"then we get less results with paging");

		assert.strictEqual(dataWithoutPaging.d.__count, dataWithPaging.d.__count,
	...
	QUnit.module("given importTextsFromFile", {
		beforeEach: function(assert){
			...
			this.filePath = require.toUrl("../resources/i18n/demokitTest.properties");
