...
sap.ui.define([
	"sap/apf/demokit/app/model/stubbedMethods",
	"sap/base/Log",
	"sap/ui/core/util/MockServer",
	"sap/ui/thirdparty/jquery",
	"sap/ui/util/XMLHelper"
], function(StubbedMethods, Log, MockServer, jQuery, XMLHelper) {
	"use strict";

	var restoreApplicationMethods;
	var initializeMockServer = function() {
		var sApplicationPath = sap.ui.require.toUrl('sap/apf/demokit/app');
		...
		var stubApplicationMethods = function() {
			oStubbedMethods = new StubbedMethods(oTokenForServices);
		...
		* */
		var startApplicationMockServer = function() {
			var oApplicationMockServer = new MockServer({
				rootUri : sAppOdataPath
			});
			var sApplicationMatadaPath = sMetaDataFilePath + "applicationMetadata.xml";
			oApplicationMockServer.attachAfter(MockServer.HTTPMETHOD.GET, aggregateData);
		...
		function aggregateData(data){
			...
			if(data && data.mParameters && data.mParameters.oFilteredData && data.mParameters.oFilteredData.results){
				...
				if(results.length > 0){
					...
					if(measures.length > 0 ){
						...
						var resultValues = [];
						results.forEach(function(result){
							if(!resultValues.includes(result[dimensions[0]])){
			...
			function sortData(aggregatedData, url){
				...
				if(beginOrderBy > 0){
					...
					if(measuresInService.indexOf(sortProperty) > -1){
		...
		* */
		var startApplicationAnnotationMockServer = function() {
			var oApplicationAnnotationMockServer = new MockServer({
				...
				requests : [ {
					...
					response : function(oXhr) {
						...
						if (oResponse.success) {
							...
							}, XMLHelper.serialize(oResponse.data));
							Log.debug("MockServer: response sent with: 200, " + XMLHelper.serialize(oResponse.data));
						}
					}
				}, {
                    method: "HEAD",
		...
		* */
		var startPersistencyMockServer = function() {
			var oPersistecyMockServer = new MockServer({
		...
		* */
		var startPersistencyAnnotationMockServer = function() {
			var oPersistencyAnnotationMockServer = new MockServer({
				...
				requests : [ {
					...
					response : function(oXhr) {
						...
						if (oResponse.success) {
							...
							Log.debug("MockServer: error reading persistence annotations");
						}
					}
				},{
                    method: "HEAD",
		...
		* */
		var startSmartBusinessMockServer = function() {
			var oSBMockServer = new MockServer({
		...
		* */
		var startModelerMockServer = function() {
			var oModelerMockServer = new MockServer({
	...
	* @alias sap.apf.demokit.app.model.initializeMockServer
