sap.ui.define([
	...
	utils.vizNodeExists = function (xml, chartType) {
		...
		return this.validateXMLValue(vizFrames[0].getAttribute("vizType"), chartType);
	};
	...
	utils.vizSkeletonExists = function (xml) {
		...
		for (var i = 0; i < dimensionDefinitions.length; i++) {
			if (
				dimensionDefinitions[i].getAttribute("name") == undefined ||
				dimensionDefinitions[i].getAttribute("name") == ""
			) {
				return false;
			}
			if (
				dimensionDefinitions[i].getAttribute("value") == undefined ||
				dimensionDefinitions[i].getAttribute("value") == ""
			) {
				return false;
			}
		}
		...
		for (var i = 0; i < measureDefinitions.length; i++) {
			if (
				measureDefinitions[i].getAttribute("name") == undefined ||
				measureDefinitions[i].getAttribute("name") == ""
			) {
				return false;
			}
			if (
				measureDefinitions[i].getAttribute("value") == undefined ||
				measureDefinitions[i].getAttribute("value") == ""
			) {
				return false;
			}
		}
	...
	utils.dataBinding = function (xml, expected) {
		// This function was using Function constructor to evaluate data attribute of vizFrame to get complete binding information
		// since used Function constructor is violation of security, we need to find another way to parse this data.
		...
		if (data.filters && expected.filters) {
			...
			for (var i = 0; i < expected.filters.length; i++) {
				...
				for (var j = 0; j < data.filters.length; j++) {
					if (
						expected.filters[i].path == data.filters[j].path &&
						expected.filters[i].operator == data.filters[j].operator &&
						expected.filters[i].value1 == data.filters[j].value1
					) {
						flag = true;
						break;
					}
		...
		if (data.sorter && expected.sorter) {
			...
			for (i = 0; i < expected.sorter.length; i++) {
				...
				for (var j = 0; j < data.sorter.length; j++) {
					if (
						expected.sorter[i].path == data.sorter[j].path &&
						expected.sorter[i].descending == data.sorter[j].descending
					) {
						flag = true;
						break;
					}
		...
		if (feedItems && expected) {
			...
			for (var i = 0; i < expected.length; i++) {
				...
				for (var j = 0; j < feedItems.length; j++) {
					if (
						expected[i].uid == feedItems[j].getAttribute("uid") &&
						expected[i].type == feedItems[j].getAttribute("type")
					) {
...
*/
	...
	utils.referenceValuePathExists = function (xml, referenceValue) {
		...
		if (indicator.indexOf("parts:") < 0) {
			return false;
		}

		if (indicator.indexOf("{path:'" + referenceValue + "'}") > 0) {
			return true;
		} else {
			return false;
		}
	...
	utils.itemsNodeExists = function (xml, expected, category, categoryDefinition) {
		...
		for (var i = 0; i < expected.length; i++) {
			...
			for (var j = 0; j < dimensionDefinitions.length; j++) {
				if (
					expected[i].name == dimensionDefinitions[j].getAttribute("name") &&
					expected[i].value == dimensionDefinitions[j].getAttribute("value")
				) {
					flag = true;
					break;
				}
	...
	utils.testNoDataSection = function () {
		...
		OvpVizAnnotationManager.checkNoData(event, container, el);
	...
	utils.genericDimensionItemsNodeExists = function (vizComp, expected, bDisplayVal) {
		...
		for (var i = 0; i < expected.length; i++) {
			...
			for (var j = 0; j < dimensions.length; j++) {
				...
				if (
					expected[i].name == dimensions[j].getName() &&
	...
	utils.checkAxisTitleVisibility = function (cardViz, value) {
		...
		var aUids = cardViz.getFeeds().map(function (f, i) {
			return f.getUid();
		});
		var flag = true;
		aUids.forEach(function (uid, i) {
			var current = oVizProperties[uid];
			if (!current || !current.title || current.title.visible !== value) {
				flag = false;
				i = uid.length;
			}
		});
