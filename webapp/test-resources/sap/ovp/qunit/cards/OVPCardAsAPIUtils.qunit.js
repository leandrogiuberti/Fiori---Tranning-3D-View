...
sap.ui.define([
    ...
    var errorSpy;
    QUnit.module("sap.ovp.cards.OVPCardAsAPIUtils", {
        beforeEach: function () {
    ...
    QUnit.test("Card Test - For API - Dynamic Link List Card", function (assert) {
        var cardTestData = {
            dataSource: {
                baseUrl: CardUtils.odataBaseUrl_salesOrder,
                rootUri: CardUtils.odataRootUrl_salesOrder,
                annoUri: CardUtils.testBaseUrl + "data/annotations_barListCard.xml"
            }
        };
        var oManifest = {
            cards: {
        ...
        oModel.getMetaModel().loaded()
            .then(function () {
                ...
                return OVPCardAsAPIUtils.createCardComponent(oView, oManifest, "card017");
            }).then(function(){
    ...
    QUnit.test("Card Test - For API - Static Link List Card", function (assert) {
        var oManifest = {
            cards: {
                card014: {
                    template: "sap.ovp.cards.linklist",
    ...
    QUnit.test("Recreate Card functionality Test - For API - Normal View Switch Card", function (assert) {
        var oManifest = {
            cards: {
                card018: {
                    model: "purchaseOrder",
        ...
        var oCreateCardComponentStub = sinon.stub(OVPCardAsAPIUtils, "createCardComponent", function (oView, oManifest) {
            assert.ok(JSON.stringify(result) === JSON.stringify(oManifest), "recreateCard function was executed correctly");
        });
        OVPCardAsAPIUtils.recreateCard(oCardProperties, oComponentData);
        oCreateCardComponentStub.restore();
    });

    QUnit.test("Recreate Card functionality Test - For API - View Switch for Analytical Card", function (assert) {
        var oManifest = {
            cards: {
                card004: {
                    model: "salesShare",
        ...
        var oCreateCardComponentStub = sinon.stub(OVPCardAsAPIUtils, "createCardComponent", function() {
            assert.ok(JSON.stringify(result) === JSON.stringify(oManifest), "recreateCard function was executed correctly");
        });
        OVPCardAsAPIUtils.recreateCard(oCardProperties, oComponentData);
        oCreateCardComponentStub.restore();
    ...
    QUnit.test("Recreate Card functionality Test - For API - View Switch for Complex Cards with entitySet inside tabs", function (assert) {
        var oManifest = {
            cards: {
                card009: {
                    model: "salesOrder",
