...
sap.ui.define([
    ...
    QUnit.module("sap.ovp.cards.PayLoadUtils", {
        beforeEach: function () { },
        afterEach: function () { },
    });
    ...
    var aCards = [
        ...
        },
        {
            ...
            settings: {
                ...
                annotationPath:
                    "com.sap.vocabularies.UI.v1.FieldGroup#Note/Data,com.sap.vocabularies.UI.v1.FieldGroup#Note1/Data",
                identificationAnnotationPath:
                    "com.sap.vocabularies.UI.v1.Identification,com.sap.vocabularies.UI.v1.Identification#Awaiting_Approval",
            },
        },
        {
            id: "card016",
    ...
    QUnit.test("Function Test ---> getPayLoadForCloneCard ---> Clone of SAP Delivered List card", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    card: {
        ...
        PayLoadUtils.getPayLoadForCloneCard(oComponentContainer).then(function (mChangeContent) {            
            ...
            assert.ok(!!oTexts[sTitle.substr(2, sTitle.length - 4)], "Title binding done properly in PayLoad");
            assert.ok(!!oTexts[sSubTitle.substr(2, sSubTitle.length - 4)], "SubTitle binding done properly in PayLoad");
            assert.ok(JSON.stringify(mChangeContent) == JSON.stringify(result), "PayLoad created for Clone of SAP Delivered List card");
            fnDone();
        });
    });

    QUnit.test("Function Test ---> getPayLoadForCloneCard ---> Clone of SAP Delivered Analytical card", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    card: {
                        "customer.Vcard16_cardchartscolumnstacked_C1": {
                            ...
                            settings: {
                                ...
                                dataPointAnnotationPath:
                                    "com.sap.vocabularies.UI.v1.DataPoint#Eval_by_Currency-Generic",
                                identificationAnnotationPath:
        ...
        PayLoadUtils.getPayLoadForCloneCard(oComponentContainer).then(function (mChangeContent) {
            ...
            assert.ok(!!oTexts[sTitle.substr(2, sTitle.length - 4)], "Title binding done properly in PayLoad");
            assert.ok(!!oTexts[sSubTitle.substr(2, sSubTitle.length - 4)], "SubTitle binding done properly in PayLoad");
            assert.ok(!!oTexts[sValueSelectionInfo.substr(2, sValueSelectionInfo.length - 4)], "ValueSelectionInfo binding done properly in PayLoad");
            assert.ok(JSON.stringify(mChangeContent) == JSON.stringify(result), "PayLoad created for Clone of SAP Delivered Analytical card");
            fnDone();
        });
    });

    QUnit.test("Function Test ---> getPayLoadForCloneCard ---> Clone of SAP Delivered Analytical card with view switch", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    card: {
        ...
        PayLoadUtils.getPayLoadForCloneCard(oComponentContainer).then(function (mChangeContent) {
            ...
            assert.ok(!!oTexts[sSubTitle.substr(2, sSubTitle.length - 4)], "SubTitle binding done properly in PayLoad");
            assert.ok(!!oTexts[sValueSelectionInfo.substr(2, sValueSelectionInfo.length - 4)], "ValueSelectionInfo binding done properly in PayLoad");
            assert.ok(!!oTexts[sTabValue1.substr(2, sTabValue1.length - 4)], "Tab level property value's binding done properly in PayLoad");
            assert.ok(!!oTexts[sTabValue2.substr(2, sTabValue2.length - 4)], "Tab level property value's binding done properly in PayLoad");
            assert.ok(JSON.stringify(mChangeContent) == JSON.stringify(result), "PayLoad created for Clone of SAP Delivered Analytical card with view switch");
            fnDone();
        });
    });

    QUnit.test("Function Test ---> getPayLoadForCloneCard ---> Clone of SAP Delivered Stack card", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    card: {
        ...
        PayLoadUtils.getPayLoadForCloneCard(oComponentContainer).then(function (mChangeContent) {            
            ...
            assert.ok(!!oTexts[sTitle.substr(2, sTitle.length - 4)], "Title binding done properly in PayLoad");
            assert.ok(!!oTexts[sSubTitle.substr(2, sSubTitle.length - 4)], "SubTitle binding done properly in PayLoad");
            assert.ok(!!oTexts[sItemText.substr(2, sItemText.length - 4)], "ItemText binding done properly in PayLoad");
            assert.ok(JSON.stringify(mChangeContent) == JSON.stringify(result), "PayLoad created for Clone of SAP Delivered Stack card");
            fnDone();
        });
    });

    QUnit.test("Function Test ---> getPayLoadForCloneCard ---> Clone of SAP Delivered Static Link List card", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    card: {
        ...
        PayLoadUtils.getPayLoadForCloneCard(oComponentContainer).then(function (mChangeContent) {            
            ...
            assert.ok(!!oTexts[sTitle.substr(2, sTitle.length - 4)], "Title binding done properly in PayLoad");
            assert.ok(!!oTexts[sSubTitle.substr(2, sSubTitle.length - 4)], "SubTitle binding done properly in PayLoad");
            assert.ok(!!oTexts[sTabTitle1.substr(2, sTabTitle1.length - 4)], "Tab level property title's binding done properly in PayLoad");
            assert.ok(!!oTexts[sTabTitle2.substr(2, sTabTitle2.length - 4)], "Tab level property title's binding done properly in PayLoad");
            assert.ok(!!oTexts[sTabSubTitle1.substr(2, sTabSubTitle1.length - 4)], "Tab level property subTitle's binding done properly in PayLoad");
            assert.ok(sTabSubTitle2 === "", "Tab level property subTitle is empty, Hence it is not translated in PayLoad");
            assert.ok(!!oTexts[sTabImageAltText1.substr(2, sTabImageAltText1.length - 4)], "Tab level property imageAltText's binding done properly in PayLoad");
            assert.ok(!!oTexts[sTabImageAltText2.substr(2, sTabImageAltText2.length - 4)], "Tab level property imageAltText's binding done properly in PayLoad");
            assert.ok(JSON.stringify(mChangeContent) == JSON.stringify(result), "PayLoad created for Clone of SAP Delivered Static Link List card");
            fnDone();
        });
    });

    QUnit.test("Function Test ---> getPayLoadForCloneCard ---> Multiple clone's from the same card", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    card: {
        ...
        PayLoadUtils.getPayLoadForCloneCard(oComponentContainer).then(function (mChangeContent) {
            var sCardId = mChangeContent.flexibilityChange.id,
                ...
                oTexts = oAppDesChange.texts;
            assert.ok(!!oTexts[sTitle.substr(2, sTitle.length - 4)], "Title binding done properly in PayLoad");
            assert.ok(sSubTitle === "", "SubTitle is empty, Hence it is not translated in PayLoad");
            assert.ok(sCardId === "customer.card017_C2", "Newly copied card Id should be customer.card017_2");
            assert.ok(oTexts[sTitle.substr(2, sTitle.length - 4)].value[""] === "Standard Dynamic Linklist Card 2", "Check card title is changed");
            assert.ok(JSON.stringify(mChangeContent) == JSON.stringify(result), "PayLoad created for Clone of SAP Delivered Stack card");
            fnDone();
        });
    });
    ...
    QUnit.test("Function Test ---> getPayLoadForEditCard ---> Edit of SAP Delivered card ---> Basic Card Settings", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    cardId: "card001",
        ...
        assert.ok(
            JSON.stringify(mChangeContent) == JSON.stringify(result),
            "PayLoad created correctly on Edit of SAP Delivered card ---> Add, Update & Delete of Basic Card Settings"
        );
    });
    ...
    QUnit.test("Function Test ---> getPayLoadForEditCard ---> Edit of SAP Delivered card ---> Basic Card Settings", function (assert) {
        ...
        assert.ok(
            JSON.stringify(mChangeContent) == JSON.stringify(result),
            "PayLoad created correctly on Edit of SAP Delivered card ---> disable cards adding to My Home, Basic Card Settings"
        );
    });

    QUnit.test("Function Test ---> getPayLoadForEditCard ---> Edit of SAP Delivered card ---> Tab Level settings change", function (assert) {
        var result = {
            appDescriptorChange: {
                parameters: {
                    cardId: "card004",
                        ...
                        propertyValue: {
                            tabs: [
                                ...
                                },
                                {
                                    ...
                                    selectionPresentationAnnotationPath:
                                        "com.sap.vocabularies.UI.v1.SelectionPresentationVariant#OtherSPV",
                                    navigation: "headerNav",
                                },
                            ],
                        },
        ...
        assert.ok(
            JSON.stringify(mChangeContent) == JSON.stringify(result),
            "PayLoad created correctly on Edit of SAP Delivered card ---> Add, Update & Delete of Tab Level Settings | Also Adding of new Tab in the card"
        );
        ...
        assert.ok(
            JSON.stringify(mChangeContent) == JSON.stringify(result),
            "PayLoad created correctly on Edit of SAP Delivered card ---> Delete one Tab ---> Properties come out to the settings level"
        );
        ...
        assert.ok(
            JSON.stringify(mChangeContent) == JSON.stringify(result),
            "PayLoad created correctly on Edit of SAP Delivered card ---> Add, Update & Delete of Tab Level Settings | Also Adding of new Tab in Static Link List card"
        );
        ...
        assert.ok(
            JSON.stringify(mChangeContent) == JSON.stringify(result),
            "PayLoad created correctly on Edit of SAP Delivered card ---> Delete one Tab from Static Link List Card"
        );
    });
