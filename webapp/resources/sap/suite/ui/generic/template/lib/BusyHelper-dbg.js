sap.ui.define(["sap/ui/base/Object",
	...
	"sap/suite/ui/generic/template/genericUtilities/FeError"
		...
		//
		// bWillShowPlaceholder is used as a parameter in the setBusy and setBusyReason methods, indicates if the busy session will show (at least one)) placeholder (if ploaceholder feature is on)
		...
		function getMethods(oTemplateContract) {
			...
			function fnMakeBusy(bWillShowPlaceholder, sMethod, oBusyEndedPromise, sReason){
				...
				bIsBusy = true;
				bIsOnlyLogicalBusyNeeded = bWillShowPlaceholder && oTemplateContract.bEnablePlaceholder;
			...
			function setBusyReason(sReason, bIsActive, bImmediate, oSessionParams, bWillShowPlaceholder) {
				if (bIsActive) {
					...
					if (!mBusyReasons[sReason]) {
						...
						fnMakeBusy(bWillShowPlaceholder, "setBusyReason", oBusyEndedPromise, sReason);
					}
				} else {
					if (mBusyReasons[sReason]) {
			...
			function setBusy(oBusyEndedPromise, bImmediate, oSessionParams, bWillShowPlaceholder) {
				extend(oBusySessionParams, oSessionParams);
				iBusyPromiseCount++;
				fnMakeBusy(bWillShowPlaceholder, "setBusy", oBusyEndedPromise, "");
