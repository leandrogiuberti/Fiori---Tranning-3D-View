sap.ui.define(["sap/ui/base/Object", "sap/base/util/extend"],function(BaseObject, extend) {
	...
	function getMethods(oTemplateContract) {
        ...
        */
		...
		/**
		 * Returns the details of the relevant/affected registry entries.
		 * @param {boolean} bRestrictToNonDraftScenario should be true if the caller needs only the non draft registry entries details
		 * and otherwise, it checks for both draft and non draft registry entries.
		 */
		function fnGetAffectedRegistryEntryInfo(bRestrictToNonDraftScenario){
			...
			aActiveComponents.forEach(function(sComponentId){ // ensure, that application callback is called for all active views
				...
				if (bRestrictToNonDraftScenario && oRegistryEntry.utils.isDraftEnabled()) {
					return;
				}
		...
		function fnPerformIfNoDataLossImpl(fnPositive, fnNegative, bIsLeaving, bIsTechnical) {
			var oRelevantRegistryEntryInfo = fnGetAffectedRegistryEntryInfo(true);
		...
		function fnGetShellDataLossPopup(oShellContext) {
			...
			if (bNeedsDataLossPopup) {
				var oRelevantRegistryEntryInfo = fnGetAffectedRegistryEntryInfo();
				var bHasChanges = oRelevantRegistryEntryInfo.relevantRegistryEntry && (oRelevantRegistryEntryInfo.hasExternalChanges || oTemplateContract.oAppComponent.getModel().hasPendingChanges());
				return !!bHasChanges;
			}
			return false;
