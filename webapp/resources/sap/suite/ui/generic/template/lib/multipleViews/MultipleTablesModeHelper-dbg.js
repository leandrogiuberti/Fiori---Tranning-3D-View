sap.ui.define([
	...
	function getMethods(oController, oTemplateUtils, oConfiguration) {
		...
		// This method returns an array of filters which should be used for all count calls.
		// Thereby, the filters which are coming from the settings of the table (aFiltersFromRebindEvent) are removed from the filters
		// which are meanwhile in the bindingParams (aCurrentRebindFilters).
		function getCommonFiltersForCounts(aCurrentRebindFilters, aFiltersFromRebindEvent){
			if (aFiltersFromRebindEvent.length === 0){
				return aCurrentRebindFilters;
			}
		...
		// This function adds the filters coming from the SmartFilterBar to the filters used for the view specified by oViewMeta.
		// Thereby, we need to ensure that filters which are not applicable to the current view (because they are not available or not filterable in the entity set) are ignored.
		// If bCount is true the filters will be used for the count call.
		// Otherwise oViewMeta is represents the view currently active. In this case we will also update our internal information about ignored filters.
		function fnGetFiltersAdaptedFromItem(aFilters, oViewMeta, bForCount){
...
//					oSelectedKeyWrapper.setState(oState.selectedKey);
					if (oState && oState.controlStates){
						Object.values(mSwitchingKeyToViewMeta).forEach(function(oViewMeta){
...
//					oSelectedKeyWrapper.attachStateChanged(fnHandler);
		...
		return {
			...
			getCommonFiltersForCounts: getCommonFiltersForCounts,
