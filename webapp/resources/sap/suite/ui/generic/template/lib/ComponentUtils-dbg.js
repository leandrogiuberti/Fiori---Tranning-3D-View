sap.ui.define([
	...
	function getMethods(oComponent, oComponentRegistryEntry) {
		...
		function fnRebindHeaderData(sBindingPath) {
			...
			let mRoutingTree = oComponentRegistryEntry.oTemplateContract.mRoutingTree;
			let oAncestorNode = mRoutingTree[oTreeNode.parentRoute];
			while (oAncestorNode.level > 0) {
				oAncestorNode.bindElement(oComponentRegistryEntry.oTemplateContract.oNavigationHost, null, true);
				oAncestorNode = mRoutingTree[oAncestorNode.parentRoute];
			}
		...
		oComponentRegistryEntry.oStatePreserverPromise = oComponentRegistryEntry.reuseComponentsReady.then(function(mReuseComponentProxies){
			...
			var oSettings = extend({
				...
				applyState: function(oState, bIsSameAsLast){
					...
					// The result of "applyState" can be a Promise or undefined.
					var vStateAppliedResult = (oComponentRegistryEntry.methods.applyState || Function.prototype)(oViewState, bIsSameAsLast);
		...
		testableHelper.testableStatic(fnRebindHeaderData, "filterHelper_fnRebindHeaderData");
		return {
