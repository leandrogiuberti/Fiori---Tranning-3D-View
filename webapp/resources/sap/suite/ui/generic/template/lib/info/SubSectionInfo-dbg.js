    ...
    function getSubSectionLoadingBehavior(bLayoutFinished, bEnteredToViewPort, bStateApplied, bRebindCompleted, activeHandler, inActiveHandler) {
        var loadingBehavior = {
            ...
            activeHandler: activeHandler || function (oSubSection, oSubSectionInfo) {
                ...
                // be non null value & subsequently will skip the binding context change & refresh 
                // call in the active handler. Actions on the sub sections are also not part of the
                // LazyLoader because of which binding context would have been set to null. In case 
                // they are not the first control in the array, context will never get associated to
                // parent context again. Therefore we check the refreshRequired property and in case 
                // true we forcefully associate the binding context & refresh subsection
                if ((aBlocks.length && aBlocks[0].getBindingContext() === null) || oSubSectionInfo.getRefreshRequired()) {
