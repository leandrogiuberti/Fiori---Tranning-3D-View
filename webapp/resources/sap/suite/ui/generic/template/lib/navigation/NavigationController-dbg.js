...
sap.ui.define(["sap/ui/base/Object",
	...
	// By creating the object as instance of self defined class cPreprocessorClass we suppress this cloning mechanism.
	var cPreprocessorClass = function () {
		// Intentionally empty: prevents UI5 deep cloning
	};
	...
	function fnAppStates2ParString(oAppStates){
		var sDelimiter = "";
		var sRet = "";
		var aPars = Object.keys(oAppStates).sort((a, b) => a.localeCompare(b));
		aPars.forEach(function(sPar){
			var vValue = oAppStates[sPar];
			if (Array.isArray(vValue)){
				var aValues = vValue.sort((a, b) => a.localeCompare(b));
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
			...
			if (Array.isArray(vValue1)){
				if (Array.isArray(vValue2)){
					if (vValue1.length === vValue2.length){
						vValue1 = vValue1.sort((a, b) => a.localeCompare(b));
						vValue2 = vValue2.sort((a, b) => a.localeCompare(b));
		...
		function getCandidateForBackNavigation(oTargetNode){
			...
			if (oCandidateHash && (oTargetNode.level === 0 || (oTemplateContract.oFlexibleColumnLayoutHandler && oTargetNode.fCLLevel === 0))){
				while (oCandidateHash.backTarget > 0 && oCandidateHash.identity.treeNode && oCandidateHash.identity.treeNode.level > oTargetNode.level) {
		...
		function fnNavigateUpAfterDeletion(iTargetViewLevel){
			...
			oCandidateHash = oCurrentHash;
			while (oCandidateHash.backTarget > 0 && (!oCandidateHash.identity || !oCandidateHash.identity.treeNode || oCandidateHash.identity.treeNode.level > iTargetViewLevel)) {
				oCandidateHash = aPreviousHashes[oCandidateHash.backTarget];
				iCandidateCount++;
			}
			...
			var oNavigationPromise = getAppStatePromiseForUpDownNavigation(oTargetIdentity.treeNode, oTargetIdentity.appStates).then(function(){
				...
				if (iMode < 0 && (oTargetIdentity.treeNode.fCLLevel === 1 || oTargetIdentity.treeNode.fCLLevel === 2) && oCandidateHash.identity.treeNode === oTargetIdentity.treeNode){
					while ( oCandidateHash.backTarget > 0 && !oTemplateContract.oFlexibleColumnLayoutHandler.areIdentitiesLayoutEquivalent(oCandidateHash.identity, oTargetIdentity)) {
						oCandidateHash = aPreviousHashes[oCandidateHash.backTarget];
		...
		function fnNavigateToMessageTarget(oMessage, sFullTarget){
			...
			for (sPathPrefix = sAssumedBindingPath; sPathPrefix && (!oTreeNode || oTreeNode.level > 0);) {
				...
				// Move sPathPrefix and oTreeNode one step up in the hierarchy
				// cut of last segment
