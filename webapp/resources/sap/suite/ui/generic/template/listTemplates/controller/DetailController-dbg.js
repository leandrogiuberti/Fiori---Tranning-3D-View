			...
			_applyCriticalityInfo: function(oEvent, smartTable){
				...
				if (sCriticalityPath) {
					...
					if (oEvent && oEvent.mParameters) {
						if (!oEvent.mParameters.bindingParams.parameters.select.includes(sCriticalityPath)) {
							oEvent.mParameters.bindingParams.parameters.select += "," + sCriticalityPath;
						}
					}
			...
			_setParamMap: function(chart) {
				...
				drillFiltersFromChart.forEach(function(oFilter) {
					if (!paramMap.hasOwnProperty(oFilter.sPath)) {
						...
						if (!paramMap[oFilter.sPath].includes(oFilter.oValue1)) {
							paramMap[oFilter.sPath].push(oFilter.oValue1);
						}
					}
				});
				this._paramMap = paramMap;
