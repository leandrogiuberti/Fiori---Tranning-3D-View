sap.ui.define(["sap/ui/core/date/UI5Date"], function(UI5Date) {
	...
	* To achieve this goal the functions should be used together with the standard serialization/deserialization methods JSON.stringify/parse.
	...
	*/
	...
	function fnDeStringify(vObject){
		...
		var bError = vObject.replaces.some(function(oReplaceInfo){ // bError will be true if any member of vObject.replaces proves that vObject is not "a valid vObject3" (see above)
			...
			if (oReplaceInfo.path.length === 0){ // oReplaceInfo defines a replacement for vObject itself -> vObject either represents undefined or a Date
				...
				return vObject.replaces.length > 1; // if vObject is either undefined or a Date there should be no other replacement info
			}
