sap.ui.define([
	...
	function getNormalizedTableSettings(oMetaModel, oOriginalSettings, Device, sEntitySet, oExtensionActions, aLineItem, fnGetSearchFieldId) {
		...
		// 4. determine type
		if (oSettings.tableSettings.type !== "StandardList" && oSettings.tableSettings.type !== "ObjectList" && sEntitySet) {
		...
		var mColumnWidthIncludingColumnHeader = Object.create(null);
		if (oSettings.columns) {
			for (var i = 0; i < Object.keys(oSettings.columns).length; i++) {
				var oColumnSettings = Object.create(null);
				var sId = Object.keys(oSettings.columns)[i];
				if (sId.indexOf("com.sap.vocabularies.UI.v1.DataField::") > -1) {
					sId = sId.split("com.sap.vocabularies.UI.v1.DataField::")[1];
				}
				oColumnSettings[sId] =  {"truncateLabel": !oSettings.columns[Object.keys(oSettings.columns)[i]].widthIncludingColumnHeader};
				Object.assign(mColumnWidthIncludingColumnHeader, oColumnSettings);
			}
		}
		if (oSettings.tableSettings && oSettings.tableSettings.widthIncludingColumnHeader) {
			Object.assign(mColumnWidthIncludingColumnHeader, {"*": {"truncateLabel": !oSettings.tableSettings.widthIncludingColumnHeader}});
		}
		oSettings.tableSettings.calculateWidthIncludingColumnHeader = mColumnWidthIncludingColumnHeader;
