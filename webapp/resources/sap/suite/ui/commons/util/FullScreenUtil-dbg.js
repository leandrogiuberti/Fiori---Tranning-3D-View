...
sap.ui.define([
	"sap/m/Dialog", "sap/ui/core/HTML", "sap/base/Log"
], function(Dialog, HTML, Log) {
	"use strict";
	...
	var FullScreenUtil = {
        toggleFullScreen: function(oControl, bIsFullScreen, oFullScreenButton, fnExternalClose) {
			...
			if (bIsFullScreen) {
				...
				//add custom property to the Dialog control
				oControl._oFullScreenDialog.getDomRef().querySelector(".sapMDialogScroll").classList.add("sapMDialogChartContainerScroll");
		...
		toggleFullScreenBrowserAPI: function(oControl, bIsFullScreen) {
            // Switch to full-screen mode using Browser API requestFullscreen
            if (bIsFullScreen) {
                oControl.getDomRef().requestFullscreen().catch((err) => {
                    Log.error(`Error attempting to enable fullscreen mode: ${err.message} (${err.name})`);
                });
            } else {
                // Exit from full-screen mode using Browser API exitFullscreen
                oControl._bFromFullScreenButtonPress = true;
                document.exitFullscreen().catch((err) => {
                    Log.error(`Error attempting to enable fullscreen mode: ${err.message} (${err.name})`);
                });
            }
        },
        //Attach fullscreenchange on the oControl to detect switching between fullscreen and non fullscreen mode
        attachFullScreenChangeEvent: function(oControl) {
            oControl.getDomRef().removeEventListener("fullscreenchange", this.onFullScreenChange.bind(oControl));
            oControl.getDomRef().addEventListener("fullscreenchange", this.onFullScreenChange.bind(oControl));
            oControl._bIsFullScreenListenerAdded = true;
        },
        onFullScreenChange: function () {
            //_toggleFullScreenButtonState should be triggered only when ESC Key is pressed to close fullscreen mode in order to change the fullscreen button state from fullscreen to non fullscreen.
            if (!document.fullscreenElement && !this._bFromFullScreenButtonPress){
                this._toggleFullScreenButtonState();
            } else {
                this._bFromFullScreenButtonPress = false;
            }
        }
    };
