...
sap.ui.define([
...
], function (jQuery, library, ProcessFlowLaneHeader, AriaProperties, Device, encodeXML, Element, Core, CoreLib, InvisibleText) {
	"use strict";
	...
	//Setting invisible text
		if (!oControl._oInvisibleText) {
			oControl._oInvisibleText = new InvisibleText();
			oControl._oInvisibleText.toStatic();
			oControl._oInvisibleText.setText(this._oResBundle.getText("PF_ARIA_PROCESS_FLOW_NODE_DESC"));
		}
	...
	ProcessFlowRenderer._renderNode = function (oRm, oControl, node, isTDTagOpen) {
		if (isTDTagOpen) {
			...
			oRm.attr("id", sNodeId);
			oRm.attr("tabindex", 0);
            oRm.attr("aria-describedby", oControl._oInvisibleText.getId());
