...
sap.ui.define([
	...
	var oGroupFeed =  Controller.extend(sControllerName, {
		...
		onInit: function() {
			...
			pTimelineReady = this._initializeTimeline();
		},
		/**
		...
		_initializeTimeline: async function() {
			...
			await this._createSocialProfile();
		},
		/**
		 ...
		 * @memberOf sap.collaboration.components.feed.views.GroupFeed
		 */
		_onFeedSourcesChange: async function(oEvent) {
			...
			if (this._oCommonUtil.isString(oFeedSources.mode)) {
				if (this._oModes[oFeedSources.mode] === undefined) {
				 var oMode = await ModeFactory.getInstance().createMode(oFeedSources.mode, this);
					this._oModes[oFeedSources.mode] = oMode;
	...
	oGroupFeed.prototype.getTimelineReady = function () {
		return pTimelineReady;
	};
