...
sap.ui.define([
	...
	*/
	ReplyPopover.prototype.init = function () {
		...
		this._pReplyPopover = this._createReplyPopover();
		this._pReplyPopover.then((oPopover) => {
			this._oReplyPopover = oPopover;
		})                   
	};
	
	ReplyPopover.prototype.fullyCreated = function() {
		return this._pReplyPopover;
	}
	...
	*/
	ReplyPopover.prototype.exit = function () {
		...
		else if ( this._pReplyPopover ) {
			this._pReplyPopover.then(() => {
				this._oReplyPopover.destroy();
			});
		}
	};
	...
	* @private
	* @returns {Promise <sap.m.ResponsivePopover>}
	* @memberOf sap.collaboration.components.controls.ReplyPopover
	*/
	ReplyPopover.prototype._createReplyPopover = async function () {
		var oReplyPopover = new ResponsivePopover({
			showHeader: false,
			placement: PlacementType.Vertical,
			contentWidth: "25rem",
			contentHeight: "455px",
			content: await this._addApp(),
			afterClose: [function(){
	...
	* @memberOf sap.collaboration.components.controls.ReplyPopover
	*/
	ReplyPopover.prototype._addApp = async function () {
		var that = this;
	...
	*/
		...
		this._oSocialProfilePage = new Page({
			...
			content: [this._oSocialProfileView]
		});
