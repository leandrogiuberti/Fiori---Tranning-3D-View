...
sap.ui.define([
	...
	/** @lends sap.collaboration.components.fiori.sharing.dialog.Component.prototype */
	{
		 ...
		 * @private
		 */
		_createSharingView: async function() {
			var self = this;
			...
			if (!this.oSharingView) {
				this.oSharingView = await View.create({
					...
					viewName: "module:sap/collaboration/components/fiori/sharing/SharingView"
				});
			} else {
		 ...
		 * @private
		 */
		createNoGroupsDialog: async function(sJamUrl) {
			if (!this.oNoGroupsView) {
				this.oNoGroupsView = await View.create({
					...
					viewName: "module:sap/collaboration/components/fiori/sharing/NoGroupsView"
				});
			}
		 ...
		 * @private
		 */
		openoNoGroupsDialog: async function(sJamUrl) {
			this._oSharingDialog.removeAllContent();
			if (!this.oNoGroupsDialog) {
				this.oNoGroupsDialog = await this.createNoGroupsDialog(sJamUrl);
			}
