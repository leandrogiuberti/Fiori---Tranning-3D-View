    ...
    _proto.constructorWithStrings = function constructorWithStrings() {
      ...
      if (rowNavigationInfo !== undefined || this.tableBlockProperties.overrideRowPress) {
        ...
        if (rowNavigationInfo?.type === "Outbound") {
          ...
          this.contextMenuOpenInNewTab = `API.onOpenInNewTabPress($event, $controller,  %{internal>contextmenu/selectedContexts}, %{internal>contextmenu/navigableContexts},{ callExtension: true, targetPath: '${rowNavigationInfo?.targetPath ?? ""}', navMode: 'openInNewTab' }, ${selectionLimitForOpenInNewTab})`;
        }
      }
      ...
      this.fieldLiveChange = this.tableBlockProperties.creationMode?.name === "InlineCreationRows" ? "API.onFieldLiveChange($event)" : undefined;
