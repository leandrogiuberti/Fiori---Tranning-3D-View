...
export default class Range extends Value {
	 ...
	 * @protected
	 */
	formatValue(internalValue: string | undefined, externalValueType: string | undefined): unknown {
		let results = super.formatValue(internalValue, externalValueType);

		if (!results) {
			const minValue = (this as unknown as AugmentedSimpleType).oFormatOptions.min || Number.MIN_SAFE_INTEGER,
				maxValue = (this as unknown as AugmentedSimpleType).oFormatOptions.max || Number.MAX_SAFE_INTEGER;
