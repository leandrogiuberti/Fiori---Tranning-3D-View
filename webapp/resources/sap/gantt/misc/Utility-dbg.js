...
sap.ui.define([
	...
	*/
	...
	/**
	* @param {string} sTimezone timezone value to compute the offset
	* @param {Date} oDate reference date
	* @returns {number} returns the non dst offset
	*/
	Utility.getNonDSTOffsetInfo = function (sTimezone, oDate) {
		var iYear = oDate.getFullYear();
		var StartDate = new Date(iYear, 0, 1);
		var EndDate = new Date(iYear, 6, 1);
		var RefDate = StartDate;

		var iStart = Utility.getTimezoneOffset(sTimezone, new Date(iYear, 0, 1));
		var iEnd = Utility.getTimezoneOffset(sTimezone, new Date(iYear, 6, 1));
		...
		return {
			utcOffset: Math.min(iStart, iEnd) * 60000,
			offset: (RefDate.getTime() - new Date(RefDate.toLocaleString("en-US", { timeZone: sTimezone })).getTime()),
			month: iStart < iEnd ? 0 : 6
		};
	};
	...
	* @param {string} sTimezone timezone value to compute the offset
	* @param {Date} oDate reference date
	* @returns {number} returns offset of the given timezone
	*/
	Utility.getTimezoneOffset = function (sTimezone, oDate) {
	...
	* @param {number} dayOfWeek reference day of the week
	* @returns {number} next week of the day
	*/
	Utility.getNextWeekday = function (refDate, dayOfWeek) {
