	...
	var GanttZoomHelper = {
		...
		fnStockChartMouseMoveHandler: function(oEvent) {
			...
			function getAllElementsFromPoint(x, y) {
				var aItems = document.elementsFromPoint(x, y);
				var sDefaultTooltip = "", sCustomTooltip = "";
				var dimensionMap = new Map(), tooltipMap = new Map();
				aItems.forEach(function(oItem) {
					if (oItem && oItem.tagName === "rect" && oItem.children && oItem.children.length && oItem.children[0].tagName === "title") {
						if (oItem.classList.contains("stockChartTooltip")) {
							oItem.style.pointerEvents = "none";
						} else {
							var incomingTooltip = oItem.children[0].innerHTML;
							var sDimensionName = oItem.getAttribute("data-text");
							var isCalendar = Core.byId(oItem.id) && Core.byId(oItem.id).isA('sap.gantt.simple.BaseCalendar');
							if (sDimensionName && incomingTooltip.trim().length > 0) {
								dimensionMap.set(sDimensionName, incomingTooltip);
							} else if (!(sDimensionName || isCalendar)) {
								// Split the incoming tooltip and check if the first part exists in the map
								var existingLines = incomingTooltip.split("\n").reverse();
								existingLines.forEach(function(line) {
									var incomingParts = line.split(":");
									if (incomingParts.length > 1) {
										var key = incomingParts[0];
										var value = incomingParts[1];
										tooltipMap.set(key, value); // Update or add the key-value pair
									}
								});
							}
							if (!isCalendar){
				...
				for (const key of dimensionMap.keys()) {
					tooltipMap.delete(key);
				}

				// Reconstruct the tooltip string from the map
				sDefaultTooltip = Array.from(tooltipMap.entries()).reverse().map(function(entry) {
					return entry[0] + ":" + entry[1];
				}).join("\n");
				sCustomTooltip = Array.from(dimensionMap.entries()).reverse().map(function(entry) {
					return entry[1];
				}).join("\n");

				if (sDefaultTooltip.length && sCustomTooltip.length) {
					return sCustomTooltip + "\n" + sDefaultTooltip;
				} else if (sCustomTooltip.length) {
					return sCustomTooltip;
				} else if (sDefaultTooltip.length) {
