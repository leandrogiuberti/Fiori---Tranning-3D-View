...
sap.ui.define([
	...
	"sap/ui/core/Lib", "sap/ui/core/IconPool", "sap/ui/Device", "sap/gantt/utils/GanttChartConfigurationUtils",
	// 3rd party lib
	"sap/ui/thirdparty/d3"
], function (Drawer, Lib, IconPool, Device, GanttChartConfigurationUtils) {
	"use strict";
	...
	SelectionPanel.prototype._drawExpandedBackground = function (aSvg, iTableWidth, iTableHeaderWidth, aDatas, oChartSchemeBackgroundConfig) {
		...
		aHederBGRect.attr("x",function(){
			return GanttChartConfigurationUtils.getRTL() ? iTableWidth - iTableHeaderWidth + 1 : 2;
		})
		.attr("y", function(d) {
			return d.y;
		})
		...
		aCntBGRect.attr("x", function(){
			return GanttChartConfigurationUtils.getRTL() ? 2 : iTableHeaderWidth + 1;
		})
		.attr("y", function(d) {
			return d.y;
		})
	...
	SelectionPanel.prototype._drawExpandedContent = function (aSvg, iTableWidth, iTableHeaderWidth, aDatas) {
		...
		.attr("x", function (d) {
			return GanttChartConfigurationUtils.getRTL() ? iTableWidth - iTableHeaderWidth - fGetLevelIndentByShape(d) - 17 : fGetLevelIndentByShape(d) + iTableHeaderWidth;
		})
		.attr("y", function (d) {
		...
		aCntG.selectAll(".iconFont").attr("x", function (d) {
			return GanttChartConfigurationUtils.getRTL() ? iTableWidth - iTableHeaderWidth - fGetLevelIndentByShape(d) : fGetLevelIndentByShape(d) + iTableHeaderWidth;
		})
		.attr("y", function (d) {
			return d.y + 19;
		})
		...
		aCntText.attr("x", function (d) {
			return GanttChartConfigurationUtils.getRTL() ? iTableWidth - iTableHeaderWidth - fGetLevelIndentByShape(d) - 26 : fGetLevelIndentByShape(d) + iTableHeaderWidth + 27;
		})
		.attr("y", function (d) {
			return d.y + 16.5;
		})
		...
		.attr("text-anchor", Device.browser.edge && GanttChartConfigurationUtils.getRTL() ? "end" : "start");
		...
		aCntCloseBtn.attr("x", function (d) {
			...
			var sInitialX = GanttChartConfigurationUtils.getRTL() ? iTableWidth - iTableHeaderWidth - fGetLevelIndentByShape(d) - iPrevTextWidth - 37 : fGetLevelIndentByShape(d) + iTableHeaderWidth + iPrevTextWidth + 35;
			return sInitialX;
		})
		.attr("y", function (d) {
			return d.y + 19;
		...
		.text( function(d) {
			...
			return oResourceBundle.getText("TLTP_CLOSE");
		});
