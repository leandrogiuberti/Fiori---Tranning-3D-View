 ...
 */
sap.ui.define([
	"sap/gantt/drawer/Drawer", "sap/gantt/misc/Utility","sap/gantt/misc/Format","sap/ui/Device", "sap/ui/core/Core", "sap/ui/core/Element",
	// 3rd party lib
	"sap/ui/thirdparty/d3"
], function (Drawer, Utility, Format, Device, Core, Element) {
	"use strict";
	...
	ShapeInRow.prototype.drawSvg = function (aSvgNode, oShape, oAxisTime, oAxisOrdinal, oStatusSet) {
		...
		if (Element.getElementById(oShape.getId()).getMetadata().getName() === "sap.gantt.shape.cal.Calendar") {
	...
	ShapeInRow.prototype._textTruncate = function (oData, oSelf, nTruncateWidth, nEllipsisWidth, id) {
		...
		if (nTextLength > nTruncateWidth) { // truncate needed
			...
			if (bEllipsisAppear) {
				if (Device.browser.name === "cr" && Device.browser.version < 96){
