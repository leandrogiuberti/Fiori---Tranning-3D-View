		...
		for (var i = 0; i < aData.length; i++) {
			...
			if (isBulk && sArrayAttribute && sTimeFilter && sEndTimeFilter){
				...
				for (var key in oTempData) {
					if ((typeof oTempData[key] == "object") && oTempData[key][sArrayAttribute] && oTempData[key][sArrayAttribute].length > 0) {
						...
						if (startArray !== null && endArray !== null && startArray <= endArray){
							...
							for (var k = 0; k < aShapeDatas.length; k++){
								if (key !== aShapeDatas[k] && Array.isArray(oTempData[aShapeDatas[k]]) && oTempData[aShapeDatas[k]].length > 0) {
