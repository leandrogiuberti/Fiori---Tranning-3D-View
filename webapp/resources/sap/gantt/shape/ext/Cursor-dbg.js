...
sap.ui.define([
	...
	Cursor.prototype.getD = function (oData, oRowInfo) {
		...
		if (this.mShapeConfig.hasShapeProperty("d")) {
			...
			if (aCenter && aCenter.length === 2 && Number.isFinite(Number(nPointHeight)) && Number.isFinite(Number(nWidth)) &&
			Number.isFinite(Number(nLength)) && Number.isFinite(Number(nHalflength))) {
				sD = "M " + aCenter.join(" ") +
