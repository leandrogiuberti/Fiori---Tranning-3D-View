...
sap.ui.define([
	...
	Triangle.prototype.getD = function (oData, oRowInfo) {
		...
		if (this.mShapeConfig.hasShapeProperty("d")) {
			...
			if (aCenter && aCenter.length === 2 && Number.isFinite(Number(nBase)) && Number.isFinite(Number(nHeight)) &&
			Number.isFinite(Number(nDisOfYaxisH)) && Number.isFinite(Number(nHalfHeight))) {
				sD = "M " + aCenter.join(" ") +
				" m 0 " + nHalfHeight +
