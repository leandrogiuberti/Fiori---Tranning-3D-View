...
sap.ui.define([
	...
	GanttChartWithTable.prototype.useSelectionPlugin = function (oSelectionPlugin) {
		...
		if (oSelectionPlugin) {
			var oSelectionMode = oSelectionPlugin.getSelectionMode() || oRightTable.getSelectionMode();
			...
			if (!oRightTable.getPlugins().length) {
				oRightTable.setSelectionMode(oSelectionMode);
			}
	...
	GanttChartWithTable.prototype._attachEvents = function () {
		...
		if (Device.browser.firefox) {
			$tableMaskSvg.off("MozMousePixelScroll.sapUiTableMouseWheel", this._onMouseWheel.bind(this));
			$tableMaskSvg.on("MozMousePixelScroll.sapUiTableMouseWheel", this._onMouseWheel.bind(this));
		} else {
			$tableMaskSvg.off("wheel.sapUiTableMouseWheel", this._onMouseWheel.bind(this));
			$tableMaskSvg.on("wheel.sapUiTableMouseWheel", this._onMouseWheel.bind(this));
		}
	};
	...
	GanttChartWithTable.prototype._bindVerticalScrollForTT = function() {
		...
		$vsb.off("scroll.sapUiTableVScrollForGanttChartWithTable", this._onSelectionPanelVSbScroll);
		$vsb.on("scroll.sapUiTableVScrollForGanttChartWithTable", this._onSelectionPanelVSbScroll.bind(this));
	};
	...
	GanttChartWithTable.prototype.autoResizeColumn = function(iColId) {
		if (iColId >= 0 && Number.isFinite(Number(iColId))) {
