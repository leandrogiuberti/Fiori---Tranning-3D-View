...
sap.ui.define(["sap/base/strings/whitespaceReplacer", "sap/m/library", "sap/ui/core/date/UI5Date", "sap/ui/core/format/DateFormat", "sap/ui/core/format/NumberFormat", "sap/ui/core/library", "sap/ui/integration/util/Utils"], function (whitespaceReplacer, sap_m_library, UI5Date, DateFormat, NumberFormat, sap_ui_core_library, IntegrationUtils) {
  "use strict";
   ...
   */
  const criticality2state = function (criticality, criticalityState) {
    if (!criticalityState) {
      return "";
    }
   ...
   */
  const calculateTrendDirection = function (aggregateValue, referenceValue, downDifferenceValue, upDifferenceValue) {
   ...
   *
   */
  const calculateCriticalityState = function (value, improvementDirection, deviationLowValue, deviationHighValue, toleranceLowValue, toleranceHighValue, criticalityState) {
    const criticality = {
      EnumMember: "None"
    };
   ...
   */
  const formatKPIValue = function (value, formatterOptions, isUnit) {
   ...
   */
  const formatDateValue = function (value, pattern) {
    let result;
    let year, month;
    switch (pattern) {
      case "yearmonth":
   ...
   */
  const targetValueFormatter = function (kpiValue, targetValue, formatterOptions) {
   ...
   *
   */
  const formatValueColor = function (value, formatterOptions, defaultValue) {
    return calculateCriticalityState(value, formatterOptions.sImprovementDirection, formatterOptions.bIsDeviationLowBinding ? defaultValue : formatterOptions.deviationLow, formatterOptions.bIsDeviationHighBinding ? defaultValue : formatterOptions.deviationHigh, formatterOptions.bIsToleranceLowBinding ? defaultValue : formatterOptions.toleranceLow, formatterOptions.bIsToleranceHighBinding ? defaultValue : formatterOptions.toleranceHigh, formatterOptions.oCriticalityConfigValues);
  };
   ...
   */
  const formatTrendIcon = function (value, formatterOptions, defaultValue) {
    return calculateTrendDirection(value, formatterOptions.bIsRefValBinding ? defaultValue : formatterOptions.referenceValue, formatterOptions.bIsDownDiffBinding ? defaultValue : formatterOptions.downDifference, formatterOptions.bIsUpDiffBinding ? defaultValue : formatterOptions.upDifference);
  };
   ...
   */
  const formatNumber = function (value1, value2, formatterOptions, textFragments) {
    ...
    const parts = [!isNaN(parseFloat(value1)) && formatterInstance ? formatterInstance.format(parseFloat(value1)) : value1, !isNaN(parseFloat(value2)) && formatterInstance ? formatterInstance.format(parseFloat(value2)) : value2];
   ...
   */
  const formatCriticality = function (criticalityValue, type) {
   ...
   */
  const formatWithPercentage = function (value) {
    return value ? `${value} %` : "";
  };
   ...
   */
  const computePercentage = function (value, target, isUnit) {
    let percentValue;
    if (value === undefined) {
      return "0";
    }
    const numerator = typeof value === "string" ? parseFloat(value) : value;
    const denominator = typeof target === "string" ? parseFloat(target) : target;
    if (isUnit === "%") {
      if (numerator > 100) {
        percentValue = "100";
      } else if (numerator <= 0) {
   ...
   */
  const formatCriticalityIcon = function (value) {
    switch (value) {
   ...
   */
  const formatCriticalityButtonType = function (val) {
    switch (val) {
      case CriticalityTypes.Negative:
      case "1":
      case 1:
   ...
   */
  const formatToKeepWhitespace = function (value) {
   ...
   */
  const returnPercentageChange = function (kpiValue, targetValue, formatterOptions) {
    if (isNaN(Number(kpiValue))) {
      return "";
    }
   ...
   */
  const formatCurrency = function (value, formatterOptions, includeText, currency, currencyCodeText) {
    value = Number(value);
   ...
   */
  const formatHeaderCount = function (value) {
    if (value) {
   ...
   */
  const formatDate = function (dateValue, formatterOptions) {
    if (dateValue) {
   ...
   */
  const kpiValueCriticality = function (value) {
    const criticality = {
      EnumMember: "None"
    };
    switch (Number(value)) {
   ...
   */
  const formatCriticalityValueState = function (value) {
    switch (value) {
      case CriticalityTypes.Negative:
      case "1":
      case 1:
   ...
   */
  const formatCriticalityColorMicroChart = function (value) {
    switch (value) {
      case CriticalityTypes.Negative:
      case "1":
      case 1:
  ...
  __exports.getFormatters = getFormatters;
  return __exports;
});
//# sourceMappingURL=CardFormatters-dbg-dbg.js.map
