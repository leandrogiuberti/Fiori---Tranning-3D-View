...
sap.ui.define(["sap/base/Log", "sap/ui/base/BindingParser", "../config/FormatterOptions", "../utils/CommonUtils", "./Formatter"], function (Log, BindingParser, ___config_FormatterOptions, ___utils_CommonUtils, ___Formatter) {
  "use strict";
   ...
   * @returns {boolean}
   */
  function isExpression(propertyValue = "") {
   ...
   * @returns {boolean}
   */
  function isI18nExpression(propertyValue = "") {
   ...
   * @returns
   */
  function hasFormatter(propertyValue = "") {
    return propertyValue.startsWith("{=") && propertyValue.endsWith("}") && (propertyValue.includes("format.") || propertyValue.includes("extension.formatters."));
  }
   ...
   * @returns
   */
  function formatValue(propertyName, propertyValueFormatters = []) {
   ...
   * @returns {string}
   */
  function getArrangements(propertyName, options) {
    ...
    const matchedUomObj = unitOfMeasures.find(function (arrangement) {
    ...
    if (matchedUomObj && matchedArrangementObj) {
      switch (matchedArrangementObj.textArrangement) {
        ...
        case "TextFirst":
          ...
          const formattedValue = getFormattedValue(updatedVal.slice(index + 2), propertyHasFormatter, matchedUOMHasFormatter, isUoMBoolean);
  ...
  function isBooleanProperty(model, propertyName) {
   ...
   * @returns {string}
   */
  function getExpressionBindingForBooleanTypes(propertyName) {
  ...
  function getFormattedValue(updatedVal, propertyHasFormatter, matchedUOMHasFormatter, isUoMBoolean) {
   ...
   * @returns
   */
  function resolvePropertyPathFromExpression(path = "", mCardManifest) {
    let {
      propertyPath
   ...
   * @returns
   */
  function parseFormatterExpression(path = "") {
   ...
   * @returns
   */
  function updateAndGetSelectedFormatters(propertyPath) {
    const formatterOptions = getFormatterConfiguration();
  ...
  __exports.getExpressionBindingForBooleanTypes = getExpressionBindingForBooleanTypes;
