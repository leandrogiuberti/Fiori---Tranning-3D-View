    ...
    return BaseObject.extend("sap.ovp.ui.UIActions", {
        constructor: function (cfg) {
            ...
            this.startHandler = function (evt) {
                that.log("startHandler");
                //	clearTimeout(this.timer);
                //	delete this.timer;
                that.captureStart(evt);
                if (that.element) {
                    that.beforeDragCallback(evt, that.element);
                    if (that.lockMode === false) {
                        if (that.tapsNumber === 2) {
                            that.mode = "double-tap";
                            return;
                        }
                        if (evt.type === "touchstart") {
                            that.timer = setTimeout(
                                function () {
                                    if (that.isResize) {
                                        that.log("mode switched to resize");
                                        that.mode = "resize";
                                        that.resizeStartCallback(evt, that.element);
                                    } else {
                                        that.log("mode switched to drag");
                                        that.mode = "drag";
                                        that.createClone();
                                        that.dragStartCallback(evt, that.element);
                                    }
                                },
                                that.switchModeDelay
                            );
                        }
                    }
                }
            ...
            this.moveHandler = function (evt) {
                that.log("moveHandler");
                that.captureMove(evt);
                switch (that.mode) {
                    case "normal":
                        if (
                            Math.abs(that.startX - that.moveX) > that.moveTolerance ||
                            Math.abs(that.startY - that.moveY) > that.moveTolerance
                        ) {
                    ...
                    case "drag":
                        evt.preventDefault();
                        that.log("-> drag");
                        that.mode = "drag-and-scroll";
                        window.addEventListener(that.mouseUpEvent, that.endHandler, true);
                        // Adding a keyup event to terminate the drag and drop in case of a key is pressed
                        window.addEventListener(that.keyUpEvent, that.endHandler, true);
                        that.translateClone();
                        that.scrollContainer = document.querySelector(that.scrollContainerSelector);
                    ...
                    case "drag-and-scroll":
                        evt.stopPropagation();
                        evt.preventDefault();
                        that.log("-> drag-and-scroll");
                        that.translateClone();
                        that.dragMoveCallback({
                            evt: evt,
                            clone: that.clone,
                    ...
                    case "resize-and-scroll":
                        evt.stopPropagation();
                        evt.preventDefault();
                        that.log("-> resize-and-scroll");
                        that.resizeMoveCallback({
                            evt: evt,
                            element: that.element,
                            draggable: that.draggable,
                            moveX: that.moveX,
            ...
            this.clickHandler = function (event) {
                ...
                if (that.preventClickFlag) {
                    that.preventClickFlag = false;
                    event.preventDefault();
                    event.stopPropagation();
                    event.stopImmediatePropagation();
                    clearTimeout(that.preventClickTimeoutId);
                }
                that.clickCallback();
            };
            ...
            this.endHandler = function (evt) {
                that.log("endHandler");
                if (
                    (evt.type == that.keyUpEvent && evt.which == KeyCodes.ESCAPE) ||
                    evt.type == that.mouseUpEvent ||
                    evt.type == that.touchEndEvent
                ) {
                    that.captureEnd(evt);
                    switch (that.mode) {
                        case "normal":
                        ...
                        case "drag":
                            that.log("-> drag");
                            that.dragEndCallback(evt, that.element, that.clone);
                            that.preventClick();
                            break;
                        case "drag-and-scroll":
                            that.log("-> drag-and-scroll");
                            window.removeEventListener(that.mouseUpEvent, that.endHandler, true);
                            that.dragEndCallback(evt, that.element, that.clone);
                            that.preventClick();
                            evt.stopPropagation();
                            evt.preventDefault();
                        ...
                        case "double-tap":
                            that.log("-> double-tap");
                            that.doubleTapCallback(evt, that.element);
                            break;
                        case "resize":
                            that.log("-> resize");
                            that.isResize = false;
                            that.resizeEndCallback(evt, that.element);
                            that.preventClick();
                            break;
                        case "resize-and-scroll":
                            that.log("-> resize-and-scroll");
                    ...
                    if (that.element) {
                        that.endCallback(evt, that.element);
                    }
                    clearTimeout(that.timer);
                    delete that.timer;
                    that.lastMoveX = 0;
                    that.lastMoveY = 0;
                    that.swapTargetElement = null;
                    that.element = null;
                    that.mode = "normal";
                }
            };
