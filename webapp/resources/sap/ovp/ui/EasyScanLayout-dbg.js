...
sap.ui.define([
    ...
    KeyboardNavigation.prototype.init = function (easyScanLayout, swapItemFunction) {
        ...
        var that = this;
        this.jqElement.on("keydown", function(oEvent) {
            that.keydownHandler.call(that, oEvent);
        });
        this.jqElement.on("keyup", function(oEvent) {
            that.keyupHandler.call(that, oEvent);
        });
        this.jqElement.find(".after").on("focus.keyboardNavigation",  function() {
            that.afterFocusHandler.call(that);
        });
        this.jqElement
            .find(".sapUshellEasyScanLayoutInner")
            .on("focus.keyboardNavigation", function() {
                that.layoutFocusHandler.call(that);
            });
        this.jqElement.on(
        ...
        this.jqElement.on("keyup", function(oEvent) {
            that.ctrlArrowUpDownHandler.call(that, oEvent);
        });
    };
    ...
    KeyboardNavigation.prototype.layoutFocusHandler = function () {
        ...
        var firstTabbableElement = this.jqElement.find(".easyScanLayoutItemWrapper:sapTabbable").first();
        firstTabbableElement = firstTabbableElement.length === 1 ? firstTabbableElement.get(0) : firstTabbableElement;
    ...
    KeyboardNavigation.prototype.layoutItemFocusHandler = function () {
        ...
        if (jqFocused) {
            ...
            if (isNotStackCardHeader ) {
                var excludeIds = [sCardHeader, "sapOvpCardAdditionalActions", "listItem", "linkListItem"];
                var isExcluded = function(id) {
                    return !excludeIds.some(function(excludeStr) {
                        return id.includes(excludeStr);
                    });
                };
                
                for (i = 0; i < labelledElement.length; i++) {
                    var id = labelledElement[i].id;
                    var isAriaLabelRequired = id && isExcluded(id);
    ...
    KeyboardNavigation.prototype.f7Handler = function (e) {
        ...
        if (jqFocusedElement.hasClass("easyScanLayoutItemWrapper")) {
            ...
            var itemWrapper = jqFocusedElement.closest(".easyScanLayoutItemWrapper");
            itemWrapper = itemWrapper.length === 1 ? itemWrapper.get(0) : itemWrapper;
            itemWrapper.focus();
        }
    };
    ...
    KeyboardNavigation.prototype.ctrlArrowHandler = function (e) {
        ...
        if (this.swapSourceElement == null) {
            ...
            if (!this.swapSourceElement.hasClass("easyScanLayoutItemWrapper")) {
                ...
                var that = this;
                this.jqElement.on("keyup.keyboardNavigation", function(oEvent) {
                    that.keyupHandler.call(that, oEvent);
                });
