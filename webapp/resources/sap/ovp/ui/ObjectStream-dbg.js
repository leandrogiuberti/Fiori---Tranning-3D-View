...
sap.ui.define([
    ...
    KeyboardNavigation.prototype.init = function (objectStream) {
        ...
        var that = this;
        this.jqElement.on("keydown.keyboardNavigation", function(oEvent) {
            that.keydownHandler.call(that, oEvent);
        });
        this.jqElement.on(
            "focus.keyboardNavigation",
            ".sapOvpObjectStreamItem",
            this.ObjectStreamFocusAccessabilityHandler
        );
    };
    ...
    KeyboardNavigation.prototype._swapItemsFocus = function (e, jqItemFrom, jqItemTo) {
        ...
        jqItemTo = jqItemTo.length === 1 ? jqItemTo.get(0) : jqItemTo;
        jqItemFrom = jqItemFrom.length === 1 ? jqItemFrom.get(0) : jqItemFrom;

        e.preventDefault();
        jqItemFrom.setAttribute("tabindex", "-1");
        jqItemTo.setAttribute("tabindex", "0");
        jqItemTo.focus();
    };
    ...
    ObjectStream.prototype.debounceScrollHandler = function () {
        ...
        var that = this;
        scrollHandlerTimerId = window.setTimeout(function() {
            that._checkEdgesVisibility.call(that);
        }, 150);
    };
    ...
    ObjectStream.prototype._initScrollVariables = function () {
        ...
        if (this.shouldShowScrollButton) {
            ...
            jQuery(this.wrapper).on("scroll.objectStream", function() {
                that.debounceScrollHandler.call(that);
            });
        } else {
