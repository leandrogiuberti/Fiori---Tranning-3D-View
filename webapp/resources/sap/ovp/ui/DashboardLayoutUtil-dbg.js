...
sap.ui.define([
    ...
    DashboardLayoutUtil.prototype.resizeLayout = function (iWidth) {
         ...
         *  added a 20px buffer to the layout width to address the condition(in windows) where the card height causes a scrollbar to appear,
         *  which in turn causes the layout to rearrange repeatedly and results in the resize handler going into an infinite loop.
         */
        if (this.oLayoutData.layoutWidthPx !== iWidth && iWidth !== 0 && Math.abs(iWidth - this.oLayoutData.layoutWidthPx) > 20) {
    ...
    DashboardLayoutUtil.prototype._getCardController = function (cardId) {
        var oCtrl = null;
        var oComponent = CoreElement.getElementById(this._getCardComponentDomId(cardId));
        if (oComponent) {
