    ...
    function PageBuildingService (vODataBase, fnDefaultFailure, bIsPersonalization, sCacheId) {
            ...
            if (typeof vParent === "string") {
                ...
                if (vParent.instanceId) {
                    ...
                    return `ChipInstanceBags(pageId='${encodeURIComponent(vParent.pageId)
                        ...
                        }',id='${encodeURIComponent(sBagId)}')${bExpand ? "?$expand=ChipInstanceProperties" : ""}`;
                }
                // parent is a page
                sPageId = vParent.id;
            }
            return `Bags(pageId='${encodeURIComponent(sPageId)
                }',id='${encodeURIComponent(sBagId)}')${bExpand ? "?$expand=Properties" : ""}`;
        }
        ...
        this.batch = function (aChangeRequests, aGetRequests, fnSuccess, fnFailure) {
            ...
            oWrapper.batch({ __batchRequests: aRequests },
                (oData) => {
                    const aBatchResponses = oData.__batchResponses;
                    const aChangeResponses = aBatchResponses[0].__changeResponses;
        ...
        this.readMetadata = function (fnSuccess, fnFailure, bNoCache) {
            const sRequestUrl = `${oWrapper.getBaseUrl()}$metadata${bNoCache ? `?${Date.now()}` : ""}`;

            fnFailure = fnFailure || this.getDefaultErrorHandler();
            oWrapper.check(fnSuccess, fnFailure);

            OData.read(sRequestUrl, (oData) => {
        ...
        this.readAllCatalogs = function (sPageId, fnSuccess, fnFailure, sFilter, sSorting, bCache) {
            ...
            let sUrl = `Pages('${encodeURIComponent(sPageId)
        ...
        this.readAllCatalogsForUser = function (sFilter, fnSuccess, fnFailure) {
            const sUrlParameter = sFilter ? `?$filter=${encodeURIComponent(sFilter)}` : "";
        ...
        this.getReferencingCatalogIds = function (sReferencedChipId, fnSuccess, fnFailure) {
            ...
            function success (oResponse) {
                const aRefCatalogIds = [];
                const aReferenceChips = oResponse.results || [];

                aReferenceChips.forEach((oChip) => {
            ...
            oWrapper.read(`Chips?$filter=${encodeURIComponent(`referenceChipId eq '${encodeODataFilterValue(sReferencedChipId)}'`)}`,
                success,
        ...
        this.readPageSet = function (sId, fnSuccess, fnFailure, bCache) {
            ...
            if (this.readPageSet.appendedParameters) {
                ...
                if (appendedParametersKeys.length > 0) {
                    ...
                    appendedParametersKeys.forEach((key) => {
        ...
        this.updateBagProperties = function (oParent, sBagId, aChangedProperties, aNewProperties,
            ...
            const aChangeRequests = [];
            const aGetRequests = [];
            let i;
            let pos;
            let oProperty;
            let sRequestUri;
            const sMETADATA = "__metadata"; // constant to make JSLint happy
            ...
            this.batch(aChangeRequests, aGetRequests,
                (aChangeResponses, aGetResponses) => {
                    const aResetResponses =
        ...
        this.readChip = function (sChipId, fnSuccess, fnFailure) {
            ...
            let sUrl = `Chips('${encodeURIComponent(sChipId)}')?$expand=ChipBags/ChipProperties`;
