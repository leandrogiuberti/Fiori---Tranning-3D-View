...
sap.ui.define(["sap/ui/thirdparty/jquery",
	...
	function(jQuery, Element, ParsingBackendMediator, RuleServices, resourcesConvertor, RtsInitiator, responseCollector, Locale, LocaleData, vocabularyConstants, library, MetadataAnalyser, Formatting, ODataModel, Filter, coreLib) {
		"use strict";
			...
			if (bValueHelpValidation && bCallback) {
				...
				for (var i = 0; i < valueHelpInfo.length; i++) {
					if (!valueHelpInfo[i].model) {
						...
						bOdataModels = false;
						break;
					} else if (!(valueHelpInfo[i].model instanceof ODataModel)) {
						// technical value help errors are raised only at validate expression level
		...
		ExpressionLanguage.prototype._validateExpression = function(sExpression, eType, bCollection, bToken, flags) {
			...
			var parserResult = this._backendParser.parseExpression(sExpression, library.BackendParserRequest.Validate, this._runtimeService,
		...
		ExpressionLanguage.prototype.getSuggestions = function(sExpression, eType, bCollection, bToken, valueListAttribute) {
			...
			var parserResult = this._backendParser.parseExpression(sExpression, library.BackendParserRequest.Suggests, this._runtimeService,
		...
		ExpressionLanguage.prototype.getSuggestionsByCategories = function(sExpression, oFilter) {
			...
			var suggs = this.getSuggestions(sExpression, library.ExpressionType.BooleanEnhanced, false).suggs;
			suggs = suggs ? suggs : [];
		...
		ExpressionLanguage.prototype.getExpressionMetadata = function(sExpression) { //always return status Success
			...
			var parserResult = this._backendParser.parseExpression(sExpression, library.BackendParserRequest.GetMetadata, this._runtimeService,
		...
		ExpressionLanguage.prototype._setConfigurationForBasic = function() {
			...
			this.filterByTypeMap = {
				"vocabulary,undefined": [{
					tokenType: library.ExpressionTokenType.vocabulary
				}],
				"reservedword,comparisonOp": [{
				...
				"reservedword,UOM": [{
					tokenType: library.ExpressionTokenType.reservedWord,
					tokenCategory: library.ExpressionCategory.UOM
				}],
				"reservedword,function": [{
					tokenType: library.ExpressionTokenType.reservedWord,
					tokenCategory: library.ExpressionCategory.func
				}],
				"reservedword,conjunctionOp": [{
					tokenType: library.ExpressionTokenType.reservedWord,
					tokenCategory: library.ExpressionCategory.conjunctionOp
				}],
				"reservedword,value": [{
					tokenType: library.ExpressionTokenType.reservedWord,
					tokenCategory: library.ExpressionCategory.value
				}],
				"reservedword,null": [{
					tokenType: library.ExpressionTokenType.reservedWord,
					tokenCategory: null
				...
				"constant.dynamic": [{
					tokenType: library.ExpressionTokenType.constant,
					tokenCategory: library.ExpressionCategory.dynamic
				}],
				"constant.fixed": [{
					tokenType: library.ExpressionTokenType.constant,
					tokenCategory: library.ExpressionCategory.fixed
				}]
			};
		};
		...
		ExpressionLanguage.prototype._groupTokensByTokenType = function(aTokens) {
			...
			for (var i = 0; i < aTokens.length; i++) {
				var token = aTokens[i];
				if (token.tokenType === library.ExpressionTokenType.whitespace) {
					continue;
				} else if (token.tokenType !== library.ExpressionTokenType.vocabulary) {
					tokenGroups.push(token);
					previousType = token.tokenType;
				} else {
					if (previousType !== library.ExpressionTokenType.vocabulary) {
						tokenGroups.push(token);
					} else {
		...
		ExpressionLanguage.prototype._getSubExpressions = function(aTokens, expression) {
			...
			for (i = aTokens.tokens.length - 1; i >= 0; i--) {
				if (aTokens.tokens[i].info) {
					...
					if (oTokenInfo.category === library.ExpressionCategory.comparisonOp || oTokenInfo.category === library.ExpressionCategory.comparisonBetweenOp ||
						oTokenInfo.category === library.ExpressionCategory.comparisonExistOp) {
		...
		ExpressionLanguage.prototype._getBasicSuggestions = function(expression, ePart) {
			if (!ePart) {
				ePart = library.SuggestionsPart.all;
			}
			...
			if (ePart === library.SuggestionsPart.all) { // pus the left suggestions
			...
			if (ePart === library.SuggestionsPart.all || ePart === library.SuggestionsPart.compPart) {
			...
			if (ePart === library.SuggestionsPart.all || ePart === library.SuggestionsPart.rightPart) {
				var sRightExpression = (aExpression[2] && aExpression[2].exp) ? aExpression[2].exp : "";
				sCompType = (ePart === library.SuggestionsPart.all) ? oSuggestions[1].type : null;
		...
		ExpressionLanguage.prototype._getLeftSuggestions = function(sEmptyString, currentValue) {
			...
			oSuggestion.tokenCategory = library.ExpressionTokenType.vocabulary;
		...
		ExpressionLanguage.prototype._getRightSuggestions = function(sLeft, sComp, currentValue, sCompType, oTokens) {
			...
			switch (sCompType) {
				case library.ExpressionCategory.comparisonOp: //in case simple comparison
					aSuggestion = this._getRightForComparisonOp(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue);
					break;
				case library.ExpressionCategory.comparisonBetweenOp: //in case simple comparison
					aSuggestion = this._geRightForBetweenOp(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue);
					break;
				case library.ExpressionCategory.comparisonExistOp: //in case exist comparison
		...
		ExpressionLanguage.prototype._getRightForComparisonOp = function(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue) {
			...
			var i;
			oFilters = [{
				tokenType: library.ExpressionTokenType.valueList
			}];
			...
			if (sLeftBusinessDataType === library.ExpressionType.TimeSpan) { // sLeftBusinessDataType = TimeSpan (duration)
			...
			} else if (sLeftBusinessDataType === library.ExpressionType.Date) { // sLeftBusinessDataType = Date (birthdate)
				oFilters = [{
					tokenType: library.ExpressionTokenType.reservedWord,
					tokenCategory: library.ExpressionCategory.value
				}, {
		...
		ExpressionLanguage.prototype._geRightForBetweenOp = function(sLeftBusinessDataType, sLefAndCompExprerssion, currentValue) {
			...
			(function() {
				...
				if (aTokens) {
					for (var i = 0; i < aTokens.length; i++) {
						if (aTokens[i].info && (aTokens[i].info.category === library.ExpressionCategory.comparisonOp || aTokens[i].info.category ===
								library.ExpressionCategory.comparisonBetweenOp || aTokens[i].info.category === library.ExpressionCategory.comparisonExistOp
							)) {
							currentValue = "";
							return;
						}
			...
			oSuggestion[0].BDT = sLeftBusinessDataType;
			if (sLeftBusinessDataType !== library.ExpressionType.TimeSpan && sLeftBusinessDataType !== library.ExpressionType.Date) {
			...
			if (sLeftBusinessDataType === library.ExpressionType.TimeSpan) { // sLeftBusinessDataType = TimeSpan (for example duration)
			...
			} else if (sLeftBusinessDataType === library.ExpressionType.Date) { // sLeftBusinessDataType = Date (for example birthdate)
			...
			for (i = 0; i < oSuggestion.length; i++) { //put value in suggestions and set the 'to' suggestion
				if (filtered[i] === "to" || (i === 2 && sLeftBusinessDataType === library.ExpressionType.TimeSpan) //logic for putting the 'to'\'and' at currentValue
					|| (i === 1 && sLeftBusinessDataType === library.ExpressionType.Date) || ((i === 1 && sLeftBusinessDataType !== library.ExpressionType
						.Date) && (i === 1 && sLeftBusinessDataType != library.ExpressionType.TimeSpan))) {
					oSuggestion[i].tokenCategory = "reservedword.undefined";
					oSuggestion[i].currentValue = (filtered[i] === "to" || filtered[i] === "and") ? filtered[i] : "to";
				} else if (sLeftBusinessDataType === library.ExpressionType.Date) {
					oSuggestion[i].currentValue = jQuery.trim(filtered[i] || "''"); //remove extra spaces before and after string [duplicate]
				} else if (sLeftBusinessDataType === library.ExpressionType.TimeSpan) {
					oSuggestion[i].currentValue = jQuery.trim(filtered[i] || "''"); //remove extra spaces before and after string [duplicate]
				} else {
					oSuggestion[i].currentValue = jQuery.trim(filtered[i]); //remove extra spaces before and after string [duplicate]
				}
		...
		ExpressionLanguage.prototype._geRightForExistOp = function(sLeftBusinessDataType, sLefAndCompExprerssion, sRight, aTokens) {
			...
			oSuggestion[0].BDT = sLeftBusinessDataType;
			var aResults;
			if (sLeftBusinessDataType === library.ExpressionType.TimeSpan) {
				aResults = aTokens.tokens.filter(function(obj) { //comp expression
					if (obj.info) {
						return obj.info.category === library.ExpressionCategory.UOM || obj.tokenType === library.ExpressionTokenType.constant;
					}
				...
				aResults = aTokens.tokens.filter(function(obj) { //comp expression
					if (obj.info) {
						return obj.info.category === library.ExpressionCategory.value;
					}
		...
		ExpressionLanguage.prototype._buildValueHelpMap = function(valueHelpInfo) {
			...
			function _buildValueHelp(_oValueHelpInfo) {
				...
				var oMetadataAnalyzer = new MetadataAnalyser(_oModel);
				...
				if (key.length > 1) {
					for (var j = 0; j < key.length; j++) {
						if (key[j] !== keyField) {
							filters.push(new Filter(key[j], "EQ", null));
						}
					}
				}

				for (var i = 0; i < _oValueHelpInfo.values.length; i++) {
					filters.push(new Filter(keyField, "EQ", _oValueHelpInfo.values[i]));
				}
