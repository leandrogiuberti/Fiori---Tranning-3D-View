sap.ui.define([
	...
	parsingBackendMediatorLib.prototype.parseExpression = function(expression, mode, vocaRTServ, paramServ, type, vocabulary, flags, valueListAttribute) {
		...
		try {
			Logger.debug("ParsingBackendMediator expression to parse: " + expression);
			...
			switch (mode) {
				...
				case constantsLib.AUTOCOMPLETE_MODE_LOWERCASE:
					Logger.debug("mode autocomplete");
					/*
					...
					*/
					...
					Logger.debug("ParsingBackendMediator: autocomplete responseObject: " + responseObject);
					return responseObject;
				case constantsLib.PARSE_MODE:
				case constantsLib.VALIDATE_MODE:
					Logger.debug("mode validate");
			...
			Logger.error("ParsingBackendMediator error: " + err);
	...
	parsingBackendMediatorLib.prototype.validateAndGetExpressionActualReturnTypeRT = function(vocaRTServ, expression, vocabulary, paramServ, raiseError, inFlags) {
		...
		{
		Logger.debug(parseResult.actualReturnType);
		}
