...
sap.ui.define([
    ...
    SiteConverter.prototype._convertPageToSite = function (page) {
        ...
        const oSections = page.payload.sections;
        for (let i = 0; i < Object.keys(oSections).length; i++) {
            const sSectionId = Object.keys(oSections)[i];
            const oSection = page.payload.sections[sSectionId];
    ...
    SiteConverter.prototype._convertSiteToPage = function (site) {
        ...
        for (let i = 0; i < Object.keys(oGroups).length; i++) {
            ...
            const oSection = oConvertedPage.payload.sections[sGroupId];
    ...
    SiteConverter.prototype._createVizPayloadObject = function (aTiles) {
        ...
        const oViz = aTiles.reduce((oReturn, oTile) => {
    ...
    SiteConverter.prototype._convertVizPayloadObjectToArray = function (oSection) {
        return oSection.layout.vizOrder.reduce((aTiles, sVizId) => {
