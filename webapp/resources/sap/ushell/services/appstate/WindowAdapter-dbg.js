...
sap.ui.define([
...
], (LimitedBuffer, Log, jQuery) => {
    "use strict";

    const WINDOW_APPSTATE_CAPACITY = 500;
    const CROSS_ORIGIN_EXCEPTION_ERROR_CODE = 18;
    ...
    WindowAdapter.prototype._init = function (oServiceInstance, oBackendAdapter, oConfig) {
        const oInitialAppStates = oConfig && oConfig.config && oConfig.config.initialAppStates || {};
        const oInitialAppStatesPromise = oConfig && oConfig.config && oConfig.config.initialAppStatesPromise;
        ...
        if (oInitialAppStatesPromise) {
            oInitialAppStatesPromise.then((oInitialAppStates) => {
                if (typeof oInitialAppStates === "object") {
                    // register all initial keys
                    Object.keys(oInitialAppStates).forEach((sKey) => {
     ...
     *   Application state key
     *
     * @returns {boolean} whether the app state is transient.
     * @private
     */
    ...
    WindowAdapter.prototype.loadAppState = function (sKey) {
        const oDeferred = new jQuery.Deferred();
        const appStateFromWindow = WindowAdapter.prototype.data.getByKey(sKey);
        ...
        try {
            if (window.opener && window.opener.sap && window.opener.sap.ushell) {
                const oAppStateFromParentWindow = window.opener.sap.ui.require("sap/ushell/services/AppState").WindowAdapter.prototype.data.getByKey(sKey);
            ...
            if (oError.code === CROSS_ORIGIN_EXCEPTION_ERROR_CODE) {
        ...
        if (this._oBackendAdapter && !this._checkIfTransient(sKey)) {
            this._oBackendAdapter.loadAppState(sKey).done((sKey, sData, persistencyMethod, persistencySettings) => {
                // save application state in the window object (key and data)
        ...
        oDeferred.reject(new Error("AppState.js - loadAppState: Application State could not be loaded"));
        return oDeferred.promise();
    };
