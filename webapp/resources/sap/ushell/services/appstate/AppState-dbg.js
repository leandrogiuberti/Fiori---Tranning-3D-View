...
sap.ui.define([
    ...
    function save () {
        ...
        this._oServiceInstance._saveAppState(this._sKey, this._sData, this._sAppName, this._sACHComponent,
            this._bTransient, this._sPersistencyMethod, this._oPersistencySettings).done(() => {
            oDeferred.resolve();
        }).fail((oError) => {
    ...
    function deleteState () {
        const oDeferred = new jQuery.Deferred();
        this._oServiceInstance._deleteAppState(this._sKey).done(() => {
            oDeferred.resolve();
        }).fail((oError) => {
            oDeferred.reject(oError);
        });
        ...
        if (this._bModifiable) {
            this.setData = function (oData) {
                try {
                    ...
                    Log.error("Data can not be serialized", oError, "sap.ushell.services.AppState.AppState");
    ...
    AppState.prototype.getData = function () {
        ...
        try {
            ...
            Log.error(`Could not parse [${this._sData}]`, oError);
        }
        return o;
    };
