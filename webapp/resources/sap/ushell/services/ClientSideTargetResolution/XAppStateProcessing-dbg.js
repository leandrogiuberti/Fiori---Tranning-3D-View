...
sap.ui.define([
    ...
    function hasRenameTo (oMatchingTarget) {
        return oMatchingTarget.inbound && oMatchingTarget.inbound.signature && oMatchingTarget.inbound.signature.parameters
            && Object.keys(oMatchingTarget.inbound.signature.parameters).some((sKey) => {
    ...
    function removeUnusedComplexParameterValuesFromDefaultList (oMatchingTarget) {
        oMatchingTarget.defaultedParamNames = oMatchingTarget.defaultedParamNames.filter((sName) => {
    ...
    function mapDefaultParameterNames (oMatchingTarget) {
        const oParameters = (ObjectPath.get("inbound.signature.parameters", oMatchingTarget) || {});
        const oMap = {};
        oMatchingTarget.defaultedParamNames.forEach((sName) => {
            const sNewName = (oParameters[sName] && oParameters[sName].renameTo) || sName;
            if (oMap[sNewName]) {
    ...
    function isPresentProperty (aList, aParamName) {
        return Array.isArray(aList) && aList.some((oMember) => {
    ...
    function isParameterContainedInSelectionVariant (oAppStateData, aParamName) {
        ...
        if (oAppStateData && oAppStateData.selectionVariant) {
            const oSelectionVariant = new SelectionVariant(JSON.stringify(oAppStateData.selectionVariant));
            const aSelectionVariantSelectOptionsPropertyNames = (oSelectionVariant.getSelectOptionsPropertyNames() || []);
            const aSelectionVariantParameterNames = (oSelectionVariant.getParameterNames() || []);
    ...
    function renameAndRemoveDuplicates (oSelectionVariant, oRecordChange, oParameters, bIgnoreAdditionalParameters) {
        const oResultingSelectionVariant = new SelectionVariant();
        const aSelectionVariantParameterNames = (oSelectionVariant.getParameterNames() || []);
        const aSelectionVariantSelectOptionsPropertyNames = (oSelectionVariant.getSelectOptionsPropertyNames() || []);
        let sParamValue;
        let aSelectOption;
        ...
        aSelectionVariantParameterNames.forEach((sParamName) => {
        ...
        aSelectionVariantSelectOptionsPropertyNames.forEach((sSelectOptionPropertyName) => {
        ...
        oResultingSelectionVariant.getParameterNames().forEach((sParamName) => {
            oSelectionVariant.addParameter(sParamName, oResultingSelectionVariant.getParameter(sParamName));
        });
        oResultingSelectionVariant.getSelectOptionsPropertyNames().forEach((sSelectOptionPropertyName) => {
    ...
    async function _mergeAppState (oNewAppState0, oMatchingTarget) {
        ...
        const oNewAppStateData = oNewAppState0.getData() || {};
        let oSelectionVariant;
        const oChangeRecorder = {};
        const oSignature = oMatchingTarget.inbound.signature;
        const oResolutionResult = oMatchingTarget.resolutionResult;
        ...
        if (hasExtendedUserDefaults(oResolutionResult)) {
            Object.keys(oResolutionResult.oNewAppStateMembers).forEach((sName) => {
                oSelectionVariant.massAddSelectOption(sName, oResolutionResult.oNewAppStateMembers[sName].Ranges);
            });
        }

        const bIgnoreAdditionalParameters = oSignature.additionalParameters === "ignored";
    ...
    async function mixAppStateIntoResolutionResultAndRename (oMatchingTarget, oAppStateService) {
        let oNewAppState;
        const oResolutionResult = oMatchingTarget.resolutionResult;
        const oSignature = oMatchingTarget.inbound && (oMatchingTarget.inbound.signature || {});
        const bChangeExistingAppState = hasExtendedUserDefaults(oResolutionResult)
        ...
        const sSourceAppStateKey = oMatchingTarget.intentParamsPlusAllDefaults["sap-xapp-state"]
        ...
        if (sSourceAppStateKey) {
            ...
            if (isValidAppStateData(oAppStateData)) {
                if (hasExtendedUserDefaults(oResolutionResult)) {
                    const oParameterDominatorMap = CstrUtils.constructParameterDominatorMap(oSignature.parameters);
                    Object.keys(oResolutionResult.oNewAppStateMembers).forEach((sAppStateMemberName) => {
                        const aDominatorParams = oParameterDominatorMap[sAppStateMemberName].dominatedBy;
        ...
        if (hasExtendedUserDefaults(oResolutionResult)) {
            const oEmptyAppState = oAppStateService.createEmptyAppState(undefined, true);
