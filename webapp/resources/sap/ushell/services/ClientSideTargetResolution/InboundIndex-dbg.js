...
sap.ui.define([
    ...
    function getSegmentByTags (aTags) {
        return aTags
            ...
            .reduce((aUnion, aInboundSegment) => {
                Array.prototype.push.apply(aUnion, aInboundSegment.filter((oInbound) => {
    ...
    function segmentInbound (oInbound, oIndex) {
        const sSemanticObject = oInbound.semanticObject;
    ...
    function createIndex (aInbounds) {
        ...
        aInbounds.forEach((oInbound, iIndex) => {
            if (!oInbound) {
                Log.warning(
