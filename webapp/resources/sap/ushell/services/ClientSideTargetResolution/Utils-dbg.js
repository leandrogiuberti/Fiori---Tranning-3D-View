...
sap.ui.define([
    ...
    function findDominatedDefaultParameters (oIntentParamsPlusAllDefaults, aDefaultedParamNames, oParameterDominatorMap) {
        const oDominatedDefaultParameters = {};
        aDefaultedParamNames.forEach((sDefaultParamName) => {
            const aDominators = oParameterDominatorMap[sDefaultParamName].dominatedBy;
            const bDominatorFound = aDominators.some((sDominator) => {
                const bDominatorInIntentOrDefault = !!oIntentParamsPlusAllDefaults[sDominator];
                const bDominatorIsNotCurrentParam = sDominator !== sDefaultParamName;
                const bDominatorIsNotDefaultParam = aDefaultedParamNames.indexOf(sDominator) === -1;
    ...
    function constructParameterDominatorMap (oParameters) {
        ...
        Object.keys(oParameters).forEach((sKey) => {
            const sRenameTo = oParameters[sKey].renameTo || sKey;
    ...
    function filterObjectKeys (oObject, fnFilterFunction, bInPlace) {
        const oObjectToFilter = bInPlace ? oObject : deepExtend({}, oObject);
        Object.keys(oObjectToFilter).forEach((sKey) => {
    ...
    function filterObject (oObject, fnFilter) {
        const oFiltered = Object.keys(oObject).reduce((oFiltered, sKey) => {
    ...
    function inboundSignatureMeetsParameterOptions (oInboundSignatureParameters, aParsedParametersWithOptions) {
        ...
        return aParsedParametersWithOptions.every((oParsedParameterWithOptions) => {
            const sParameterName = oParsedParameterWithOptions.name;
            const oOptions = oParsedParameterWithOptions.options;
            const oInboundParameterValue = oInboundSignatureParameters[sParameterName];

            return Object.keys(oOptions).every((sOptionName) => {
                const vOptionValue = oOptions[sOptionName];
    ...
    function extractParameters (aParamsToExtract, oWithParameters) {
        const oExtractedParameters = {};
        aParamsToExtract.forEach((sParameter) => {
