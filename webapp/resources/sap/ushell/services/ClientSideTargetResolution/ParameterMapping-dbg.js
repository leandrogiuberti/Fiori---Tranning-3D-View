...
sap.ui.define([
    ...
    function mapParameterNamesAndRemoveObjects (oMatchingTarget) {
        const oNewParameters = {};
        const oOldParamsPlusAllDefaults = oMatchingTarget.intentParamsPlusAllDefaults;

        Object.keys(oOldParamsPlusAllDefaults).sort().forEach((sParamName) => {
            const sRenameTo = getRenameParameterName(sParamName, oMatchingTarget.inbound.signature, oOldParamsPlusAllDefaults[sParamName]);
