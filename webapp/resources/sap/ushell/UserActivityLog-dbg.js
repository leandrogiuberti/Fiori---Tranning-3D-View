...
sap.ui.define([
    ...
    UserActivityLogClass.prototype = {
        ...
        activate: function (/* clean */) {
            ...
            this._observedLaunchpadActions.forEach((item) => {
            ...
            this._observedGeneralActions.forEach((item) => {
            ...
            this._observedEventHubEvents.forEach((item) => {
        ...
        deactivate: function () {
            ...
            this._observedLaunchpadActions.forEach((item, i, arr) => {
            ...
            this._observedGeneralActions.forEach((item, i, arr) => {
        ...
        _tileOnTapDecorator: function (origFunc) {
            ...
            return function (/* event, ui */) {
                ...
                if (this.isA("sap.ushell.ui.launchpad.PlusTile")) {
                    ...
                    let sNavigationHash = hasher.getHash();
                     ...
                     */
                    if (sNavigationHash) {
                        const oNavObj = urlParsing.parseShellHash(sNavigationHash);
                    ...
                    const oLastNavigationActionData = that._getLastNavActionFromStorage();
                    ...
                    // Get tile title
                    const oTile = Element.getElementById(this.getId());
                    const oModel = oTile.getModel();
        ...
        _addMessageInternal: function (type, messageText, messageID) {
            const aLoggingQueue = this._getLoggingQueueFromStorage();
            const oLoggedMessage = {
                type: null
            };
            for (const prop in this.messageType) {
            ...
            extend(oLoggedMessage, {
                ...
                time: new Date(),
                toString: function () {
                    const aStringBuffer = [this.type, this.time];
        ...
        _handleAction: function (sChannelId, sEventId, oData) {
            ...
            switch (sEventId) {
                ...
                case "deleteGroup":
                    ...
                    break;
                case "addTile":
                    const oModel = oData.catalogTileContext.oModel;
                    const sPath = oData.catalogTileContext.sPath;
                    const oTileContext = oModel.getProperty(sPath);
                    const sTileID = (oTileContext && oTileContext.id) || "";
                    const oGroupContext = oData.groupContext.getObject();
                    const sGroupID = (oGroupContext && oGroupContext.id) || "";
                ...
                case "appOpened":
                    ...
                    const lastNavigationActionData = this._getLastNavActionFromStorage();
        ...
        _getLoggingQueueFromStorage: function () {
            const sLoggingQueue = this._getFromSessionStorage("sap.ushell.UserActivityLog.loggingQueue");
            let aQueue = [];
            if (sLoggingQueue) {
                try {
        ...
        _getLastNavActionFromStorage: function () {
            const sLastNavigationActionData = this._getFromSessionStorage("sap.ushell.UserActivityLog.lastNavigationActionData");
    ...
    const UserActivityLog = new UserActivityLogClass();
