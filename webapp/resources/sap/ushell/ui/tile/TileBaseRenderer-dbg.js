...
sap.ui.define([
    ...
    TileBaseRenderer.highlight = function (aHighlightTerms, sText) {
        let i;
        let regexpHighlightTerm;
        // immediately escape string for displaying as HTML
        let sEscapedText = encodeXML(sText).replace(/&#xa;/g, "<br>");
    ...
    TileBaseRenderer.render = function (oRm, oControl) {
        ...
        oRm.accessibilityState(oControl, {
            label: ushellResources.i18n.getText("tileTypeListItem")
                + ushellResources.i18n.getText("TileDetails_lable")
                + ushellResources.i18n.getText("TileTitle_lable")
                + oControl.getTitle(),
            level: "3"
        });
        oRm.openEnd();
        ...
        if (oControl.getSubtitle()) {
            ...
            oRm.accessibilityState(oControl, {
                label: ushellResources.i18n.getText("TileSubTitle_lable") + oControl.getSubtitle(),
                level: "4"
            });
            oRm.openEnd();
            // note: this mustn't be escaped, as highlight already does that
        ...
        if (oControl.getInfo() || ((typeof (this.getInfoPrefix) === "function")) && this.getInfoPrefix(oControl)) {
            ...
            oRm.accessibilityState(oControl, {
                label: ushellResources.i18n.getText("TileInfo_lable") + oControl.getInfo()
            });
            oRm.openEnd();
