...
sap.ui.define([
     ...
     * @private
     */
    const TileContainer = Control.extend("sap.ushell.ui.launchpad.TileContainer", /** @lends sap.ushell.ui.launchpad.TileContainer.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
                scrollType:
                ...
                showNoData:
                    { type: "boolean", group: "Misc", defaultValue: false },
                noDataText:
                    { type: "string", group: "Misc", defaultValue: "" }, // default value is set in init
                isGroupLocked:
    ...
    TileContainer.prototype.init = function () {
        ...
        this.setProperty("noDataText", ushellResources.i18n.getText("noFilteredItems"), true);

        this._sDefaultValue = ushellResources.i18n.getText("new_group_name");

        this.oNoLinksText = new Text({
            text: ushellResources.i18n.getText("emptyLinkContainerInEditMode")
        }).addStyleClass("sapUshellNoLinksAreaPresentTextInner");

        this.oTransformationErrorText = new Text({
            text: ushellResources.i18n.getText("transformationErrorText")
        ...
        const fnFocusOut = function () {
            this.getDomRef("groupheader").focus();
            this._stopEdit();
            const oEditInputFieldDomRef = this.oEditInputField.getDomRef();
        ...
        this.oEditInputField = new Input({
            ...
            onAfterRendering: function () {
                ...
                setTimeout(() => {
                    const $Ref = this.oEditInputField.$("inner");
                    $Ref.focus();
                    // jQuery Plugin "selectText"
    ...
    TileContainer.prototype.onBeforeRendering = function () {
        const oTitleText = this.getDomRef("titleText");
        if (oTitleText) {
            oTitleText.removeEventListener("click", this.fnTitleTextClickHandler);
        }

        const aTiles = this.getTiles();
        const aLinks = this.getLinks();
        const aTilesAndLinks = aTiles.concat(aLinks);

        aTilesAndLinks.forEach((oTile) => {
            const oDomRef = oTile.getDomRef();
            if (oDomRef) {
                const sOldValue = oDomRef.getAttribute("data-oldTabindex");
                if (sOldValue) {
    ...
    TileContainer.prototype.onAfterRendering = function () {
        ...
        aTilesAndLinks.forEach((oTile) => {
            const oDomRef = oTile.getDomRef();
            if (oDomRef) {
                const sOldValue = oDomRef.getAttribute("tabindex");
                if (sOldValue) {
        ...
        const aCards = aTiles.filter((tile) => {
    ...
    TileContainer.prototype._titleTextClickHandler = function () {
        const bEnableRenameLockedGroup = this.getModel() && this.getModel().getProperty("/enableRenameLockedGroup");
        const bEditMode = (bEnableRenameLockedGroup || !this.getIsGroupLocked())
    ...
    TileContainer.prototype.isLinkPersonalizationOveride = function () {
        TileContainer.prototype.updateLinks = function (sReason) {
            const oGroupsBox = this.getParent();
            const bTabsMode = oGroupsBox && oGroupsBox.getDisplayMode && oGroupsBox.getDisplayMode() === "tabs";
    ...
    TileContainer.prototype._resizeCards = function (cards) {
        let oCard;
        let oManifest;
        const fSingleCellSize = 5.5;
        const fTileMargin = 0.4375;
        let iCardRows;
        let iCardCols;
        let sCardWidth;
        let sCardHeight;
    ...
    TileContainer.prototype._stopEdit = function () {
        const sCurrentTitle = this.getHeaderText();
        let sNewTitle = this.oEditInputField.getValue();
        let bHasChanged;
