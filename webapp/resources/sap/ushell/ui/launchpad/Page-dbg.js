...
sap.ui.define([
     ...
     * @private
     */
    const Page = Control.extend("sap.ushell.ui.launchpad.Page", /** @lends sap.ushell.ui.launchpad.Page.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
    ...
    Page.prototype._saveFocus = function (oEvent) {
        const oSrcControl = oEvent.srcControl;
        ...
        // Visualization Control
            ...
            const aGridItems = oSrcControl.getItems();

            this._oLastFocusedViz = aGridItems.find((oControl) => {
    ...
    Page.prototype._handleBeforeFastNavigationFocus = function (oEvent) {
        ...
        if (bEdit && this._oLastFocusedSection) {
            ...
            const oSection = aSections.find((section) => {
    ...
    Page.prototype.onBeforeRendering = function () {
        const iNrOfSections = this.getSections().length;
        const aAddSectionButtons = this.getAggregation("_addSectionButtons") || [];
        let oAddSectionButton;
        ...
        for (let i = aAddSectionButtons.length; i < iNrOfSections + 1; i++) {
    ...
    Page.prototype.setNoSectionsText = function (text) {
        ...
        const oNoSectionText = this.getAggregation("_noSectionText");
    ...
    Page.prototype._focusNextVisualization = function (oInfo) {
        const aSections = this.getSections();
        let iSectionIndex = this.indexOfSection(oInfo.section);
        const oOrigEvent = oInfo.event.getParameter ? oInfo.event.getParameter("event") : oInfo.event;
        const oDomRef = oOrigEvent.target.firstElementChild;
        let oSection;
        let bFocused;
    ...
    Page.prototype._handleKeyboardPageNavigation = function (oEvent) {
        ...
        for (let i = 0; i < aSections.length; i++) {
            const oSectionDomRef = aSections[i].getDomRef();
    ...
    Page.prototype._isSectionVisible = function (section, bAtLeastOneViz) {
        const bEdit = bAtLeastOneViz ? false : this.getEdit();
    ...
    Page.prototype._getSectionAreaItems = function (oElement) {
        let aItems; let sDisplayFormat;

        // get displayFormat assuming the element is a visualization
        const oBindingContext = oElement.getBindingContext();
        if (oBindingContext) {
            sDisplayFormat = oBindingContext.getProperty("displayFormatHint");
        }

        const aSections = this.getSections();
        const oTargetSection = aSections.find((oSection) => {
    ...
    Page.prototype.announceMove = function () {
        const sSectionMovedMessage = resources.i18n.getText("PageRuntime.Message.SectionMoved");
