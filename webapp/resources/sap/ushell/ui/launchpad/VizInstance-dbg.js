...
sap.ui.define([
     ...
     * @private
     */
    const VizInstance = VizInstanceBase.extend("sap.ushell.ui.launchpad.VizInstance", /** @lends sap.ushell.ui.launchpad.VizInstance.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
                title: {
    ...
    VizInstance.prototype.setContent = function (content) {
        const oGridData = this.getLayoutData();
        ...
        const sDataHelpId = this.getDataHelpId();
        const sVizRefId = this.getVizRefId();
        if (sDataHelpId) {
    ...
    VizInstance.prototype._disableTabindexOfContent = function (content) {
        if (content) {
            if (content.isA("sap.ui.core.ComponentContainer")) {
                const sComponentName = content.getComponent();
                const oComponent = Component.getComponentById(sComponentName);
                if (oComponent) {
                ...
                const oFlexBox = content;
                const oItem = content.getItems()[0];
    ...
    VizInstance.prototype._disableTabindexOfGenericTile = function (genericTile) {
        function removeTabindex (control) {
            const oFocusDomRef = control.getFocusDomRef();
            if (oFocusDomRef) {
                this._sGenericTileId = oFocusDomRef.id;
        ...
        removeTabindex.call(this, genericTile);
    ...
    VizInstance.prototype.getFocusDomRef = function () {
        ...
        const oContent = this.getContent();
        let aPossibleControls = oContent.findAggregatedObjects(true, (oControl) => {
        ...
        if (oContent.isA("sap.ui.core.ComponentContainer")) {
            ...
            if (oComponent?.findAggregatedObjects) {
                aPossibleControls = oComponent.findAggregatedObjects(true, (oControl) => {
                    return oControl.isA("sap.m.GenericTile") || oControl.isA("sap.f.Card") || oControl.isA("sap.m.SlideTile");
                });
    ...
    VizInstance.prototype.onAfterRendering = function () {
        ...
        if (oDomRef.parentElement) {
            ...
            if (this._sGenericTileId) {
                oDomRef.parentElement.setAttribute("aria-labelledby", this._sGenericTileId);
            }
        }
    };
    ...
    VizInstance.prototype.getAvailableDisplayFormats = function () {
        const aSupportedDisplayFormats = this.getSupportedDisplayFormats();
        const sDisplayFormat = this.getDisplayFormat();

        const aAvailableDisplayFormats = aSupportedDisplayFormats.filter((displayFormat) => {
            return displayFormat !== sDisplayFormat;
        });

        const iCompactIndex = aAvailableDisplayFormats.indexOf("compact");
    ...
    VizInstance.prototype._getRemoveIconVBox = function () {
        if (!this._oRemoveIconVBox) {
            const oMBundle = Library.getResourceBundleFor("sap.m");
    ...
    VizInstance.prototype._getActionModeButtonIconVBox = function () {
        if (!this._oActionModeButtonIconVBox) {
            ...
            const fnOriginalOnKeyUpFunction = this._oActionModeIcon.onkeyup;
