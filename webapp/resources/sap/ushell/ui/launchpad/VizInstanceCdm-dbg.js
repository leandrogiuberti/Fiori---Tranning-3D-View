...
sap.ui.define([
     ...
     * @since 1.78
     */
    const VizInstanceCdm = VizInstance.extend("sap.ushell.ui.launchpad.VizInstanceCdm", /** @lends sap.ushell.ui.launchpad.VizInstanceCDM.prototype */ {
        metadata: {
            library: "sap.ushell"
        },
    ...
    VizInstanceCdm.prototype.load = function (bIsCustomVizType) {
        ...
        oComponentPromise
            .then((oComponent) => {
                ...
                const oContainer = new ComponentContainer({ component: oComponent });
    ...
    VizInstanceCdm.prototype._loadStandardVizType = function () {
        const oComponentData = this._getComponentConfiguration();
    ...
    VizInstanceCdm.prototype._loadCustomVizType = function () {
        const oAppProperties = this._getCustomComponentConfiguration();
        const oParsedShellHash = {};
        const aWaitForBeforeInstantiation = [];

        return Container.getServiceAsync("Ui5ComponentLoader")
            .then((oComponentLoader) => {
    ...
    VizInstanceCdm.prototype._getComponentConfiguration = function () {
        const oVizType = this.getInstantiationData().vizType;
        const oVizConfig = this.getVizConfig();
        ...
        const bIncludeVizType = ObjectPath.get(["sap.platform.runtime", "includeManifest"], oVizType);
        const bIncludeVizConfig = ObjectPath.get(["sap.platform.runtime", "includeManifest"], oVizConfig);
    ...
    VizInstanceCdm.prototype._getCustomComponentConfiguration = function () {
        const oComponentConfiguration = this._getComponentConfiguration();
    ...
    VizInstanceCdm.prototype.setPreview = function (bPreview) {
        const bRefresh = !!this.getProperty("preview") && !bPreview;
