...
sap.ui.define([
    ...
    TileRenderer.render = function (rm, tile) {
        ...
        const oTileContainer = tile.getParent();

        if (!oTileContainer) {
            return;
        }

        const bNoContainerMode = tile.getNoContainerMode();
        ...
        if (oTileView && oTileView.getContent) {
            const aContent = oTileView.getContent();
        ...
        const ariaDescribedBy = `${oTileContainer.getId()}-titleText`;
