...
sap.ui.define([
    ...
    const ExtendedChangeDetection = ManagedObject.extend("sap.ushell.ui.launchpad.ExtendedChangeDetection", /** @lends sap.ushell.ui.launchpad.ExtendedChangeDetection.prototype */ {
        metadata: {
            library: "sap.ushell",
            events: {
                /**
        ...
        constructor: function (aggregationName, control, siblingAggregationNames) {
            ...
            const bEnabled = Config.last("/core/spaces/extendedChangeDetection/enabled");
    ...
    ExtendedChangeDetection.prototype._updateAggregation = function () {
        const oBindingInfo = this._oControl.getBindingInfo(this._sAggregationName);
        const oBinding = oBindingInfo.binding;

        const bUpdateFromSibling = this._isUpdateFromSibling();

        // It's important to call getContexts after lastContextData was evaluated
        // because it overrides the aLastContextData cache
        const aLastContexts = oBinding.aLastContextData || [];
        const aLastItemKeys = aLastContexts.map((sContext) => {
            // The context data might be only a key or a stringified object
            try {
                const oContext = JSON.parse(sContext);
        ...
        const aCurrentItemKeys = aContexts.map((oContext) => {
            return oContext.getProperty(oBindingInfo.key);
        });

        const aDiff = aContexts.diff;
        let i;
        const aInsertedItems = [];
        const aRemovedItems = [];
        const oRemovedControls = {};
        let bItemsDeleted = false;
        let bItemsAdded = false;
        if (aDiff) {
            ...
            for (i = 0; i < aInsertedItems.length; i++) {
                const oInsertedItem = aInsertedItems[i];
                const oRemovedControl = oRemovedControls[oInsertedItem.item];
            ...
            for (const sKey in oRemovedControls) {
    ...
    ExtendedChangeDetection.prototype._isUpdateFromSibling = function () {
        const sIdentifier = `${this._oControl.getId()}:${this._sAggregationName}`;
    ...
    ExtendedChangeDetection.prototype._refreshSiblingBindings = function () {
        if (this._aSiblingAggregationNames) {
            for (let index = 0; index < this._aSiblingAggregationNames.length; ++index) {
                const sSiblingName = this._aSiblingAggregationNames[index];
                const sSiblingIdentifier = `${this._oControl.getId()}:${sSiblingName}`;

                const oSiblingBinding = this._oControl.getBinding(sSiblingName);
    ...
    ExtendedChangeDetection.prototype._createAndInsertItem = function (context, bindingInfo, index) {
        const oItem = bindingInfo.factory(null, context);
    ...
    ExtendedChangeDetection.prototype._updateBindingContexts = function (contexts) {
        const aItems = this._oAggregation.get(this._oControl);
