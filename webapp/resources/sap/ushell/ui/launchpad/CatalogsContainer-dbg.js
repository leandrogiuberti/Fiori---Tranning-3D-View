...
sap.ui.define([
     ...
     * @private
     */
    const CatalogsContainer = Control.extend("sap.ushell.ui.launchpad.CatalogsContainer", /** @lends sap.ushell.ui.launchpad.CatalogsContainer.prototype */{
        metadata: {
            library: "sap.ushell",
            properties: {
        ...
        renderer: {
            ...
            render: function (oRm, oControl) {
                const aCatalogs = oControl.getCatalogs();

                oRm.openStart("div", oControl);
                oRm.class("sapUshellCatalogsContainer");
                oRm.openEnd(); // div - tag

                for (let i = 0, len = aCatalogs.length; i < len; ++i) {
    ...
    CatalogsContainer.prototype.applyPagingCategoryFilters = function (nAllocatedUnits, sCatalog) {
        ...
        if (!this.filters.catalogEntry) {
            this.filters.catalogEntry = function (entry) {
                ...
                const bIsSelectedCatalog = entry.getProperty("title") === this.sCatalogTerm;
                ...
                // we need to check if this catalog was fully loaded or event not loaded, and if so to load it.
                const indexEntry = this.indexingMaps.onScreenPathIndexMap[sPath];
    ...
    CatalogsContainer.prototype.processFiltering = function (entry) {
        const aCatalogs = this.getCatalogs();
        const sPath = entry.getPath();
        const indexEntry = this.indexingMaps.onScreenPathIndexMap[sPath];
        const oCatalogEntry = aCatalogs[indexEntry.aItemsRefrenceIndex];
        let prevAllocatedUnits;
        ...
        if (indexEntry.isVisible && (!this.catalogPagination[sPath] || this.catalogPagination[sPath] === "start")) {
            if (this.catalogPagination[sPath] === undefined) {
                ...
                oCatalogEntry.catalogState = {};
            } else if (this.catalogPagination[sPath] === "start") {
                const numOfVisibleElementsInCatalog = oCatalogEntry.nNumberOfVisibleElements;
            ...
            oCatalogEntry.handleElements("appBoxesContainer");
            oCatalogEntry.handleElements("customTilesContainer");

            if (oCatalogEntry.catalogState.appBoxesContainer === "full" && oCatalogEntry.catalogState.customTilesContainer === "full") {
            ...
            const nrOfVisibleElementsInCatalog = oCatalogEntry.nNumberOfVisibleElements;
            const fullNumOfVisibleElementsInCatalog = nrOfVisibleElementsInCatalog.appBoxesContainer + nrOfVisibleElementsInCatalog.customTilesContainer;
    ...
    CatalogsContainer.prototype.addNewItem = function (elementToDisplay, sName) {
        ...
        if (!oNewCatalog.filters) {
            ...
            oNewCatalog.filters.appBoxesContainer.pagination = function (entry) {
                const indexEntry = this.indexingMaps.appBoxesContainer.onScreenPathIndexMap[entry.getPath()];
            ...
            oNewCatalog.filters.customTilesContainer.pagination = function (entry) {
                const indexEntry = this.indexingMaps.customTilesContainer.onScreenPathIndexMap[entry.getPath()];
    ...
    CatalogsContainer.prototype.filterElements = function () {
        ...
        const indexSearchMissingFilteredElem = TileContainerUtils.markVisibleOnScreenElements(aBindingContexts, this.indexingMaps, false);
