...
sap.ui.define([
     ...
     * @private
     */
    const CompactArea = Control.extend("sap.ushell.ui.launchpad.section.CompactArea", /** @lends sap.ushell.ui.launchpad.section.CompactArea.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
                showEmptyLinksArea:
    ...
    CompactArea.prototype.focusItem = function (itemIndex) {
        ...
        if (itemIndex >= 0) {
            const oFocusDomRef = aItems[itemIndex].getFocusDomRef() || aItems[itemIndex].getDomRef(); // getFocusDomRef may return null;
    ...
    CompactArea.prototype.onmousedown = function (oEvent) {
        const itemIndex = this.getDOMItemIndex(oEvent.srcControl.getDomRef());
        let iFocusedItemIndex = this.getFocusedItemIndex();
    ...
    CompactArea.prototype.getDOMItemIndex = function (oElement) {
        let itemIndex = -1;
        this.getItems().forEach((oItem, index) => {
            const oDomRef = oItem.getDomRef();
