...
sap.ui.define([
     ...
     * @deprecated since 1.120
     */
    const LinkTileWrapper = Control.extend("sap.ushell.ui.launchpad.LinkTileWrapper", /** @lends sap.ushell.ui.launchpad.LinkTileWrapper.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
                uuid:
    ...
    LinkTileWrapper.prototype._onFailedStatePress = function () {
        if (!this.getTileActionModeActive() && (this._getTileState() === "Failed")) {
            if (!this.FailedTileDialog) {
                sap.ui.require(["sap/ushell/ui/launchpad/FailedTileDialog"], (FailedTileDialog) => {
    ...
    LinkTileWrapper.prototype._getTileState = function () {
        const aTileViews = this.getTileViews();
        let oTileView;
        for (let i = 0; (i < aTileViews.length) && !oTileView; ++i) {
