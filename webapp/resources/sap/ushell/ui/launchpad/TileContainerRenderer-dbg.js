...
sap.ui.define([
    ...
    TileContainerRenderer.render = function (oRm, tileContainer) {
        const aHeaderActions = tileContainer.getHeaderActions();
        const aBeforeContent = tileContainer.getBeforeContent();
        const aAfterContent = tileContainer.getAfterContent();
        const aLinks = tileContainer.getLinks();
        const iNrOfLinks = aLinks.length;
        const that = this;
        ...
        if (aBeforeContent.length && tileContainer.getTileActionModeActive()) {
            ...
            oRm.openEnd(); // div - tag
            aBeforeContent.forEach((oBeforeContent) => {
        ...
        if (tileContainer.getShowHeader()) {
            ...
            if (tileContainer.getEditMode()) {
                ...
                const sTagName = tileContainer.getHeaderLevel().toLowerCase();
            ...
            if (tileContainer.getIsGroupLocked()) {
                ...
                oRm.openEnd(); // div - tag
                aHeaderActions.forEach((oHeaderAction) => {
        ...
        let bHasVisibleContent = false;
        tileContainer.getTiles().forEach((oTile) => {
            if (oTile.getVisible()) {
        ...
        // Links rendering
        const bLineModeContainer = tileContainer.getSupportLinkPersonalization();
        if (iNrOfLinks > 0 || bLineModeContainer) {
            ...
            if (bLineModeContainer) {
                ...
                let aLinkCustomData;
                let bLinkPosInsetFound;
                let bLinkSetSizeFound;
                let bLinkDataHelpIdFound;
                ...
                aLinks.forEach((link, index) => {
                    ...
                    aLinkCustomData.forEach((oCustomData) => {
        ...
        if (aAfterContent.length && tileContainer.getTileActionModeActive()) {
            ...
            oRm.class("sapContrastPlus");
            oRm.openEnd(); // div - tag
            aAfterContent.forEach((oAfterContent) => {
    ...
    TileContainerRenderer._isUserActivityCard = function (oCard) {
        ...
        const sCardTitle = oManifest && oManifest["sap.card"] && oManifest["sap.card"].header && oManifest["sap.card"].header.title;
