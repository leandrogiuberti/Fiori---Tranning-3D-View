...
sap.ui.define([
...
], (Supportability, URI, utils, LaunchpadError) => {
    "use strict";
     ...
     * @private
     */
    function HttpClient (basePath, defaultOptions) {
        // Initialize state
    ...
    HttpClient.prototype.abortAll = function () {
        Object.keys(this._oOngoingRequests).forEach((sKey) => {
     ...
     * @private
     */
    HttpClient.prototype._executeRequest = async function (sRequestMethod, sResourcePath, oData) {
        const oDefaultHeaders = {};
        const bSapStatisticsEnabled = Supportability.isStatisticsEnabled();
        if (bSapStatisticsEnabled) {
            oDefaultHeaders["sap-statistics"] = bSapStatisticsEnabled;
        }

        // Build URI and create XHR
        let oCompleteResourceURI;
        const oResourceURI = new URI(sResourcePath);
        if (!oResourceURI.is("url")) {
            throw new Error("The provided resource path is not a valid URL.");
        }
        ...
        const oHeaders = Object.assign({}, oDefaultHeaders, this._oDefaultOptions.headers, oData && oData.headers);
        ...
        return this._getCSRFTokenHeader(sRequestMethod, oCompleteResourceURI)
            .then((oCSRFHeader) => {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    const sCompleteResourceURI = oCompleteResourceURI.href();
                    ...
                    oXhr.onerror = function () {
                        const oResponse = this._getFormattedResponse(oXhr);
                    ...
                    Object.keys(oHeaders).forEach((sKey) => {
                        oXhr.setRequestHeader(sKey, oHeaders[sKey]);
                    });

                    let sBody = oData && oData.data || "";
    ...
    HttpClient.prototype._getCSRFTokenHeader = function (sRequestMethod, oAbsoluteResourceURI) {
        ...
        switch (sRequestMethod) {
            ...
            case "POST":
                ...
                return this._executeRequest("HEAD", sRequestURL).then((oResponse) => {
                    const sCSRFToken = this._getCSRFTokenFromHeader(oResponse.responseHeaders);
            ...
            case "DELETE":
                ...
                return this._executeRequest("OPTIONS", sRequestURL).then((oResponse) => {
                    const sCSRFToken = this._getCSRFTokenFromHeader(oResponse.responseHeaders);
    ...
    HttpClient.prototype._onLoad = function (fnResolve, fnReject, oOriginalRequest) {
        const oFormattedResponse = this._getFormattedResponse(oOriginalRequest.xhr);

        const sCSRFToken = this._getCSRFTokenFromHeader(oFormattedResponse.responseHeaders);
        if (sCSRFToken) {
            const sKey = this._getCSRFTokenKey(new URI(oOriginalRequest.url));
        ...
        const oError = new LaunchpadError(`Failed to execute HTTP request: ${oFormattedResponse.status} (${oFormattedResponse.status})`, oFormattedResponse);
        fnReject(oError);
    };
    ...
    HttpClient.prototype._getCSRFTokenFromHeader = function (aResponseHeaders) {
        const oCSRFToken = aResponseHeaders.filter((oHeader) => {
