...
sap.ui.define([
...
], (Log, ObjectPath, Container) => {
    "use strict";

    const DYNAMIC_BASE_CHIP_ID = "X-SAP-UI2-CHIP:/UI2/DYNAMIC_APPLAUNCHER";
    const STATIC_BASE_CHIP_ID = "X-SAP-UI2-CHIP:/UI2/STATIC_APPLAUNCHER";
    ...
    chipsUtils.getCatalogTilePreviewTitle = function (oCatalogTile) {
        const sAppLauncherTitle = chipsUtils.getBagText(oCatalogTile, "tileProperties", "display_title_text");
    ...
    chipsUtils.getCatalogTilePreviewSubtitle = function (oCatalogTile) {
        const sAppLauncherSubTitle = chipsUtils.getBagText(oCatalogTile, "tileProperties", "display_subtitle_text");
    ...
    chipsUtils.getCatalogTilePreviewIcon = function (oCatalogTile) {
        const sAppLauncherIcon = chipsUtils._getConfigurationProperty(oCatalogTile, "tileConfiguration", "display_icon_url");
    ...
    chipsUtils.getCatalogTileTargetURL = function (oCatalogTile) {
        const sAppLauncherTarget = chipsUtils._getConfigurationProperty(oCatalogTile, "tileConfiguration", "navigation_target_url");
    ...
    chipsUtils.getCatalogTilePreviewIndicatorDataSource = function (oCatalogTile) {
        ...
        const oTileConfiguration = chipsUtils._getAppLauncherTileConfiguration(oCatalogTile);
    ...
    chipsUtils.getTargetUrlFromSimplifiedChip = function (oSimplifiedChip, URLParsing) {
        const oCustomTileTarget = this.getCustomTileTargetFromSimplified(oSimplifiedChip);
        if (oCustomTileTarget) {
            return `#${URLParsing.constructShellHash(oCustomTileTarget)}`;
        }

        const oTileConfiguration = this.getTileConfigurationFromSimplifiedChip(oSimplifiedChip);
    ...
    chipsUtils.getIndicatorDataSourceFromSimplifiedChip = function (oSimplifiedChip) {
        ...
        const sServiceUrl = oTileConfiguration.service_url;
    ...
    chipsUtils.getTileSizeFromSimplifiedChip = function (oSimplifiedChip) {
        const oConfiguration = oSimplifiedChip.configuration;
        const sRow = oConfiguration.row || 1;
        const sCol = oConfiguration.col || 1;
    ...
    chipsUtils.getInfoFromSimplifiedChip = function (oSimplifiedChip) {
        const oTileProperties = oSimplifiedChip.bags.tileProperties;
    ...
    chipsUtils.getKeywordsFromSimplifiedChip = function (oSimplifiedChip) {
        const oTileProperties = oSimplifiedChip.bags.tileProperties;
        const sKeywordsText = oTileProperties && oTileProperties.texts.display_search_keywords;
    ...
    chipsUtils.isCustomTileFromSimplifiedChip = function (oSimplifiedChip) {
        const sChipId = oSimplifiedChip.chipId;
    ...
    chipsUtils.loadChipInstanceFromSimplifiedChip = function (oSimplifiedChip) {
        const oBags = oSimplifiedChip.bags;
        const oRawData = {
        ...
        return Container.getServiceAsync("PageBuilding")
            .then((oPageBuildingService) => {
                ...
                const oChipInstance = oFactory.createChipInstance(oRawData);

                this.addBagDataToChipInstance(oChipInstance, oBags);
                return new Promise((resolve, reject) => {
                    oChipInstance.load(resolve, (sErrorMessage) => {
    ...
    chipsUtils.addBagDataToChipInstance = function (oChipInstance, oBags) {
        ...
        for (const sBagId in oBags) {
            ...
            try {
                ...
                Log.error("chipsUtils.addBagDataToChipInstance:", oError);
            }
        }
    };
    ...
    chipsUtils.getCustomTileTargetFromSimplified = function (oSimplifiedChip) {
        ...
        // Smart Business tile
        const oTileConfiguration = JSON.parse(oSimplifiedChip.configuration.tileConfiguration || "{}");
