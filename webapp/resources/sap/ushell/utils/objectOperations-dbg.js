...
sap.ui.define([
    ...
    objectOperations.getMember = function (oObject, sAccessPath) {
        const oPathSegments = sAccessPath.split(".");
        let oNextObject = oObject;
        if (!oObject) {
            return undefined;
        }
        oPathSegments.forEach((sSegment) => {
    ...
    objectOperations.updateProperties = function (oTarget, oSource) {
        Object.keys(oTarget).forEach((sPropertyName) => {
    ...
    objectOperations.getNestedObjectProperty = function (aObjects, oAccessPath, oDefault) {
        ...
        const MISSING_OR_INVALID_PARAMETER = "Missing or invalid parameter";
        ...
        if (!Array.isArray(oAccessPath)) {
            aAccessPaths = Array.apply(null, Array(aObjects.length)).map(() => { return oAccessPath; });
        }

        // Retrieve nested property for each object and take the first one found
        const oNestedObjectProperty = aObjects.reduce((oMatch, oObject, iIndex) => {
            if (oMatch !== undefined) {
                return oMatch;
            }
