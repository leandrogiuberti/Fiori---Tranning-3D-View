...
sap.ui.define(["sap/base/Log", "sap/ushell/utils/UrlParsing"], (Log, urlParsing) => {
    ...
    function UrlShortening () {
        ...
        this._replaceSapXAppStateRawWithKeyIfRequired = function (value, oStoreContext) {
            ...
            const sKey = oStoreContext && oStoreContext.getNextKey();
        ...
        this.compactHash = function (sUrl, aRetainParameterList, oStoreContext) {
            let prependHash;
            const aRectifiedRetainParameterList = [this.ABBREV_PARAM_NAME, "sap-system", "sap-xapp-state"];
            if (aRetainParameterList) {
                aRetainParameterList.forEach((sArg) => {
                    aRectifiedRetainParameterList.push(sArg);
                });
            }
            // decompose the URL
            const oSegments = urlParsing.parseShellHash(sUrl);
            if (!oSegments) {
                Log.error(
            ...
            const value = oSegments && oSegments.params && oSegments.params["sap-xapp-state"] && oSegments.params["sap-xapp-state"][0];
            if (value) {
            ...
            const sKey = oStoreContext && oStoreContext.getNextKey();
            const oResult = this._splitParameters(oSegments.params, aRectifiedRetainParameterList, sKey);
            if (!oResult.key) {
                // no shortening required or parameter can not be shortened
                return { hash: sUrl };
            }
            const sResString = urlParsing.paramsToString(oResult.tailParams);
        ...
        this.checkHashLength = function (sURL) {
            ...
            const oResult = this._splitParameters(oSegments.params, [], "DummyKey");
            if (oResult.key) {
        ...
        this._findIndex = function (aArray, sTest) {
            let r = aArray.indexOf(sTest);
            if (r >= 0) {
                return r; // exact match
            }
            // poor mans findIndex
            r = -1;
            aArray.every((sArr, iIndex) => {
        ...
        this._cmpByList = function (aArray, oA, oB) {
            ...
            const i1 = this._findIndex(aArray, oA);
            const i2 = this._findIndex(aArray, oB);
            if (i1 >= 0 && i2 >= 0) {
                if ((i1 - i2) !== 0) {
                    return i1 - i2;
                }
        ...
        this.expandParamGivenRetrievalFunction = function (sUrl, sParamName, fRetrieveValue) {
            ...
            // decompose the URL
            const segments = urlParsing.parseShellHash(sUrl);
            // non parsable or does it have a special parameter name?
            const sKey = segments && segments.params && segments.params[sParamName] && segments.params[sParamName][0];
            if (!sKey) {
                return sUrl;
            }
            prependHash = "";
            ...
            const val = fRetrieveValue(sKey);
            if (!val) {
                return sUrl;
            }
            const paramsExpanded = this._blendParameters(segments.params, sParamName, val);
        ...
        this._blendParameters = function (oParams, sParamName, sValue) {
            const newParams = urlParsing.parseParameters(`?${sValue}`);
            let a;
    ...
    const urlShortening = new UrlShortening();
