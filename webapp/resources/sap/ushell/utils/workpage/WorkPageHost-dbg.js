...
sap.ui.define([
    ...
    return function (sId, mSettings) {
        const oHost = new Host(sId, mSettings);
         ...
         */
        oHost.setResolveDestination(async (sDestinationName, oCard) => {
            if (!sDestinationName || !oCard) {
                throw new Error("No destination name or card instance given.");
            }
            ...
            return oHost._destinationResolver.resolveCardDestination(sDestinationName, oCard).catch((oError) => {
                const sReason = oError.details?.reason;
         ...
         */
        oHost.attachCardInitialized((oEvent) => {
            const oCard = oEvent.getParameter("card");
         ...
         */
        oHost.getContextValue = async function (sPath) {
            if (!sPath) {
         ...
         */
        oHost.getContext = async function () {
            if (!oHost._oHostContext) {
                try {
                    const oUserInfoService = await Container.getServiceAsync("UserInfo");
                    try {
                        const oBundle = await oHost._getBundle();
                        oHost._oHostContext = oHost._applyHostContext(oUserInfoService, oBundle);
                    } catch (oError) {
                        oHost._oHostContext = oHost._applyHostContext(oUserInfoService);
                    }
                } catch (oError) {
                    oHost._oHostContext = {};
                }
