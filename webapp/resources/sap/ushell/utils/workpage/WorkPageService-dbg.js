...
sap.ui.define([
     ...
     * @private
     */
    WorkPage.prototype._validateData = async function (oPageData) {
        Log.debug("cep/editMode: load Page: validate", "Work Page service");

        if (oPageData.errors && oPageData.errors.length > 0) {
            const sErrorMessage = oPageData.errors
                .map((oError) => { return oError.message; })
                .join(",\n");
        ...
        if (!ObjectPath.get("data.workPage", oPageData)) {
            ...
            throw new Error("Work Page data is empty");
        }
