                ...
                try {
                    this._dataPromise = new Promise(async (resolve, reject) => {
                        ...
                        try {
                            ...
                            return reject(new LaunchpadError("Failed to fetch destinations", { reason: DestinationResolver.ERROR_CODES.COULD_NOT_RETRIEVE_DESTINATIONS }));
                        } catch (oError) {
                            // DestinationResolver: Couldn't Load Destinations from Service
                            return reject(new LaunchpadError("Failed to fetch destinations", { reason: DestinationResolver.ERROR_CODES.COULD_NOT_RETRIEVE_DESTINATIONS }, oError));
                        }
                    });
                } catch (oError) {
        ...
        resolveCardDestination (sDestinationName, oCard) {
            return new Promise((resolve, reject) => {
                if (!sDestinationName || !oCard) {
                    return reject(new LaunchpadError("Could not resolver card destination", { reason: ERROR_CODES.NO_CARD_OR_DESTINATION_NAME }));
                }
                this._getContentProviderDestinationMappings().then((mProviders) => {
                    ...
                    if (sResolvedUrl) {
                        ...
                        return reject(new LaunchpadError("Could not resolver card destination", { reason: ERROR_CODES.RESOLVED_URL_EMPTY }));
                    }
                    // DestinationResolver: No Destinations Found
                    return reject(new LaunchpadError("Could not resolver card destination", { reason: DestinationResolver.ERROR_CODES.NO_DESTINATION_FOUND }));
                }).catch((oError) => {
                    if (!this._sSiteId) {
