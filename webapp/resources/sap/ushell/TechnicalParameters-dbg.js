...
sap.ui.define([
    ...
    function getValueFromComponent (sParameterName, oApplicationComponent) {
        if (oApplicationComponent) {
            const oComponentData = oApplicationComponent.getComponentData() || {};
            const aTechnicalParameters = oComponentData.technicalParameters || {};
    ...
    function getValueFromManifest (sPath, oApplicationComponent) {
        let vValue = oApplicationComponent.getManifestEntry(sPath);
    ...
    function getValueFromContainer (sParameterName, oApplicationContainer) {
        const aReservedParameters = oApplicationContainer.getReservedParameters();
    ...
    function getParameters (oFilterOptions) {
        ...
        return Object.keys(oTechnicalParameters).filter((oTechnicalParameterName) => {
            const oParameter = oTechnicalParameters[oTechnicalParameterName];
            return Object.keys(oFilterOptions).every((sFilterName) => {
                const vDesiredPropertyValue = oFilterOptions[sFilterName];
                const bHasParameter = oParameter.hasOwnProperty(sFilterName);
                if (bHasParameter) {
                    return oParameter[sFilterName] === vDesiredPropertyValue;
                }
                return false;
            });
        }).map((sMatchingParameter) => {
            return Object.keys(oTechnicalParameters[sMatchingParameter]).reduce((oParameterDescription, sParameterAttributeName) => {
