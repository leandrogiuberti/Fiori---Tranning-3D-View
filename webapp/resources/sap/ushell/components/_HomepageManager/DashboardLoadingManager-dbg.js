...
sap.ui.define([
    ...
    const DashboardLoadingManager = BaseObject.extend("sap.ushell.components._HomepageManager.DashboardLoadingManager", {
        metadata: {},
        ...
        manageDynamicTiles: function () {
            ...
            let oTile;
            let oTileObject;
            const oNewActiveDynamicTiles = {};
            const oCloneActiveTiles = deepExtend({}, this.oActiveDynamicTiles);

            // handle old dynamic tiles (that not exist in the visible tile that will be handle next in loop
            for (let index = 0; index < this.currentVisibleTiles.length; index++) {
                const oVisTile = this.currentVisibleTiles[index];
                if (!oVisTile.bIsExtanded) {
                    delete oCloneActiveTiles[oVisTile.oTile.uuid];
                }
            }

            for (const oKey in oCloneActiveTiles) {
            ...
            for (let iVisibleTileIndex = 0; iVisibleTileIndex < this.currentVisibleTiles.length; iVisibleTileIndex++) {
                const oVisObj = this.currentVisibleTiles[iVisibleTileIndex];
        ...
        manageBusyIndicatorTiles: function () {
            ...
            let iVisibleTileIndex;
            let oCurrentKey;
            let oCurrentTile;
            let oOtherBusyIndicatorTile;
            const aRemoveBusyIndicator = [];
            let bIsInVisibleTile;
            const aAddBusyIndicator = [];
        ...
        _refreshTiles: function () {
            for (let iVisibleTileIndex = 0; iVisibleTileIndex < this.currentVisibleTiles.length; iVisibleTileIndex++) {
                const oTile = this.currentVisibleTiles[iVisibleTileIndex].oTile;
                const tileObject = oTile.object;
                if (tileObject) {
                    for (let iRefreshTilesIndex = 0; iRefreshTilesIndex < this.aRefreshTiles.length; iRefreshTilesIndex++) {
