            ...
            try {
                ...
                if (this.oConfig.TILE_PROPERTIES.semanticMeasure) {
                    ...
                    if (this.oConfig.TILE_PROPERTIES.COLUMN_NAMES) {
                        for (let j = 0; j < this.oConfig.TILE_PROPERTIES.COLUMN_NAMES.length; j++) {
                ...
                if (!cachedValue) {
                    ...
                    const finalQuery = sap.ushell.components.tiles.indicatorTileUtils.util.prepareQueryServiceUri(that.oChip.url.addSystemToServiceUrl(sUri), entitySet, measures, null, variants, 3);
                    if (this.oConfig.TILE_PROPERTIES.semanticMeasure) {
                    ...
                    this.comparisionChartODataRef = finalQuery.model.read(finalQuery.uri, null, null, true, (data) => {
                        if (data && data.results && data.results.length) {
                            ...
                            for (let i = 0; i < that.oConfig.TILE_PROPERTIES.FINALVALUE.length; i++) {
        ...
        flowWithoutDesignTimeCall: function () {
            ...
            if (this.oChip.visible.isVisible() && !this.firstTimeVisible) {
                ...
                this.fetchKpiValue(function (kpiValue) {
                    ...
                    const calculatedKpiValue = this.CALCULATED_KPI_VALUE;
                    let applyColor; let calculatedValueForScaling;
                    for (let i = 0; i < calculatedKpiValue.length; i++) {
                    ...
                    const oControlNumeric = that.oDualComparisonView.oGenericTile.getTileContent()[0].getContent();
                    const oControlComparison = that.oDualComparisonView.oGenericTile.getTileContent()[1].getContent();
                ...
                sap.ushell.components.tiles.indicatorTileUtils.util.getParsedChip(
                    this.oChip.configuration.getParameterValueAsString("tileConfiguration"), this.oChip.preview.isEnabled(), (config) => {
                        ...
                        if (that.oChip.preview.isEnabled()) {
                            ...
                            that.setTitle();
                            that.oDualComparisonView.oGenericTile.attachPress(() => {
