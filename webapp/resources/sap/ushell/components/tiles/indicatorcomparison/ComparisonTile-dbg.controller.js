...
sap.ui.define([
    ...
    const ComparisonTileController = generic.extend("sap.ushell.components.tiles.indicatorcomparison.ComparisonTile", {
        onInit: function () {
            this.KPI_VALUE_REQUIRED = false;
        },
        _processDataForComparisonChart: function (data, measure, unit) {
            const finalOutput = []; const LABEL_MAPPING = {}; let i; let tempObject; let l;
            let tempVar;
            let aTitles = [];
            const that = this;
            let unitValue = null;
            ...
            const columnName = that.oConfig.TILE_PROPERTIES.COLUMN_NAMES || that.oConfig.EVALUATION.COLUMN_NAMES;
            for (i = 0; i < columnName.length; i++) {
                const temp = {};
                const columnObject = columnName[i];
                temp.value = Number(eachData[columnObject.COLUMN_NAME]);
                let calculatedValueForScaling = Number(eachData[columnObject.COLUMN_NAME]);

                let isEvaluationThresholdMeasure = false;
                const tMeasures = that._getEvaluationThresholdMeasures();
                const mIndex = tMeasures ? Array.prototype.indexOf.call(tMeasures, columnObject.COLUMN_NAME) : -1;
                if (mIndex > -1) {
                ...
                const c = that.isCurrencyMeasure(columnObject.COLUMN_NAME);
        ...
        fetchChartData: function (bRefreshClick, isAutoRefresh, fnSuccess, fnError) {
            function checkIfDataPresent (data, columnNames) {
                ...
                if (data && data.results && data.results.length) {
                    for (let i = 0, l = columnNames.length; i < l && !isPresent; i++) {
            ...
            try {
                ...
                if ((tempData && !tempData.rightData) || !cachedValue || (!isCacheValid && that.oTileApi.visible.isVisible()) || isRefreshClick || (isAutoRefresh && that.oTileApi.visible.isVisible()) || that.getView().getViewData().refresh) {
                    if (that.kpiValueFetchDeferred) {
                        ...
                        const finalQuery = sap.ushell.components.tiles.indicatorTileUtils.util.prepareQueryServiceUri(that.oRunTimeODataModel, entitySet, measures, null, variants, 3);
