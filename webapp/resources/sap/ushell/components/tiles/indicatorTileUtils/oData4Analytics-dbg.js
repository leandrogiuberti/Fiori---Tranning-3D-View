...
sap.ui.define([
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Model.prototype = {
        ...
        _init: function (oModelReference, sAnnotationJSONDoc) {
            ...
            if (oModelReference && oModelReference.aWorkaroundID) {
                ...
                for (let i = 0; i < aWorkaroundID.length; i++) {
            ...
            for (let i = -1, oContainer; oContainer = oSchema.entityContainer[++i];) {
            ...
            let j;
            for (let i = -1, oType; oType = aEntityType[++i];) {
            ...
            for (let i = -1, oType; oType = aUnsortedEntityTypes[++i];) {
            ...
            let oParameterization;
            let oAssoc;

            for (let i = -1, oType; oType = aParameterEntityTypes[++i];) {
                ...
                oParameterization = new sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Parameterization(oEntityType,
                    oEntitySet);
            ...
            for (let i = -1, oType; oType = aQueryResultEntityTypes[++i];) {
        ...
        mergeV2AnnotationLevel: function (aMetadata, aAnnotation, oUI5ODataModelAnnotatableObject) {
            for (let i = -1, oAnnotation; oAnnotation = aAnnotation[++i];) {
                for (let j = -1, oMetadata; oMetadata = aMetadata[++j];) {
                    ...
                    if (oAnnotation.extensions != undefined) {
                        ...
                        for (let l = -1, oAnnotationExtension; oAnnotationExtension = oAnnotation.extensions[++l];) {
                            let bFound = false;
                            for (let m = -1, oMetadataExtension; oMetadataExtension = oMetadata.extensions[++m];) {
                    ...
                    for (let k = -1, oUI5ODataModelAnnotatableSubObject; oUI5ODataModelAnnotatableSubObject = oUI5ODataModelAnnotatableObject.aSubObject[++k];) {
                        for (const propName in oAnnotation) {
        ...
        _getEntitySetsOfType: function (oSchema, sQTypeName) {
            ...
            for (let i = -1, oEntityContainer; oEntityContainer = oSchema.entityContainer[++i];) {
                for (let j = -1, oEntitySet; oEntitySet = oEntityContainer.entitySet[++j];) {
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.QueryResult.prototype = {
        ...
        _init: function (oModel, oEntityType, oEntitySet, oParameterization, oAssocFromParamsToResult) {
            ...
            let oDimensionAttribute;
            let oDimension;
            for (let i = -1, oProperty; oProperty = aProperty[++i];) {
                if (oProperty.extensions == undefined) {
                    continue;
                }
                for (let j = -1, oExtension; oExtension = oProperty.extensions[++j];) {
                    if (!oExtension.namespace == sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.constants.SAP_NAMESPACE) {
                        continue;
                    }
                    ...
                    switch (oExtension.name) {
                        case "aggregation-role":
                            switch (oExtension.value) {
                                case "dimension":
                                    oDimension = new sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Dimension(this, oProperty);
                                    this._oDimensionSet[oDimension.getName()] = oDimension;
                                    break;
                                case "measure":
                                    const oMeasure = new sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Measure(this, oProperty);
                            ...
                            break;
                        case "attribute-for":
                            oDimensionAttribute = new sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.DimensionAttribute(this,
                                oProperty);
            ...
            if (oModel._oActivatedWorkarounds.IdentifyTextPropertiesByName) {
                ...
                for (let i = -1, sPropertyName; sPropertyName = aMatchedTextPropertyName[++i];) {
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Parameterization.prototype = {
            ...
            for (let i = -1, oProperty; oProperty = aProperty[++i];) {
                ...
                for (let j = -1, oExtension; oExtension = oProperty.extensions[++j];) {
                    ...
                    switch (oExtension.name) {
                        ...
                        case "parameter":
                            const oParameter = new sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Parameter(this, oProperty);
        ...
        setTargetQueryResult: function (oQueryResult, oAssociation) {
            ...
            for (let i = -1, oNavProp; oNavProp = aNavProp[++i];) {
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.Parameter.prototype = {
        ...
        _init: function (oParameterization, oProperty) {
            ...
            if (oProperty.extensions != undefined) {
                for (let i = -1, oExtension; oExtension = oProperty.extensions[++i];) {
                    if (!oExtension.namespace == sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.constants.SAP_NAMESPACE) {
                        continue;
                    }
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.EntityType.prototype = {
        ...
        _init: function (oModel, oSchema, oEntityType) {
            ...
            for (let i = -1, oProperty; oProperty = oEntityType.property[++i];) {
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.FilterExpression.prototype = {
        ...
        addSetCondition: function (sPropertyName, aValues) {
            ...
            for (let i = -1, oValue; oValue = aValues[++i];) {
                this._aFilterCondition.push({
                    property: oProperty,
        ...
        getExpressionAsUI5FilterArray: function () {
            ...
            for (let i = -1, oCondition; oCondition = this._aFilterCondition[++i];) {
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.SortExpression.prototype = {
        ...
        _containsSorter: function (sPropertyName) {
            let oResult = null;
            for (let i = -1, oCurrentSorter; oCurrentSorter = this._aSortCondition[++i];) {
        ...
        getExpressionsAsUI5SorterArray: function () {
            ...
            for (let i = -1, oCondition; oCondition = this._aSortCondition[++i];) {
    ...
    sap.ushell.components.tiles.indicatorTileUtils.odata4analytics.QueryResultRequest.prototype = {
        ...
        addToAggregationLevel: function (aDimensionName) {
            ...
            for (let i = -1, sDimName; sDimName = aDimensionName[++i];) {
        ...
        setMeasures: function (aMeasureName) {
            ...
            this._oMeasures = {};
            for (let i = -1, sMeasName; sMeasName = aMeasureName[++i];) {
        ...
        includeDimensionKeyTextAttributes: function (sDimensionName, bIncludeKey, bIncludeText, aAttributeName) {
            ...
            for (let i = -1, sDimName; sDimName = aDimName[++i];) {
                if (bIncludeKey != null) {
        ...
        includeMeasureRawFormattedValueUnit: function (sMeasureName, bIncludeRawValue,
            ...
            for (let i = -1, sMeasName; sMeasName = aMeasName[++i];) {
            ...
            switch (sQueryOptionName) {
                case "$select": {
                    ...
                    if (this._oValueSetResult.attributes) {
                        for (let i = -1, sAttrName; sAttrName = this._oValueSetResult.attributes[++i];) {
                            sSelectOption += (sSelectOption == "" ? "" : ",")
