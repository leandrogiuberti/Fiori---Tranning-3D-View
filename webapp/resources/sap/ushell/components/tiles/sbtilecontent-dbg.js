...
sap.ui.define([
    ...
    "sap/ui/core/Control"
], (TileContent, Renderer, Control) => {
    "use strict";

    const timestamp = Control.extend("numeric.TileContent_Timestamp", {
        ontap: function (e) {
        ...
        renderer: function (r, c) {
            ...
            r.writeStyles();
            r.write(">");
            const ts = c.getParent().getTimestamp();
            if (ts) {
                if (!c.getParent().getRefreshOption()) {
                    r.writeEscaped(ts);
    ...
    return TileContent.extend("sap.ushell.components.tiles.sbtilecontent", {
        ...
        getAltText: function () {
            let a = TileContent.prototype.getAltText.apply(this, arguments);
