...
sap.ui.define([
    ...
    const InfoTileValueColor = commonsLibrary.InfoTileValueColor;

    // shortcut for sap.suite.ui.commons.InfoTileSize
    const InfoTileSize = commonsLibrary.InfoTileSize;

    // shortcut for sap.suite.ui.commons.LoadState
    const LoadState = commonsLibrary.LoadState;
    ...
    sap.ui.controller("tiles.indicatorHarveyBall.HarveyBallTile", {
        ...
        attachHandlers: function () {
            const that = this;
            const oChip = this.getVariable("chip");
            if (oChip.visible) {
                oChip.visible.attachVisible((bVisible) => {
                    if (bVisible) {
                        that.onVisible();
                    } else {
            ...
            this.getVariable("tile").attachPress(() => {
                that.abortAllOpenODataRequests();
                const indicatorUtils = that.getVariable("indicatorUtils");
        ...
        initializeVariables: function () {
            ...
            const oTile = oView.oTile;
            const oChip = oViewData.chip;
        ...
        readChipConfiguration: function () {
            const that = this;
            const oChip = this.getVariable("chip");
            const utils = this.getVariable("indicatorUtils");
            utils.getParsedChip(this.getVariable("configurationString"), (evalData) => {
        ...
        loadPreviewData: function () {
            const oTitle = this.getVariable("indicatorUtils").getTileTitleSubtitle(this.getVariable("chip"));
        ...
        abortAllOpenODataRequests: function () {
            for (const each in this._oDataRequests) {
                try {
        ...
        fetchKpiValue: function (oParam) {
            ...
            function makeODataRequest (oModel, sUri, oConfig, fnS, fnE) {
                const oDataRef = oModel.read(sUri, null, null, true, (data) => {
            ...
            const indicatorUtils = this.getVariable("indicatorUtils");
            const cachedValue = this.getVariable("cache").getKpivalueById(this.getVariable("chipId"));
            if (cachedValue) {
                oParam.fnS.call(that, cachedValue);
            } else {
                const sMeasure1 = oParam.measure1; const sMeasure2 = oParam.measure2;
                const sUri = oParam.url; const sEntitySet = oParam.entitySet;
                const variantFilters = oParam.filters;
                const fractionValueFilters = oParam.fractionFilters.concat(variantFilters);
                let oQuery1;
                semaphore = 2;// to keep track of race around condition
                fractionValueFilters.forEach((cur) => { // this code written only for now. to handler the exception thrown by prepareQueryServiceUri
                ...
                const oQuery2 = indicatorUtils.prepareQueryServiceUri(this.getVariable("chip").url.addSystemToServiceUrl(sUri), sEntitySet, sMeasure2, null, fractionValueFilters);
                const unitColumn1 = oQuery1.unit[0];
                const unitColumn2 = oQuery2.unit[0];
                kpiData = {
                    kpiValue: null,
                    fractionValue: null,
                    unit: ""
        ...
        _setThresholdValues: function (data) {
            const oThreshold = this.getVariable("oThreshold");
            let each;
            if (oThreshold) {
        ...
        getThresholdObject: function () {
            let oThreshold = this.getVariable("oThreshold");
            if (!oThreshold) {
                const aThresholds = this.getVariable("evalData").EVALUATION_VALUES;
                oThreshold = {};
                for (let i = 0, l = aThresholds.length; i < l; i++) {
        ...
        getTrendColor: function (kpiValue) {
            let color = "Neutral";
            let oThreshold; let goalType; let WL; let WH; let CL; let CH;
        ...
        _getThresholdMeasures: function () {
            const aThresholds = [];
            const oThreshold = this.getThresholdObject();
            for (const each in oThreshold) {
                if (oThreshold[each].COLUMN_NAME) {
        ...
        onEvalFiltersFetched: function () {
            ...
            if (this.getVariable("chip").visible.isVisible() && !this.getVariable("hasAppeared")) {
                try {
                    const kpiDefinition = this.getVariable("evalData");
                    const sUri = kpiDefinition.EVALUATION.ODATA_URL;
                    const sEntitySet = kpiDefinition.EVALUATION.ODATA_ENTITYSET;
                    const tileProperties = this.getVariable("tileProperties");
                    let harveyFilters = [];
                    const variantFilters = this.getVariable("indicatorUtils").prepareFilterStructure(kpiDefinition.EVALUATION_FILTERS, kpiDefinition.ADDITIONAL_FILTERS);
                    const fractionMeasure = kpiDefinition.EVALUATION.COLUMN_NAME;
                    let totalMeasure = fractionMeasure;
                    let thresholdMeasures = [];
        ...
        fetchEvalFilters: function (fnS) {
            const that = this;
            try {
                const evaluationData = this.getVariable("cache").getEvaluationById(this.getVariable("chipId"));
                if (evaluationData) {
                    this.getVariable("evalData").EVALUATION_FILTERS = evaluationData.EVALUATION_FILTERS;
                    fnS();
                } else {
                    const oDataRef = this.getVariable("indicatorUtils").getFilterFromRunTimeService(this.getVariable("evalData"), (filter) => {
        ...
        onKpiValueFetched: function (kpiValue, kpiValueUnit, fractionValue, fractionValueUnit) {
            const indicatorUtil = this.getVariable("indicatorUtils");
            let scaledKpiValue = kpiValue;
            let scaledFractionValue = fractionValue;
            const evalData = this.getVariable("evalData");
            const scalingFactor = evalData.EVALUATION.SCALING;
            if (scalingFactor == -2) {
                scaledKpiValue *= 100;
                scaledFractionValue *= 100;
            }
            let c = this.isACurrencyMeasure(this.getVariable("totalMeasure"));
            ...
            const navTarget = indicatorUtil.getNavigationTarget(this.getVariable("evalData"), this.getVariable("system"));
