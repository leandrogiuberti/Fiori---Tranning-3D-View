    ...
    sap.ui.controller("tiles.indicatorDualDeviation.DualDeviation", {
        ...
        fetchKpiValue: function (fnSuccess, fnError) {
            ...
            try {
                ...
                let sThresholdObject = this.setThresholdValues();
                        ...
                        try {
                            this.queryServiceUriODataReadRef = this.QUERY_SERVICE_MODEL.read(oQuery.uri, null, null, true, (data) => {
                                ...
                                if (data && data.results && data.results.length) {
                                    ...
                                    let oScaledValue = "";
                                    let calculatedValueForScaling = kpiValue;
                                    ...
                                    const trendIndicator = that.getTrendIndicator(sThresholdObject.trendValue, calculatedValueForScaling);
                                    if (that.oConfig.EVALUATION.SCALING == -2) {
                                        that._updateTileModel({
                                            scale: "%"
                                        });
        ...
        flowWithoutDesignTimeCall: function () {
            const that = this;
            let formattedValue; let formattedTargetvalue;
            ...
            if (this.oTileApi.visible.isVisible() && !this.firstTimeVisible) {
                ...
                this.fetchKpiValue(function (kpiValue, thresholdObject) {
                    let calculatedValueForScaling = Number(kpiValue);
                    ...
                    const deviationTileObj = {};
                    const thresholdsArrayObjAndColor = this.getThresholdsObjAndColor(thresholdObject);
                    const actualKpiObj = { value: Number(kpiValue), color: thresholdsArrayObjAndColor.returnColor };
                    ...
                    const evalValue = this.DEFINITION_DATA.EVALUATION_VALUES;
                    let calculatedTargetValue;
                    if (this.DEFINITION_DATA.EVALUATION.VALUES_SOURCE == "MEASURE") {
                        ...
                        for (let itr = 0; itr < evalValue.length; itr++) {
                    ...
                    const valueObjDeviation = valueObjNumeric;

                    const oControlNumeric = that.oDualDeviationView.oGenericTile.getTileContent()[0].getContent();
                    const oControlDeviation = that.oDualDeviationView.oGenericTile.getTileContent()[1].getContent();
                ...
                sap.ushell.components.tiles.indicatorTileUtils.util.getParsedChip(
                    that.oTileApi.configuration.getParameterValueAsString("tileConfiguration"), that.oTileApi.preview.isEnabled(), (config) => {
                        ...
                        if (that.oTileApi.preview.isEnabled()) {
                            ...
                            that.oDualDeviationView.oGenericTile.attachPress(() => {
