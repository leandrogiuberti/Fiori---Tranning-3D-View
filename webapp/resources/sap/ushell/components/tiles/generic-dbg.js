...
sap.ui.define([
    ...
    const generic = Controller.extend("sap.ushell.components.tiles.generic", {
        getTile: function () {
        ...
        getRelativeTime: function () {
            ...
            const relativeTime = sap.ushell.components.tiles.indicatorTileUtils.util.getTimeDifference(curDate - tempCacheTime);
            let retDate;
        ...
        setTimeStamp: function (/* date */) {
            ...
            const oFormat = DateFormat.getDateTimeInstance({ relative: true, relativeSource: "auto", style: "short" });
        ...
        isACurrencyMeasure: function (measure) {
            const entitySet = this.DEFINITION_DATA.EVALUATION.ODATA_ENTITYSET;
        ...
        isCurrencyMeasure: function (measure) {
            ...
            if (cachedValue && cachedValue.Data) {
                let kpiData = cachedValue.Data && JSON.parse(cachedValue.Data);
                if (sap.ushell.components.tiles.indicatorTileUtils.util.isDualTile(that.oConfig)) {
                    ...
                    if (kpiData && kpiData.data && kpiData.data.length) {
                        for (let i = 0; i < kpiData.data.length; i++) {
                            if (kpiData.data[i] && kpiData.data[i].measure == measure) {
                                if (typeof kpiData.data[i].isCurM === "boolean") {
                ...
                } if (kpiData && typeof kpiData.isCurM === "boolean") {
        ...
        setThresholdValues: function () {
            ...
            try {
                ...
                if (this.DEFINITION_DATA.EVALUATION.VALUES_SOURCE == "MEASURE") {
                    const cacheData = sap.ushell.components.tiles.indicatorTileUtils.cache.getKpivalueById(that.oConfig.TILE_PROPERTIES.id);
        ...
        fetchKpiValue: function (callback, fnError, bRefreshClick, bAutoRefresh) {
            ...
            try {
                ...
                const sThresholdObject = this.setThresholdValues();
                const sMeasure = sThresholdObject.fullyFormedMeasure;
                const cachedValue = sap.ushell.components.tiles.indicatorTileUtils.cache.getKpivalueById(that.oConfig.TILE_PROPERTIES.id);
                const chipUpdateTime = that.oTileApi.configuration.getParameterValueAsString("timeStamp");
                const isCacheValid = sap.ushell.components.tiles.indicatorTileUtils.util.isCacheValid(that.oConfig.TILE_PROPERTIES.id,
                    chipUpdateTime, that.chipCacheTime, that.chipCacheTimeUnit, that.tilePressed);
                const isRefreshClick = sap.ushell.components.tiles.indicatorTileUtils.util.getBoolValue(bRefreshClick);
                const keyCallCheck = this.getKeyForCallCheck();
                    ...
                    if ((!cachedValue || (!isCacheValid && that.oTileApi.visible.isVisible()) || isRefreshClick || (bAutoRefresh && that.oTileApi.visible.isVisible())
                        ...
                        if (that.kpiValueFetchDeferred) {
                            ...
                            if (oQuery) {
                                ...
                                this.queryServiceUriODataReadRef = this.QUERY_SERVICE_MODEL.read(oQuery.uri, null, null, true, (data) => {
                                    ...
                                    if (data && data.results && data.results.length && data.results[0][that.DEFINITION_DATA.EVALUATION.COLUMN_NAME] != null) {
                                        ...
                                        if (!sap.ushell.components.tiles.indicatorTileUtils.util.isDualTile(that.oConfig)) {
                                            ...
                                            const tempCacheData = sap.ushell.components.tiles.indicatorTileUtils.cache.getKpivalueById(that.oConfig.TILE_PROPERTIES.id);
                                            let avilableCacheData;
                                            if (tempCacheData) {
                                        ...
                                        const key = `${that.oConfig.TILE_PROPERTIES.id}data`;
                                        let runningJob = sap.ushell.components.tiles.indicatorTileUtils.util.getScheduledJob(key);
                                        if (runningJob) {
                                            clearTimeout(runningJob);
                                            runningJob = undefined;
                        ...
                        if (cachedValue && cachedValue.Data) {
                            ...
                            if (that.chipCacheTime &&
                                !sap.ushell.components.tiles.indicatorTileUtils.util.isDualTile(that.oConfig)) {
                                that.setTimeStamp(cachedValue.CachedTime);
                            }
                            callback.call(that, kpiValue, sThresholdObject);
                        } else {
                            that.setNoData();
        ...
        parse_sapclient: function () {
            const filters = this.oConfig.EVALUATION_FILTERS;
            ...
            let filter;
            const SAP_CLIENT = "P_SAPClient";
            const reserved_placeholder = "$$$";
            ...
            if (filter) {
                jQuery.when(sap.ushell.components.tiles.indicatorTileUtils.util.getHanaClient()).done((client) => {
        ...
        fetchEvaluation: function (chipConfig, callback, bRefreshClick, isAutoRefresh) {
            const that = this;
            const sPlatform = this.oConfig.TILE_PROPERTIES.sb_metadata || "HANA";
            ...
            if ((that.oTileApi.visible.isVisible() && !that.firstTimeVisible) || isRefreshClick || isAutoRefresh) {
                jQuery.when(sap.ushell.components.tiles.indicatorTileUtils.util.getFrontendCache(chipConfig, that.oTileApi)).done((isCached) => {
                    ...
                    if (isCached || Number(that.oTileApi.configuration.getParameterValueAsString("isSufficient"))) {
                        ...
                        try {
                            if (!sap.ushell.components.tiles.indicatorTileUtils.util.isDualTile(that.oConfig) || !that.dataCallInProgress) {
                                ...
                                const evaluationData = sap.ushell.components.tiles.indicatorTileUtils.cache.getEvaluationById(that.oConfig.TILE_PROPERTIES.id);
                                if (evaluationData) {
                                    ...
                                    sap.ushell.components.tiles.indicatorTileUtils.util.getFilterFromRunTimeService(that.oConfig, that.oTileApi, (filter) => {
        ...
        refreshHandler: function (bRefreshClick, bAutoRefresh) {
            const that = this;
            const isRefreshClick = sap.ushell.components.tiles.indicatorTileUtils.util.getBoolValue(bRefreshClick);
            const isAutoRefresh = sap.ushell.components.tiles.indicatorTileUtils.util.getBoolValue(bAutoRefresh);
            if (!that.firstTimeVisible || isRefreshClick || isAutoRefresh) {
                that.fetchEvaluation(that.oConfig, () => {
                    let sUrl;
                    if (that.oConfig.TILE_PROPERTIES.tileType == "NT" ||
                        ...
                        that.getRunTimeODataModel(sUrl, () => {
            ...
            try {
                sap.ushell.components.tiles.indicatorTileUtils.util.getParsedChip(
                    that.oTileApi.configuration.getParameterValueAsString("tileConfiguration"), that.oTileApi.preview.isEnabled(), (config) => {
                        that.oConfig = config;
                        const title = sap.ushell.components.tiles.indicatorTileUtils.util.getChipTitle(that.oConfig);
                        const subtitle = sap.ushell.components.tiles.indicatorTileUtils.util.getChipSubTitle(that.oConfig);
        ...
        onAfterTileRendering: function () {
            ...
            this.getChipConfiguration(() => {
                ...
                if (that.oTileApi.preview.isEnabled()) {
                    ...
                    if (!sap.ushell.components.tiles.indicatorTileUtils.util.isDualTile(that.oConfig)) {
                        that.oKpiTileView.oNVConfS.attachRefresh(() => {
                            that.oKpiTileView.oGenericTile.setState(LoadState.Loading);
                            that.refreshHandler(true);
                        });
                    }

                    that.fetchEvaluation(that.oConfig, () => {
                        that.DEFINITION_DATA = that.oConfig;
                        const sUrl = that.oTileApi.url.addSystemToServiceUrl(that.oConfig.EVALUATION.ODATA_URL);
                        const chipUpdateTime = that.oTileApi.configuration.getParameterValueAsString("timeStamp");
                        const isCacheValid = sap.ushell.components.tiles.indicatorTileUtils.util.isCacheValid(that.oConfig.TILE_PROPERTIES.id,
                            chipUpdateTime, that.chipCacheTime, that.chipCacheTimeUnit, that.tilePressed);
        ...
        setToolTip: function (applyColor, calculatedValueForScaling, tileType) {
            ...
            let oControl;
            const sThresholdObject = this.setThresholdValues();
            const measure = this.oConfig.EVALUATION.COLUMN_NAME;
            const isACurrencyMeasure = this.isCurrencyMeasure(measure);
            let valueObj;
        ...
        getTrendColor: function (sThresholdObj) {
            const that = this;
            let warningLowValue;
            let criticalLowValue;
            let warningHighValue;
            let criticalHighValue;
            try {
