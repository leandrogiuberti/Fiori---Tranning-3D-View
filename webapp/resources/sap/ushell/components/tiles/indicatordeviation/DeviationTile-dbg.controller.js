...
sap.ui.define([
    ...
    const DeviationTileController = generic.extend("sap.ushell.components.tiles.indicatordeviation.DeviationTile", {
        onInit: function () {
        ...
        doProcess: function (kpiValue, sThresholdObject) {
            ...
            let c = this.isCurrencyMeasure(this.oConfig.EVALUATION.COLUMN_NAME);
            const formattedValue = sap.ushell.components.tiles.indicatorTileUtils.util.getLocaleFormattedValue(Number(calculatedValueForScaling), this.oConfig.EVALUATION.SCALING, this.oConfig.EVALUATION.DECIMAL_PRECISION, c, this.CURRENCY_CODE);

            this.CALCULATED_KPI_VALUE = Number(kpiValue);
            const deviationTileObj = {};
            const applyColor = this.getThresholdsObjAndColor(sThresholdObject).returnColor;
            const actualKpiObj = { value: Number(kpiValue), color: applyColor };
