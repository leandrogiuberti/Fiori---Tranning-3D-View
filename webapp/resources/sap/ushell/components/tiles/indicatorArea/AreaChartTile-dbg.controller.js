...
sap.ui.define([
    ...
    const AreaChartTileController = generic.extend("sap.ushell.components.tiles.indicatorArea.AreaChartTile", {
        onInit: function () {
        ...
        onAfterFinalEvaluation: function (bRefreshClick, isAutoRefresh) {
            ...
            const variantData = sap.ushell.components.tiles.indicatorTileUtils.util.prepareFilterStructure(this.DEFINITION_DATA.EVALUATION_FILTERS, this.DEFINITION_DATA.ADDITIONAL_FILTERS);
            ...
            if (sap.ushell.components.tiles.indicatorTileUtils.util.isDualTile(that.oConfig)) {
                if (cachedValue) {
                    tempData = cachedValue.Data && JSON.parse(cachedValue.Data);
                }
            }
            const chipUpdateTime = that.oTileApi.configuration.getParameterValueAsString("timeStamp");
            const isCacheValid = sap.ushell.components.tiles.indicatorTileUtils.util.isCacheValid(that.oConfig.TILE_PROPERTIES.id, chipUpdateTime, that.chipCacheTime, that.chipCacheTimeUnit, that.tilePressed);
            ...
            function _applyData (data, type) {
                ...
                let c = that.isCurrencyMeasure(that.oConfig.EVALUATION.COLUMN_NAME);
                let formattedFirstYLabel = sap.ushell.components.tiles.indicatorTileUtils.util.getLocaleFormattedValue(firstCalculatedValueForScaling, that.oConfig.EVALUATION.SCALING, that.oConfig.EVALUATION.DECIMAL_PRECISION, c, that.CURRENCY_CODE);
                if (that.oConfig.EVALUATION.SCALING == -2) {
                ...
                let formattedLastYLabel = sap.ushell.components.tiles.indicatorTileUtils.util.getLocaleFormattedValue(lastCalculatedValueForScaling, that.oConfig.EVALUATION.SCALING, that.oConfig.EVALUATION.DECIMAL_PRECISION, c, that.CURRENCY_CODE);
                if (that.oConfig.EVALUATION.SCALING == -2) {
                    formattedLastYLabel += " %";
                }
                ...
                function buildChartItem (sName, a, b, type) {
                ...
                const TileRef = that.getView().oNVConfContS; // that.getTile().getTileContent()[0].getContent();
            ...
            if ((tempData && !tempData.rightData) || !cachedValue || (!isCacheValid && that.oTileApi.visible.isVisible()) || isRefreshClick || (isAutoRefresh && that.oTileApi.visible.isVisible()) || that.getView().getViewData().refresh) {
                if (that.kpiValueFetchDeferred) {
                    ...
                    const oQuery = sap.ushell.components.tiles.indicatorTileUtils.util.prepareQueryServiceUri(that.oRunTimeODataModel, sEntitySet, fullyFormedMeasure, dimensionName, variantData);
                    if (oQuery) {
                            ...
                            this.trendChartODataReadRef = oQuery.model.read(oQuery.uri, null, null, true, (data) => {
                                ...
                                if (data && data.results && data.results.length) {
                                    ...
                                    if (writeData && writeData.data && writeData.data.results && writeData.data.results.length) {
                                        for (let itr = 0; itr < writeData.data.results.length; itr++) {
                                    ...
                                    if (that.DEFINITION_DATA.TILE_PROPERTIES.frameType == FrameType.TwoByOne) {
                                        ...
                                        that.scheduleJob(that.cacheTime);
                                    }
                                } else {
                                    that.setNoData();
                                }
                ...
                try {
                    let kpiData;
                    const tileType = that.oConfig && that.oConfig.TILE_PROPERTIES && that.oConfig.TILE_PROPERTIES.tileType;
