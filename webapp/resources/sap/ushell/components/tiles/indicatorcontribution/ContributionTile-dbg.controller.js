...
sap.ui.define([
    ...
    const ContributionTileController = generic.extend("sap.ushell.components.tiles.indicatorcontribution.ContributionTile", {
        onInit: function () {
            this.KPI_VALUE_REQUIRED = false;
        },
        _processDataForComparisonChart: function (data, measure, dimension, unitProperty) {
            const semanticColor = this.oConfig.TILE_PROPERTIES.semanticColorContribution;
            const finalOutput = [];
            let tempVar;
            let unitValue;
            for (let i = 0; i < data.results.length; i++) {
                ...
                const c = this.isCurrencyMeasure(measure);
            ...
            try {
                ...
                if ((tempData && !tempData.rightData) || !cachedValue || (!isCacheValid && that.oTileApi.visible.isVisible()) || isRefreshClick || (isAutoRefresh && that.oTileApi.visible.isVisible()) || that.getView().getViewData().refresh) {
                    if (that.kpiValueFetchDeferred) {
                        ...
                        const finalQuery = sap.ushell.components.tiles.indicatorTileUtils.util.prepareQueryServiceUri(that.oRunTimeODataModel, entitySet, measure, dimension, variants, 3);
                        if (this.oConfig.TILE_PROPERTIES.semanticMeasure) {
