...
sap.ui.define([
    ...
    const IllustratedMessageType = mobileLibrary.IllustratedMessageType;
     ...
     */
    const WorkPageCell = Control.extend("sap.ushell.components.workPageBuilder.controls.WorkPageCell", /** @lends sap.ushell.components.workPageBuilder.controls.WorkPageCell.prototype */ {
        metadata: {
            library: "sap.ushell",
            properties: {
                /**
        ...
        renderer: {
            ...
            render: function (rm, workPageCell) {
                ...
                const aWidgets = workPageCell.getAggregation("widgets");
    ...
    WorkPageCell.prototype._deregisterResizeHandles = function () {
        if (this._aRegistrationIds.length > 0) {
            this._aRegistrationIds.forEach((sRegistrationId) => {
    ...
    WorkPageCell.prototype._resizeCard = function (oCard, oEvent) {
        const oDomRef = this.getAggregation("_gridContainer") &&
        ...
        const iWidth = parseFloat(window.getComputedStyle(oDomRef).width);
    ...
    WorkPageCell.prototype.getGridContainer = function () {
        let oGridContainer = this.getAggregation("_gridContainer");
        if (!oGridContainer) {
            oGridContainer = this._createGridContainer()
                .attachColumnsChange((oEvent) => {
