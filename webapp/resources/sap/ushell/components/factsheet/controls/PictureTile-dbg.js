...
sap.ui.define([
     ...
     */
    const PictureTile = CustomTile.extend("sap.ushell.components.factsheet.controls.PictureTile", /** @lends sap.ushell.components.factsheet.controls.PictureTile.prototype */ {
        metadata: {
            deprecated: true,
            library: "sap.ushell",
            properties: {
     ...
     * @function
     */
    PictureTile.prototype.setTileContent = function (vTileContent) {
        this.setContent(null);
        if (vTileContent) {
            const image = vTileContent.getImage();

            this.setContent(image);
        } else {
            this.setContent(null);
        }
        this.setAssociation("tileContent", vTileContent);
    ...
    PictureTile.prototype.switchVisibility = function (bVisible) {
        const $delBtn = this._oDeletePictureButton.$();
    ...
    PictureTile.prototype._tileSwipedHandler = function (/* oEvent */) {
        const $deleteBtn = this._oDeletePictureButton.$();
