...
sap.ui.define([
    ...
    const ManifestPropertyHelper = {
        ...
        extractCardData: function (manifest) {
            const oAction = this._getNavigationAction(manifest);
            let sTileConfiguration;
            let oSemanticParameters;
            let sSemanticParameters;
            let sSemanticObject;
            let sSemanticAction;
            let sTargetUrl;
            let bUseSemanticObject;
            const aSearchKeywords = ObjectPath.get(["sap.app", "tags", "keywords"], manifest);
            let sSearchKeywords;
            ...
            if (oSemanticParameters) {
                sSemanticParameters = UrlParsing.urlParametersToString(oSemanticParameters);
            }
        ...
        mergeCardData: function (manifest, cardData) {
            const oManifest = JSON.parse(JSON.stringify(manifest));
            const oNavigationAction = this._getNavigationAction(oManifest);
            let aSearchKeywords;
        ...
        getCardData: function (chipInstance) {
            ...
            const oBagProperties = this.getTranslatablePropertiesFromBag(oTilePropertiesBag);
            const oCardData = JSON.parse(sTileConfiguration || "{}");
            ...
            Object.keys(oBagProperties).forEach((key) => {
        ...
        _parseParameters: function (parameters) {
            const oParameters = UrlParsing.parseParameters(`?${parameters}`);
        ...
        _getNavigationAction: function (manifest) {
            const aActions = ObjectPath.get(["sap.card", "header", "actions"], manifest);
            let oAction;
