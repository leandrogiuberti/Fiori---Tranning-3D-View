    ...
    return View.extend("sap.ushell.components.appfinder.HierarchyFoldersView", {
        createContent: function (oController) {
            ...
            this.oItemTemplate = new StandardListItem({
                ...
                type: "Navigation",
                press: function () {
                    const backButton = that.pageMenu.$().find(".sapMBarLeft button");
                    ...
                    const path = this.getBindingContextPath();
            ...
            this.oList = new List({
                ...
                updateFinished: function () {
                    const aListItems = this.getItems();

                    that.finishEasyAccessAnimation(true);
                    aListItems.forEach((oListItem) => {
            ...
            this._navBackButton = new Button({
                ...
                visible: false,
                press: function () {
                    const pathChunks = this.treePath.split("/");
                    const newPathChunks = pathChunks.slice(0, pathChunks.length - 2);
        ...
        finishEasyAccessAnimation: function () {
            ...
            this.jqFolderClone.on("animationend", () => {
                this.pageMenu.$().removeClass("forwardToViewAnimation backToViewAnimation");
                const backButton = this.pageMenu.$().find(".sapMBarLeft button");
        ...
        updatePageBindings: function (path, forwardAnimation) {
            const bShowBack = path.split("/").length > 2;
            this.treePath = path;
