...
sap.ui.define([
    ...
    utilsCdm.mapOne = function (sKey, oSrc, oApp, oVisualization, oVisualizationType, oSite) {
        ...
        const oVizConfig = readVisualizations.getConfig(oVisualization);
        ...
        ["desktop", "tablet", "phone"].forEach((sMember) => {
        ...
        // Hide Tile Intent
        const oSapHideIntentLinkParam = this.getMember(oInbound, "signature.parameters.sap-hide-intent-link");
        ...
        const oSapPlatformRuntime = this.getMember(oApp, "sap|platform|runtime");
        ...
        let sTileTechnicalInformation;
        let sAppId;

        // Pick indicator data source
        const oIndicatorDataSource = this.getMember(oVisualization, "vizConfig.sap|flp.indicatorDataSource");

        // Collect tile component data
        let oTempTileComponent = {};

        // Include manifest from visualization type
        if (!isEmptyObject(oVisualizationType)) {
            const sType = this.getMember(oVisualizationType, "sap|app.type");
            if (sType === "card") {
                bIsCard = true;
                ...
                const oComponentProperties = this.getMember(oVisualizationType, "sap|platform|runtime.componentProperties");
        ...
        // Tile size
        const sTileSize = this.getNestedObjectProperty([oVizConfig, oApp, oVisualizationType], "sap|flp.tileSize");

        // Tile description
        const sTileDescription = this.getNestedObjectProperty([oVizConfig, oApp, oVisualizationType], "sap|app.description");
        ...
        const sContentProviderId = readApplications.getContentProviderId(oApp) || "";
        ...
        // URL Template
        const sTemplateName = this.getMember(oApp, "sap|integration.urlTemplateId");
        const oTemplatePayload = this.getTemplatePayloadFromSite(sTemplateName, oSite);
        if (oTemplatePayload) {
    ...
    utilsCdm.getTemplatePayloadFromSite = function (sTemplateName, oSite) {
        ...
        const sTemplateNameEscaped = sTemplateName.replace(/[.]/g, "|");
    ...
    utilsCdm._formatApplicationType = function (oResolutionResult, oApp) {
        ...
        const sComponentName = this.getMember(oApp, "sap|platform|runtime.componentProperties.name") || this.getMember(oApp, "sap|ui5.componentName");
    ...
    utilsCdm.formatSite = function (oSite) {
        ...
        const aInbounds = [];
        try {
            const aSiteApplications = Object.keys(oSite.applications || {}).sort();
            aSiteApplications.forEach((sApplicationKey) => {
                try {
                    const oApp = oSite.applications[sApplicationKey];
                    const oApplicationInbounds = this.getMember(oApp, "sap|app.crossNavigation.inbounds");
                    if (oApplicationInbounds) {
                        const lst2 = Object.keys(oApplicationInbounds).sort();
                        lst2.forEach((sInboundKey) => {
                            const oInbound = oApplicationInbounds[sInboundKey];
                            const oResolvedInbound = that.mapOne(sInboundKey, oInbound, oApp, undefined, undefined, oSite);
                            if (oResolvedInbound) {
                    ...
                    Log.error(
                        `Error in application ${sApplicationKey}:`,
                        oError
                    );
                }
            });
        } catch (oError) {
            Log.error("FormatSite failed", oError);
            return [];
        }
    ...
    utilsCdm.toHashFromInbound = function (oInbound, sAppId) {
        ...
        const oParams = ObjectPath.get("signature.parameters", oInbound) || {};
        ...
        const sConstructedHash = urlParsing.constructShellHash(oShellHash);
        if (!sConstructedHash) {
            return undefined;
        }
    ...
    utilsCdm.toHashFromOutbound = function (oOutbound) {
        ...
        const oParams = oOutbound.parameters || {};
        Object.keys(oParams).forEach((sKey) => {
    ...
    utilsCdm.toHashFromVizData = function (oVizData, oApplications) {
        ...
        const oVizTarget = oVizData.target;
        if (oVizTarget.type === "URL") {
            // Navigation with an external URL
            return oVizTarget.url;
        }

        const sAppId = oVizTarget.appId;
        const sInboundId = oVizTarget.inboundId;
        let oTarget;
        if (sAppId && sInboundId) {
            // Navigation with appId & inboundId
            const oInboundTarget = readApplications.getInboundTarget(oApplications, sAppId, sInboundId);
            oTarget = {};
    ...
    utilsCdm.toHashFromTarget = function (oTarget) {
        try {
            const oParams = {};
            const oRawParams = ObjectPath.get("parameters", oTarget) || {};
            Object.keys(oRawParams).forEach((sKey) => {
    ...
    utilsCdm.toTargetFromHash = function (sHash) {
        let oTarget = urlParsing.parseShellHash(sHash);
        if (oTarget !== undefined) {
            ...
            if (Object.keys(oParams).length > 0) {
                oTarget.parameters = [];
                Object.keys(oParams).forEach((sKey) => {
                    const aValues = Array.isArray(oParams[sKey]) ? oParams[sKey] : [oParams[sKey]];
