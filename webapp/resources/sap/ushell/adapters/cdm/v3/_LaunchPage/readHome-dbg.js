...
sap.ui.define([
    ...
    readHome.getGroupsArrayFromSite = function (oSite) {
        ...
        oSite.site.payload.groupsOrder.forEach((sGroupId, iIndex) => {
    ...
    readHome.getDefaultGroup = function (aGroups) {
        const aDefaultGroups = aGroups.filter((oGroup) => {
    ...
    readHome.getTileTitle = function (oResolvedTiles, oTile) {
        ...
        const oResolvedTile = oResolvedTiles[oTile.id];
        if (oResolvedTile) {
    ...
    readHome.isCard = function (oResolvedObjects, oItem) {
        const oResolvedObject = oResolvedObjects[oItem.id];
        if (oResolvedObject) {
    ...
    readHome.getInbound = function (oAppDescriptor, sInboundId) {
        const oInbound = readApplications.getInbound(oAppDescriptor, sInboundId);
