...
sap.ui.define([
    ...
    const oContentProviderLabelPromises = {};
    ...
    readUtils.getCdmParts = function (oSite, oTile) {
        const oViz = readVisualizations.get(oSite, readHome.getTileVizId(oTile)) || {};
        const oVizConfig = readVisualizations.getConfig(oViz);
        const oApp = readVisualizations.getAppDescriptor(oSite, readVisualizations.getAppId(oViz));
        const oInbound = readApplications.getInbound(oApp, readVisualizations.getInboundId(oViz));
    ...
    readUtils.harmonizeTarget = function (oTarget) {
        const vParameters = oTarget.parameters;
        let oParametersOut = {};
    ...
    readUtils._evaluateDisplayFormat = function (oVizData, oSite) {
        const sDisplayFormatHint = oVizData.displayFormatHint;
        const oVizType = readVisualizations.getType(oSite, oVizData.vizType);
        const sTileSize = readVisualizations.getTileSize(oVizType);

        // Set standard fallback displayFormat to StandardWide for 1x2 tiles
        const sStandardDisplayFormat = sTileSize === "1x2" ? DisplayFormat.StandardWide : DisplayFormat.Standard;
        const aDefaultDisplayFormats = [sStandardDisplayFormat, DisplayFormat.Compact];
        ...
        const sDefaultDisplayFormat = readVisualizations.getDefaultDisplayFormat(oVizType) || sStandardDisplayFormat;
    ...
    readUtils._addBookmarkInstantiationData = function (oVizData, oSite) {
        if (oVizData.vizType) {
            const oVizType = readVisualizations.getType(oSite, oVizData.vizType);
            const sVizTypeType = ObjectPath.get(["sap.app", "type"], oVizType || {});
            ...
            if (oVizType && sVizTypeType !== "chipVizType") {
                ...
                const oChipConfig = readVisualizations.getChipConfigFromVizReference(oVizData);
                oVizData._instantiationData = {
                    platform: "ABAP",
    ...
    readUtils.getBookmarkVizTypeIds = function (oBookmark) {
        ...
        const sCdmVizType = oBookmark.vizType;
        if (sCdmVizType && !aIds.includes(sCdmVizType)) {
            aIds.push(sCdmVizType);
        }

        const oChipConfig = readVisualizations.getChipConfigFromVizReference(oBookmark);
        const sChipId = oChipConfig && oChipConfig.chipId;
