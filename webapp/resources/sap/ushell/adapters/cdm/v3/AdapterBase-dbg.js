...
sap.ui.define([
    ...
    AdapterBase.prototype._ensureLoaded = function () {
        ...
        this._loaded = new Promise((fnResolve, fnReject) => {
            this._getSiteData()
                .then((oSite) => {
                ...
                .catch((oError) => {
                    oLogger.error("Delivering homepage groups failed", oError);
                    fnResolve([]);
                    // ensure that following _ensureLoaded calls get resolved with a new parameter
                    delete that._loaded;
                });
    ...
    AdapterBase.prototype._resolveGroupTile = function (oTile, oSite) {
        ...
        oResultPromise.done((oResolvedTile) => {
            ...
            if (oError instanceof LaunchpadError) {
                mFailureCache[oTile.id] = {
                    logLevel: oError.details?.logLevel || oLogLevel.ERROR,
                    message: oError.message
                };
            } else {
                mFailureCache[oTile.id] = {
                    logLevel: oLogLevel.ERROR,
                    message: oError.toString()
                };
            }
    ...
    AdapterBase.prototype._resolveTileByVizId = function (oTile, oSite) {
        ...
        let oOutbound;
        let sExternalUrl;
        let oHarmonizedOutbound;

        // Reject tile resolution
        function createdRejectedDeferred (iLogLevel, sMessage) {
            const oError = new LaunchpadError(sMessage, { logLevel: iLogLevel });
        ...
        if (!isPlainObject(oSite)) {
            return createdRejectedDeferred(oLogLevel.ERROR, "Cannot resolve tile: oSite must be an object");
        }
        if (!isPlainObject(oTile)) {
            return createdRejectedDeferred(oLogLevel.ERROR, "Cannot resolve tile: oTile must be an object");
        }

        // Get visualization and type
        const sVisualizationId = oTile.vizId;

        const oVisualization = deepClone(oReadVisualization.get(oSite, sVisualizationId || "") || {});

        const sVisualizationTypeId = oTile.vizType || oReadVisualization.getTypeId(oVisualization);
        const oVisualizationType = oReadVisualization.getType(oSite, sVisualizationTypeId);
        if (!oVisualizationType) {
            return createdRejectedDeferred(oLogLevel.ERROR, `Cannot resolve tile '${oTile.id}': no visualization type found for vizTypeId '${sVisualizationTypeId}'`);
        }
        ...
        if (sAppId) {
            ...
            if (!oAppDescriptor) {
                return createdRejectedDeferred(oLogLevel.INFO, `Tile '${oTile.id}' filtered from result: no app descriptor found for appId '${sAppId}' (dangling app reference)`);
            }

            oInboundResult = oReadHomePageUtils.getInbound(oAppDescriptor, oReadVisualization.getTarget(oVisualization).inboundId);
            if (!oInboundResult) {
                return createdRejectedDeferred(oLogLevel.ERROR, `Cannot resolve tile '${oTile.id}': app '${sAppId}' has no navigation inbound`);
            }
        ...
        if (!this._isFormFactorSupported(oApp)) {
            return createdRejectedDeferred(oLogLevel.INFO, `Tile '${oTile.id}' filtered from result: form factor not supported`);
        }
        ...
        if (sExternalUrl || sHash) {
            ...
            if (oTile.target) {
                const oTarget = deepClone(oTile.target);
    ...
    AdapterBase.prototype._allPromisesDone = function (aPromises) {
        ...
        if (aPromises.length === 0) {
            ...
            const aNoneFailingPromises = aPromises.map((oPromise) => {
                const oDeferredAlways = new jQuery.Deferred();
    ...
    AdapterBase.prototype._logTileResolutionFailures = function (oFailedResolvedTiles) {
        ...
        Object.keys(oFailedResolvedTiles).forEach((sKey) => {
            const oFailureInfo = oFailedResolvedTiles[sKey];
    ...
    AdapterBase.prototype.getCardManifest = function (oGroupCard) {
        const oResolvedCard = this._mResolvedTiles[oGroupCard.id];
        const oResolutionResult = oResolvedCard.tileResolutionResult;
    ...
    AdapterBase.prototype._getTileUiComponentContainer = function (oTile, oResolvedTile, bIsCatalogTile) {
        ...
        Container.getServiceAsync("Ui5ComponentLoader")
            .then((oComponentLoaderService) => {
                oComponentLoader = oComponentLoaderService;
                const oTileComponentData = this._createTileComponentData(oTile, bIsCatalogTile, oResolvedTile);
                return oTileComponentData;
            })
            .then((oTileComponentData) => {
            ...
            .then((oTileComponentData) => {
                ...
                if (!oTileComponentProperties.name) {
                    throw new Error(`Cannot find name of tile component for tile with id: '${oTile.id}'`);
                }
                ...
                function _tileCreateCompContainer () {
                    const oDeferred = new jQuery.Deferred();
    ...
    AdapterBase.prototype._createTileComponentData = function (oTile, bIsCatalogTile, oResolutionResult) {
        ...
        const sNumberUnit = oTile.numberUnit || (oResolutionResult.tileResolutionResult && oResolutionResult.tileResolutionResult.numberUnit);
    ...
    AdapterBase.prototype._enhanceTileComponentData = function (oTile, oComponentData) {
        let oPrefixDataSourcePromise = Promise.resolve();
        const sContentProviderId = this.getTileContentProviderId(oTile);
        ...
        return oPrefixDataSourcePromise.then(() => {
            return oComponentData;
        });
    };
