...
sap.ui.define([
    ...
    function result (oUri, oUriParent, oUriRelative, oUriBase, oUriAbsolute) {
        const oResult = {};
        if (oUri) {
            oResult.uri = oUri.toString();
        }
        if (oUriParent) {
            oResult.uriParent = oUriParent.toString();
        }
        if (oUriRelative) {
            oResult.uriRelative = oUriRelative.toString();
        }
        if (oUriBase) {
            oResult.uriBase = oUriBase.toString();
        }
        if (oUriAbsolute) {
            oResult.uriAbsolute = oUriAbsolute.toString();
        }
        return oResult;
    }
    ...
    function joinPaths () {
        const longPath = new URI("/1/2/3/4/5/6/7/8/9/");
        let oResult = longPath;
        for (let i = 0; i < arguments.length; i++) {
            oResult = URI.joinPaths(oResult, arguments[i]);
        }
        return oResult.relativeTo(longPath);
    }
    ...
    function directoryPath (oUri) {
        const sDirectory = oUri.directory();
        if (sDirectory === "/") {
    ...
    function transform (sUri, sUriParent, sUriBase, sUriNewBase) {
        ...
        const oUri = (isNonEmptyString(sUri)) ? new URI(sUri) : null;
        if (!oUri) {
        ...
        const oUriParent = (isNonEmptyString(sUriParent)) ? new URI(sUriParent) : null;
        ...
        let oUriPageAbsolute = new URI().search("");
        ...
        let oUriBase = (isNonEmptyString(sUriBase)) ? new URI(makePath(sUriBase)) : oUriPageAbsolute;
        ...
        const oUriNewBase = (isNonEmptyString(sUriNewBase)) ? new URI(sUriNewBase) : oUriPageAbsolute;
        ...
        if (!isRelative(oUriBase) && !isRelative(oUriNewBase)) {
            ...
            const oUriNewBaseToBaseRelative = new URI(directoryPath(oUriBase)).relativeTo(directoryPath(oUriNewBase));
            let oUriAbsolute;
            if (isNonEmptyString(sUriParent)) {
                ...
                const oUriParentNew = joinPaths(oUriNewBaseToBaseRelative, oUriParent);
                const oUriRelative = joinPaths(oUriParentNew, oUri);
            ...
            const oUriNew = joinPaths(oUriNewBaseToBaseRelative, oUri);
