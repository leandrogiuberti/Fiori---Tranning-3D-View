...
sap.ui.define([
    ...
    function LaunchPageAdapter (oUnused, sParameter, oAdapterConfiguration) {
        ...
        this._resolveTileByAppId = function (oTile, oSite) {
            function createRejectDeferred (iLogLevel, sMessage) {
            ...
            if (!isPlainObject(oTile)) {
                return createRejectDeferred(oLogLevel.ERROR, "Cannot resolve tile: oTile must be an object");
            }
            if (!isPlainObject(oSite)) {
                return createRejectDeferred(oLogLevel.ERROR, "Cannot resolve tile: oSite must be an object");
            }

            const sAppId = oTile.appId || oTile.appIDHint;
            if (!sAppId) {
                return createRejectDeferred(oLogLevel.ERROR,
            ...
            if (!oAppDescriptor) {
                return createRejectDeferred(oLogLevel.INFO,
                    ["Tile '",
                        oTile.id,
        ...
        this._assureLoaded = function () {
            ...
            Container.getServiceAsync("CommonDataModel").then((oCDMSiteService) => {
                oCDMSiteService.getSite()
                    ...
                    .fail((oError) => {
                        Log.error("Delivering homepage groups failed", oError);
            ...
            oCdmSiteService.getSite()
                .done((oSite) => {
                    ...
                    oCdmSiteService.save()
                        ...
                        .fail((oError) => {
                            Log.error("Failed to update the group title", oError);
        ...
        this.moveGroup = function (oGroup, nNewIndex) {
            ...
            oCdmSiteService.getSite()
                .done((oSite) => {
                    ...
                    if (!aGroupsOrder) {
                        return oDeferred.reject(new Error("groupsOrder not found - abort operation of adding a group.")).promise();
                    ...
                    if (!aGroupsOrderAfterMove) {
                        return oDeferred.reject(new Error("invalid move group operation - abort.")).promise();
                    }
        ...
        this.getTileView = function (oGroupTile) {
            ...
            return new jQuery.Deferred((oDeferred) => {
                return that._getTileView(oGroupTile, false)
                    ...
                    .catch((oError) => {
                        Log.error(`Tile with ID '${oGroupTile.id}' could not be initialized`, oError, oGroupTile.tileType);
                        oDeferred.reject(oError);
                    });
            }).promise();
        };
        ...
        this._getTileUiComponentContainer = function (oTile, oResolvedTile, bIsCatalogTile) {
            ...
            if (!oTileComponentProperties.name) {
                return oDeferred
                    .reject(new Error(`Cannot find name of tile component for tile with id: '${oTile.id}'`))
                    .promise();
            }
            ...
            oCdmSiteService.getSite()
                .done((oSite) => {
                    ...
                    oCdmSiteService.save()
                        ...
                        .fail((oError) => {
                            Log.error("Failed to remove tile", oError);
                            oDeferred.reject(oError, oSite.groups[oGroup.identification.id]);
                        });
                })
                .fail((oError) => {
                ...
                jQuery.when(oGroup || that.getDefaultGroup(), oCdmSiteService.getSite())
                    .done((oGroup, oSite) => {
                        ...
                        oResolveTilePromise
                            ...
                            .fail((oError) => {
                                Log.error("Bookmark creation failed because:", oError);
