...
sap.ui.define([
...
], (ServiceFactory, Service, Log, ObjectPath, isPlainObject) => {
    "use strict";
    ...
    function addCallAllowedCheck (oPublicServiceContext, oPublicServiceInstance, sPublicServiceMethod) {
        const fnOriginalMethod = oPublicServiceInstance[sPublicServiceMethod];

        oPublicServiceInstance[sPublicServiceMethod] = function () {
            let i;
            const sComponentId = getActiveComponentId();
            ...
            const aArgs = new Array(arguments.length);
    ...
    function setPublicMethods (oUshellServiceInstance, oPublicServiceInstance, oPublicServiceContext, bAddCallProtection) {
        getAllProperties(oUshellServiceInstance)
            ...
            .filter((sName) => {
                ...
                const vValue = oUshellServiceInstance[sName];
                oPublicServiceInstance[sName] = vValue;

                return typeof vValue === "function";
            })
            .forEach((sFunctionName) => { // special function treatment
    ...
    function isValidContext (oContext) {
        ...
        const fnGetId = ObjectPath.get("scopeObject.getId", oContext);
    ...
    function createServiceFactory (sUshellServiceName, bAddCallProtection) {
         ...
         * @private
         */
        const Ui5ServiceFactory = ServiceFactory.extend(`sap.ushell.ui5service.${sUshellServiceName}Factory`, /** @lends sap.ushell.Ui5ServiceFactory.prototype */ {

            createInstance: function (oPublicServiceContext) {
                return new Promise((fnResolve, fnReject) => {
                    getServiceAsync(sUshellServiceName)
                        .then((oUshellService) => {
                            if (!isValidContext(oPublicServiceContext)) {
                                ...
                                fnReject(new Error(`Invalid Context for ${sUshellServiceName} service`));
                                return;
                            }

                            const oUi5ServiceInfo = getServiceInfo(oPublicServiceContext, oUshellService, bAddCallProtection);
                            ...
                            const Ui5Service = Service.extend(`sap.ushell.ui5service.${sUshellServiceName}`, {
                                getInterface: function () {
                                    return oUi5ServiceInfo;
                                }
                            });

                            const oUi5ServiceInstance = new Ui5Service(oPublicServiceContext);
