...
sap.ui.define([
    ...
    utils.removeDuplicatedActions = function (aActions) {
        if (Array.isArray(aActions)) {
            const aFilteredActions = aActions.reduce((aResult, sItem) => {
    ...
    utils.storeSapSystemData = function (oSapSystemData, sSapSystemSrc) {
        let sKey;
        let oLocalStorage;
        let sStringifiedSapSystemData;
        const aSystemIds = [oSapSystemData.id];
        ...
        try {
            ...
            Log.error("Cannot stringify and store expanded system data:", oError);
        }
    ...
    utils.getLocalSystemInSidFormat = function () {
        ...
        const oSystem = oContainer.getLogonSystem();
        const sSystemName = oSystem.getSystemName();
        const sSystemClient = oSystem.getClient();
    ...
    utils.storeSapSystemToLocalStorage = function (oArgs) {
        ...
        if (utils.isPlainObject(oParams["sap-system"])) {
            const oSapSystemData = oParams["sap-system"];
            const sSapSystemSrc = oParams["sap-system-src"];
    ...
    utils.localStorageSetItem = function (sKey, sValue, bLocalEvent) {
        ...
        try {
            ...
            Log.warning("Error calling localStorage.setItem():", oError, "sap.ushell.utils");
        }
    };
    ...
    utils.isFeatureBitEnabled = function (iFeatureBit) {
        ...
        const oPrivateEpcm = utils.getPrivateEpcm();
        if (oPrivateEpcm) {
            try {
                ...
                Log.error("failed to get feature bit vector via call getNwbcFeatureBits on private epcm object", oError, "sap.ushell.utils");
            }
        }
    ...
    utils.isSAPLegacyApplicationType = function (sType, sFramework, bGuiVariantsOnly) {
        const arrTypes = bGuiVariantsOnly === true ? ["TR", "GUI", "NWBC"] : ["TR", "GUI", "NWBC", "WDA", "WCF"];
    ...
    utils.getParamKeys = function (sUrl) {
        const aAppStateKeysArray = [];
        const aAppStateNamesArray = [];
        let aParams;
        ...
        if (sUrl.indexOf("sap-xapp-state=") > 0) {
            ...
            if (aParams && aParams.length === 2) {
                try {
                    const sDecodedParam = decodeURIComponent(aParams[1]);
                    aAppStateNamesArray.push("sap-xapp-state-data");
    ...
    utils.filterOutParamsFromLegacyAppURL = function (sUrl) {
        ...
        const URL_PARAMS_FILTER = ["sap-ach",
            "sap-fiori-id",
    ...
    function _extractUi5Version (sVersion) {
        ...
        const oMatch = /\d+(\.\d+){0,2}/.exec(sVersion);
        if (oMatch && oMatch[0]) {
            return oMatch[0];
        }
        return undefined;
    ...
    utils.getUi5Version = function () {
        return new Promise((resolve, reject) => {
            sap.ui.require(["sap/ui/VersionInfo"], (VersionInfo) => {
                VersionInfo.load()
                    .then((oVersionInfo) => {
    ...
    utils.isNativeWebGuiNavigation = function (oResolvedNavigationTarget) {
        const sApplicationType = ObjectPath.get("applicationType", oResolvedNavigationTarget);
        const bNativeNWBCNavigation = ObjectPath.get("appCapabilities.nativeNWBCNavigation", oResolvedNavigationTarget);
    ...
    utils.Map.prototype.toString = function () {
        const aResult = ["sap.ushell.utils.Map("];
    ...
    utils.getParameterValueBoolean = function (sParameterName, sParams) {
        const oUriParameters = new URLSearchParams(sParams || window.location.search);
        const aArr = oUriParameters.getAll(sParameterName);
        const aTruthy = ["true", "x"];
        const aFalsy = ["false", ""];
        if (!aArr || aArr.length === 0) {
            return undefined;
        }
        const sValue = aArr[0].toLowerCase();
    ...
    utils.call = function (fnSuccess, fnFailure, bAsync) {
        ...
        try {
            ...
            sMessage = oError.message || oError.toString();
            Log.error(`Call to success handler failed: ${sMessage}`,
                oError, // may be undefined: supported in Chrome, FF; not supported in Safari, IE
                "sap.ushell.utils");
            if (fnFailure) {
                fnFailure(oError);
            }
    ...
    utils.getVisibleTiles = function () {
        const nWindowHeight = document.body.clientHeight;
        const oControl = Element.getElementById("dashboardGroups");
        const oNavContainer = Element.getElementById("viewPortContainer");
        const shellHdrHeight = 0;
        const aTiles = [];
        const aVisibleTiles = [];
    ...
    utils.recalculateBottomSpace = function () {
        const jqContainer = jQuery("#dashboardGroups").find(".sapUshellTileContainer:visible");
        const lastGroup = jqContainer.last();
        const headerHeight = jQuery(".sapUshellShellHead > header").height();
        const lastGroupHeight = lastGroup.parent().height();
        const groupTitleMarginTop = parseInt(lastGroup.find(".sapUshellContainerTitle").css("margin-top"), 10);
        const groupsContainerPaddingBottom = parseInt(jQuery(".sapUshellDashboardGroupsContainer").css("padding-bottom"), 10);
        let nBottomSpace;
    ...
    utils.calcVisibilityModes = function (oGroup, personalization) {
        ...
        let bIsVisibleInActionMode = true;
        const aLinks = oGroup.pendingLinks && oGroup.pendingLinks.length ? oGroup.pendingLinks : oGroup.links;
        const hasVisibleTiles = utils.groupHasVisibleTiles(oGroup.tiles, aLinks);
    ...
    utils.groupHasVisibleTiles = function (groupTiles, groupLinks) {
        ...
        let tileIndex;
        let tempTile;
        let tiles = !groupTiles ? [] : groupTiles;
        const links = !groupLinks ? [] : groupLinks;
     ...
     * @returns {Promise} Resolves once all handlers are done.
     */
    utils.invokeUnfoldingArrayArguments = async function (fnFunction, aArguments) {
        if (!Array.isArray(aArguments[0])) {
            return utils.promisify(fnFunction.apply(this, aArguments));
        }

        const aArgArray = aArguments[0];
        if (aArgArray.length === 0) {
            return [];
        }

        const aPromise = aArgArray.map(async (nThArgs) => {
            if (!Array.isArray(nThArgs)) {
                const sErrorMessage = "Expected Array as nTh Argument of multivalue invocation: "
            ...
            return utils.promisify(fnFunction.apply(this, nThArgs))
                .then((vResult) => {
                    // this a relict from the past when jQuery was used, which allows to resolve multiple values
                    return [vResult];
                });
        });
    ...
    utils.formatDate = function (sCreatedAt) {
        ...
        const iCreatedAt = utils._convertToUTC(new Date(sCreatedAt));
        const iNow = utils._convertToUTC(utils._getCurrentDate());
        const iTimeGap = iNow - iCreatedAt;
        const iDays = parseInt(iTimeGap / (1000 * 60 * 60 * 24), 10);
        if (iDays > 0) {
            if (iDays === 1) {
        ...
        const iHours = parseInt(iTimeGap / (1000 * 60 * 60), 10);
        if (iHours > 0) {
            if (iHours === 1) {
        ...
        const iMinutes = parseInt(iTimeGap / (1000 * 60), 10);
        if (iMinutes > 0) {
            if (iMinutes === 1) {
        ...
        return new Promise((resolve) => {
            sap.ui.require(["sap/ushell/Container"], (Container) => {
                return Promise.all([
                    ...
                    const oURLParsingService = aResults[0];
                    const oNavigationService = aResults[1];
                    const oNavigationArguments = {};
                    ...
                    const aActionParts = action.split("&/");
    ...
    utils.moveElementInsideOfArray = function (aArray, nSourceIndex, nTargetIndex) {
        ...
        const oElement = aArray.splice(nSourceIndex, 1)[0];
    ...
    utils.shallowMergeObject = function (oTarget /* , ...rest */) {
        return Array.prototype.slice.call(arguments, 1, arguments.length)
            ...
            .reduce((oResult, oSource) => {
                oSource.properties.forEach((sProperty) => {
    ...
    utils.isRootIntent = function (sIntent) {
        ...
        const sRootIntentConfigPath = "renderers.fiori2.componentData.config.rootIntent";
        const sConfiguredIntent = ObjectPath.get(sRootIntentConfigPath, window["sap-ushell-config"]) || "#Shell-home";
        const sRootIntentNoHash = sConfiguredIntent.replace("#", "");
        const sIntentNoHash = sIntent.replace("#", "");
    ...
    utils.generateRandomKey = function () {
        ...
        let sResult = "";
        const aRandomValues = new window.Uint32Array(40);

        window.crypto.getRandomValues(aRandomValues);

        function getRandomAlphaNumeric (i) {
            const randomIndex = aRandomValues[i] % sChars.length;
    ...
    utils.getHideEmptySpacesEnabled = function () {
        ...
        return new Promise((resolve) => {
            ...
            .then((Config) => {
                const bHideEmptySpacesEnabled = Config.last("/core/spaces/hideEmptySpaces/enabled");
                ...
                return utils._getUserSettingPersContainer()
                    ...
                    .then((oUserSettings) => {
                        const bUserEnabled = (oUserSettings || {}).hideEmptySpaces !== false;
    ...
    utils.setHideEmptySpacesEnabled = function (bHide) {
        ...
        return new Promise((resolve) => {
            ...
            .then((Config) => {
                ...
                return utils._getUserSettingPersContainer()
                    .then((oPersContainer) => {
                        return oPersContainer.getPersData().then((oUserSettings) => {
                            oUserSettings = oUserSettings || {};
                            const bOldValue = oUserSettings.hideEmptySpaces !== false;
                            ...
                            return oPersContainer.setPersData(oUserSettings).then(() => {
    ...
    utils.sanitizeTimeoutDelay = function (iDelay) {
        ...
        const iMaxDelay = 2147483647; // (2^31 − 1)
     ...
     * @param {boolean} [bWrapDeferredResult=false] Whether to wrap the result of a jQuery Deferred in an array. This can be used when multiples values are expected.
     * @returns {Promise} The wrapped value
     */
    utils.promisify = function (vValue, bWrapDeferredResult) {
        if (typeof vValue === "object") {
            if (vValue.done) { // jQuery.Deferred
                return new Promise((resolve, reject) => {
                    vValue
                        .done((...args) => {
                            if (bWrapDeferredResult) {
                                resolve(args);
                                return;
                            }
    ...
    /**
     * Casts any value to a jQuery.Deferred object.
     * @param {any} vValue The value to cast
     * @returns {jQuery.Promise} The created Deferred object
     *
     * @since 1.141.0
     * @private
     */
    utils.toDeferred = function (vValue) {
        const oDeferred = new jQuery.Deferred();

        const pPromise = this.promisify(vValue);
        pPromise
    ...
    utils.isColdStart = function () {
        const oContainer = sap.ui.require("sap/ushell/Container");
        const oRenderer = oContainer.getRendererInternal("fiori2");
        const bNoCoreViewNavigated = !oRenderer || !oRenderer.getCurrentCoreView();
