 ...
 * 
 */
sap.ui.define(["sap/m/MessageBox", "sap/ui/core/format/FileSizeFormat", "sap/ui/core/format/NumberFormat", "./i18n", "sap/ui/core/Locale", "sap/base/i18n/Localization", "sap/ui/core/EventBus", "sap/ui/core/Element"], function (MessageBox, FileSizeFormat, NumberFormat, __i18n, Locale, Localization, EventBus, UI5Element) {
  "use strict";
  ...
  function _isDisplayed(element) {
    ...
    if (element && element.nodeType) {
      ...
      if (element instanceof HTMLElement && element.offsetParent !== null && !!(element.offsetWidth || element.offsetHeight || element.getClientRects().length)) {
        return true;
      }
    }
  ...
  class SearchFocusHandler {
    ...
    setFocus() {
      ...
      const doSetFocus = function () {
        ...
        if (!controlDomRef ||
        // condition 1
        document.querySelectorAll(".sapUshellSearchTileContainerDirty").length > 0 ||
        // condition 3
   ...
   */
  function calculateTooltipForwordEllipsis(elementToBeChecked) {
    ...
    const targetMeasureElement = findChildWithClass(elementToBeChecked, ["sapUshellSearchLink", "sapUshellSearchTextText"], ["sapMLnkText", "sapMTextLineClamp"] //sapMTextLineClamp span only occurs in multiline SearchText
    );

    // check whether content is overflowing
    if (!(targetMeasureElement.offsetWidth < targetMeasureElement.scrollWidth || targetMeasureElement.offsetHeight < targetMeasureElement.scrollHeight)) {
  ...
  function applyForwardEllipsisTruncation(targetMeasureElement, firstBElement) {
    const iconSpan = targetMeasureElement.querySelector(".sapUshellSearchTextIcon");
  ...
  function isElementVisible(elementToBeMeasured, parentElement) {
    if (!elementToBeMeasured) {
      return false;
    }

    // Get the bounding rectangle of the element
    const elToBeMeasuredRect = elementToBeMeasured.getBoundingClientRect();

    // Get the bounding rectangle of the parent container
    const parentElRect = parentElement.getBoundingClientRect();

    // Check if the element's rectangle is within the parent's rectangle
    const isVisible = elToBeMeasuredRect.top >= parentElRect.top && elToBeMeasuredRect.left >= parentElRect.left && elToBeMeasuredRect.bottom <= parentElRect.bottom && elToBeMeasuredRect.right <= parentElRect.right;
    return isVisible;
  }
   ...
   */
  function findChildWithClass(currentElement, parentClasses, childClasses) {
    ...
    if (hasAnyParentClass) {
      ...
      for (const childClass of childClasses) {
        const found = currentElement.querySelector(`.${childClass}`);
        if (found) {
          return found;
        }
      }
  ...
  __exports.calculateTooltipForwordEllipsis = calculateTooltipForwordEllipsis;
...
//# sourceMappingURL=SearchHelper-dbg.js.map
