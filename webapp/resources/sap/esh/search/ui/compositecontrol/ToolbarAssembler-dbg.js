...
sap.ui.define(["sap/ui/core/IconPool", "../i18n", "sap/ui/Device", "sap/m/OverflowToolbarLayoutData", "sap/m/library", "sap/m/OverflowToolbar", "sap/m/OverflowToolbarButton", "../controls/SearchSpreadsheet", "../eventlogging/UserEvents", "sap/m/ToolbarSpacer", "sap/m/ToolbarSeparator", "../error/errors", "sap/m/Button", "sap/m/ActionSheet", "sap/m/IconTabBar", "sap/m/IconTabFilter", "sap/m/SegmentedButton", "sap/m/SegmentedButtonItem", "sap/ui/model/BindingMode", "sap/m/ViewSettingsDialog", "sap/ui/core/Element", "sap/m/OverflowToolbarToggleButton"], function (IconPool, __i18n, Device, OverflowToolbarLayoutData, sap_m_library, OverflowToolbar, OverflowToolbarButton, __SearchSpreadsheet, ___eventlogging_UserEvents, ToolbarSpacer, ToolbarSeparator, __errors, Button, ActionSheet, IconTabBar, IconTabFilter, SegmentedButton, SegmentedButtonItem, BindingMode, ViewSettingsDialog, Element, OverflowToolbarToggleButton) {
  ...
  class ToolbarAssembler {
    ...
    assembleFilterButton() {
      ...
      const filterBtn = new OverflowToolbarToggleButton(this.compositeControl.getId() + "-searchBarFilterButton", {
        ...
        press: oEvent => {
          ...
          const isVisible = oEvent.getParameter("pressed");
          oModel.setFacetVisibility(isVisible);
          oModel.eventLogger.logEvent(isVisible ? {
        ...
        visible: {
          ...
          formatter: businessObjSearchEnabled => {
            // do not show button on phones
            // do not show in value-help mode
            // only show if business object search is active
            return !Device.system.phone && !oModel.config.optimizeForValueHelp && oModel.config.facets && businessObjSearchEnabled;
          }
        }
      });
    ...
    assembleResultviewSelectionButton() {
      const oModel = this.compositeControl.getModelInternal();
      const selectionBtn = new OverflowToolbarToggleButton(this.compositeControl.getId() + "-searchBarResultviewSelectionButton", {
        icon: IconPool.getIconURI("multi-select"),
        tooltip: {
          parts: [{
            path: "/resultviewSelectionVisibility"
          }],
          formatter: resultviewSelectionVisibility => {
            return resultviewSelectionVisibility ? i18n.getText("hideSelectionBtn_tooltip") : i18n.getText("showSelectionBtn_tooltip");
          }
        },
        pressed: {
          path: "/resultviewSelectionVisibility"
        ...
        visible: {
          ...
          formatter: businessObjSearchEnabled => {
            ...
            // do not show in value-help mode
            // only show if business object search is active
            return !Device.system.phone && !oModel.config.optimizeForValueHelp && oModel.config.showSelectionToggleButton && oModel.config.facets && businessObjSearchEnabled;
          }
        }
      });
      selectionBtn.addStyleClass("searchBarResultviewSelectionButton");
      selectionBtn.setLayoutData(new OverflowToolbarLayoutData({
        priority: OverflowToolbarPriority.NeverOverflow
      }));
      return selectionBtn;
    }
    assembleGenericButtonsToolbar() {
      ...
      if (oModel?.config?.searchResultTablePersonalization !== false) {
        tablePersonalizeButton = new OverflowToolbarButton((this.compositeControl.getId() ? this.compositeControl.getId() + "-" : "") + "tablePersonalizeButton", {
...
//# sourceMappingURL=ToolbarAssembler-dbg.js.map
