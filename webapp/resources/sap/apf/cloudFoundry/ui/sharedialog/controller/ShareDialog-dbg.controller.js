sap.ui.define([
	...
	"sap/ui/core/library"
], function (Controller, JSONModel, Library, MessageBox, LaunchPageUtils, Fragment, IconPool, coreLibrary) {
	"use strict";
		...
		*/
		onInit: function() {
			...
			if (oViewData.oController.configEditor.isSaved()) {
				this._pDialogInitialized = this.initializeDialog();
			} else {
		...
		/**
		 * Expose pDialogInitialized promise (for tests only)
		 * @private
		 * @returns {Promise<undefined>}
		 */
		whenDialogInitialized() {
			return this._pDialogInitialized;
		},

		/**
		 * Initialize the dialog, load the configuration, create the UI JSON-Model, open the dialog
		 */
		initializeDialog: async function() {
			var oDialog = this.oDialog = await this.loadFragment({
				name: "sap.apf.cloudFoundry.ui.sharedialog.fragment.ShareDialog"
			});
			var oView = this.getView();
			var oViewData = oView.getViewData();
			...
			oTilePreview.addDelegate({
				onAfterRendering: function() {
					oTilePreview.getDomRef().setAttribute("tabindex", -1);
				}
			});
		 ...
		 */
		getDialog: function() {
			return this.getView().byId("shareDialog");
		},
		/**
