...
sap.ui.define(
	...
	function(coreConstants, CoreFilter, formatter, exportToGlobal, Common) {
		"use strict";
		/**
		...
		function _getAllTheConfiguredSmartFilters(oPrintModelInstance) {
			...
			if (oSmartFilter !== undefined && oSmartFilter !== null) {
				...
				if(aUi5Filter.length > 0){
					var oFilter = CoreFilter.transformUI5FilterToInternal(oPrintModelInstance.oCoreApi.getMessageHandler(), aUi5Filter[0]);
	...
	otherwise get formatted filter values for application specific filters from APF****/
		function _getAllTheConfiguredAppSpecificFormattedFilters(oPrintModelInstance) {
			...
			var callback = oPrintModelInstance.oUiApi.getEventCallback(coreConstants.eventTypes.printTriggered);
