...
sap.ui.define([
...
], function(Print, utils, Button, Dialog, Input, mobileLibrary, Text, Device, Item, coreLibrary, BaseController, View, JSONModel) {
	"use strict";
	 ...
	 * @name sap.apf.ui.reuse.controller.toolbar
	 */
	return BaseController.extend("sap.apf.ui.reuse.controller.toolbar", /** @lends sap.apf.ui.reuse.controller.toolbar.prototype */ {
		/**
		*@this {sap.apf.ui.reuse.controller.toolbar}
		*/
		/**
		 * Refresh carousel on new Analysis path
		 */
		...
		/**
		 * Sets compact mode for dialogs when application is running in desktop.
		 *
		 * @param {sap.ui.core.Control} Dialog Instance on which style class has to be applied
		 */
		...
		/**
		 * Opens the save dialog when Save or SaveAs button is pressed from the toolbar
		 * @param {boolean} bSaveAs Boolean to determine whether Save or Save As button is pressed from the toolbar
		 */
		...
		/**
		 * Opens an overlay which holds saved analysis Paths.
		 *
		 * @param {boolean} bIsPathGalleryWithDelete to determine whether path gallery with delete mode has to be opened or not. if bIsPathGalleryWithDelete === true then open pathgallery with delete mode
		 * @see {@link sap.apf.ui.view.pathGallery}
		 */
		openPathGallery : function(bIsPathGalleryWithDelete) {
			var jsonData = {};
			var self = this;
			var i, oMessageObject;
			self.oCoreApi.readPaths(async function(data, metaData, msgObj) {
				if (msgObj === undefined && (typeof data === "object")) {
					...
					if (bIsPathGalleryWithDelete) {
						await self.openSavedPathGallery(jsonData, self, "deleteAnalysisPath");
					} else {
						await self.openSavedPathGallery(jsonData, self, "pathGallery");
					}
					self.oUiApi.getLayoutView().setBusy(false);
				} else {
		...
		openSavedPathGallery : async function(jsonData, context, viewName) {
			if (context.oPathGalleryDialog[viewName] === undefined || (await context.oPathGalleryDialog[viewName]).bIsDestroyed) {
				context.oPathGalleryDialog[viewName] = View.create({
					viewName : `module:sap/apf/ui/reuse/view/${viewName}.view`,
					viewData : {
						oInject : context.oViewData
					}
				});
			}
			const oDialogView = await context.oPathGalleryDialog[viewName];
			oDialogView.getViewData().jsonData = jsonData;
			var pathGalleryDialog = oDialogView.getController().oDialog;
			if ((!pathGalleryDialog) || (pathGalleryDialog && !pathGalleryDialog.isOpen())) {
				oDialogView.getController().openPathGallery();
			}
		},
		doPrint : function() {
			var oPrint = new Print(this.oViewData);
			oPrint.doPrint();
		},
		/**
		 * Getter for save dialog. Opens a new dialog for saving analysis Path.
		 *
		 * @param {boolean} bSafeAS callback for save
		 * @param {object} reset callback for save
		 * @param {any} aPath callback for save
		 */
		...
		/**
		 * Executes operations on click of "Ok" button of New Analysis Path dialog.
		 */
		...
		/**
		 * Executes operations on click of "Ok" btton of Open Analysis Path dialog.
		 */
		...
		/**
		 * Getter for New Analysis Path dialog.
		 */
		...
		/**
		 * Confirm dialog before overwriting path.
		 */
		...
		/**
		 * On click event of open button in Menu Popover
		 * @param {boolean} bIsPathGalleryWithDelete Boolean to determine whether path gallery with delete mode has to be opened or not
		 */
