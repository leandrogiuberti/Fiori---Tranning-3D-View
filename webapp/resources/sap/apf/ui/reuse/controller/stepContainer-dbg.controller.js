...
sap.ui.define([
	...
	function _setViewSettingsIcon(oController) {
		...
		var sortButton = new Icon({
			...
			press: async function () {
				const viewSettingsDialog = await oController.getCurrentRepresentation().getViewSettingDialog();
				oController.getView().addDependent(viewSettingsDialog);
	...
	function _setChartToolbarContent(oController) {
		...
		if (oCoreApi.getSteps().indexOf(oActiveStep) !== 0){
