...
], function(UI5MockServer, parseTextPropertyFile, utils, Log, deepExtend, extend, Storage, jQuery) {
	'use strict';
			...
			_postProcessDemokitData: function(oEvent) {
				...
				if (oXhr && oFilteredData) {
					...
					if (containsPagination) {
						var urlParamsWithoutPagination = extend({}, parsedUrl.parameters);
				...
				if (results.length > 0) {
					...
					if (measures.length > 0) {
						results.forEach(function(result) {
							...
							if (!hashTable[hash]) {
								hashTable[hash] = deepExtend({}, result);
			...
			_attachGUIDs: function(results) {
				return results.map(function(dataPoint) {
					if (dataPoint.__metadata) {
						...
						var newDataPoint = deepExtend({}, dataPoint);
