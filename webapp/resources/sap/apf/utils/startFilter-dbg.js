...
sap.ui.define([
...
],function(Filter, exportToGlobal, deepExtend, jQuery){
	'use strict';
	/**
	 * @private
	 * @class Start Filter
	 ...
	 * @alias sap.apf.utils.StartFilter
	 */
	function StartFilter(inject, config, context) {
		...
		var contextNotInValueHelp = deepExtend([], context);
		var preselectionNotInValueHelp;
		if (config.preselectionDefaults && config.preselectionDefaults.length > 0) {
			preselectionNotInValueHelp = deepExtend([], config.preselectionDefaults);
		} else if (typeof config.preselectionFunction === "function"){
			preselectionNotInValueHelp = deepExtend([], config.preselectionFunction());
		}
		...
		this.setSelectedValues = function(values, isSetInitially) {
			...
			if (values.type === 'internalFilter') {
				...
				externallySetSelectedValues = deepExtend([], values);
			}
			if(!isSetInitially){
				determineSelectedAndResolve(++concurrentDetermineSelectedCount);
			}
		...
		this.deserialize = function(serializedStartFilter) {
			...
			if(Array.isArray(serializedStartFilter.selectedValues)){
				contextNotInValueHelp = deepExtend([], serializedStartFilter.selectedValues);
				context = deepExtend([], serializedStartFilter.selectedValues);
			} else {
		...
		function determineValues(currentCount, concurrenCount) {
			...
			if (valueHelpConfigured && Array.isArray(context)) {
			...
			} else if (valueHelpConfigured  && ((config.preselectionDefaults && config.preselectionDefaults.length > 0) || typeof config.preselectionFunction === "function")) {
				getValueHelp(currentCount, concurrenCount).then(function(response) {
					var valueHelpList = response.data;
				...
				deferredDetermination.resolve(valueHelpList);
			} else if (((config.preselectionDefaults && config.preselectionDefaults.length > 0) || typeof config.preselectionFunction === "function") && !valueHelpConfigured && !config.filterResolutionRequest && !context) {
				...
				deferredDetermination.resolve(prependedValueHelpList);
			} else if (((config.preselectionDefaults && config.preselectionDefaults.length > 0) || typeof config.preselectionFunction === "function") && config.filterResolutionRequest && !context) {
				if (this.isMultiSelection()) {
					prependedValueHelpList = prependListValuesIfNotContainedInResponse(preselectionNotInValueHelp, valueHelpList);
				} else if(preselectionNotInValueHelp.length > 0 ){
					prependedValueHelpList = prependListValuesIfNotContainedInResponse([ preselectionNotInValueHelp[0] ], valueHelpList);
		...
		function prependListValuesIfNotContainedInResponse(list, response) {
			var valueList = deepExtend([], response);
		...
		function determineSelected(currentCount) {
			...
			if (externallySetSelectedValues) {
				if (!config.invisible) {
					checkAgainstValueHelpAndResolve(deepExtend([], externallySetSelectedValues), currentCount);
				} else {
					selectedValues = externallySetSelectedValues;
					resolvePromise();
				}
			} else if (typeof config.preselectionFunction === "function" && !valueHelpConfigured) {
				...
				sendFilterResolutionRequest(currentCount, concurrentDetermineSelectedCount).then(function(response) {
					...
					checkAgainstValueHelpAndResolve(deepExtend([], filterResolutionList), currentCount);
				});
				...
				if (typeof config.preselectionFunction === "function") {
					preselectionValues = config.preselectionFunction();
				} else {
					preselectionValues = config.preselectionDefaults;
				}
				checkAgainstValueHelpAndResolve(deepExtend([], preselectionValues), currentCount);
			} else if (Array.isArray(context)) {
			...
			function resolvePromise() {
				if (selectedValues && selectedValues.type === 'internalFilter') {
					...
					deferredDetermination.resolve(deepExtend([], selectedValues));
				}
			}
	...
	exportToGlobal("sap.apf.utils.StartFilter", StartFilter);
