...
sap.ui.define([
...
], function(uiCoreDateFormat, Hashtable, exportToGlobal, jQuery) {
	'use strict';
	...
	var utils = {
		...
		getUriParameters: function() {
			...
			// convert to the legacy structure internally used by sap/base/util/UriParameters
			const result = {};
		...
		extractFunctionFromModulePathString: function(sFunctionPath) {
			if (typeof sFunctionPath === "function") {
		...
		checkAndCloseDialog: function(dialog) {
			if (dialog !== undefined) {
				if (typeof dialog.isA === "function" && dialog.isA("§sap.m.ViewSettingsDialog")) {
					dialog.destroy();
	...
	Object.keys(utils).forEach(function(key) {
		exportToGlobal("sap.apf.utils." + key, utils[key]);
	});
	/*END_COMPATIBILITY*/
