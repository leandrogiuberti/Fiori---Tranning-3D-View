...
], function(ConfigurationObjects, exportToGlobal, extend){
	'use strict';
	/**
	 * @private
	...
	var ElementContainer = function(name, ElementConstructor, inject, dataFromCopy) {
		...
		this.copyElement = function(id) {
			...
			if (element.copy && typeof element.copy === "function") {
				...
				copiedElement = ConfigurationObjects.deepDataCopy(element);
			}
		...
		this.copy = function(newName) {
			...
			return new ElementContainer((newName || name), ElementConstructor, inject, copiedData);
		};
		/**
		 * private functions
		 */
		function createElementWithId(element, elementId, SpecifiedConstructor) {
			...
			if(typeof SpecifiedConstructor === 'function'){
				...
				if (ElementConstructor !== ElementContainer) {
			...
			if (element) {
				created = extend(created, element);
			}
			elements.setItem(elementId, created);
			return created;
		}
	};
	exportToGlobal("sap.apf.modeler.core.ElementContainer", ElementContainer);
