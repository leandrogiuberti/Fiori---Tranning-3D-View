...
sap.ui.define([
           ...
           ], function(hashtable, coreConstants, filter, DateFormat, exportToGlobal, utils, jQuery) {
               "use strict";
	/**
	 * @private 
	...
	function TextPool(inject, applicationId, existingTexts) {
		...
		var defaultInitialTextElement = {
			TextElement : coreConstants.textKeyForInitialText,
		...
		this.removeTexts = function(textElementKeys, application, callback) {
			...
			for(i = 0; i < len; i++) {
				if (textElementKeys[i] === coreConstants.textKeyForInitialText) {
					continue;
				}
				batchRequests.push({
					method : "DELETE",
		...
		this.isInitialTextKey = function(textKey) {
			return (textKey === coreConstants.textKeyForInitialText);
		};
		/**
		...
		this.addTextsAndSave = function(textElements, callback, application) {
			...
			var inputParameters;
			for(i = 0; i < len; i++) {
				if (textElements[i] === coreConstants.textKeyForInitialText) {
					continue;
				}
		...
		this.get = function(id) {
			...
			if (id === coreConstants.textKeyForInitialText) {
		...
		this.setTextAsPromise = function(textElementDescription, format) {
			...
			if (!textElementDescription) {
				deferred.resolve(coreConstants.textKeyForInitialText);
			}
