sap.ui.define([
	...
	return BaseController.extend("sap.apf.modeler.ui.controller.stepRequest", {
		setLabelDisplayOptionTypeAsPromise : function(optionsValueModelBuilder) {
			...
			oLabelDisplayOptions = coreConstants.representationMetadata.labelDisplayOptions;
			sPropertyName = textManipulator.removePrefixText(oController.byId("idOptionalRequestField").getSelectedKey(), oTextReader(modelerConstants.texts.NOTAVAILABLE));
		...
		fireRelevantEvents : function(oEvt) {
			...
			if (oEvt.getSource() !== oController.byId("idOptionalRequestField")) {
				sFilterProperty = [ textManipulator.removePrefixText(oController.byId("idOptionalRequestField").getSelectedKey(), oController.getView().getViewData().oTextReader(modelerConstants.texts.NOTAVAILABLE)) ];
				...
				oController.getView().fireEvent(modelerConstants.events.step.SETDATAREDUCTIONSECTION);
			}
			...
			oController.getView().fireEvent(modelerConstants.events.step.UPDATEFILTERMAPPINGFIELDS);
		},
		getSource : function() {
