...
sap.ui.define([
...
], function(coreConstants, BaseController, modelerConstants, DisplayOptionsValueBuilder, nullObjectChecker, textManipulator, textPoolHelper, jQuery) {
	"use strict";
	var oTranslationFormat = textPoolHelper.TranslationFormatMap.STEPFILTERPROPERTY_LABEL;
	...
	var stepFilterMapping = BaseController.extend("sap.apf.modeler.ui.controller.stepFilterMapping", {
		setLabelDisplayOptionTypeAsPromise : function(optionsValueModelBuilder) {
			...
			var oTextReader = oController.getView().getViewData().oTextReader;
			var displayOptionsValueBuilder = new DisplayOptionsValueBuilder(oTextReader, optionsValueModelBuilder);
			...
			var sPropertyName = textManipulator.removePrefixText(oController.byId("idOptionalRequestField").getSelectedKey(), oTextReader(modelerConstants.texts.NOTAVAILABLE));
		...
		getSelectedKeysForProperties : function() {
			...
			var oTextReader = oController.getView().getViewData().oTextReader;
			sSelectedKey = textManipulator.removePrefixText(oController.byId("idOptionalRequestField").getSelectedKey(), oTextReader(modelerConstants.texts.NOTAVAILABLE));
		...
		updateFilterMappingFields : function() {
			...
			if (nullObjectChecker.checkIsNotNullOrUndefinedOrBlank(sSource)) {
				oController.getAllEntitiesAsPromise(sSource).done(function(entities){
					...
					//set properties
					sSelectProperty = textManipulator.removePrefixText(oController.byId("idOptionalRequestField").getSelectedKey(), oTextReader(modelerConstants.texts.NOTAVAILABLE));
