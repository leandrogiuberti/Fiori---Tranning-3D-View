...
sap.ui.define([
	...
	/**
	 * Controller for view.toolbar.
	 * @class Toolbar controller
	 * @name sap.apf.modeler.ui.controller.toolbar
	 */
	return MVCController.extend("sap.apf.modeler.ui.controller.toolbar", /** @lends sap.apf.modeler.ui.controller.toolbar.prototype */ {
		/**
		...
		/**
		 * Sets the text for each of the toolbar buttons.
		 *
		 * @private
		 */
		...
		/**
		 * Enables copy and delete button in the toolbar.
		 *
		 * @private
		 */
		...
		/**
		 * Disables copy and delete button in the toolbar.
		 *
		 * @private
		 */
		...
		/**
		 * Sets text for all the menu items present in add menu fragment.
		 *
		 * @private
		 */
		...
		/**
		 * Sets text for all items present in existing step dialog fragment.
		 *
		 * @private
		 */
		...
		/**
		 * Enables and disables menu items in add menu fragment based on selected node object type.
		 *
		 * @private
		 * @param {Object} Context of the selected node
		 * @param {sap.ui.unified.Menu} addMenu root control of the addMenu fragment which contains all the menu items
		 */
		...
		/**
		 * Handler for add button in the toolbar.
		 *
		 * @private
		 */
		_handlePressAddButton : function(oEvent) { //handler for add button in the toolbar above tree control
			...
			var oNavigationHandlerInstance = navigationHandler.getInstance();
			...
			var copyConfigEditor = this.oConfigListInstance.configEditor ? deepExtend({}, this.oConfigListInstance.configEditor) : undefined;
			...
			var addAction = function() {
				// create add menu only once
				this._pAddMenu ??=
					oSelf.loadFragment({
						autoPrefixId: false, // do not prefix control IDs with view ID
						id: "idAddMenuFragment",
						name: "sap.apf.modeler.ui.fragment.addMenu"
					}).then((addMenu) => {
						this.addMenu = addMenu;
					});
				this._pAddMenu.then(() => {
					this._setAddMenuText();
					// enable/disable menu items has to be set to default (creating a new instance each time)
					...
					this.addMenu.open(false, oAddButton, Dock.BeginTop, Dock.BeginBottom, oAddButton); // opens the menu right below the add button
				});
			};
		...
		/**
		 * Handler for menu items of add button in the toolbar.
		 *
		 * @private
		 */
		 ...
		 *
		 * @private
		 * @param {string} sCopiedConfigurationId Copied configuration id
		 * @param {Object} newContextForCopiedNode New context of the copied node
		 */
		...
		/**
		 * Handler for copy button in the toolbar.
		 *
		 * @private
		 */
			...
			var copyAction = function() {
				...
				switch (oSelectedTreeNodeDetails.nodeObjectType) {
					case constants.configurationObjectTypes.FACETFILTER:
						...
						var oTranslationFormatForFacetFilterLabel = textPoolHelper.TranslationFormatMap.FACETFILTER_LABEL;
						this.oTextPool.setTextAsPromise(sFacetFilterLabel, oTranslationFormatForFacetFilterLabel).done(function(sFacetFilterLabelId) {
							...
							var oCopiedFacetFilterForTree = deepExtend({}, oFacetFilterForTree);
					...
					case constants.configurationObjectTypes.CATEGORY:
						...
						var oTranslationFormatForCategoryLabel = textPoolHelper.TranslationFormatMap.CATEGORY_TITLE;
						this.oTextPool.setTextAsPromise(sCategoryLabel, oTranslationFormatForCategoryLabel).done(function(sCategoryLabelId) {
							...
							var oCopiedCategoryForTree = deepExtend({}, oCategoryForTree);
					...
					case constants.configurationObjectTypes.NAVIGATIONTARGET:
						...
						var oCopiedNavTargetForTree = deepExtend({}, oNavTargetForTree);
					...
					case constants.configurationObjectTypes.STEP:
						...
						this.oTextPool.setTextAsPromise(sStepTitle, oTranslationFormatForStepTitle).done(function(sStepTitleId) {
							...
							var oCopiedStepForTree = deepExtend({}, oStepForTree);
							...
							for(var index = 0; index < aCategoriesForCopiedStep.length; index++) {
								var oStepInTree = deepExtend({}, oCopiedStepForTree);
					...
					case constants.configurationObjectTypes.REPRESENTATION:
						...
						var oCopiedRepForTree = deepExtend({}, oRepForTree);
						...
						for(var categoryIndex = 0; categoryIndex < aCategoriesForParentStep.length; categoryIndex++) {
							var repInTree = deepExtend({}, oCopiedRepForTree);
							var paramsForStep = {
								arguments : {
									configId : oSelf.oConfigListInstance.configId,
					...
					case constants.configurationObjectTypes.CONFIGURATION:
						...
						this.configurationHandler.copyConfiguration(oSelectedTreeNodeDetails.nodeAPFId, function(sCopiedConfigurationId) {
							...
							oCopySelf.configurationHandler.loadConfiguration(tempConfigId, function(configurationEditor) {
								...
								var oTranslationFormat = textPoolHelper.TranslationFormatMap.APPLICATION_TITLE;
		...
		/**
		 * Handler for delete button in the toolbar.
		 *
		 * @private
		 */
		...
		/**
		 * Opens a confirmation dialog before deleting a node from the tree.
		 *
		 * @private
		 */
		_openDeleteConfirmationDialog : function(sDialogMessage) {
			var oSelf = this;
			var oContext = extend(oSelf.oConfigListInstance, {
				closeDialog : oSelf.closeDialog.bind(oSelf)
			});

			this._pConfirmationDialog ??=
				// Note: can't use this.loadFragment as a different controller is required
				Fragment.load({
					id: "idConfigConfirmationDialogFragment",
					name: "sap.apf.modeler.ui.fragment.deleteConfirmationDialog",
					controller: oContext
				}).then((confirmationDialog) => {
					this.confirmationDialog = confirmationDialog
					this.getView().addDependent(confirmationDialog);
				});

			this._pConfirmationDialog.then(() => {
		...
		/**
		 * Sets text for the delete confirmation dialog.
		 *
		 * @private
		 */
		...
		/**
		 * Handler for move up button in the toolbar.
		 *
		 * @private
		 */
		...
		/**
		 * Handler for move down button in the toolbar.
		 *
		 * @private
		 */
		...
		/**
		 * Moves the node up or down depending upon which button is pressed in the toolbar.
		 *
		 * @private
		 * @param {Object} selectedNodeContext Selected node context
		 * @param {boolean} bIsDown Boolean stating whether down button is pressed
		 */
		...
		/**
		 * Handler for existing step menu item in the toolbar.
		 *
		 * @private
		 */
		_handleAddExistingStepPress : function() {
			...
			this._pAddExistingStepDialog ??=
				this.loadFragment({
					autoPrefixId: false, // do not prefix control IDs with view ID
					id: "idExistingStepDialogFragment",
					name: "sap.apf.modeler.ui.fragment.existingStepDialog"
				}).then((addExistingStepDialog) => this.addExistingStepDialog = addExistingStepDialog);

			this._pAddExistingStepDialog.then(() => {
		...
		/**
		 * Handler for press of ok in the existing step dialog.
		 *
		 * @private
		 */
		...
		/**
		 * Returns teh representation icon for a representation type.
		 *
		 * @private
		 * @param {string} sRepresentationName Representation type
		 * @returns {string} Picture or icon of the representation type
		 */
		...
		/**
		 * Handler for search in the existing step dialog.
		 *
		 * @private
		 */
