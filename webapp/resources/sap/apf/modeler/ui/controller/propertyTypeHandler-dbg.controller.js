...
function(ModelerConstants, mPropertyTypeFactory, mNullObjectChecker, PropertyTypeState, propertyTypeOrchestration, deepExtend, Controller, jQuery) {
	'use strict';
	...
	function _instantiateSubViews(oController, oPropertyOrchestration) {
		...
		aPropertiesToBeCreated.forEach(function(oPropertyInformation) {
			var pView = _createSubView(oController, oPropertyInformation, oController.nCounter, oPropertyOrchestration);
			aPromises.push(pView.then((oView) => oView.getController().initPromise));
		});
	 ...
	 * @private
	 */
	async function _createSubView(oController, oPropertyInformation, nIndexOfNewView, oPropertyOrchestration) {
		var oViewData = deepExtend({}, oController.getView().getViewData().oViewDataForPropertyType);
		...
		var oView = await mPropertyTypeFactory.createPropertyTypeView(oViewData, sViewId);
	 ...
	 * @constructor
	 */
	var propertyTypeHandler = Controller.extend("sap.apf.modeler.ui.controller.propertyTypeHandler", {
		...
		handlePressOfAdd : function(oEvent) {
			...
			propertyTypeOrchestration.getConsumableAndAvailablePropertiesAsPromise(representationId, aggregationRole, oStepPropertyMetadataHandler)
				.then(async function (oAvailableConsumableProperties) {
					...
					if (mNullObjectChecker.checkIsNotUndefined(oPropertyTypeData.sProperty)) {
						oView = await _createSubView(oController, oPropertyTypeData, nIndexOfNewView, oPropertyOrchestration);
