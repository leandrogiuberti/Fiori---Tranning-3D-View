...
], function(BaseController, modelerConstants, nullObjectChecker, optionsValueModelBuilder, textManipulator, GridData) {
	"use strict";

	var facetFilterVHR = BaseController.extend("sap.apf.modeler.ui.controller.facetFilterVHR", {
		...
		fireRelevantEvents : function(oEvent) {
			...
			if (oEvent.getSource() === oController.byId("idSource")) {
				oController.getView().fireEvent(modelerConstants.events.facetFilter.UPDATEPROPERTIES);
			}
			if (oEvent.getSource() !== oController.byId("idOptionalRequestField")) {
				aAlias = [ textManipulator.removePrefixText(oController.byId("idOptionalRequestField").getSelectedKey(), oTextReader(modelerConstants.texts.NOTAVAILABLE)) ];
			...
			if (oController.oParentObject.getUseSameRequestForValueHelpAndFilterResolution()) {
				oController.getView().fireEvent(modelerConstants.events.facetFilter.USESAMEASVHR);
			}
		},
