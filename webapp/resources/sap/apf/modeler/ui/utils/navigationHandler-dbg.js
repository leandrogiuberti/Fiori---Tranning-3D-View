...
sap.ui.define([
	...
	var _openMessageDialogForSwitchState = async function(configListInstance, switchState, callback) {
		...
		dialogInstance.oConfirmNavigationDialog = await Fragment.load({
			type: "XML",
			id: "idMessageDialogFragment",
			name: "sap.apf.modeler.ui.fragment.unsavedDataConfirmationDialog",
			controller: handlerContext
		});
	...
	var _openMessageDialogForValidationState = async function(configListInstance, switchState, callback) {
		var oCoreApi = configListInstance.oCoreApi;
		...
		handlerContext[currSwitchStateKey] = switchState[currSwitchStateKey];
		//Create New Instance of dialog
		dialogInstance.oConfirmValidationDialog = await Fragment.load({
			type: "XML",
			id: "idMandatoryValidationDialogFragement",
			name: "sap.apf.modeler.ui.fragment.mandatoryDialog",
			controller: handlerContext
		});
