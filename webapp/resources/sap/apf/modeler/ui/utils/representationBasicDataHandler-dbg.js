...
sap.ui.define([
...
], function(modelerUiConstants, propertyTypeOrchestration, View, ViewType) {
	'use strict';
	...
	RepresentationBasicDataHandler.prototype._prepareCommonView = function(oRepresentationBasicDataHandler, sPropertyType, aPropertyTypeHandlersToBeCreated, oPropertyOrchestration) {
		...
		const pView = View.create({
			...
			return oView.getController().initPromise;
		});
		oRepresentationBasicDataHandler.propertyTypeHandlerPromises.push(pView);
	...
	RepresentationBasicDataHandler.prototype.instantiateBasicDataAsPromise = function() {
		...
		return propertyTypeOrchestration.getConsumableAndAvailablePropertiesAsPromise(representationId, modelerUiConstants.aggregationRoles.DIMENSION, basicDataHandler.oStepPropertyMetadataHandler)
			...
			.then(() => jQuery.when(... this.propertyTypeHandlerPromises));
	};
