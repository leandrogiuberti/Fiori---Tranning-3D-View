...
sap.ui.define([
	...
	function HierarchicalStep(messageHandler, stepConfig, factory, representationId, coreApi) {
		Step.constructor.call(this, messageHandler, stepConfig, factory, representationId, coreApi);
		...
		function getSelectString(hierarchyProperty, hierarchyAnnotations) {
			...
			return uriGenerator.getSelectString(selectStrings);
		}
		...
		this.update = function(filter, callbackAfterRequest) {
			metadataInitialized.done(function(metadata, hierarchyAnnotations, entityTypeMetadata) {
				...
				var path = "/" + uriGenerator.generateOdataPath(messageHandler, metadata, entitySet, filter, metadata.getUriComponents(entitySet).navigationProperty);
				var controlObject = {};
		...
		/**
		 * Expose metadataInitialized for tests
		 * @private
		 * @returns {jQuery.Promise}
		 */
		this.whenMetadataInitialized = function() {
			return metadataInitialized.promise();
		}
	}
