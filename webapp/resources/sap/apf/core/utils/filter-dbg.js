...
sap.ui.define([
...
], function(FilterTerm, constants, Ui5Filter, exportToGlobal, utils, jQuery){
	'use strict';
	/**
	 * @private
	...
	Filter.prototype.mapToSelectOptions = function(getAllParameters) {
		...
		function Visitor (){
			...
			this.processTerm = function(term) {
				if(!parameters?.includes(term.getProperty())){
					addProperty(term);
				} else {
					addParameter(term);
				}
		...
		function transform(oUI5Filter, bAnd) {
			...
			if (oUI5Filter.aFilters) {
				oUI5Filter.aFilters.forEach(function(filter) {
					...
					} else if (filter.sPath) { //CoreFilter.mapToSapUI5FilterExpression() creates an empty filter object, therefore an additional check for filter.path is required
						if (bAnd) {
	...
	/*BEGIN_COMPATIBILITY*/
	exportToGlobal("sap.apf.core.utils.Filter", Filter);
	/*END_COMPATIBILITY*/
