...
sap.ui.define([
...
], function(MessageObject, CoreFilter, CoreFilterTerm, utils, EncodeURL, isPlainObject){
	'use strict';
	/**
		...
		function retrieveParameters() {
			...
			if (numberOfParameters > 0) {
				for(i = 0; i < numberOfParameters; i++) {
					if (filter && filter instanceof CoreFilter) {
					...
					if (parameterTerm instanceof CoreFilterTerm) {
	...
	uriGenerator.buildUri = function(oMsgHandler, sEntityType, aSelectProperties, oFilterForRequest, oFilter, sortingFields, oPaging, sFormat, fnFormatValue, sNavigationProperty, oMetadata) {
		...
		function addFilterToUri(oFilter, fnFormatValue) {
			if (!(oFilter && oFilter instanceof CoreFilter) || oFilter.isEmpty()) {
				return '';
			}
		...
		function addSorting(sortingFields, aSelectProperties) {
			...
			switch (true) {
				case Array.isArray(sortingFields):
					...
					break;
				case isPlainObject(sortingFields):
			...
			function makeOrderByValue(oOrderBy, aSelectProperties) {
				var sValue = '';
				if (aSelectProperties?.includes(oOrderBy.property)) {
