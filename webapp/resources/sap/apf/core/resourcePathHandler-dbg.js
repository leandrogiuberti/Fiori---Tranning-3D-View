	...
	function ResourcePathHandler(oInject) {
		...
		function loadAnalyticalConfigFromService(analyticalConfigurationId) {
			...
			var odataProxy = new Proxy({
				...
				entityTypes : {
					configuration : coreConstants.entitySets.configuration,
					texts : coreConstants.entitySets.texts
				}
			}, {
				instances : {
					coreApi : coreApi,
		...
		function loadTextFromService(applicationId, odataProxy) {
			...
			var filter = new CoreFilter(messageHandler, 'Language', 'eq', coreConstants.developmentLanguage);
		...
		function loadAnalyticalConfigFromFile() {
			var sUrl = thisModule.getResourceLocation(coreConstants.resourceLocation.analyticalConfigurationLocation);
			var oMessageObject;
			if (sUrl !== "") {
				coreApi.ajax({
					url : sUrl,
		...
		function configureMessageHandlingAsPromise() {
			coreApi.loadMessageConfiguration(messageDefinition, true);
			return loadMessagesFromConfigurationFileAsPromise(coreConstants.resourceLocation.applicationMessageDefinitionLocation, false);
		}
		...
		function checkPersistenceConfiguration(oConfig) {
			...
			if (oConfig.analyticalConfiguration) {
				if (!oConfig.analyticalConfiguration.service) {
					oMessageObject = messageHandler.createMessageObject({
						code : coreConstants.message.code.errorInAnalyticalConfig,
		 ...
		 * @param {string} sResourceIdentifier type coreConstants.resourceLocation.*
		...
		function setDefaultLocations() {
			...
			oHT.setItem(coreConstants.resourceLocation.apfUiTextBundle, sApfLocation + "resources/i18n/apfUi.properties");
			oHT.setItem(coreConstants.resourceLocation.applicationMessageDefinitionLocation, "");
			oHT.setItem(coreConstants.resourceLocation.applicationMessageTextBundle, "");
			oHT.setItem(coreConstants.resourceLocation.applicationUiTextBundle, "");
			oHT.setItem(coreConstants.resourceLocation.analyticalConfigurationLocation, "");
		}
		function processApplicationConfiguration(conf, analyticalConfigurationId) {
			function saveRestrictedCopyOfApplicationConfiguration(oApplicationConfiguration) {
				oConfigurationProperties = deepExtend({}, oApplicationConfiguration);
			...
			if (!oInject.functions.isUsingCloudFoundryProxy()) {
				...
				if (!oPersistenceConfiguration.path.entitySet) {
					oPersistenceConfiguration.path.entitySet = coreConstants.entitySets.analysisPath;
				}
			}
			...
			var sUrl;
			var oProperty;
			for (oProperty in coreConstants.resourceLocation) {
				if (!coreConstants.resourceLocation.hasOwnProperty(oProperty)) {
					continue;
				}
				if (oProperty === coreConstants.resourceLocation.analyticalConfigurationLocation && analyticalConfigurationId ) {
					continue;
				}
				...
				} else if (oProperty === coreConstants.resourceLocation.apfUiTextBundle || oInject.instances.fileExists.check(sUrl, true)) {
					oHT.setItem(oProperty, sUrl);
		...
		function resolveCorePromiseAfterResourceLoading() {
			...
			jQuery.when(deferredConfigurationLoaded).done(function(){
				sApfBundleUrl = thisModule.getResourceLocation(coreConstants.resourceLocation.apfUiTextBundle);
				sApplicationUIBundleUrl = thisModule.getResourceLocation(coreConstants.resourceLocation.applicationUiTextBundle);
				sApplicationMessageBundleURL =  thisModule.getResourceLocation(coreConstants.resourceLocation.applicationMessageTextBundle);
		...
		if (oInject.constructors && oInject.constructors.ProxyForAnalyticalConfiguration) {
			...
			Proxy = oInject.constructors && oInject.constructors.RuntimeProxy || RuntimeProxy;
		} else if (useLrepProxy()) {
			Proxy = oInject.constructors && oInject.constructors.LayeredRepositoryProxy || LayeredRepositoryProxy;
		} else {
			Proxy = OdataProxy;
		}
