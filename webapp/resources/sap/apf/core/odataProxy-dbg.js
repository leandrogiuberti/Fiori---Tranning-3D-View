...
sap.ui.define([
...
], function(exportToGlobal, utils, coreConstants, Localization, OData, jQuery) {
	"use strict";
	var EntitySets = coreConstants.entitySets;
	...
	function OdataProxy(serviceConfiguration, inject) {
		...
		this.readEntity = function(entitySetName, callback, inputParameters, selectList,  application) {
			var entitySet = EntitySets[entitySetName];
		...
		function buildBatchRequests(requestConfigurations) {
			...
			coreApi.getXsrfToken(serviceRoot).done(function(xsrfToken){
				...
				var counter = 0;
				for(i = 0; i < len; i++) {
					entitySet = EntitySets[requestConfigurations[counter].entitySetName];
					getMetadata(serviceRoot, entitySet).done(function(entityMetadata){
						var url, method, request; 
						var bSelectListSet = false;
						url = EntitySets[requestConfigurations[counter].entitySetName];
		...
		function buildUrlForReadCollection(entitySet, inputParameters, selectList, filter) {
			...
			getMetadata(serviceRoot, entitySet).done(function(entityMetadata){
				...
				if (entitySet === EntitySets.application) {
					if (sQueryString) {
						sQueryString = sQueryString + '&';
					}
