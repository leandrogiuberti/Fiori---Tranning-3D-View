...
sap.ui.define([
...
], function(coreConstants, CoreFilter, filterTerm, exportToGlobal, deepExtend) {
	"use strict";
	...
	function Binding(oInject, oBindingConfig, oFactory, sRepresentationId) {
		...
		this.getFilter = function(oContextInfo) {
			...
			var methodTypes = coreConstants.filterMethodTypes;
			var aIndices = [];
			var result;
			...
			if (oSelectedRepresentation.getSelectionAsArray) {
				...
				return afterGetFilter(new CoreFilter(oInject.instances.messageHandler), oContextInfo);
			}
			...
			*/
			if (aIndices === undefined) {
				result = CoreFilter.createEmptyFilter(oInject.instances.messageHandler, aRequiredFilters);
				return afterGetFilter(result, oContextInfo);
			}
			if (aIndices.length === aCachedData.length || aIndices.length === 0) {
				return afterGetFilter(new CoreFilter(oInject.instances.messageHandler), oContextInfo);
			}
			result = CoreFilter.createFromArray(oInject.instances.messageHandler, aRequiredFilters, aCachedData, aIndices);
			return afterGetFilter(result, oContextInfo);
		};
		/**
		...
		this.getRequestOptions = function(filterChanged) {
			if (typeof this.getSelectedRepresentation().getRequestOptions === "function") {
		...
		this.getSelectedRepresentationInfo = function() {
			...
			var oRepType = deepExtend({}, aRepresentationInfo[nSelectedRepresentation]);
			delete oRepType.id;
		...
		oBindingConfig.representations.forEach(function(representation, index) {
			...
			aRepresentationInfo[index] = deepExtend({}, oFactory.getConfigurationById(sRepTypeId)); // return clone
			...
			aRepresentationInfo[index].parameter = deepExtend({}, representation.parameter);
		});
