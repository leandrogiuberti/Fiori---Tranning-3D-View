...
sap.ui.define([
          ...
          ],	function(utilsUtils, Hashtable, exportToGlobal, deepExtend, each, UI5ODataModel, ODataUtils, jQuery) {
	'use strict';
	...
	function Metadata(oInject, sAbsolutePathToServiceDocument) {
		...
		function defineApiResult(input) {
			...
			function convertAnnotationName(sAlternativeName, property) {
				...
				if (Array.isArray(object[property]) === true) {
					...
					if (property !== sAlternativeName){ // do not map identically
						each(object[property], function(objProperty, itsValue) {
			...
			object = deepExtend({}, input);
			each(object, function(sProperty) {
				switch (sProperty) {
	...
	exportToGlobal("sap.apf.core.Metadata", Metadata);
