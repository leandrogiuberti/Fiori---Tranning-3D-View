...
sap.ui.define([
...
], function(coreConstants, CoreFilter, exportToGlobal, trace, deepExtend, jQuery) {
	'use strict';
	var MessageCode = coreConstants.message.code;
	...
	function Path(oInject) {
		...
		this.getSteps = function() {
			return deepExtend([], aStepInstances);
		};
		/**
		 * @function
		...
		this.makeStepActive = function(oStep) {
			...
			oMessageHandler.check(bStepIsInPath, "An unknown step can't be an active step.", MessageCode.errorCheckWarning);
			if (bStepIsInPath) {
		...
		this.makeStepInactive = function(oStep) {
			...
			oMessageHandler.check(bStepIsActive, "Only an active step can be removed from the active steps.", MessageCode.errorCheckWarning);
			if (bStepIsActive) {
				var indexOfStep = aActiveSteps.indexOf(oStep);
		...
		this.stepIsInPath = function(oStep) {
			var indexOfStep = aStepInstances.indexOf(oStep);
			return (indexOfStep >= 0);
		};
		/**
		...
		this.getActiveSteps = function() {
			return deepExtend([], aActiveSteps);
		};
		/**
		...
		this.moveStepToPosition = function(oStep, nPosition, fnStepProcessedCallback) {
			var nIndexOfStep = aStepInstances.indexOf(oStep);
			...
			jQuery.when(coreApi.getCumulativeFilter(), coreApi.getSmartFilterBarAsPromise()).done(function(cumulativeStartFilter, smartFilterBarInstance){
				...
				if(smartFilterBarInstance){
					...
					filterArrayFromSFB.forEach(function(filterFromSFB){
						resultFilter.addAnd(CoreFilter.transformUI5FilterToInternal(oMessageHandler, filterFromSFB));
					});
					...
					if (analyticalParameterDefinition && analyticalParameterDefinition.length > 0) {
						for (i = 0; i < analyticalParameterDefinition.length; i++) {
							...
							if (filterData && filterData.length) {
								...
								resultFilter.addAnd(new CoreFilter(oMessageHandler,propertyName, "EQ", value ));
							}
						}
					}
				}
	...
	exportToGlobal("sap.apf.core.Path", Path);
