...
sap.ui.define([
...
], function(constants, utils, MessageObject, HashTable, exportToGlobal, Log, deepExtend, Device, jQuery) {
	'use strict';
	...
	function MessageHandler(bLogOnCreate) {
		...
		this.getLogMessages = function() {
			return deepExtend([], aLogMessages);
		};
		/**
		...
		function enrichInfoInMessageObject(oMessageObject) {
			...
			if (oMessageConfiguration === undefined) {
				oMessageConfiguration = deepExtend({}, oDefaultMessageConfiguration);
	...
	exportToGlobal("sap.apf.core.MessageHandler", MessageHandler);
