sap.ui.define([
	...
	function Step(oMessageHandler, oStepConfig, oFactory, sRepresentationId, oCoreApi) {
		...
		oMessageHandler.check(oStepConfig.binding !== undefined, "No binding assigned to step " + oStepConfig.id + " in analytical configuration", coreConstants.message.code.errorCheckConfiguration);
		...
		var oAdditionalConfigurationProperties = deepExtend({}, oStepConfig);
		/**
		 * @private
		 * @description Type
		 * @returns {string}
		 ...
		 * @returns {string}
		 */
		this.title = deepExtend({}, oStepConfig.title);
		/**
		 * @private
		...
		if (oStepConfig.longTitle) {
			this.longTitle = deepExtend({}, oStepConfig.longTitle);
		}
		/**
		 * @private
		 ...
		 * @returns {string}
		 */
		this.thumbnail = deepExtend({}, oStepConfig.thumbnail);
		/**
		 * @private
		...
		this.setData = function(oDataResponse, oFilterThatHasBeenUsedToRetrieveData) {
			...
			oCachedRequestOptions = extend({}, oBinding.getRequestOptions(bFilterChanged));
		...
		this.getFilterInformation = function(activeStep, index) {
			...
			function getFilterMappingValues (mappedProperty, mappingRequest) {
				...
				oCoreApi.getMetadata(mappingRequest.service).done(function(filterMappingMetadata){
					var textProperty;
					var mappedPropertyMetadata = new MetadataProperty.constructor(filterMappingMetadata.getPropertyMetadata(mappingRequest.entityType, mappedProperty));
	...
	/*BEGIN_COMPATIBILITY*/
	exportToGlobal("sap.apf.core.Step", Step);
	/*END_COMPATIBILITY*/
