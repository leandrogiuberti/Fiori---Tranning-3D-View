...
sap.ui.define([
	...
	ReadRequest,
	deepExtend,
	ImportedControlConfiguration,
	ImportedSelectOption,
	ODataModel,
	OData,
	jQuery
) {
	...
	function Instance(oApiInject) {
		...
		this.ajax = function(oSettings) {
			var settingsAndInject = deepExtend({}, oSettings);
		...
		this.odataRequest = function(oRequest, fnSuccess, fnError, oBatchHandler) {
			...
			var request = (oApiInject && oApiInject.functions && oApiInject.functions.odataRequest) || odataRequestWrapper;
		...
		this.getSmartFilterbarDefaultFilterValues = function() {
			const defaultFilterValues = jQuery.Deferred();

			// prefer injected constructors
			const ControlConfiguration = oApiInject.constructors.SFBControlConfiguration ?? ImportedControlConfiguration;
			const SelectOption = oApiInject.constructors.SFBSelectOption ?? ImportedSelectOption;
			...
			oApiInject.functions.getCombinedContext().done(function(filter){
				filter.getProperties().forEach(function(property){
					...
					controlConfig.push(new ControlConfiguration(config));
				});
		...
		this.createReadRequest = function(requestConfiguration) {
			...
			return new ReadRequest(oInject, oRequest, oRequestConfiguration.service, oRequestConfiguration.entityType);
		};
		/**
		...
		oTextResourceHandler = new ((oApiInject.constructors.TextResourceHandler) || TextResourceHandler)(oInject);
		...
		oConfigurationFactory = new ((oApiInject.constructors.ConfigurationFactory) || ConfigurationFactory)(oInject);
		...
		oMetadataFactory = new (oApiInject.constructors.MetadataFactory || MetadataFactory)(oInjectMetadataFactory);
		oPath = new (oApiInject.constructors.Path || Path.constructor)(oInject);
		oSessionHandler = new (oApiInject.constructors.SessionHandler || SessionHandler)(oInject);
		...
		if (isUsingCloudFoundryProxy()) {
			...
			CtorAjaxHandler = (oApiInject.constructors.AjaxHandler || AjaxHandler);
		...
		if (oApiInject.constructors.Persistence) {
			...
			oInjectPersistence.instances.ajaxHandler = oAjaxHandler;
			oPersistence = new (oApiInject.constructors.AnalysisPathProxy || AnalysisPathProxy)(oInjectPersistence);
		} else {
		...
		oResourcePathHandler = new (oApiInject.constructors.ResourcePathHandler || ResourcePathHandler)(oInjectRessourcePathHandler);
	...
	/*BEGIN_COMPATIBILITY*/
	exportToGlobal("sap.apf.core.Instance", Instance);
	/*END_COMPATIBILITY*/
