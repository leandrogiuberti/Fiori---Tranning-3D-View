...
sap.ui.define([
...
], function(exportToGlobal, utils, Filter, FilterTerm, filterSimplify, Localization, OData){
	'use strict';
	/**
			...
			metadataPromise.done(function(oMetadata){
				oCoreApi.getXsrfToken(sServiceRootPath).done(function(xsrfToken){
					...
					function checkFilterForRequiredProperties(oFilter) {
						...
						if (!aFilterableProperties?.includes(sRequiredFilterProperty)) {
						...
						if (!aPropertiesInFilter?.includes(sRequiredFilterProperty)) {
							oMessageObject2 = oMessageHandler.createMessageObject({
								code : '5005',
								aParameters : [ sEntitySet, sRequiredFilterProperty ]
							});
	...
	exportToGlobal("sap.apf.core.Request", Request);
	return Request;
}, true /*Global_Export*/);
