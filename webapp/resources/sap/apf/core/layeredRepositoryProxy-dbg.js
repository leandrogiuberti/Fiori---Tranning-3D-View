...
], function(utils, constants, exportToGlobal, Hashtable, LrepConnector, parseTextPropertyFile, deepExtend, jQuery) {
	"use strict";
	/**
	...
	function LayeredRepositoryProxy(serviceConfiguration, inject) {
		...
		function init() {
			if (inject.constructors && inject.constructors.LrepConnector) {
				...
				connector = LrepConnector.createConnector();
			}
		...
		function updateConfiguration(configurationData, callback, layer) {
			...
			promiseForChangeList.then(function(changeList){
				...
				promiseForApplicationData.then(function(result) {
					...
					analyticalConfiguration = deepExtend(analyticalConfiguration, {
