...
sap.ui.define([
	...
	'use strict';
	var MessageCodes = coreConstants.message.code;
	...
	function ConfigurationFactory(oInject) {
		...
		var setItem = function(oItem) {
			oInject.instances.messageHandler.check(oItem !== undefined && oItem.hasOwnProperty("id") !== false, "oItem is undefined or property 'id' is missing", MessageCodes.errorCheckConfiguration);
			if (!idRegistry) {
				idRegistry = new Hashtable(oInject.instances.messageHandler);
			}
			var result = idRegistry.setItem(oItem.id, oItem);
			oInject.instances.messageHandler.check((result === undefined), "Configuration includes duplicated identifiers (IDs): " + oItem.id + "", MessageCodes.errorCheckConfigurationWarning);
		};
		...
		function addObject(configurationObject) {
			if (!coreConstants.configurationObjectTypes.hasOwnProperty(configurationObject.type)) {
			...
			if (configurationObject.type === coreConstants.configurationObjectTypes.facetFilter && !(configurationObject.property)) {
				oInject.instances.messageHandler.putMessage(oInject.instances.messageHandler.createMessageObject({
					code : "5034"
				}));
			}
		...
		function loadSteps(aSteps) {
			oInject.instances.messageHandler.check(aSteps !== undefined && aSteps instanceof Array !== false, "aSteps is missing or not an Array", MessageCodes.errorCheckConfiguration);
		...
		function loadRequests(aRequests) {
			oInject.instances.messageHandler.check(aRequests !== undefined && aRequests instanceof Array !== false, "aRequests is missing or not an Array", MessageCodes.errorCheckConfiguration);
		...
		function loadBinding(oBinding) {
			...
			oInject.instances.messageHandler.check(oBinding.representations !== undefined && oBinding.representations instanceof Array !== false, "representations for binding " + oBinding.id + " not defined",
					MessageCodes.errorCheckConfiguration);
			checkRepresentationForID();
			setItem(oBinding);
		}
		function loadBindings(aBindings) {
			oInject.instances.messageHandler.check(aBindings !== undefined && aBindings instanceof Array !== false, "aBindings is missing or not an Array", MessageCodes.errorCheckConfiguration);
			aBindings.forEach(function(binding) {
				loadBinding(binding);
			});
		}
		function loadNavigationTargets(navigationTargets) {
			oInject.instances.messageHandler.check(navigationTargets !== undefined && navigationTargets instanceof Array !== false, "navigationTarget is missing or not an Array", MessageCodes.errorCheckConfiguration);
		...
		function loadCategories(aCategories) {
			oInject.instances.messageHandler.check(aCategories !== undefined && aCategories instanceof Array !== false, "aCategories is missing or not an Array", MessageCodes.errorCheckConfiguration);
			aCategories.forEach(function(category) {
				var aSteps = category.steps;
				oInject.instances.messageHandler.check(aSteps !== undefined && aSteps instanceof Array !== false, "steps for category " + category.id + " are missing or not an Array", MessageCodes.errorCheckConfiguration);
				aSteps.forEach(function(step) {
		...
		function loadRepresentationType(oRepresentationTypeConfig) {
			...
			if (!isPredefinedRepresentation(oRepresentationTypeConfig.id)) {
				...
				if (typeof representationConstructorCandidate !== "function") {
					oInject.instances.messageHandler.putMessage(oInject.instances.messageHandler.createMessageObject({
						code : '5030',
		...
		function loadRepresentationTypes(aRepresentationTypes) {
			oInject.instances.messageHandler.check(aRepresentationTypes !== undefined && aRepresentationTypes instanceof Array !== false, "aRepresentationInfo is missing or not an Array", MessageCodes.errorCheckConfiguration);
		...
		function loadFacetFiltersInDesignTime(aFacetFilters) {
			oInject.instances.messageHandler.check(aFacetFilters !== undefined && aFacetFilters instanceof Array !== false, "Facet filter configuration is missing or not an Array", MessageCodes.errorCheckConfiguration);
			if(aFacetFilters.length === 0) {
				idRegistry.setItem(coreConstants.existsEmptyFacetFilterArray, true);
				return;
			}
		...
		function StepTemplate(oStepConfig, oFactory) {
			function getRepresentations(oStepConfig, oConfigurationFactory) {
				...
				oInject.instances.messageHandler.check(false, 'Binding of step with ID "' + oStepConfig.id + '" does not contain any representations.', MessageCodes.errorCheckConfigurationWarning);
			}
			function getRepresentationInfo(oStepConfig, oConfigurationFactory) {
				...
				if (oStepConfig.binding) {
					...
					aRepresentations.forEach(function(representation) {
						var oRepresentationType = deepExtend({}, oConfigurationFactory.getConfigurationById(representation.representationTypeId));
						oRepresentationType.representationId = representation.id;
						oRepresentationType.representationLabel = representation.label;
						oRepresentationType.parameter = deepExtend({}, representation.parameter);
						localRepresentationInfo.push(oRepresentationType);
					});
					return localRepresentationInfo;
				}
				oInject.instances.messageHandler.check(false, 'Step with ID "' + oStepConfig.id + '" does not contain any binding references.', MessageCodes.errorCheckConfigurationWarning);
			}
			var oStepTemplate = deepExtend({}, oStepConfig);
			...
			oStepTemplate.getRepresentationInfo = function() {
				var aReprInfo = deepExtend([], aRepresentationInfo); // clone deep
		...
		this.getNavigationTargets = function() {
			...
			return deepExtend([], navigationTargets);
		};
		/**
		 * @private
		...
		this.getSmartFilterBarConfiguration = function() {
			...
			deferredConfigurationLoaded.done(function(){
				...
				if(typeof SFBConfig === 'object' && SFBConfig.service && ( SFBConfig.entityType || SFBConfig.entitySet )) {
					deferred.resolve(deepExtend({}, SFBConfig));
				}else{
					deferred.resolve();
				}
			});
		...
		this.getFacetFilterConfigurations = function() {
			...
			var facetFilters = deepExtend([], originalFacetFilters);
		...
		this.createStep = function(sStepId, sRepresentationId) {
			...
			const EffectiveStep = oInject.constructors.Step || Step.constructor;
			oInject.instances.messageHandler.check((oStepConfig !== undefined && (oStepConfig.type === "step" || oStepConfig.type === "hierarchicalStep")), "Error - referenced object is undefined or has not type step", MessageCodes.errorCheckConfiguration);
			oInject.instances.messageHandler.check(EffectiveStep !== undefined, "Step must be defined ", MessageCodes.errorCheckConfiguration);
			oInject.instances.messageHandler.check(typeof EffectiveStep === "function", "Step must be constructor function");
			if(oStepConfig.type === 'hierarchicalStep'){
				oInject.instances.messageHandler.check(typeof HierarchicalStep === "function", "HierarchicalStep must be constructor function");
				return new HierarchicalStep(oInject.instances.messageHandler, oStepConfig, this, sRepresentationId, oInject.instances.coreApi);
			}
			return new EffectiveStep(oInject.instances.messageHandler, oStepConfig, this, sRepresentationId, oInject.instances.coreApi);
		};
		/**
		 * @private
		...
		this.createBinding = function(sBindingId, oTitle, oLongTitle, sRepresentationId) {
			...
			oInject.instances.messageHandler.check((oBindingConfig !== undefined && oBindingConfig.type === "binding"), "Error - oBindingConfig is undefined or has not type binding", MessageCodes.errorCheckConfiguration);
			oBindingConfig.oTitle = oTitle;
			oBindingConfig.oLongTitle = oLongTitle;
			const EffectiveBinding = oInject.constructors.Binding || Binding;
			return new EffectiveBinding(oInject, oBindingConfig, this, sRepresentationId);
		};
		/**
		 * @private
		...
		this.createRequest = function(request) {
			...
			return new ((oInject && oInject.constructors && oInject.constructors.Request) || Request)(oInject, oRequestConfig);
		};
