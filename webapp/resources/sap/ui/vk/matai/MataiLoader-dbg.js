		...
		return function() {
			return promise || (promise = new Promise(function(resolve, reject) {
				...
				function finishLoading(sceneBuilder, result) {
					...
					// Forcefully set progress to 100% for those cases when count does not reach totalCount due to missing items (such as no thumbnail for a modelview).
					updateProgress(sceneBuilder, "", sceneBuilder._progress.totalCount);
							...
							case "fileLoaded":
								if (data.result === OK) {
									...
									if (context.waitingCount === 0) {
										// There are no more files waiting to be loaded.
										worker.postMessage({
											method: "buildScene",
											args: { sceneBuilderId: sceneBuilderId }
										});
			...
			getWorker().then(
				...
				function(worker) {
					...
					sceneBuilders.set(sceneBuilder._id, {
						...
						// (Promise<Response>|null) is a promise that resolves when the dependency
						// loader finishes loading the file or `null` if loading has not started
						// yet.
						files: new Map(),
						// The number of files waiting for being processed, equals the number of
						// items in `files` with value `false` plus 1 for the top level file.
						waitingCount: 1
					});
					...
					worker.postMessage(
						{
							...
							args: {
								...
								buffer: buffer,
								fileName: url,
								locale: Localization.getLanguageTag().toString()
							}
						},
						[buffer]
					);
