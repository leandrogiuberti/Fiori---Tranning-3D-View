...
sap.ui.define([
	 ...
	 * @return {float[]} an array of 12 or 16 numbers
	 * @static
	 * @public
	 */
	TransformationMatrix.parseValue = function(value) {
		var componentType = DataType.getType("float");
	 ...
	 * @return {float[]} The matrix 4x4 with [0, 0, 0, 1] in the last row.
	 * @static
	 * @public
	 */
	 ...
	 *
	 * A matrix can be safely converted to a 4x3 matrix if the last row contains <code>0 0 0 1</code>.
	 ...
	 * 0  0  0  1
	 * </pre>
	 * @param {float[]} matrix4x4 The matrix to convert. The last row must be [0, 0, 0, 1].
	 * @return {float[]} The matrix 4x3 with the last row removed from matrix4x4.
	 * @public
	 * @static
	 */
	TransformationMatrix.convertTo4x3 = function(matrix4x4) {
		var m = matrix4x4;
		assert(TransformationMatrix.canConvertTo4x3(m), "The transformation matrix is invalid. The last row must be [0, 0, 0, 1].");
