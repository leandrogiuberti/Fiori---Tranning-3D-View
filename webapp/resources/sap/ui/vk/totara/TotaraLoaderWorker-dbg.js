...
(function() {
	...
	async function sendGetScene(data) {
		const [json, sceneResponse] = await Promise.all([
			sendRequest(`scenes/${data.sceneId}/info?includeInMetering=true`, data, "json"),

			// NOTE: The scene metadata is returned by the `scenes/{sceneId}` endpoint.
			//
			// That endpoint also returns the full scene tree by default. We use sid 9223372036854775807 (Java
			// Long.MAX_VALUE) to limit the scene tree to one node only. As there is no node with this sid in the
			// database, no scene tree is actually returned.
			sendRequest(`scenes/${data.sceneId}?sid=9223372036854775807`, data, "json")
		]);

		json.metadata = sceneResponse.scene.metadata;
	...
	function sendGetView(data) {
		...
		sendRequest(request, data, "text")
			.then(function(response) {
				...
				const dynamicContents = json.dynamicContents;
				...
				if (nodes) {
					...
					nodes.forEach(function(node) {
						...
						if ("dynamicContent" in node && dynamicContents) {
							node.dynamicContentId = dynamicContents[node.dynamicContent].id;
						}
				...
				if (dynamicContents) {
					commands.push({
						...
						jsonContent: {
							sceneId: sceneId,
							dynamicContents: json.dynamicContents
						},
						isInitial: isInitial
					});
				}
