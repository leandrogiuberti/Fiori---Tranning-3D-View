...
sap.ui.define([
	...
	MapContainer.prototype.init = function() {
		...
		// navbar
		this._oNavbar = new Toolbar({
			// Use ToolbarDesign.Auto
			width: "auto"
		});
		// scroll container for list panel stack
		this._oScrollCont = new ScrollContainer({
			horizontal: false,
			vertical: true,
			focusable: false
		});
		...
		this._oRectZoomButton = new ToggleButton({
		...
		this._oZoomInButton = new Button({
			icon: "sap-icon://add",
			type: sap.m.ButtonType.Transparent,
		...
		if (Device.system.phone) {
			this._oMenuOpenButton = new Button({
			...
			this._oMenuCloseButton = new Button({
				type: sap.m.ButtonType.Transparent,
	...
	MapContainer.prototype.setListPanelStack = function(oPanel) {
		if (Device.system.phone) {
			...
			oPanel.setWidth("100%");
			this._oMenuOpenButton.setVisible(true);
		}
		...
		if (content !== null) {
			...
			if (this._isSupportingMapLayerSwitch && control.getMapConfiguration() !== null && this.getShowMapLayer()) {
				...
				if (layers.length > 1) {
					...
					this._currentText = new MText().addStyleClass("mapLayerSelectedText");
					this.addDependent(this._currentText); // bind life/render cycle to MapContainer

					this._box = new HBox().addStyleClass("mapContainerHboxPopover");
	...
	MapContainer.prototype.onAfterRendering = function() {
		...
		if (this._shouldRenderMapLayerSwitch) {
			...
			layers.forEach(function(v) {
				...
				var itemText = new MText({ text: v.name }).addStyleClass("mapLayerPopoverItemText");
