...
sap.ui.define([
	 ...
	 *       					<code>dynamicContentId</code>: String. The id of the dynamic content object the node is associated with. Optional<br/>
	...
	SceneBuilder.prototype.createNode = function(nodeInfo, sceneId) {
		...
		if (nodeInfo.dynamicContentId) {
			const node = this._nodes.get(nodeInfo.sid);
			if (node) {
				const visible = nodeInfo.visible;
				node.userData.parametricVisible = visible !== undefined ? visible : true; // dynamic content is a parametric rectangle
			}
		}
		...
		if (node.parent.userData.objectType === ObjectType.Hotspot) {
			...
			userData.objectType = ObjectType.DynamicContent;
