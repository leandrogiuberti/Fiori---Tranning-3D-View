...
sap.ui.define([
	...
	ViewportGestureHandler.prototype.activateCamera = function(camera, timeInterval, pauseWhenNoCameraChange, transitionEffect) {
		...
		if (viewportCamera.getCameraRef().isPerspectiveCamera !== camera.isPerspectiveCamera) {
			var vkCamera = camera.isPerspectiveCamera ? new PerspectiveCamera() : new OrthographicCamera();
			vkCamera.getCameraRef().copy(camera);
			viewport.setCamera(vkCamera);
		} else {
			viewportCamera.getCameraRef().copy(camera);
			viewportCamera.update(viewport._width, viewport._height);
			delete viewportCamera._dummyCamera;
		}
	...
	ViewportGestureHandler.prototype.animateCameraUpdate = function() {
		return this._cameraController.animateCameraUpdate();
	};
