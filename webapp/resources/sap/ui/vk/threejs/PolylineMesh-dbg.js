	...
	PolylineMesh.prototype = Object.assign(Object.create(THREE.Mesh.prototype), {
		...
		computeLineDistances: (function() {
			...
			return function(matWorldViewProj, viewportSize, nearZ) {
				...
				let dist = this.material.dashOffset, t;

				if (!indices) {
					b.copy(vertices[0]).applyMatrix4(matWorldViewProj);
				}
				for (let i = 0, j = 0, l = data.count; i < l; i++, j += 2) {
					if (indices) {
						a.copy(vertices[indices[i << 1]]).applyMatrix4(matWorldViewProj);
						b.copy(vertices[indices[(i << 1) + 1]]).applyMatrix4(matWorldViewProj);
						dist = this.material.dashOffset;
					} else {
						a.copy(b);
