...
sap.ui.define([
			...
			if (annotations) {
				annotations.forEach(function(annotationInfo) {
					if (annotationInfo.node === nodeRef) {
						...
						annotation.setDisplay(vsm?.getVisibilityState(nodeRef) ?? nodeRef.visible);
					}
				}, this);
			}
		}
	...
	Viewport.prototype._findBackgroundNode = function() {
		var nativeScene = this._getNativeScene();
		const vsm = this._getViewStateManagerThreeJS();
		if (nativeScene == null || vsm == null) {
			return null;
		}
		var findBackgroundChildNodeRecursively = function(nodeRefs) {
			for (var i = 0; i < nodeRefs.length; i++) {
				var node = nodeRefs[i];
				if (node._vkGetNodeContentType() === NodeContentType.Background && vsm.getVisibilityState(node) === true && (node.name === "plane" || node.name === "sphere")) {
					if (node.parent.name === "Background" && vsm.getVisibilityState(node.parent) === false) {
						return null;
					}
					return node;
				} else {
	...
	function _updateDynamicObjects(renderer, camera, scene, viewport, vsm) {
	...
	Viewport.prototype.render = function() {
		...
		vsm?.applyNodeStates?.(this.getSelectionDisplayMode() === SelectionDisplayMode.Outline);
		...
		try {
			...
			const someObjectsWereUpdated = _updateDynamicObjects(renderer, nativeCamera, nativeScene, this, vsm);
			...
			if (this._camera.getUsingDefaultClipPlanes() || (nativeCamera.isOrthographicCamera && nativeCamera.zoom <= 0)) {
				...
				if (mustUpdateDynamicObjectsAgain && boundingBox.isEmpty()) {
					// If the bounding box is empty, it means that there are no visible objects in the scene currently,
					// so we use bounding box of all objects in the scene in order to get "good" znear and zfar.
					boundingBox = currentScene._computeBoundingBox(false, false, true);
				}
				...
				if (!boundingBox.isEmpty()) {
					...
					if (mustUpdateDynamicObjectsAgain) {
						...
						_updateDynamicObjects(renderer, nativeCamera, nativeScene, this, vsm);
					}
			...
			if (vsm?._outlinedNodes.size > 0) {
				this._renderOutline(nativeScene, nativeCamera, Array.from(vsm._outlinedNodes), vsm._outlineColorABGR, vsm);
			}

			if (this.getSelectionDisplayMode() === SelectionDisplayMode.Outline && vsm?._selectedNodes.size > 0) {
				this._renderOutline(nativeScene, nativeCamera, Array.from(vsm._selectedNodes), vsm._highlightColorABGR, vsm);
			}
	...
	Viewport.prototype._renderOutline = function(scene, camera, nodes, colorABGR, vsm) {
		const c = abgrToColor(colorABGR);
		const color = new THREE.Color(c.red / 255.0, c.green / 255.0, c.blue / 255.0);
		this._outlineRenderer.render(this._renderer, scene, camera, nodes, color, vsm.getOutlineWidth(), vsm._jointCollection);
	};
		...
		try {
			...
			nodeRefs.forEach(function(nodeRef) {
				nodeRef.traverseVisible(function(node) {
					node._vkUpdate?.(renderer, cameraRef, viewportSize, null, vsm);
	...
	Viewport.prototype._setContent = function(content) {
		...
		if (content) {
			...
			this._outputSettings = content.outputSettings;
		}
	...
	Viewport.prototype._activateSingleView = function(view, playViewGroup, skipCameraTransitionAnimation) {
		...
		this._outputSettings = view.outputSettings;
