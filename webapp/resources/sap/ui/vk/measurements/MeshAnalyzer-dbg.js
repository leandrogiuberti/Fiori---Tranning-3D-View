...
sap.ui.define([
	...
	MeshAnalyzer.prototype.intersectSphere = function(position, radius, allowFace, allowEdge, allowVertex, _allowClosedContour, _allowCurvedEdge, pointRadius) {
		...
		if (!this._triangleCount && !this._lineCount && this._vertexCount && allowVertex) {
			...
			const vertexRadius2 = pointRadius ? pointRadius * pointRadius : radius2;
			for (let vi = 0, count = this._vertexCount; vi < count; ++vi) {
				...
				if ((vertexDistance <= vertexRadius2) && (!ret.vertexDistance || vertexDistance < ret.vertexDistance)) {
					ret.vertexDistance = vertexDistance;
					ret.vertexId = vi;
				}
			}
