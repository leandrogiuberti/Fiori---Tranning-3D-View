	...
	var FormatUtil = {
		 ...
		 */
		 _extractCompositeKey: function(sLongKey) {
			...
			if (sMatch && sMatch[1]) {
				const oKeyValues = sMatch[1]
									.split(",")
									.reduce(function(oKey, sItem){
										const [sKey, sValue] = sItem.split("=");
										if (sKey && sValue) {
											const sNewValue = sValue.startsWith("guid") ? sValue.replace(/'$/g,"") : sValue.replace(/^'|'$/g, "");
											oKey[sKey] = sNewValue;
										}
										return oKey;
									}, {});
