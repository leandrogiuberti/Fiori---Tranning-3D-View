						...
						if (oValue && oValue.hasOwnProperty("low")) {// The data in the model corresponds to low and high Objects
							...
							if (oValue && oValue.hasOwnProperty("ranges")) { // Check if the data is for an unrestricted filter
								...
								while (iLen--) {
									...
									if (oRange.operation === ValueHelpRangeOperation.Empty || oValue?.conditionTypeInfo?.data?.operation === "NODATE") {
							...
							// Item filters
							while (iLen--) {
								const sKey = aValue[iLen].key;
								if (!aKeys.includes(sKey)) {
									aArrayFilters.push(new Filter(sField, FilterOperator.EQ, sKey));
									aKeys.push(sKey);
								}
							}
