...
sap.ui.define([
	...
	SmartChart.prototype._personalisationModelDataChange = function(oEvent) {
		...
		this._toggleIllustratedMessage();
	...
	SmartChart.prototype._toggleIllustratedMessage = function() {
		this._bInvalidChartType = false;
		if (!this._oChart?.getBlocked()) {
			const oIllustratedMessage = this._oVBox;
			const aUnAvailableCharts = this._oChart.getAvailableChartTypes().unavailable;
			const sChartType = this._oChart.getChartType();
			const iUnavailableIndex = aUnAvailableCharts.findIndex((item) => item.chart === sChartType);
			if (iUnavailableIndex !== -1) {
				oIllustratedMessage?.setVisible(true);
				this._oChart.setVisible(false);
				const oError = aUnAvailableCharts[iUnavailableIndex].error;
				this._sErrorMsg = MDCChartUtil.getErrorMessageForMissingMeasuresAndDimensions(sChartType, oError);
				this._oIllustratedMessage?.setDescription(this._sErrorMsg);
				this._bInvalidChartType = true;
			} else {
				oIllustratedMessage?.setVisible(false);
				this._oChart.setVisible(true);
			}
		}
	};
	...
	SmartChart.prototype._createChart = function() {
		...
		if (oChart) {
			...
			const oIllustratedMessage = this._createUIErrorForInvalidChartType();
	...
	/**
	 * Creates an IllustratedMessage for the case when the chart type is invalid.
	 * @private
	 * @since 1.130.0
	 * @returns {sap.m.IllustratedMessage} The IllustratedMessage instance
	 */
	SmartChart.prototype._createUIErrorForInvalidChartType = function() {
